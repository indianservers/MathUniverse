...
## Lesson 63 / Mockup 0045 - Complex Numbers

Reworked individually against the target mockup with a dedicated `complex-coefficients-draggable-plane-point-conjugate-vector-modulus-argument-projection-model`. The real and imaginary coefficients drive the plotted point, vector from the origin, horizontal and vertical projections, conjugate reflection, coordinate labels, modulus, argument, component legend, and result cards from one lesson-owned calculation model.

Both coefficient sliders are native interactive ranges and recalculate every representation immediately. The complex point is also directly draggable on the plane, snapping to integer coordinates and updating both sliders and all calculations. Reveal answer, five lesson tabs, language, Workspace, Reset, Share, previous/next navigation, and footer navigation all perform real actions; no visible control is decorative. The shared number adapter only dispatches Lesson 63 to this separate component.

Final 1060x1484 integrated browser validation changes the real coefficient to -2 and verifies `-2 + 2i`; changes the imaginary coefficient to 4 and verifies conjugate `-2 - 4i`, modulus 4.472, and argument 116.57 degrees; physically drags the point to `(4, -1)` and verifies `4 - 1i`, conjugate `4 + 1i`, modulus 4.123, and argument -14.04 degrees; reveals the practice answer; changes tab and language; opens Workspace; and resets. The final frame matches the target at hero x=226-1044 and y=102-332, tabs y=342-397, workspace y=407-1296, complex plane x=245-743 and y=513-1114, legend y=1133-1200, right rail x=772-1044, navigation y=1306-1360, and footer y=1367-1475. It reports no horizontal overflow and zero console messages.
...
## Lesson 77 / Mockup 0059 - Comparing Fractions

Reworked individually against the target mockup with a dedicated `dual-editable-fractions-draggable-unit-bars-lcm-common-units-cross-product-ordering-shared-number-line-practice-model`. Two independently editable fractions drive their original-unit bars, LCM, scale factors, common-unit bars, exact converted numerators, decimal approximations, comparison symbol, conclusion, and shared number-line positions from one lesson-owned calculation model.

Both numerator/denominator pairs are directly editable with valid-denominator and numerator guards. Every original bar piece is clickable and a native draggable/drop target, so a physical drop changes the represented numerator and recalculates the entire proof. All five proof-stage tabs, language, Share, Workspace, Reset, and the `3/4` versus `7/9` practice prompt perform real state changes. The shared number adapter only dispatches Lesson 77 to this separate component.

Final 1072x1467 integrated browser validation verifies `3/4 = 21/28`, `4/7 = 16/28`, and `3/4 > 4/7`; edits B to `6/8` and verifies equality at `6/8`; edits A to `2/4` and verifies the less-than result; physically drags an A bar piece to restore `3/4`; changes B to `3/8` and verifies greater-than; exercises Number line, Hindi, Share, and Workspace states; loads practice and verifies `27/36 < 28/36`; then resets to the target pair. The final frame matches the target at surface x=230-1056 and y=101-1467, hero y=101-332, tabs y=343-391, proof/side layout y=400-1316, understand y=400-638, conversion y=642-935, comparison y=935-1119, number line y=1119-1255, practice y=1259-1316, navigation y=1325-1374, and footer y=1382-1467. It reports no horizontal or vertical overflow and zero console messages.
...
## Lesson 185 / Mockup 0242 - Position Vectors

Reworked individually against the target mockup with a dedicated `origin-anchored-multiple-position-vectors-coordinate-magnitude-direction-practice` model. Three independent points A, B, and C generate origin-anchored vectors, coordinate cards, magnitudes, and atan2 directions; the practice point P has a separate state and calculation path.

Pointer and keyboard movement for all three lesson points and practice P, six exact coordinate fields, grid visibility, integer/half-unit snap behavior, active-point cycling, five stages, workspace link, Share, Reset, practice reset, wrong/correct grading, and adjacent navigation are working controls. Final 1024x1536 validation verifies every initial vector, independent A/B/C changes, grid/snap states, stage progression, practice movement and exact `(2,-3)` match, reset, links/routes, exact geometry, overflow, and console output.
...
## Lesson 198 / Mockup 0255 - Free Point

Reworked individually with a dedicated `independent-free-point-coordinate-construction` model. Physical point drag, keyboard movement, point/select/pan/zoom/delete tools, snap mode, coordinate and appearance editing, replayable undo history, lesson tabs, worked-example visibility, checked Q practice, routes, overflow, console output, and exact 1024x1536 framing are validated.

Evidence: `0255-reference.png`, `0255-desktop.png`, `0255-dedicated-target-validation.json`.

## Lesson 199 / Mockup 0256 - Point on Object
...
## Lesson 203 / Mockup 0260 - Line Through Two Points
