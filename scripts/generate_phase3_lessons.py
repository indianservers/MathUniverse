"""Generate the checked-in Phase 3 lesson catalog from the planning workbook."""
...
OUTPUT = Path(__file__).resolve().parents[1] / "src" / "modules" / "lessons" / "catalog" / "phase3.generated.ts"
PHASE_THREE_CATEGORIES = {"Calculus", "Data and Probability"}
TOPIC_ADAPTERS = {
    "Limits and Differential Calculus": "calculus",
    "Integral Calculus and Differential Equations": "calculus",
...
for values in sheet.iter_rows(min_row=2, values_only=True):
    ...
    if row["ID"] is None or row["Main Category"] not in PHASE_THREE_CATEGORIES:
        continue
    ...
    lessons.append({
        "id": lesson_id, "phase": 3, "slug": title_slug, "categorySlug": category_slug,
...
assert len(lessons) == 163, f"Expected 163 Phase 3 lessons, got {len(lessons)}"
assert len(ids) == len(set(ids)), "Duplicate Phase 3 IDs"
assert len({lesson["route"] for lesson in lessons}) == len(lessons), "Duplicate Phase 3 routes"
...
OUTPUT.write_text(
    ...
    f"export const phaseThreeLessons = {payload} as const satisfies readonly LessonSourceDefinition[];\n",
    encoding="utf-8",
)
