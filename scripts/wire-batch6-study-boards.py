#!/usr/bin/env python3
"""Wire LessonTopicStudyBoard into remaining TargetLessons 2001-2025 and 10057-10220."""
...
IDS = set(range(2001, 2026)) | set(range(10057, 10221))
...
def insert_board(text: str, lesson_id: int, view_prop: str) -> str:
    if already_wired(text, lesson_id):
        return text
    interaction = " onInteraction={onInteraction}" if re.search(r"\bonInteraction\b", text) else ""
    board = (
        f"      <LessonTopicStudyBoard lessonId={{{lesson_id}}} "
        f"{view_prop}{interaction} />\n"
    )
...
def main() -> None:
    ...
    if missing:
        ...
        print("all 2001-2025 and 10057-10220 have study boards")
