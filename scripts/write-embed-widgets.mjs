...
for (const [kind, api] of widgets) {
  const source = `/* Math Universe embed widget: ${api} (${kind}.js)\n * Usage: ${api}.embed(element, { objects:[{type,x,y,size,...}] })\n */\n(function(root){\n"use strict";\nvar KIND=${JSON.stringify(kind)};\nvar API=${JSON.stringify(api)};\n${engine}\nautostart(KIND, API);\n})(typeof window!=="undefined"?window:globalThis);\n`;
  writeFileSync(join(publicDir, `${kind}.js`), source);
}

writeFileSync(
  join(publicDir, "embed.html"),
  `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width, initial-scale=1" />
  <title>Math Universe embed</title>
  <style>html,body,#mu-embed-stage{margin:0;height:100%;background:transparent}</style>
</head>
<body>
  <div id="mu-embed-stage"></div>
  <script>
    (function () {
      var params = new URLSearchParams(location.search);
      var kind = params.get("kind") || "twodgraph";
      var allowed = { twodgraph: 1, twodgeometry: 1, threedgraph: 1, threedgeometry: 1 };
      if (!allowed[kind]) kind = "twodgraph";
...
console.log("wrote embed widgets");
