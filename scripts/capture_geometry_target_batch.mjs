...
const allLessons = [
  ...
  [247, 304, "locus-generator", 1006, 1563],
  ...
  [255, 312, "concurrency-test", 1004, 1566],
  [256, 313, "concyclicity-test", 1024, 1536],
];
...
for (const [id, mockup, slug, width, height] of lessons) {
  ...
  let status;
  if (id === 256) {
    const surface=page.locator(selector),pointA=page.locator('[data-testid="concyclicity-point-a"]'),pointB=page.locator('[data-testid="concyclicity-point-b"]'),pointC=page.locator('[data-testid="concyclicity-point-c"]'),pointD=page.locator('[data-testid="concyclicity-point-d"]');
    if((await surface.getAttribute("data-object-model"))!=="four-point-circumcircle-determinant-angle-residual"||Math.abs(Number(await surface.getAttribute("data-center-x")))>.001||Math.abs(Number(await surface.getAttribute("data-center-y")))>.001||Math.abs(Number(await surface.getAttribute("data-radius"))-Math.sqrt(26))>.001||(await surface.getAttribute("data-concyclic"))!=="true"||(await surface.getAttribute("data-determinant"))!=="0.000000")throw new Error("Concyclicity initial exact circle model is invalid");
    for(const [handle,dx,dy,label] of [[pointA,18,-16,"A"],[pointB,-20,15,"B"],[pointC,-18,-20,"C"],[pointD,32,-26,"D"]]){const beforeX=await handle.getAttribute("data-x"),box=await handle.boundingBox();if(!box)throw new Error(`Concyclicity point ${label} is not draggable`);await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await page.mouse.down();await page.mouse.move(box.x+Number(dx),box.y+Number(dy),{steps:6});await page.mouse.up();if((await handle.getAttribute("data-x"))===beforeX)throw new Error(`Dragging point ${label} did not update coordinates`)}if((await surface.getAttribute("data-concyclic"))!=="false"||Math.abs(Number(await surface.getAttribute("data-radial-residual")))<.01)throw new Error("Off-circle dragged construction was still classified concyclic");
    const initialValues={A:[-5,1],B:[1,5],C:[5,-1],D:[-1,-5]};for(const [key,values] of Object.entries(initialValues)){await page.getByRole("spinbutton",{name:`${key} x coordinate`}).fill(String(values[0]));await page.getByRole("spinbutton",{name:`${key} y coordinate`}).fill(String(values[1]))}if((await surface.getAttribute("data-concyclic"))!=="true")throw new Error("Exact coordinates did not restore the fitted circle");
    await page.getByRole("spinbutton",{name:"D x coordinate"}).fill("-5");await page.getByRole("spinbutton",{name:"D y coordinate"}).fill("1");if((await surface.getAttribute("data-points-distinct"))!=="false"||(await surface.getAttribute("data-concyclic"))!=="false")throw new Error("Duplicate fourth point was accepted as a four-point concyclic set");await page.getByRole("spinbutton",{name:"D x coordinate"}).fill("-1");await page.getByRole("spinbutton",{name:"D y coordinate"}).fill("-5");
    for(const [key,values] of Object.entries({A:[-2,0],B:[0,0],C:[2,0]})){await page.getByRole("spinbutton",{name:`${key} x coordinate`}).fill(String(values[0]));await page.getByRole("spinbutton",{name:`${key} y coordinate`}).fill(String(values[1]))}if((await surface.getAttribute("data-circle-valid"))!=="false"||(await surface.getAttribute("data-concyclic"))!=="false")throw new Error("Collinear first three points incorrectly defined a circumcircle");await page.getByRole("button",{name:"Reset",exact:true}).first().click();
    await page.getByRole("button",{name:"measurement",exact:true}).click();if((await surface.getAttribute("data-object-view"))!=="measurement")throw new Error("Concyclicity object-view strip did not switch modes");await page.getByRole("button",{name:"primary-control",exact:true}).click();await page.getByRole("slider",{name:"Practice D radial scale"}).fill("1.3");await page.getByRole("button",{name:"Check practice"}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();await page.getByRole("slider",{name:"Practice D radial scale"}).fill("1");await page.getByRole("button",{name:"Check practice"}).click();status=await page.getByRole("status").filter({hasText:"Correct. The fourth point"}).innerText();
  } else if (id === 255) {
    const surface=page.locator(selector),pointA=page.locator('[data-testid="concurrency-point-a"]'),pointB=page.locator('[data-testid="concurrency-point-b"]'),pointC=page.locator('[data-testid="concurrency-point-c"]'),pointD=page.locator('[data-testid="concurrency-point-d"]'),pointE=page.locator('[data-testid="concurrency-point-e"]'),pointF=page.locator('[data-testid="concurrency-point-f"]');
    if((await surface.getAttribute("data-object-model"))!=="triangle-cevians-exact-ceva-concurrency"||Math.abs(Number(await surface.getAttribute("data-ceva-product"))-1)>.001||(await surface.getAttribute("data-concurrent"))!=="true"||(await surface.getAttribute("data-triangle-valid"))!=="true")throw new Error("Concurrency initial Ceva model is invalid");
    const fBox=await pointF.boundingBox();if(!fBox)throw new Error("Ceva side point F is not draggable");await page.mouse.move(fBox.x+fBox.width/2,fBox.y+fBox.height/2);await page.mouse.down();await page.mouse.move(fBox.x+34,fBox.y+22,{steps:6});await page.mouse.up();if((await surface.getAttribute("data-concurrent"))!=="false"||Math.abs(Number(await surface.getAttribute("data-ceva-product"))-1)<.02)throw new Error("Dragging F did not break the Ceva product and concurrency");
    for(const [handle,dx,dy,label] of [[pointD,-22,0,"D"],[pointE,18,-20,"E"]]){const box=await handle.boundingBox();if(!box)throw new Error(`Ceva side point ${label} is not draggable`);await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await page.mouse.down();await page.mouse.move(box.x+Number(dx),box.y+Number(dy),{steps:5});await page.mouse.up()}
    await page.getByRole("spinbutton",{name:"AF over FB exact value"}).fill("1.25");await page.getByRole("spinbutton",{name:"BD over DC exact value"}).fill("1.2");await page.getByRole("spinbutton",{name:"CE over EA exact value"}).fill("0.6666667");if(Math.abs(Number(await surface.getAttribute("data-ceva-product"))-1)>.002||(await surface.getAttribute("data-concurrent"))!=="true")throw new Error("Exact Ceva ratios did not restore concurrency");
    for(const [handle,dx,dy,label] of [[pointA,25,-20,"A"],[pointB,20,10,"B"],[pointC,-18,16,"C"]]){const box=await handle.boundingBox();if(!box)throw new Error(`Triangle vertex ${label} is not draggable`);await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await page.mouse.down();await page.mouse.move(box.x+Number(dx),box.y+Number(dy),{steps:5});await page.mouse.up();if((await surface.getAttribute("data-concurrent"))!=="true")throw new Error(`Moving vertex ${label} broke affine Ceva concurrency`)}
    await page.getByRole("button",{name:"Zoom in concurrency graph"}).click();await page.getByRole("button",{name:"Zoom out concurrency graph"}).click();const cBox=await pointC.boundingBox(),bBox=await pointB.boundingBox();if(!cBox||!bBox)throw new Error("Triangle vertices disappeared before degeneracy test");await page.mouse.move(cBox.x+cBox.width/2,cBox.y+cBox.height/2);await page.mouse.down();await page.mouse.move(bBox.x+bBox.width/2,bBox.y+bBox.height/2,{steps:8});await page.mouse.up();if((await surface.getAttribute("data-triangle-valid"))!=="false"||(await surface.getAttribute("data-concurrent"))!=="false")throw new Error("Degenerate triangle was accepted as a valid concurrent construction");await page.getByRole("button",{name:"Reset",exact:true}).first().click();await page.getByRole("button",{name:"measurement",exact:true}).click();if((await surface.getAttribute("data-object-view"))!=="measurement")throw new Error("Concurrency object-view strip did not switch modes");await page.getByRole("button",{name:"primary-control",exact:true}).click();
    await page.getByRole("slider",{name:"Practice Ceva F ratio"}).fill("2");await page.getByRole("button",{name:"Check my answer"}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();await page.getByRole("slider",{name:"Practice Ceva F ratio"}).fill("1");await page.getByRole("button",{name:"Check my answer"}).click();status=await page.getByRole("status").filter({hasText:"Great! Correct"}).innerText();
  } else if (id === 254) {
    ...
    await page.getByRole("button",{name:"Toggle grid"}).click();await page.getByRole("button",{name:"Toggle labels"}).click();if((await surface.getAttribute("data-grid"))!=="false"||(await surface.getAttribute("data-labels"))!=="false")throw new Error("Exact Proof display controls did not update layers");await page.getByRole("button",{name:"Toggle grid"}).click();await page.getByRole("button",{name:"Toggle labels"}).click();
    for(const [name,value] of [["A x exact value","-1"],["A y exact value","2"],["B x exact value","3"],["B y exact value","-2"],["Translation x exact value","3"],["Translation y exact value","-1"]])await page.getByRole("spinbutton",{name}).fill(value);if(Math.abs(Number(await surface.getAttribute("data-original-length"))-Math.sqrt(32))>.001||Math.abs(Number(await surface.getAttribute("data-image-midpoint-x"))-4)>.001)throw new Error("Exact Proof coordinate controls did not recalculate exact premises");
    await page.getByRole("combobox",{name:"Proof reason 2",exact:true}).selectOption("Measurement from diagram");await page.getByRole("button",{name:"Check proof"}).click();if((await surface.getAttribute("data-proof-valid"))!=="false")throw new Error("Invalid proof reason was accepted");await page.getByRole("status").filter({hasText:"Proof incomplete"}).waitFor();await page.getByRole("combobox",{name:"Proof reason 2",exact:true}).selectOption("Distance preserved under translation");await page.getByRole("button",{name:"Check proof"}).click();await page.getByRole("status").filter({hasText:"All steps are valid"}).waitFor();
    await page.getByRole("radio",{name:"Exact proof practice B"}).check();await page.getByRole("combobox",{name:"Practice proof reason 2"}).selectOption("Measurement");await page.getByRole("button",{name:"Check answers"}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();await page.getByRole("radio",{name:"Exact proof practice A"}).check();await page.getByRole("combobox",{name:"Practice proof reason 2"}).selectOption("Definition of translation");await page.getByRole("button",{name:"Check answers"}).click();status=await page.getByRole("status").filter({hasText:"Correct. A′ = (2, 1)"}).innerText();
  } else if (id === 252) {
    const surface=page.locator(selector),pointA=page.locator('[data-testid="conjecture-point-a"]'),pointB=page.locator('[data-testid="conjecture-point-b"]'),vector=page.locator('[data-testid="conjecture-vector-handle"]');
    if((await surface.getAttribute("data-object-model"))!=="measured-translation-conjecture-trial-engine"||(await surface.getAttribute("data-length-difference"))!=="0.0000"||(await surface.getAttribute("data-midpoint-shift"))!=="2.0000"||(await surface.getAttribute("data-claim-status"))!=="unverified")throw new Error("Conjecture Testing initial translation model is invalid");
    for(const [handle,dx,dy,label] of [[pointA,32,-18,"A"],[pointB,-20,-30,"B"],[vector,42,20,"translation vector"]]){const box=await handle.boundingBox();if(!box)throw new Error(`Conjecture ${label} is not draggable`);await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await page.mouse.down();await page.mouse.move(box.x+Number(dx),box.y+Number(dy),{steps:6});await page.mouse.up();if(Math.abs(Number(await surface.getAttribute("data-length-difference")))>.001)throw new Error(`Dragging ${label} broke translation length invariance`)}
    await page.getByRole("spinbutton",{name:"A x exact value"}).fill("-2");await page.getByRole("spinbutton",{name:"A y exact value"}).fill("1");await page.getByRole("spinbutton",{name:"B x exact value"}).fill("3");await page.getByRole("spinbutton",{name:"B y exact value"}).fill("-2");await page.getByRole("spinbutton",{name:"Translation x exact value"}).fill("2");await page.getByRole("spinbutton",{name:"Translation y exact value"}).fill("1");if(Math.abs(Number(await surface.getAttribute("data-length-difference")))>.001||Math.abs(Number(await surface.getAttribute("data-midpoint-shift"))-Math.sqrt(5))>.001)throw new Error("Conjecture exact controls did not update measured translation");
    const statement=page.getByRole("textbox",{name:"Conjecture statement"});await statement.fill("The midpoint stays at the same coordinates after translation.");await page.getByRole("button",{name:"Check with current construction"}).click();if((await surface.getAttribute("data-claim-status"))!=="disproved"||(await surface.getAttribute("data-trial-count"))!=="1")throw new Error("False midpoint conjecture did not produce a counterexample");await page.getByRole("button",{name:"New test case"}).click();if((await surface.getAttribute("data-claim-status"))!=="disproved")throw new Error("Varied trial failed to retain midpoint counterexample");
    await page.getByRole("button",{name:"Clear",exact:true}).click();await statement.fill("A translation preserves the length of segment AB.");await page.getByRole("button",{name:"Check with current construction"}).click();await page.getByRole("button",{name:"New test case"}).click();if((await surface.getAttribute("data-claim-status"))!=="likely"||(await surface.getAttribute("data-trial-count"))!=="2")throw new Error("True length conjecture was not supported across trials");
    await page.getByRole("radio",{name:"Conjecture challenge A"}).check();await page.getByRole("button",{name:"Check my answer"}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();await page.getByRole("radio",{name:"Conjecture challenge C"}).check();await page.getByRole("button",{name:"Check my answer"}).click();status=await page.getByRole("status").filter({hasText:"Correct. Translation moves"}).innerText();
  } else if (id === 251) {
    ...
    const surface=page.locator(selector),contact=page.locator('[data-testid="envelope-contact-point"]');
    if((await surface.getAttribute("data-object-model"))!=="parameterized-tangent-family-parabola-envelope"||(await surface.getAttribute("data-slope"))!=="1.000"||(await surface.getAttribute("data-contact-x"))!=="0.500"||(await surface.getAttribute("data-contact-y"))!=="0.250"||(await surface.getAttribute("data-line-intercept"))!=="-0.250"||(await surface.getAttribute("data-trace-count"))!=="23")throw new Error("Envelope initial tangent-family model is invalid");
    const slopeBefore=await surface.getAttribute("data-slope"),box=await contact.boundingBox();if(!box)throw new Error("Envelope contact point is not draggable");await page.mouse.move(box.x+box.width/2,box.y+box.height/2);await page.mouse.down();await page.mouse.move(box.x+36,box.y-28,{steps:6});await page.mouse.up();if((await surface.getAttribute("data-slope"))===slopeBefore)throw new Error("Dragging tangency point did not recover a new family parameter");const cx=Number(await surface.getAttribute("data-contact-x")),cy=Number(await surface.getAttribute("data-contact-y"));if(Math.abs(cy-cx*cx)>.002)throw new Error("Dragged contact point left the detected envelope");
    await page.getByRole("spinbutton",{name:"m (slope) exact value"}).fill("2");if((await surface.getAttribute("data-contact-x"))!=="1.000"||(await surface.getAttribute("data-contact-y"))!=="1.000"||(await surface.getAttribute("data-line-intercept"))!=="-1.000")throw new Error("Slope control did not derive the correct tangent line");await page.getByRole("spinbutton",{name:"c (offset) exact value"}).fill("2");if((await surface.getAttribute("data-contact-y"))!=="3.000"||(await surface.getAttribute("data-envelope"))!=="y=x^2+2")throw new Error("Offset control did not shift line and envelope together");
    const traceBefore=await surface.getAttribute("data-trace-count");await page.getByRole("checkbox",{name:"Accumulated tangency traces"}).uncheck();await page.getByRole("spinbutton",{name:"m (slope) exact value"}).fill("3");if((await surface.getAttribute("data-trace-count"))!==traceBefore)throw new Error("Disabled trace accumulation still recorded contact points");await page.getByRole("checkbox",{name:"Accumulated tangency traces"}).check();await page.getByRole("spinbutton",{name:"m (slope) exact value"}).fill("4");if(Number(await surface.getAttribute("data-trace-count"))!==Number(traceBefore)+1)throw new Error("Enabled trace accumulation did not record contact point");await page.getByRole("button",{name:"CLEAR TRACES"}).click();if((await surface.getAttribute("data-trace-count"))!=="0")throw new Error("Clear traces did not clear accumulated contacts");
    await page.getByRole("button",{name:"Reset",exact:true}).first().click();await page.getByRole("radio",{name:"Envelope challenge B"}).check();await page.getByRole("button",{name:"Check Idea"}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();await page.getByRole("radio",{name:"Envelope challenge A"}).check();await page.getByRole("button",{name:"Check Idea"}).click();status=await page.getByRole("status").filter({hasText:"Correct! The envelope"}).innerText();
  } else if (id === 249) {
    const surface=page.locator(selector),point=page.locator('[data-testid="linkage-point-p"]');
    if((await surface.getAttribute("data-object-model"))!=="fixed-foci-flexible-tether-ellipse"||(await surface.getAttribute("data-valid"))!=="true"||(await surface.getAttribute("data-total-length"))!=="9.000"||(await surface.getAttribute("data-focus-distance"))!=="7.000"||(await surface.getAttribute("data-semi-major"))!=="4.500"||Math.abs(Number(await surface.getAttribute("data-semi-minor"))-Math.sqrt(8))>.001||(await surface.getAttribute("data-sum-distance"))!=="9.000")throw new Error("Moving linkage initial ellipse model is invalid");
    const thetaBefore=await surface.getAttribute("data-theta"),pointBox=await point.boundingBox();if(!pointBox)throw new Error("Linkage point P is not draggable");await page.mouse.move(pointBox.x+pointBox.width/2,pointBox.y+pointBox.height/2);await page.mouse.down();await page.mouse.move(pointBox.x-45,pointBox.y+38,{steps:7});await page.mouse.up();if((await surface.getAttribute("data-theta"))===thetaBefore||Math.abs(Number(await surface.getAttribute("data-sum-distance"))-9)>.001)throw new Error("Dragging P did not move on the constant-sum ellipse");
    await page.getByRole("button",{name:"Animate",exact:true}).click();const animatedBefore=await surface.getAttribute("data-theta");await page.waitForTimeout(220);if((await surface.getAttribute("data-theta"))===animatedBefore)throw new Error("Linkage animation did not advance theta");await page.getByRole("button",{name:"Pause",exact:true}).click();const paused=await surface.getAttribute("data-theta");await page.waitForTimeout(150);if((await surface.getAttribute("data-theta"))!==paused)throw new Error("Linkage animation did not pause");
    await page.getByRole("spinbutton",{name:"Link AB exact value"}).fill("2");await page.getByRole("spinbutton",{name:"Link BC exact value"}).fill("2");if((await surface.getAttribute("data-valid"))!=="false"||await page.locator('[data-testid="linkage-locus"]').count())throw new Error("Invalid short tether still rendered a closed ellipse");await page.getByRole("spinbutton",{name:"Link AB exact value"}).fill("4");await page.getByRole("spinbutton",{name:"Link BC exact value"}).fill("5");
    await page.getByRole("spinbutton",{name:"Translate x exact value"}).fill("2");await page.getByRole("spinbutton",{name:"Translate y exact value"}).fill("-1");await page.getByRole("spinbutton",{name:"Scale exact value"}).fill("2");await page.getByRole("spinbutton",{name:"Rotation (°) exact value"}).fill("90");if((await surface.getAttribute("data-center-x"))!=="2.000"||(await surface.getAttribute("data-center-y"))!=="-1.000"||(await surface.getAttribute("data-rendered-major"))!=="9.000"||Math.abs(Number(await surface.getAttribute("data-sum-distance"))-18)>.001)throw new Error("Linkage transform controls did not update ellipse geometry");
    await page.getByRole("button",{name:"Reset",exact:true}).first().click();await page.getByRole("radio",{name:"Linkage practice A"}).check();await page.getByRole("button",{name:"Check",exact:true}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();await page.getByRole("radio",{name:"Linkage practice B"}).check();await page.getByRole("button",{name:"Check",exact:true}).click();status=await page.getByRole("status").filter({hasText:"Correct! For total"}).innerText();
  } else if (id === 248) {
    const surface=page.locator(selector),pointA=page.locator('[data-testid="equidistant-point-a"]'),pointB=page.locator('[data-testid="equidistant-point-b"]'),anchor=page.locator('[data-testid="equidistant-anchor-p"]');
    if((await surface.getAttribute("data-object-model"))!=="dependent-perpendicular-bisector-equal-distance"||(await surface.getAttribute("data-ap"))!=="3.0000"||(await surface.getAttribute("data-bp"))!=="3.0000"||(await surface.getAttribute("data-difference"))!=="0.0000"||!(await surface.getAttribute("data-equation"))?.includes("x"))throw new Error("Equidistant initial perpendicular-bisector model is invalid");
    const aBox=await pointA.boundingBox();if(!aBox)throw new Error("Equidistant point A is not draggable");await page.mouse.move(aBox.x+aBox.width/2,aBox.y+aBox.height/2);await page.mouse.down();await page.mouse.move(aBox.x+36,aBox.y-18,{steps:6});await page.mouse.up();if((await surface.getAttribute("data-difference"))!=="0.0000")throw new Error("Dragging A did not keep dependent P on the new locus");
    const bBox=await pointB.boundingBox();if(!bBox)throw new Error("Equidistant point B is not draggable");await page.mouse.move(bBox.x+bBox.width/2,bBox.y+bBox.height/2);await page.mouse.down();await page.mouse.move(bBox.x-18,bBox.y-36,{steps:6});await page.mouse.up();if((await surface.getAttribute("data-difference"))!=="0.0000")throw new Error("Dragging B broke the equal-distance invariant");
    const pBefore=await anchor.getAttribute("data-y"),pBox=await anchor.boundingBox();if(!pBox)throw new Error("Equidistant locus anchor P is not draggable");await page.mouse.move(pBox.x+pBox.width/2,pBox.y+pBox.height/2);await page.mouse.down();await page.mouse.move(pBox.x+18,pBox.y-54,{steps:6});await page.mouse.up();if((await anchor.getAttribute("data-y"))===pBefore||(await surface.getAttribute("data-difference"))!=="0.0000")throw new Error("Dragging P did not move along the perpendicular bisector");
    await page.getByRole("spinbutton",{name:"A (x, y) x coordinate"}).fill("-4");await page.getByRole("spinbutton",{name:"A (x, y) y coordinate"}).fill("2");await page.getByRole("spinbutton",{name:"B (x, y) x coordinate"}).fill("2");await page.getByRole("spinbutton",{name:"B (x, y) y coordinate"}).fill("6");const equation=await surface.getAttribute("data-equation");if(!equation?.includes("3x")||!equation.includes("2y")||!equation.includes("5"))throw new Error("Endpoint controls did not derive the exact perpendicular-bisector equation");
    await page.getByRole("spinbutton",{name:"Equidistant translate x"}).fill("1");await page.getByRole("spinbutton",{name:"Equidistant translate y"}).fill("-1");if((await surface.getAttribute("data-midpoint"))!=="0.000,3.000"||(await surface.getAttribute("data-difference"))!=="0.0000")throw new Error("Translation did not preserve midpoint-dependent equal distances");
    await page.getByRole("checkbox",{name:"Equidistant grid"}).uncheck();await page.getByRole("checkbox",{name:"Equidistant axes"}).uncheck();await page.getByRole("checkbox",{name:"Equidistant labels"}).uncheck();await page.getByRole("button",{name:"Equidistant (angle)",exact:false}).click();await page.getByRole("button",{name:"Reset",exact:true}).first().click();
    await page.getByRole("radio",{name:"Practice equation A"}).check();await page.getByRole("button",{name:"Check answer"}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();await page.getByRole("radio",{name:"Practice equation B"}).check();await page.getByRole("button",{name:"Check answer"}).click();status=await page.getByRole("status").filter({hasText:"Correct! The perpendicular"}).innerText();
  } else if (id === 247) {
    const surface=page.locator(selector),anchor=page.locator('[data-testid="locus-anchor"]'),moving=page.locator('[data-testid="locus-moving-point"]');
    if((await surface.getAttribute("data-object-model"))!=="anchor-radius-transformed-circle-locus"||(await surface.getAttribute("data-center-x"))!=="2.000"||(await surface.getAttribute("data-center-y"))!=="1.000"||(await surface.getAttribute("data-radius"))!=="4.000"||(await surface.getAttribute("data-point-x"))!=="6.000"||(await surface.getAttribute("data-distance"))!=="4.000")throw new Error("Locus Generator initial circle model is invalid");
    const radiusBefore=await surface.getAttribute("data-radius"),pointBefore=await moving.getAttribute("data-x"),anchorBox=await anchor.boundingBox();if(!anchorBox)throw new Error("Locus anchor A is not draggable");await page.mouse.move(anchorBox.x+anchorBox.width/2,anchorBox.y+anchorBox.height/2);await page.mouse.down();await page.mouse.move(anchorBox.x+31,anchorBox.y-31,{steps:6});await page.mouse.up();if((await surface.getAttribute("data-radius"))!==radiusBefore||(await moving.getAttribute("data-x"))===pointBefore)throw new Error("Dragging anchor did not translate locus while preserving radius");
    const movingBox=await moving.boundingBox();if(!movingBox)throw new Error("Locus moving point P is not draggable");await page.mouse.move(movingBox.x+movingBox.width/2,movingBox.y+movingBox.height/2);await page.mouse.down();await page.mouse.move(movingBox.x+31,movingBox.y-31,{steps:6});await page.mouse.up();if((await surface.getAttribute("data-radius"))===radiusBefore||Number(await surface.getAttribute("data-trace-count"))<1)throw new Error("Dragging P did not update radius and trace history");
    await page.getByRole("spinbutton",{name:"A x (h) exact value"}).fill("1");await page.getByRole("spinbutton",{name:"A y (k) exact value"}).fill("-2");await page.getByRole("spinbutton",{name:"Radius r exact value"}).fill("3");await page.getByRole("spinbutton",{name:"Translate x exact value"}).fill("2");await page.getByRole("spinbutton",{name:"Translate y exact value"}).fill("1");await page.getByRole("spinbutton",{name:"Scale s exact value"}).fill("2");if((await surface.getAttribute("data-center-x"))!=="3.000"||(await surface.getAttribute("data-center-y"))!=="-1.000"||(await surface.getAttribute("data-radius"))!=="6.000")throw new Error("Locus transform controls did not update center and scaled radius");
    await page.getByRole("button",{name:"pause-trace"}).click();const pausedBox=await moving.boundingBox();if(!pausedBox)throw new Error("Moving point disappeared while trace paused");await page.mouse.move(pausedBox.x+pausedBox.width/2,pausedBox.y+pausedBox.height/2);await page.mouse.down();await page.mouse.move(pausedBox.x-20,pausedBox.y+20,{steps:5});await page.mouse.up();if((await surface.getAttribute("data-trace-count"))!=="0")throw new Error("Paused trace still recorded points");
    await page.getByRole("button",{name:"Reset",exact:true}).first().click();for(const [name,value] of [["Practice A x (h)","0"],["Practice A y (k)","0"],["Practice Radius r","1"],["Practice Translate x","2"],["Practice Translate y","2"],["Practice Scale s","2"]])await page.getByRole("textbox",{name}).fill(value);await page.getByRole("button",{name:"Check",exact:true}).click();await page.getByRole("status").filter({hasText:"Not yet"}).waitFor();
    for(const [name,value] of [["Practice A x (h)","-2"],["Practice A y (k)","-1"],["Practice Radius r","2.5"],["Practice Translate x","0"],["Practice Translate y","0"],["Practice Scale s","1"]])await page.getByRole("textbox",{name}).fill(value);await page.getByRole("button",{name:"Check",exact:true}).click();status=await page.getByRole("status").filter({hasText:"Correct! Great"}).innerText();
  } else if (id === 246) {
