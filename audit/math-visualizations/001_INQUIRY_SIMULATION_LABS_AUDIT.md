# Audit Report - Inquiry Simulation Labs

## 1. Basic Information

| Field | Details |
|---|---|
| Visualization Name | Inquiry Simulation Labs |
| Concept Area | Math |
| File Path | src/components/inquiry/InquirySimulationLabs.tsx |
| Route / Page | / |
...
- Source file was inspected: src/components/inquiry/InquirySimulationLabs.tsx (210 lines).
- Rendering/interaction evidence: SVG, sliders, inputs, formula logic/text.
...
| Formula edge cases are not clearly guarded in source. | Undefined or impossible math states can mislead learners if rendered as normal results. | Medium | Add explicit guards and student-facing messages for zero, invalid, and out-of-domain states. |
...
If interaction is indirect, provide presets or example buttons. Sliders should show min, max, current value, and the exact math quantity being changed.
...
- src/components/inquiry/InquirySimulationLabs.tsx
...
|---|---|---|---|
| Visualization is placeholder or not educationally complete. | src/components/inquiry/InquirySimulationLabs.tsx | High impact on correctness, learning, accessibility, or maintenance. | Add a complete visual model, guided task, formula connection, and tested interactions. |
| Formula edge cases are not clearly guarded in source. | src/components/inquiry/InquirySimulationLabs.tsx | Medium impact on correctness, learning, accessibility, or maintenance. | Add explicit guards and student-facing messages for zero, invalid, and out-of-domain states. |
| No adjacent automated test found. | src/components/inquiry/InquirySimulationLabs.tsx | Medium impact on correctness, learning, accessibility, or maintenance. | Add math-kernel tests and a render smoke test for default and edge states. |
...
1. What do you think will change in Inquiry Simulation Labs before you move the control?
...
- Date: 2026-06-30
- Changed files: `src/components/inquiry/InquirySimulationLabs.tsx`
- Update summary: refreshed the inquiry lab presentation as part of the 2026 welcome-screen UI pass, improving contrast, spacing, panel hierarchy, and action clarity.
- Verification: included in `npm run lint`, `npm run build`, and `npx vitest run --maxWorkers=1 --reporter=dot`.
- Remaining limitation: the original Grade 6 recommendation still applies until the lab adds stronger guided narration and keyboard-first interaction checks.

## Phase 2 Upgrade Notes

- Date: 2026-07-01
- Changed files: `src/components/inquiry/InquirySimulationLabs.tsx`, `src/components/ui/LearningScaffolds.tsx`, `src/components/inquiry/InquirySimulationLabs.test.tsx`
- Update summary: added a Grade 6-oriented task scaffold with try-first, predict, observe, explain, and common-mistake prompts; added teacher notes, screen-reader live summaries, radio semantics for prediction choices, and SVG title/description text for the slope diagram.
...
- Remaining limitation: the inquiry lab is improved for guided classroom use, but the simulations still need deeper keyboard-only interaction testing and separate math-kernel tests for every lab type.
