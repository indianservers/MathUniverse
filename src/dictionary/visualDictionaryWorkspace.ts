...
export type DictionaryProgress = Record<
  string,
  { viewed?: string; practiced?: string; mastered?: string }
>;
...
export function dictionaryRangeForLetter(
  letter: string,
): DictionaryLetterRange | "All" {
...
export function findDictionaryTerm(
  ...
  return terms.find(
    (entry) =>
      ...
      dictionarySlug(entry.term) === normalized,
  );
}
...
export function filterDictionaryTerms(
  ...
  return terms
    .map((entry, index) => ({
      entry,
      index,
      score: searchScore(entry, filters.query ?? ""),
    }))
    .filter(
      ({ entry, score }) =>
        score > 0 &&
        (!filters.letter ||
          filters.letter === "All" ||
          entry.term.toUpperCase().startsWith(filters.letter)) &&
        (!filters.category ||
...
export function relatedDictionaryTerms(
  terms: VisualDictionaryTerm[],
  selected: VisualDictionaryTerm,
  limit = 5,
) {
  ...
  return terms
    ...
    .map((entry) => ({
      entry,
      score:
        (entry.category === selected.category ? 5 : 0) +
        (entry.kind === selected.kind ? 3 : 0) +
        entry.keywords.filter((keyword) => selectedKeywords.has(keyword))
          .length,
    }))
