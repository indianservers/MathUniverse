...
} from "./visualDictionaryWorkspace";
...
describe("visual dictionary workspace", () => {
  it("creates stable slugs and resolves direct URL selections", () => {
    ...
    expect(
      findDictionaryTerm(visualDictionaryTerms, "pythagorean-theorem")?.term,
    ).toBe("Pythagorean theorem");
  });
  ...
  it("ranks exact and partial term searches before keyword matches", () => {
    expect(
      filterDictionaryTerms(visualDictionaryTerms, { query: "matrix" })[0].term,
    ).toBe("Matrix");
    expect(
      filterDictionaryTerms(visualDictionaryTerms, { query: "pythag" })[0].term,
    ).toBe("Pythagorean theorem");
  });

  it("searches formula and example content", () => {
    const matches = filterDictionaryTerms(visualDictionaryTerms, {
      query: "det(A - lambda I)",
    });
    expect(
      matches.some((entry) => entry.term === "Characteristic equation"),
    ).toBe(true);
  });
  ...
  it("combines letter, category, and visual filters", () => {
    ...
    expect(
      matches.every(
        (entry) =>
          entry.term.startsWith("C") &&
          entry.category === "Geometry" &&
          entry.kind === "circle",
      ),
    ).toBe(true);
  });
  ...
  it("returns contextual related terms without the selected term", () => {
    const selected = visualDictionaryTerms.find(
      (entry) => entry.term === "Abacus",
    )!;
    ...
    expect(related.some((entry) => entry.category === selected.category)).toBe(
      true,
    );
  });
});
