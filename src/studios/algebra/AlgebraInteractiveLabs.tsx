...
import { useEffect, useMemo, useState, type DragEvent, type PointerEvent, type ReactNode } from "react";
...
export function Plot({ expressions, shade = [], dashed = [], handles }: { expressions: string[]; shade?: Array<"above" | "below" | null>; dashed?: boolean[]; handles?: { h: number; k: number; a: number; onChange: (next: { h?: number; k?: number; a?: number }) => void } }) {
  ...
  return (
    <div className="alg-graph-stack">
    <StudioGraphWidget expressions={expressions} view={{ xMin: -6, xMax: 6, yMin: -8, yMax: 8 }} />
    <svg
      className="alg-graph"
      data-studio-graph="1"
      viewBox="0 0 700 380"
      ...
      onPointerMove={(event: PointerEvent<SVGSVGElement>) => {
        ...
        const worldX = (x - 350) / 50;
        const worldY = (190 - y) / 11;
        const hx = 350 + handles.h * 50;
        const ky = 190 - handles.k * 11;
        if (Math.hypot(x - hx, y - ky) < 36) handles.onChange({ h: Math.max(-5, Math.min(5, worldX)), k: Math.max(-8, Math.min(8, worldY)) });
        else if (Math.abs(x - hx) < 28) handles.onChange({ a: Math.max(-4, Math.min(4, worldY - handles.k)) });
      }}
    >
      ...
      {handles ? (
        <>
          <circle cx={350 + handles.h * 50} cy={190 - handles.k * 11} r="8" fill="#10b981" />
          <text x={358 + handles.h * 50} y={186 - handles.k * 11} fontSize="12" fill="#047857">h,k</text>
          <circle cx={350 + handles.h * 50} cy={190 - (handles.a + handles.k) * 11} r="8" fill="#0891b2" />
          <text x={358 + handles.h * 50} y={186 - (handles.a + handles.k) * 11} fontSize="12" fill="#0e7490">A</text>
        </>
      ) : null}
    </svg>
    </div>
...
export function Challenge({ expected, prompt, onNew }: { expected: number | string; prompt: string; onNew?: () => void }) {
...
export function CasGateway() {
  ...
  const figureOk = candidate.trim() !== "" && (expressionsEquivalent(draft.split("=")[0] || draft, candidate) || (result ? expressionsEquivalent(result, candidate) : false));
  const compute = () => {
  ...
  return <div className="alg-page alg-dash-page" data-mode-canvas={mode}><AlgebraLabHeading subtitle="Solve, simplify, factor, expand, and substitute with the connected CAS." modes={modes} mode={mode} onMode={(next) => { setMode(next); history.replace({ ...history.state, result: "" }); }} onUndo={history.undo} onRedo={history.redo} canUndo={history.canUndo} canRedo={history.canRedo} onReset={() => history.reset()} helpTitle={`${mode} help`} helpBody="The CAS never uses eval(). Invalid expressions return an error. History stores each successful or failed compute.">CAS Step Explorer</AlgebraLabHeading><p role="status">{focus}</p><div className="alg-cas-notice"><b>Connected to the existing CAS workspace</b><Link to="/workspace/data/cas">Open CAS Workspace</Link></div><div className="alg-dash-banner" data-lab-mode={mode}><b>Step explorer · {mode}</b><small>Each compute rewrites the expression with the same engine as the CAS workspace. Click a history row to restore it.</small></div><div className="alg-dash-layout alg-cas-layout"><Card title="CAS expression"><label className="alg-field">CAS expression<input value={draft} onChange={(e) => history.replace({ ...history.state, draft: e.target.value, result: "" })} onKeyDown={(e) => { if (e.key === "Enter") compute(); }} /></label>{mode === "Substitute" && <Numeric label="Substitute x" value={x} onChange={(v) => history.replace({ ...history.state, x: v, result: "" })} />}<button type="button" className="alg-gradient-button" onClick={compute}>Compute {mode}</button><label className="alg-field">Candidate to check against the figure<input value={candidate} onChange={(e) => setCandidate(e.target.value)} placeholder="2*(x-5)*(x+1)" /></label><p>{candidate ? (figureOk ? "Candidate matches the figure." : "Not equivalent to the current expression.") : "Enter a candidate to check against the graph."}</p></Card><Card title={`${mode} result`}><Result>{result || "Enter an expression and compute."}</Result><ol className="alg-cas-steps">{entries.map((entry) => <li key={entry}><button type="button" onClick={() => { const parts = entry.split(" → "); const left = parts[0] ?? ""; const expr = left.slice(left.indexOf(":") + 1).trim(); history.replace({ ...history.state, draft: expr || draft, result: parts[1] ?? result }); }}>{entry}</button></li>)}</ol><button type="button" onClick={() => history.commit({ ...history.state, entries: [], result: "" })}>Clear history</button></Card><Card title="Expression graph"><Plot expressions={[draft.split("=")[0] || "0"]} /></Card></div><Challenge expected="2*(x-5)*(x+1)" prompt="Factor 2x²−8x−10, or enter an equivalent factored form." onNew={() => history.commit({ ...history.state, draft: "x^2-5*x+6" })} /><LabStrip onSelect={(title) => { if (title === "Try") { const answer = runCasOperation("2*x^2-8*x-10", "factor"); history.commit({ ...history.state, draft: "2*x^2-8*x-10", result: answer.ok ? answer.output : answer.output }); } setFocus(title === "Challenge" ? "Check a candidate by substituting." : title === "Try" ? "Factor 2x²−8x−10." : title === "Why" ? "Equivalent forms share values." : title === "Understand" ? "CAS applies the same algebra laws." : "Each operation rewrites the expression."); }} items={[["Observe", "Each operation rewrites the expression."], ["Understand", "CAS applies the same algebra laws."], ["Why", "Equivalent forms share values."], ["Try", "Factor 2x²−8x−10."], ["Challenge", "Check a candidate by substituting."]]} /></div>;
}
