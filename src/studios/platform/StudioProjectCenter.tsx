...
import {
  ...
  redoCheckpoint, responsiveStudioLayout, transferStudioObject, undoCheckpoint, type ProjectHistory,
...
export default function StudioProjectCenter() {
  const [title, setTitle] = useState("Cross-studio investigation");
  const [viewport, setViewport] = useState(960);
  const [points, setPoints] = useState(80_000);
  const [answer, setAnswer] = useState(1);
  const [history, setHistory] = useState<ProjectHistory<{ title: string }>>(() => createHistory({ title: "Cross-studio investigation" }));
  const project = useMemo(() => {
    const next = createStudioProject(title, "algebra", new Date("2026-09-04T00:00:00Z"));
  ...
  const saveProject = () => localStorage.setItem("math-universe:shared-project", exportStudioProject(project));

  return <StudioPageShell title="Studio Project Center" subtitle="Move mathematical work between studios with reproducible projects, checkpoints, accessible controls, and adaptive computation." breadcrumbs={["Home", "Studio", "Project Center"]} difficulty="All levels" estimatedMinutes={20} status={[{ id: "platform", label: "Shared capabilities", value: 10, tone: "green" }]}>
    <div className="space-y-4">
      ...
      <section className="grid gap-3 md:grid-cols-2 xl:grid-cols-3" aria-label="Ten shared studio enhancements">
        <Capability id="PLATFORM-01" title="1. Shared project format"><p className="mt-2 text-xs text-slate-600">One versioned JSON project carries objects and parameters across every studio.</p><button type="button" onClick={saveProject} className="mt-3 rounded-lg bg-slate-900 px-3 py-2 text-xs font-bold text-white">Save project locally</button><Output>{project}</Output></Capability>
        <Capability id="PLATFORM-02" title="2. Shareable deep links"><p className="mt-2 text-xs text-slate-600">The complete project is encoded in the URL, not hidden server state.</p><button type="button" onClick={() => void navigator.clipboard?.writeText(deepLink)} className="mt-3 rounded-lg border px-3 py-2 text-xs font-bold">Copy project link</button><Output>{deepLink}</Output></Capability>
        ...
        <Capability id="PLATFORM-05" title="5. Named undo checkpoints"><div className="mt-3 flex flex-wrap gap-2"><button type="button" className="rounded-lg border px-3 py-2 text-xs font-bold" onClick={() => setHistory((current) => checkpoint(current, { title }, `Rename to ${title}`))}>Checkpoint</button><button type="button" className="rounded-lg border px-3 py-2 text-xs font-bold" onClick={() => setHistory(undoCheckpoint)}>Undo</button><button type="button" className="rounded-lg border px-3 py-2 text-xs font-bold" onClick={() => setHistory(redoCheckpoint)}>Redo</button></div><Output>{history}</Output></Capability>
