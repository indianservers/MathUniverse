...
export function geogebraCasParitySummary() {
  ...
  const byPriority = countBy(
    report.filter((entry) => entry.status === "missing"),
...
export function missingGeoGebraCasCommands(
  priority?: GeoGebraParityEntry["priority"],
) {
  return geogebraCasParityReport().filter(
    (entry) =>
      entry.status === "missing" && (!priority || entry.priority === priority),
  );
}
...
function parityEntry(command: string): GeoGebraParityEntry {
  ...
  if (direct) {
    return {
      ...
      note:
        direct.name === command
          ? "Implemented with the same command name."
          : `Covered by local command or alias ${direct.name}.`,
    };
  }
...
function parityCategory(
  command: string,
  local?: CasCommandSpec,
): GeoGebraParityEntry["category"] {
  ...
  if (
    /Factor|Root|Complex|ToComplex|ToExponential|ToPolar|ToPoint/.test(command)
  )
    return "complex";
  if (
    /Matrix|Eigen|Jordan|LU|QR|SVD|Characteristic|Minimal|Dimension/.test(
      command,
    )
  )
    return "linear-algebra";
  if (
    /Cauchy|ChiSquared|Exponential|Distribution|Gamma|HyperGeometric|Pascal|Poisson|Normal|Weibull|Zipf/.test(
      command,
    )
  )
    return "probability";
  if (/Fit|Covariance|Sample|Variance|Mean|Median|SD/.test(command))
    return "statistics";
  if (/Delete|Element|First|Last|Sequence|Shuffle|Take|Unique/.test(command))
    return "lists-data";
  ...
  if (/Integral|Derivative|Limit|SolveODE|Taylor|Laplace/.test(command))
    return "calculus";
  return "algebra";
}
...
function parityPriority(command: string): GeoGebraParityEntry["priority"] {
  if (
    /CFactor|CIFactor|ComplexRoot|RootList|SolveCubic|SolveQuartic|CharacteristicPolynomial|LUDecomposition|QRDecomposition|SVD|JordanDiagonalization|Poisson|Exponential|Gamma|ChiSquared|TDistribution|FDistribution/.test(
      command,
    )
  )
    return "P0";
  if (
    /Groebner|Random|Fit|Sequence|Element|First|Last|Take|Unique|ExtendedGCD|ModularExponent|HyperGeometric|Pascal|Weibull|Zipf|Cauchy/.test(
      command,
    )
  )
    return "P1";
...
function countBy<T extends string>(
  items: GeoGebraParityEntry[],
  key: (item: GeoGebraParityEntry) => T,
) {
