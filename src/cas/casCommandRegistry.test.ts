...
import {
  casCommandArgumentRange,
  casCommandCatalog,
  casCommandRegistrySummary,
...
describe("casCommandRegistry", () => {
  it("tracks implemented, partial, and planned CAS command coverage", () => {
    ...
    expect(summary.byCategory["Equation Solving"].implemented).toBeGreaterThan(
      5,
    );
  ...
  it("normalizes GeoGebra-style aliases and searches metadata", () => {
    ...
    expect(searchCasCommands("laplace").map((item) => item.name)).toContain(
      "Laplace",
    );
  });
  ...
  it("keeps commands discoverable with examples and signatures", () => {
    expect(
      casCommandCatalog.every(
        (command) =>
          command.signature && command.description && command.examples.length,
      ),
    ).toBe(true);
  });
  ...
  it("suggests close commands and validates argument counts", () => {
    expect(
      suggestCasCommands("Derivitive").map((command) => command.name),
    ).toContain("Derivative");
    expect(casCommandArgumentRange("Solve")?.label).toBe("1 to 2 arguments");
    expect(validateCasCommandArguments("Factor", []).errors[0]).toContain(
      "Factor expects 1 argument",
    );
    expect(
      validateCasCommandArguments("Substitute", ["x^2"]).errors[0],
    ).toContain("at least one assignment");
    expect(
      validateCasCommandArguments("Determinant", ["[1,2]", "[3,4]"]).errors,
    ).toEqual([]);
  });
});
