...
export function convertFromMeters(value: number, unit: ARUnit) {
  if (!Number.isFinite(value)) {
    throw new Error("Measurement value must be a finite number.");
  }
...
export function convertARMeasurement(value: number, from: ARUnit, to: ARUnit) {
  return convertFromMeters(convertToMeters(value, from), to);
}
