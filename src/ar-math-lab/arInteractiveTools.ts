...
export const arLessonCards: ARLessonCard[] = [
  {
    ...
    keyObservations: [
      "Peaks occur when sin(x) and sin(y) have the same sign.",
      "Valleys occur when one is positive and the other is negative.",
      "The surface repeats periodically.",
    ],
    tryThis: [
      "Increase amplitude a.",
      "Increase frequency k.",
      "Turn on wireframe.",
      "View from top to see the pattern.",
    ],
  ...
  },
  {
    ...
    keyObservations: [
      "Minimum point is at the origin.",
      "z increases as distance from origin increases.",
      "Horizontal cross-sections are circles.",
    ],
    tryThis: [
      "Show horizontal cross-section.",
      "Change z-scale.",
      "Compare with z = x^2 - y^2.",
    ],
  },
  {
    id: "surface-saddle",
  ...
  },
  {
    ...
    title: "Cone",
    concept:
      "A cone has a circular base and a curved surface meeting at one vertex.",
    keyObservations: [
      "Slant height depends on radius and height.",
  ...
  },
  {
    ...
    keyObservations: [
      ...
      "Horizontal cross-section is a circle.",
      "Vertical cross-section is a rectangle.",
    ],
    tryThis: [
      "Animate radius.",
      "Show curved surface area.",
      "Compare with cone of the same radius and height.",
    ],
  ...
  },
  {
    ...
    title: "Sphere",
    concept:
      "A sphere is the set of all points at the same distance from a center.",
    keyObservations: [
      "Every radius is equal.",
      "Central cross-section is a circle.",
      "Volume grows with radius cubed.",
    ],
    tryThis: [
      "Animate radius.",
...
export function createObjectDimensionMeasurement(
  solid: ARGeneratedGeometrySolid,
  key: string,
): ARMeasurement {
  const value =
    solid.calculatedValues.values[key] ?? solid.dimensions[key]?.value ?? 0;
  const unit = key.toLowerCase().includes("area")
    ? `${solid.unit}^2`
  ...
  return {
    ...
    type: key.toLowerCase().includes("slant")
      ? "slant_height"
...
export function measurementLabel(
  ...
  if (type === "point_coordinate")
    return `Point coordinate y = ${format(value)}`;
  return `${readable(type)} = ${format(value)} ${unit}`;
}

export function createAnimation(
  objectId: string,
  target: ARAnimation["target"],
  property: string,
): ARAnimation {
  return {
...
export function lessonFor(
  ...
  if (solid)
    return (
      arLessonCards.find((card) => card.objectType === solid.solidType) ??
      arLessonCards[0]
    );
  const equation = graph?.equation.toLowerCase() ?? "";
  if (equation.includes("sin(x)") && equation.includes("sin(y)"))
...
export function feedbackFor(
  ...
  if (lower.includes("x^2 + y^2"))
    feedback.push(
      "This is a paraboloid. Try enabling horizontal cross-section to see circular level curves.",
    );
  if (
    /sin\s*\(\s*10/.test(lower) ||
    lower.includes("10x") ||
    lower.includes("10*x")
  )
    feedback.push(
      "High frequency detected. Increase resolution for better quality, or reduce frequency for smoother mobile performance.",
    );
  if (
    solid?.solidType === "cone" &&
    Math.abs((solid.calculatedValues.values.slantHeight ?? 0) - 13) < 0.01
  )
    feedback.push(
      "Slant height is 13 cm. This forms a 5-12-13 right triangle.",
    );
  if (solid && solid.maxDimensionMeters > 10)
    feedback.push(
      "This object is very large. Fit to View or Miniature Mode is recommended.",
    );
  if (
    graph &&
    (graph.settings.resolutionX > 100 || graph.settings.resolutionY > 100)
  )
    feedback.push(
      "High resolution may reduce performance on mobile. Recommended resolution: 60.",
    );
  if (!feedback.length)
    feedback.push(
      "Try measuring, animating, or comparing this object to reveal the underlying math.",
    );
  return feedback;
}
...
export function comparisonSummary(
  ...
  if (objectA.kind === "solid" && objectB.kind === "solid") {
    if (
      new Set([objectA.solid.solidType, objectB.solid.solidType]).has("cone") &&
      new Set([objectA.solid.solidType, objectB.solid.solidType]).has(
        "cylinder",
      )
    )
      return "Both objects can share the same radius and height. The cone occupies one-third of the cylinder's volume.";
    if (
      new Set([objectA.solid.solidType, objectB.solid.solidType]).has(
        "sphere",
      ) &&
      ...
      return "The hemisphere has exactly half the volume of a sphere with the same radius.";
  ...
  const eqA =
    objectA.kind === "graph" ? objectA.graph.equation : objectA.solid.name;
  const eqB =
    objectB.kind === "graph" ? objectB.graph.equation : objectB.solid.name;
  if (eqA.includes("x^2 + y^2") && eqB.includes("x^2 - y^2"))
    return "The paraboloid curves upward in all directions, while the saddle curves upward in one direction and downward in another.";
  return `Compare ${eqA} with ${eqB}. Check shape, formula, scale, and cross-sections.`;
}

export function serializeScene(
  data: Omit<ARSceneSaveData, "version" | "module" | "savedAt">,
): ARSceneSaveData {
  return {
    version: "1.0",
    module: "ARMathLab",
...
export function parseSceneJson(raw: string): ARSceneSaveData {
  ...
  if (
    parsed.module !== "ARMathLab" ||
    !Array.isArray(parsed.graphs) ||
    !Array.isArray(parsed.solids)
  )
    throw new Error(
      "Invalid scene file. Please import a valid AR Math Lab JSON scene.",
    );
  return {
...
function defaultMeasurementPoints(
  type: ARMeasurementType,
...
function firstFormulaResult(solid: ARGeneratedGeometrySolid) {
  return (
    solid.calculatedValues.formulas.find(
      (formula) => formula.label === "Volume",
    )?.result ??
    solid.calculatedValues.formulas[0]?.result ??
    "not calculated"
  );
}
