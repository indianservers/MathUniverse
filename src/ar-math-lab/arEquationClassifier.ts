...
const dimensionPattern =
  /\b(radius|diameter|height|length|width|depth|side)\s*[:=]?\s*(-?\d+(?:\.\d+)?)\s*(mm|millimeters?|cm|centimeters?|m|meters?|metres?|inch(?:es)?|in|ft|feet|foot)\b/gi;

export function classifyEquationInput(
  input: string,
): EquationClassificationResult {
  ...
  if (!normalizedInput) {
    return result(
      ...
      [],
      "3d-preview",
      "Please enter an equation. Example: z = sin(x) * sin(y).",
      { errors: ["Please enter an equation. Example: z = sin(x) * sin(y)."] },
    );
  }

  const solid = geometrySolids.find((candidate) =>
    new RegExp(`\\b${candidate}\\b`, "i").test(normalizedInput),
  );
  if (solid) {
    const geometry = parseGeometrySolidInput(normalizedInput);
    const flatDimensions = geometry.ok
      ? Object.entries(geometry.solid.dimensions).map(([name, dimension]) => ({
          name,
          ...dimension,
        }))
      : Object.entries(geometry.dimensions).map(([name, dimension]) => ({
          name,
          ...dimension,
        }));
    return {
      ...result(
          ...
          : geometry.message,
        {
          ...
          solidType: geometry.ok
            ? geometry.solid.solidType
            : geometry.solidType,
          solidDimensions: geometry.ok
            ? geometry.solid.dimensions
            : geometry.dimensions,
          missingInputs: geometry.ok ? [] : geometry.missingInputs,
          suggestedScaleMode: geometry.ok
            ? geometry.solid.displayScaleMode
            : "fit-to-view",
  ...
  if (/\b(coordinate|xyz|axes?|axis|grid)\b/i.test(normalizedInput)) {
    return result(
      ...
      "high",
      variables,
      extractDimensions(normalizedInput),
      "ar",
      "Detected a coordinate system for placing axes or a grid in real space.",
      { suggestedRenderer: "unsupported" },
    );
  }

  if (
    /\b(measure|measurement|distance|room|table|desk|wall|height)\b/i.test(
      normalizedInput,
    ) &&
    extractDimensions(normalizedInput).length > 0
  ) {
    return result(
      "measurement_demo",
      normalizedInput,
      ...
      extractDimensions(normalizedInput),
      "camera-preview",
      "Detected a measurement-based visualization. Camera preview is the best fallback.",
      { suggestedRenderer: "unsupported" },
    );
  }
  ...
  if (looksLikeParametricSurface(lower)) {
    return result(
      ...
      [],
      "3d-preview",
      "Detected a parametric surface x(u,v), y(u,v), z(u,v).",
      {
        ...
        educationalHint:
          "A parametric surface uses two inputs, u and v, to draw a sheet in 3D space.",
      },
    );
  }
  ...
  if (looksLikeParametricCurve(lower)) {
    return result(
      ...
      [],
      "3d-preview",
      "Detected a 3D parametric curve x(t), y(t), z(t).",
      {
        ...
        suggestedResolution: {
          t:
            lower.includes("sin(2t)") || lower.includes("sin(2*t)") ? 500 : 300,
        },
        suggestedParameters: suggestedParameterValues(parameters),
        educationalHint:
          "A parametric curve traces one moving point through 3D space as t changes.",
      },
    );
  }
  ...
  if (looksLikeExplicitSurface(lower)) {
    ...
    return result(
      ...
      [],
      "3d-preview",
      "Detected an explicit surface z = f(x,y). Generate Graph will create a mesh.",
      {
        ...
        suggestedResolution: {
          x: lower.includes("sqrt") ? 80 : lower.includes("^2") ? 50 : 60,
          y: lower.includes("sqrt") ? 80 : lower.includes("^2") ? 50 : 60,
        },
        suggestedParameters: suggestedParameterValues(parameters),
        educationalHint: explicitHint(lower),
        warnings:
          variables.includes("x") || variables.includes("y")
            ? []
            : [
                "For a 3D surface, use x and/or y as input variables, or define sliders for parameters.",
              ],
      },
    );
  }
  ...
  return result(
    ...
    "This equation type is not supported yet. Try z = f(x, y) or a parametric equation.",
    {
      parameters,
      suggestedRenderer: "unsupported",
      errors: ["Unsupported equation type."],
    },
  );
}
  ...
  return Array.from(
    new Set(
      Array.from(input.matchAll(/\b[A-Za-z]\b/g))
        ...
        .filter(
          (name) =>
            !variableSet.has(name.toLowerCase()) &&
            !knownFunctions.has(name.toLowerCase()),
        ),
    ),
  ).sort();
}
...
function inferExplicitRanges(
  lower: string,
): EquationClassificationResult["suggestedRanges"] {
...
function inferCurveRange(lower: string): [number, number] {
  ...
  if (lower.includes("sin(2t)") || lower.includes("sin(2*t)"))
    return [0, Math.PI * 2];
  return [0, Math.PI * 6];
}

function inferParametricSurfaceRanges(
  lower: string,
): EquationClassificationResult["suggestedRanges"] {
  if (lower.includes("r * sin(u)") || lower.includes("r*sin(u)"))
    return { u: [0, Math.PI], v: [0, Math.PI * 2] };
  if (lower.includes("z = v") || lower.includes("z=v"))
    return { u: [0, Math.PI * 2], v: [-2, 2] };
  return { u: [0, Math.PI * 2], v: [0, Math.PI * 2] };
}

function explicitHint(lower: string) {
  if (lower.includes("sin(x") && lower.includes("sin(y"))
    return "This is a wave surface. The height z changes based on both x and y.";
  if (lower.includes("x^2 + y^2"))
    return "This is a paraboloid. As x and y move away from zero, z increases, creating a bowl shape.";
  if (lower.includes("x^2 - y^2"))
    return "This is a saddle surface. The surface curves upward in one direction and downward in another.";
...
function classifyRecognizedImplicit(lower: string): {
  ...
  if (
    /x\^2\s*\+\s*y\^2\s*=/.test(lower) &&
    !/\bz\b/.test(lower.split("=")[0])
  ) {
    return {
    ...
    return [
      {
        ...
        value,
        unit,
        meters: convertToMeters(value, unit),
      },
    ];
  });
}
...
function collectVariables(input: string) {
  const candidates = new Set(
    Array.from(input.matchAll(/\b[xytzuv]\b/gi)).map((match) =>
      match[0].toLowerCase(),
    ),
  );
