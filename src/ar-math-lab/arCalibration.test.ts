...
import {
  ...
  devicePointToMath,
  mathPointToDevice,
  validateARCalibration,
  type ARCalibration,
} from "./arCalibration";
import {
  convertARMeasurement,
  convertFromMeters,
  convertToMeters,
} from "./arUnits";
...
describe("AR units and calibration", () => {
  it("round-trips every supported unit through metres", () => {
    ...
    for (const unit of units)
      expect(
        convertFromMeters(convertToMeters(12.345, unit), unit),
      ).toBeCloseTo(12.345, 12);
  ...
  it("round-trips points between explicit math and device frames", () => {
    ...
    expect(
      devicePointToMath(mathPointToDevice(mathPoint, calibration), calibration),
    ).toEqual(mathPoint);
  });
  ...
  it("reports RMS calibration error in physical metres", () => {
    ...
    const observed = references.map((point) =>
      mathPointToDevice(point, calibration),
    );
    expect(calibrationResidual(references, observed, calibration)).toBeCloseTo(
      0,
      12,
    );
  });
  ...
  it("rejects invalid scale and non-orthonormal axes", () => {
    expect(() =>
      validateARCalibration({ ...calibration, metersPerDeviceUnit: 0 }),
    ).toThrow(/positive/);
    expect(() =>
      validateARCalibration({
        ...calibration,
  ...
  it("discloses whether measurements are verified or illustrative", () => {
    ...
    expect(
      calibrationDisclosure({ ...calibration, source: "device-estimate" }),
    ).toMatchObject({ status: "approximate", label: "Approximate scale" });
  });
});
