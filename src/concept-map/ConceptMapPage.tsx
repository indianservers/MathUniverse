...
export default function ConceptMapPage() {
  ...
  const path = useMemo(
    () => findLearningPath(pathStartId, selectedId),
    [pathStartId, selectedId],
  );
  const pathIds = useMemo(
    () => new Set(pathActive ? path.map((concept) => concept.id) : []),
    [path, pathActive],
  );
  const focusIds = useMemo(
    () =>
      selectedId ? getImmediateConnectionIds(selectedId) : new Set<string>(),
    [selectedId],
  );
  ...
  const visibleConcepts = useMemo(() => {
    ...
    if (showPrerequisites) {
      const ids = new Set([
        selectedId,
        ...getPrerequisites(selectedId).map((concept) => concept.id),
        ...pathIds,
      ]);
    ...
    if (showNext) {
      const ids = new Set([
        selectedId,
        ...(selectedConcept?.nextConcepts ?? []),
        ...pathIds,
      ]);
      return conceptNodes.filter((concept) => ids.has(concept.id));
    }
    if (hideUnrelated || classroomMode || pathActive)
      return conceptNodes.filter(
        (concept) =>
          focusIds.has(concept.id) ||
          pathIds.has(concept.id) ||
          (filtersActive && filteredIds.has(concept.id)),
      );
    return conceptNodes.filter(
      (concept) =>
        (masteredVisible || !masteredSeedIds.includes(concept.id)) &&
        (!filtersActive || filteredIds.has(concept.id)),
    );
  }, [
    classroomMode,
  ...
  useEffect(() => {
    if (
      ...
      !filteredConcepts.some((concept) => concept.id === selectedId)
    )
      setSelectedId(filteredConcepts[0].id);
  }, [filteredConcepts, filtersActive, selectedId]);
  ...
  const startPath = () => {
    if (!selectedId) setSelectedId("unit-circle");
    setPathActive(true);
  ...
  return (
    <main
      className={`concept-map-page ${classroomMode ? "classroom" : ""} ${filtersOpen ? "filters-open" : ""} ${detailsOpen ? "details-open" : ""}`}
    >
      <header className="concept-page-header">
        <div className="concept-heading-copy">
        ...
        <div className="concept-hero-stats" aria-label="Concept map coverage">
          <span>
            <Network />
            <strong>{conceptNodes.length}</strong>
            <small>Concepts</small>
          </span>
          <span>
            <Route />
            <strong>{conceptEdges.length}</strong>
        ...
        <button
          type="button"
          className="concept-start-path"
          onClick={startPath}
        >
          <BookOpen />
          {pathActive ? "Continue learning path" : "Start learning path"}
          <ArrowRight />
        </button>
      </header>
      ...
      <section className="concept-map-layout">
        <ConceptMapSidebar
          ...
          masteredVisible={masteredVisible}
          onClassroomMode={setClassroomMode}
          onFilters={setFilters}
          onLearningMode={setLearningMode}
          onMasteredVisible={setMasteredVisible}
        ...
        <div className="concept-main-column">
          <div
            className="concept-context-toolbar"
            aria-label="Graph relationship controls"
          >
            <button
              type="button"
              aria-label="Show prerequisites"
              className={showPrerequisites ? "active" : ""}
              onClick={() => {
            ...
            <button
              type="button"
              className={hideUnrelated ? "active" : ""}
              onClick={() => setHideUnrelated((value) => !value)}
            >
              <EyeOff />
              Hide unrelated
            </button>
            <select
              ...
              value={filters.relationships?.length ? "filtered" : "all"}
              onChange={(event) =>
                event.target.value === "all" &&
                setFilters({ ...filters, relationships: undefined })
              }
            >
              <option value="all">All relationships</option>
              {filters.relationships?.length && (
                <option value="filtered">
                  {filters.relationships.length} types selected
                </option>
              )}
            </select>
            {pathActive && (
          ...
          <ConceptMapCanvas
            edges={visibleEdges}
            focusIds={focusIds}
            onOpen={(concept) => concept.route && navigate(concept.route)}
        ...
        <ConceptMapNodeDetails
          concept={selectedConcept}
          learningMode={learningMode}
          masteredIds={masteredSeedIds}
      ...
      <button
        type="button"
        className="concept-mobile-filter"
      ...
      {(filtersOpen || detailsOpen) && (
        <button
          type="button"
          className="concept-mobile-backdrop"
