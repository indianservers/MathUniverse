...
const relationshipFilters: Array<{
  id: ConceptEdgeType;
  label: string;
  style: string;
}> = [
...
export default function ConceptMapSidebar({
  ...
  const categories = Array.from(
    new Set(conceptNodes.map((node) => node.category)),
  );
  ...
  return (
    <aside
      className="concept-panel concept-sidebar"
      aria-label="Concept map filters"
    >
      <ConceptMapSearch filters={filters} onFilters={onFilters} />
      <p className="concept-count">
        <strong>{visibleCount}</strong> of {totalCount} concepts visible
      </p>
      ...
      <section>
        ...
        <div
          className="concept-segmented"
          role="group"
          aria-label="Learning mode"
        >
          {(["student", "teacher", "explorer"] as LearningMode[]).map(
            (mode) => (
              <button
                key={mode}
                type="button"
                className={learningMode === mode ? "active" : ""}
                onClick={() => onLearningMode(mode)}
              >
                {mode}
              </button>
            ),
      ...
      <div className="concept-toggle-list">
        <label className="concept-toggle">
          <span>Show mastered</span>
          <input
            type="checkbox"
            checked={masteredVisible}
            onChange={(event) => onMasteredVisible(event.target.checked)}
          />
        </label>
        <label className="concept-toggle">
          ...
          <input
            ...
            onChange={(event) => onClassroomMode(event.target.checked)}
          />
        </label>
      </div>
      ...
      <section>
        ...
        <div className="concept-category-list">
          {shownCategories.map((category) => {
            const count = conceptNodes.filter(
            ...
            return (
              ...
              >
                <i
                  style={{ background: conceptCategoryInfo[category].color }}
                />
                <span>{conceptCategoryInfo[category].label}</span>
                <b>{count}</b>
              </button>
            );
          })}
        ...
        <button
          type="button"
          className="concept-show-more"
          onClick={() => setShowAllCategories((value) => !value)}
        >
          {showAllCategories ? <ChevronUp /> : <ChevronDown />}
          {showAllCategories
            ? "Show less"
            : `Show ${categories.length - shownCategories.length} more`}
        </button>
      </section>
        ...
        <div className="concept-relationship-list">
          {relationshipFilters.map((relationship) => {
            const checked =
              ...
              filters.relationships.includes(relationship.id);
            return (
              <label key={relationship.id}>
                <i className={relationship.style} />
                <span>{relationship.label}</span>
                <input
                  type="checkbox"
                  checked={checked}
                  onChange={() => {
                    const current = filters.relationships?.length
                      ? filters.relationships
                      : relationshipFilters.map((item) => item.id);
                    const next = toggleValue(current, relationship.id);
                    onFilters({
                      ...filters,
                      relationships:
                        next.length === relationshipFilters.length
                          ? undefined
                          : next,
                    });
                  }}
