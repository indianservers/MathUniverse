...
function edgePath(source: ConceptNode, target: ConceptNode) {
  ...
  const midY =
    (source.y + target.y) / 2 -
    Math.min(44, Math.abs(source.x - target.x) / 10);
...
export default function ConceptMapCanvas({
  ...
  const conceptById = useMemo(
    () => new Map(visibleConcepts.map((concept) => [concept.id, concept])),
    [visibleConcepts],
  );
  ...
  const clampView = (next: { x: number; y: number; zoom: number }) => {
    const zoom = Math.min(
      MAX_ZOOM,
      Math.max(MIN_ZOOM, Number(next.zoom.toFixed(2))),
    );
    const width = size.width / zoom;
    const height = size.height / zoom;
    return {
      x: Math.min(Math.max(next.x, 0), Math.max(0, MAP_WIDTH - width)),
      y: Math.min(Math.max(next.y, 0), Math.max(0, MAP_HEIGHT - height)),
      zoom,
    };
  };
  const fitView = () =>
    setView(
      clampView({
        x: 0,
        y: 0,
        zoom: Math.min(size.width / MAP_WIDTH, size.height / MAP_HEIGHT) * 0.94,
      }),
    );
  ...
  const centerSelected = () => {
    ...
    setView((current) =>
      clampView({
        x: concept.x - size.width / current.zoom / 2,
        y: concept.y - size.height / current.zoom / 2,
        zoom: Math.max(current.zoom, 1.05),
      }),
    );
  };
  const zoomAtCenter = (delta: number) =>
    setView((current) => {
      ...
      return clampView({
        x: current.x + (size.width / current.zoom - size.width / nextZoom) / 2,
        y:
          current.y + (size.height / current.zoom - size.height / nextZoom) / 2,
        zoom: nextZoom,
      });
    });
  ...
  const navigateMiniMap = (event: ReactMouseEvent<HTMLButtonElement>) => {
    ...
    setView((current) =>
      clampView({
        x: x - size.width / current.zoom / 2,
        y: y - size.height / current.zoom / 2,
        zoom: current.zoom,
      }),
    );
  };
  ...
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      ...
      if (event.key === "+" || event.key === "=") zoomAtCenter(0.12);
      if (event.key === "-") zoomAtCenter(-0.12);
    };
  ...
  const handlePointerMove = (event: PointerEvent<SVGSVGElement>) => {
    ...
    setView((current) =>
      clampView({
        ...current,
        x: drag.viewX - (event.clientX - drag.x) * (viewWidth / rect.width),
        y: drag.viewY - (event.clientY - drag.y) * (viewHeight / rect.height),
      }),
    );
  };
  ...
  return (
    <section
      ref={shellRef}
      className="concept-panel concept-canvas-card"
      aria-label="Interactive math concept graph"
    >
      <div className="concept-floating-tools" aria-label="Graph controls">
        ...
        <button
          type="button"
          title="Centre selected concept"
          aria-label="Centre selected concept"
      ...
      <div className="concept-canvas-shell">
        <svg
          ...
          role="img"
          aria-label="Interactive mathematics knowledge graph"
        ...
        >
          ...
          <g>
            {edges.map((edge) => {
              ...
              const highlighted =
                pathIds.has(edge.source) && pathIds.has(edge.target);
              return (
                <path
                  key={edge.id}
          ...
          <g>
            {visibleConcepts.map((concept) => {
              ...
              const searchMatch = searchMatchIds.has(concept.id);
              const muted =
                searchActive && !searchMatch && !focused && !selected;
              return (
                <g
                  key={concept.id}
                  aria-label={`${concept.title}, ${category.label}, ${concept.difficulty}`}
                  className={`concept-node ${selected ? "selected" : ""} ${focused ? "focused" : ""} ${inPath ? "path" : ""} ${searchMatch ? "search-match" : ""} ${muted ? "muted" : ""}`}
                  role="button"
                  tabIndex={0}
                  transform={`translate(${concept.x} ${concept.y})`}
                  onClick={() => onSelect(concept.id)}
                  onDoubleClick={() => onOpen(concept)}
                  onKeyDown={(event) => {
                    if (event.key === "Enter") onSelect(concept.id);
                  }}
                ...
                >
                  ...
                  {(hoveredId === concept.id || selected) && (
                    <foreignObject x="-96" y="48" width="192" height="58">
                      <div className="concept-node-tooltip">
                        <strong>{concept.title}</strong>
                        <span>
                          {category.label} · {concept.difficulty}
                        </span>
                      </div>
                    </foreignObject>
                  )}
      ...
      <div className="concept-canvas-legend">
        {Object.values(conceptCategoryInfo)
          .slice(0, 6)
          .map((category) => (
      ...
      <button
        ...
        aria-label="Navigate graph using minimap"
        onClick={navigateMiniMap}
      >
        <svg viewBox={`0 0 ${MAP_WIDTH} ${MAP_HEIGHT}`}>
          <rect x={view.x} y={view.y} width={viewWidth} height={viewHeight} />
          <g>
            {visibleConcepts.map((concept) => (
              <circle
                key={concept.id}
                ...
                cy={concept.y}
                r="12"
                fill={conceptCategoryInfo[concept.category].color}
              />
            ))}
          </g>
        </svg>
