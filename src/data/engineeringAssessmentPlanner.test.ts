...
describe("engineering assessment planner", () => {
  it("creates one readiness plan for every engineering domain", () => {
    ...
    expect(
      engineeringMathDomains.every((domain) => plannedDomainIds.has(domain.id)),
    ).toBe(true);
    expect(plans.every((plan) => plan.score >= 0 && plan.score <= 100)).toBe(
      true,
    );
  });
  ...
  it("adds actionable drill routes and evidence", () => {
    ...
    expect(
      plans.every((plan) => plan.drillRoute.startsWith("/syllabus-lab/")),
    ).toBe(true);
