...
const engineeringLevel = syllabusLevels.find(
  (level) => level.id === "engineering",
);
export const engineeringSyllabusTopics = engineeringLevel?.topics ?? [];

const domainDefinitions: Omit<
  EngineeringMathDomain,
  "topics" | "nativeRoutes" | "priority" | "status"
>[] = [
  {
    ...
    semesterBand: "M1",
    purpose:
      "Multivariable calculus, curve tracing, Jacobians, multiple integrals, special integrals, and engineering optimization foundations.",
    prerequisiteUnits: [
      "Functions",
      "Trigonometry",
  ...
  },
  {
    ...
    semesterBand: "M1-M2",
    purpose:
      "First-order, higher-order, Cauchy-Euler, IVP/BVP, and physical-system equation workflows.",
  ...
  },
  {
    ...
    semesterBand: "M1-M2",
    purpose:
      "Rank, consistency, eigen systems, diagonalization, quadratic forms, and numerical matrix workflows.",
    prerequisiteUnits: [
      "Matrices",
      "Vectors",
  ...
  },
  {
    ...
    semesterBand: "M2-M3",
    purpose:
      "Laplace, Fourier, Z-transform, convolution, impulse/unit-step models, and signal-domain reasoning.",
    prerequisiteUnits: [
      "Calculus",
  ...
  },
  {
    ...
    semesterBand: "M3",
    purpose:
      "PDE classification, heat/wave/Laplace equations, separation of variables, and numerical boundary methods.",
    prerequisiteUnits: [
  ...
  },
  {
    ...
    semesterBand: "M3-M4",
    purpose:
      "Root finding, interpolation, numerical integration, ODE/PDE solvers, error analysis, and iterative linear algebra.",
    prerequisiteUnits: [
      "Algebra",
      "Calculus",
      "Linear algebra",
  ...
  },
  {
    ...
    semesterBand: "M3-M4",
    purpose:
      "Random variables, distributions, inference, regression, reliability, Markov chains, queueing, and time series.",
    prerequisiteUnits: [
      "Set theory",
      "Counting",
      "Calculus",
  ...
  },
  {
    ...
    semesterBand: "M4",
    purpose:
      "Linear programming, simplex, transportation, assignment, network planning, game theory, and variational methods.",
    prerequisiteUnits: [
      "Linear algebra",
      "Calculus",
      "Graph theory",
  ...
  },
  {
    ...
    semesterBand: "M2-M3",
    purpose:
      "Gradient, divergence, curl, line/surface integrals, and Green/Gauss/Stokes theorem interpretation.",
    prerequisiteUnits: [
      "Vectors",
      "3D geometry",
      "Multivariable calculus",
  ...
  },
  {
    ...
    semesterBand: "M3-M4",
    purpose:
      "Complex functions, residues, Bessel/Legendre models, Sturm-Liouville systems, poles/zeros, and response analysis.",
...
function routesForTopics(topics: SyllabusTopic[]) {
  return Array.from(
    new Set(
      topics
        .map((topic) => topic.linkedVisualization.route)
...
function statusFor(
  topics: SyllabusTopic[],
  routes: string[],
): EngineeringBuildStatus {
  if (topics.length === 0) return "Build next";
  if (
    routes.length >= topics.length &&
    topics.every((topic) => topic.linkedVisualization.available)
  )
    return "Live";
  return "Mapped";
}

export const engineeringMathDomains: EngineeringMathDomain[] =
  domainDefinitions.map((domain) => {
    const topics = engineeringSyllabusTopics.filter((topic) => {
      if (topic.unit === domain.title) return true;
      if (
        domain.title === "Transforms and Signal Mathematics" &&
        topic.unit === "Transforms"
      )
        return true;
      if (
        domain.title === "Partial Differential Equations" &&
        topic.unit === "Partial Differential Equations"
      )
        return true;
      if (
        domain.title === "Numerical Methods" &&
        topic.unit === "Numerical Methods"
      )
        return true;
      if (
        domain.title ===
        "Engineering Probability, Statistics, and Stochastic Processes"
      )
        return [
          "Engineering Probability and Statistics",
          "Stochastic Processes",
        ].includes(topic.unit);
      if (domain.title === "Optimization and Operations Research")
        return ["Optimization", "Operations Research"].includes(topic.unit);
      if (domain.title === "Vector Calculus and Field Theory")
        return topic.unit === "Vector Calculus";
      if (
        domain.title ===
        "Complex Analysis, Special Functions, and Control Mathematics"
      )
        return [
          "Complex Analysis",
          "Special Functions",
...
export const engineeringMathMilestones: EngineeringMathMilestone[] = [
  {
    ...
    deliverables: [
      "Domain map",
      "topic-to-route inventory",
      "formula families",
      "workspace targets",
    ],
    acceptanceTargets: [
      "Every domain has native route targets",
      "No existing 2D/3D workspace layout is rewritten",
    ],
  },
  {
    ...
    deliverables: [
      "Compact Engineering Math hub",
      "Math Lab entry",
      "left navigation entry",
    ],
    acceptanceTargets: [
      "Learners can open engineering domains without external redirects",
      "All lab buttons resolve to app routes",
    ],
  },
  {
    id: "simulation-depth",
    ...
    deliverables: [
      "PDE presets",
      "numerical method iteration tables",
      "seeded stochastic simulations",
      "3D field presets",
    ],
    acceptanceTargets: [
      "Deterministic tests cover formula outputs",
      "Large simulations use performance budgets",
    ],
  },
];
...
export const engineeringMathSummary = {
  ...
  liveDomainCount: engineeringMathDomains.filter(
    (domain) => domain.status === "Live",
  ).length,
  nativeRouteCount: new Set(
    engineeringMathDomains.flatMap((domain) => domain.nativeRoutes),
  ).size,
  formulaFamilyCount: new Set(
    engineeringMathDomains.flatMap((domain) => domain.formulaFamilies),
  ).size,
};
