...
describe("engineering worked examples", () => {
  ...
  it("gives each domain multiple worked examples with real steps", () => {
    engineeringMathDomains.forEach((domain) => {
      ...
      expect(examples.every((example) => example.answerCheck.length > 0)).toBe(
        true,
      );
      expect(examples.every((example) => example.application.length > 20)).toBe(
        true,
      );
    });
  });
  ...
  it("keeps examples route-backed into native app surfaces", () => {
    expect(
      engineeringWorkedExamples.every((example) =>
        example.route.startsWith("/"),
      ),
    ).toBe(true);
    expect(
      engineeringWorkedExamples.every((example) =>
        example.route.startsWith("/syllabus-lab/"),
      ),
    ).toBe(true);
    expect(workedExampleSummary().routeCount).toBeGreaterThan(8);
    expect(workedExampleSummary().stepCount).toBeGreaterThanOrEqual(
      engineeringWorkedExamples.length * 3,
    );
  });

  it("includes recognizable engineering examples", () => {
    expect(
      workedExamplesForDomain("numerical-methods").some((example) =>
        example.title.includes("Newton"),
      ),
    ).toBe(true);
    expect(
      workedExamplesForDomain("vector-calculus-fields").some((example) =>
        example.title.includes("Divergence"),
      ),
    ).toBe(true);
    expect(
      workedExamplesForDomain("complex-special-control").some((example) =>
        example.title.includes("pole"),
      ),
    ).toBe(true);
  });
