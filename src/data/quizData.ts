...
const banks: ConceptBank[] = [
  ...
  },
  {
    ...
    numeric: [
      ...
      { label: "ap", build: (v) => { const a = 3 + (v % 5); const d = 2 + (v % 4); const n = 4 + (v % 8); const answer = a + (n - 1) * d; return { question: `In AP with first term ${a} and difference ${d}, find a_${n}.`, ...placeAnswer(String(answer), [String(answer + d), String(a * n), String(d * n)], v + 3), explanation: `a_n=a+(n-1)d=${a}+${n - 1}(${d})=${answer}.` }; } },
