...
describe("NCERT concept audit", () => {
  ...
  it("has a supported visualization and complete learning copy for every concept", () => {
    for (const concept of ncertConcepts) {
      expect(supportedNCERTVisualTypes.has(concept.visual), concept.id).toBe(
        true,
      );
  ...
  it("keeps slider defaults inside their configured ranges", () => {
    for (const concept of ncertConcepts) {
      expect(
        concept.defaultA,
        `${concept.id}: slider A`,
      ).toBeGreaterThanOrEqual(concept.minA);
      expect(concept.defaultA, `${concept.id}: slider A`).toBeLessThanOrEqual(
        concept.maxA,
      );
      expect(
        concept.defaultB,
        `${concept.id}: slider B`,
      ).toBeGreaterThanOrEqual(concept.minB);
      expect(concept.defaultB, `${concept.id}: slider B`).toBeLessThanOrEqual(
        concept.maxB,
      );
      if (concept.sliderC) {
        expect(
          concept.defaultC,
          `${concept.id}: slider C`,
        ).toBeGreaterThanOrEqual(concept.minC ?? Number.NEGATIVE_INFINITY);
        expect(concept.defaultC, `${concept.id}: slider C`).toBeLessThanOrEqual(
          concept.maxC ?? Number.POSITIVE_INFINITY,
        );
      }
    }
  });
