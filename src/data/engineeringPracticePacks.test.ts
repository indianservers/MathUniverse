...
import {
  engineeringPracticePacks,
  practiceCoverageSummary,
  practicePackForDomain,
} from "./engineeringPracticePacks";

describe("engineering practice packs", () => {
  it("covers every engineering math domain", () => {
    const coveredDomains = new Set(
      engineeringPracticePacks.map((pack) => pack.domainId),
    );
    expect(
      engineeringMathDomains.every((domain) => coveredDomains.has(domain.id)),
    ).toBe(true);
    expect(practiceCoverageSummary().coveredDomainCount).toBe(
  ...
  it("keeps prompts route-backed and level-balanced", () => {
    ...
    expect(
      prompts.every(
        (prompt) =>
          prompt.route.startsWith("/syllabus-lab/") ||
          prompt.route.startsWith("/formulas"),
      ),
    ).toBe(true);
    expect(levels.has("foundation")).toBe(true);
    expect(levels.has("exam")).toBe(true);
    expect(levels.has("challenge")).toBe(true);
    expect(practiceCoverageSummary().examPromptCount).toBeGreaterThanOrEqual(
      engineeringMathDomains.length,
    );
  });
  ...
  it("finds the selected domain practice pack", () => {
    ...
    expect(
      calculusPack?.prompts.some((prompt) =>
        prompt.route.includes("partial-derivative"),
      ),
    ).toBe(true);
  });
