...
describe("engineering portfolio projects", () => {
  it("covers every engineering math domain with multiple projects", () => {
    ...
    expect(projectSummary().coveredDomainCount).toBe(
  ...
  it("keeps projects route-backed with meaningful assessment artifacts", () => {
    expect(
      engineeringProjects.every((project) =>
        project.workspaceRoute.startsWith("/"),
      ),
    ).toBe(true);
    expect(
      engineeringProjects.every((project) =>
        project.workspaceRoute.startsWith("/syllabus-lab/"),
      ),
    ).toBe(true);
    expect(
      engineeringProjects.every((project) => project.deliverables.length >= 3),
    ).toBe(true);
    expect(
      engineeringProjects.every((project) => project.rubric.length >= 3),
    ).toBe(true);
    expect(
      engineeringProjects.every((project) => project.evidence.length >= 3),
    ).toBe(true);
    expect(
      engineeringProjects.every((project) => project.estimatedMinutes > 0),
    ).toBe(true);
  });
  ...
  it("summarizes project depth for the hub", () => {
    ...
    expect(summary.deliverableCount).toBeGreaterThanOrEqual(
      engineeringProjects.length * 3,
    );
  ...
  it("includes recognizable engineering project briefs", () => {
    expect(
      projectsForDomain("optimization-operations-research").some((project) =>
        project.title.includes("LP"),
      ),
    ).toBe(true);
    expect(
      projectsForDomain("complex-special-control").some((project) =>
        project.title.includes("Pole"),
      ),
    ).toBe(true);
    expect(
      projectsForDomain("partial-differential-equations").some((project) =>
        project.title.includes("Heat"),
      ),
    ).toBe(true);
  });
