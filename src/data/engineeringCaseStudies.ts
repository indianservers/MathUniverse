...
export const engineeringCaseStudies: EngineeringCaseStudy[] = [
  study("engineering-calculus", "heat-sink-surface-sensitivity", "Heat-sink surface sensitivity", "Thermal design", "A cooling fin profile must be tuned without exceeding material volume, so learners compare surface slope, cross-sections, and local sensitivity.", ["partial derivatives", "surface plots", "constrained optimization"], ["/workspace/3d?v_surface_scale=1&v_cross_section_z=0", "/formulas?q=partial%20derivative"], ["partial derivative", "double integral", "Jacobian"], ["surface screenshot", "sensitivity note", "volume constraint"], ["Explains why one variable is held fixed", "connects slope to heat-transfer area", "states the engineering tradeoff"], 55),
  study("engineering-calculus", "robot-arm-jacobian-check", "Robot arm Jacobian check", "Robotics", "A two-joint arm needs a quick workspace conversion from joint motion to endpoint motion before path planning.", ["Jacobian", "coordinate transforms", "local linearization"], ["/formulas?q=jacobian", "/linear-algebra"], ["Jacobian", "Taylor expansion", "determinant"], ["joint-variable table", "Jacobian matrix", "singularity warning"], ["Identifies input and output coordinates", "checks determinant behavior", "reports a path-planning risk"], 50),

  study("engineering-differential-equations", "rc-filter-startup-response", "RC filter startup response", "Electronics", "A sensor circuit must be ready within a time limit after power-on, so learners estimate transient response and steady state.", ["first-order ODE", "time constant", "initial values"], ["/math/slope-fields", "/syllabus-lab/laplace-transform-workflow"], ["linear first-order ODE", "Laplace derivative rule"], ["ODE setup", "response curve", "settling-time note"], ["Uses the initial condition", "finds steady state", "compares visual and analytic response"], 45),
  study("engineering-differential-equations", "vehicle-suspension-damping", "Vehicle suspension damping", "Mechanical systems", "A suspension model needs damping classification so overshoot and return time can be explained from equation roots.", ["second-order ODE", "auxiliary equation", "damping cases"], ["/syllabus-lab/higher-order-ode-characteristic-lab", "/workspace/graph"], ["auxiliary equation", "Cauchy-Euler trial"], ["root table", "damping classification", "motion sketch"], ["Classifies roots correctly", "connects roots to motion", "states a comfort or safety implication"], 60),

  study("engineering-linear-algebra", "camera-transform-pipeline", "Camera transform pipeline", "Computer graphics", "A 3D point cloud must be transformed through rotation, scale, and projection while preserving the meaning of basis vectors.", ["matrix multiplication", "eigen directions", "basis transforms"], ["/linear-algebra", "/workspace/3d"], ["eigen equation", "normal equations"], ["transform matrix", "before-after vector view", "basis explanation"], ["Labels each transformation stage", "checks matrix dimensions", "explains preserved directions"], 55),
  study("engineering-linear-algebra", "sensor-calibration-least-squares", "Sensor calibration least-squares", "Instrumentation", "Noisy calibration readings produce an overdetermined system, so learners fit coefficients and interpret residual error.", ["least squares", "rank", "projection"], ["/syllabus-lab/rank-consistency-row-reduction", "/linear-algebra"], ["normal equations", "rank-nullity theorem"], ["data table", "normal equation setup", "residual note"], ["Explains why exact solution is unlikely", "checks rank", "interprets residual size"], 50),

  study("transforms-signals", "motor-drive-frequency-signature", "Motor-drive frequency signature", "Signal processing", "A motor current trace contains harmonics, and learners identify dominant components before diagnosing vibration risk.", ["Fourier series", "spectrum", "harmonic approximation"], ["/syllabus-lab/fourier-transform-spectrum-lab", "/workspace/graph"], ["Fourier series", "convolution theorem"], ["signal sketch", "harmonic table", "diagnostic note"], ["Connects peaks to harmonics", "mentions approximation error", "states an engineering interpretation"], 50),
  study("transforms-signals", "control-input-convolution", "Control input convolution", "Control systems", "A system response must be predicted from a known impulse response and a shaped input command.", ["convolution", "Laplace transform", "impulse response"], ["/syllabus-lab/laplace-transform-workflow", "/formulas?q=convolution"], ["convolution theorem", "transfer function"], ["input response setup", "transform-domain product", "time-response note"], ["Defines input and impulse response", "uses transform pairing", "checks initial behavior"], 55),
