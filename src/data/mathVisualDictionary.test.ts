...
describe("mathVisualDictionary", () => {
  it("includes at least 300 enriched visual dictionary additions", () => {
    ...
    expect(
      enrichedTerms.every(
        (entry) =>
          entry.description && entry.explanation && entry.representation,
      ),
    ).toBe(true);
  });
  ...
  it("keeps examples as examples rather than visualization instructions", () => {
    const drawingInstructions = visualDictionaryTerms.filter((entry) =>
      /^(draw|show|use|represent|highlight|shade|mark)\b/i.test(
        entry.example ?? "",
      ),
    );
  ...
  it("includes a broad math-symbol dictionary with searchable examples", () => {
    const symbolTerms = visualDictionaryTerms.filter(
      (entry) =>
        entry.kind === "text" &&
        entry.term.includes("(") &&
        entry.description &&
  ...
  it("keeps term names unique", () => {
    const normalizedTerms = visualDictionaryTerms.map((entry) =>
