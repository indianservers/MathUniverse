...
import {
  ...
  phaseOneAccuracyConcepts,
  phaseOneAccuracySummary,
  phaseOneRoadmapCoverage,
} from ".";
...
describe("Phase 1 strengthened concept contracts", () => {
  ...
  it("covers every Phase 1 domain with multiple topic-specific concepts", () => {
    for (const domain of PHASE_ONE_DOMAINS) {
      expect(accuracyConceptsForDomain(domain).length).toBeGreaterThanOrEqual(
        3,
      );
    }
  });
  ...
  it("provides all example modes and misconception checks", () => {
    ...
    for (const concept of phaseOneAccuracyConcepts) {
      expect(new Set(concept.examples.map((example) => example.kind))).toEqual(
  ...
  it("indexes contracts by stable ID", () => {
    expect(accuracyConceptById("algebra.linear-equation")?.title).toContain(
      "Linear",
    );
    expect(
      accuracyConceptById("calculus.derivative")?.validation.tolerance,
    ).toBe(1e-5);
  });
  ...
  it("traces every roadmap row to a domain-correct contract", () => {
    ...
    for (const item of phaseOneRoadmapCoverage) {
      expect(
        accuracyConceptById(item.conceptId)?.domain,
        `${item.topic} is not mapped correctly`,
      ).toBe(item.domain);
    }
  });
