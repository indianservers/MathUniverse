...
export const formulaVisualizerConfigs: FormulaVisualizerRouteConfig[] = [
  ...
  },
  {
    ...
    title: "Proportional Reasoning-2 Formula Visualizer",
    shortTitle: "Proportional Reasoning-2",
    route: "/visual-formulas/proportional-reasoning-2",
    category: "Ratio and Proportion",
    subtitle: "Equivalent ratios, cross multiplication, map scale, pie slices, and direct/inverse proportion in a compact Class 8 visual lab.",
    description: "A focused Grade 8 Part II formula visualizer that keeps ratio, map, sharing, and proportion models in one tabbed workspace.",
    defaultFormulaId: "cross-multiplication",
    formulaLibraryRoute: "/formulas/proportional-reasoning-2",
    relatedRoutes: [
      { label: "Class 8 NCERT lab", href: "/ncert/class-8-proportional-reasoning-2" },
      { label: "Map scale tab", href: "/ncert/class-8-proportional-reasoning-2?tab=map-scale" },
      { label: "Ratio visual proofs", href: "/visual-proofs/ratios" },
    ],
    examples: [
      { title: "Equivalent ratio", description: "Check 2:3 and 8:12 by cross multiplication.", values: { a: 2, b: 3, c: 8, n: 12, p: 50 } },
      { title: "Map scale", description: "Use 3.2 cm on a 1:50000 map.", values: { a: 3.2, b: 50000, c: 1, n: 4, p: 50 } },
      { title: "Ratio split", description: "Split 900 into the ratio 2:3:4.", values: { a: 2, b: 3, c: 4, n: 900, p: 50 } },
    ],
    searchTerms: ["proportional reasoning visualizer", "ratio visual formula", "cross multiplication", "map scale", "representative fraction", "direct proportion", "inverse proportion", "pie chart ratio"],
    teacherNotes: classroomNotes(
      "Move from equal ratios to cross-products, then reuse the same idea for maps, sharing, and pie sectors.",
      "Which stays constant here: a ratio or a product?",
      "Students often forget to convert map and real distances into the same unit before forming RF.",
      "Ask students to split 900 in 2:3:4, then convert the same ratio into pie angles.",
      "Create one direct proportion example and one inverse proportion example from daily life.",
      ["Fractions", "Multiplication", "Units"],
      ["Similarity", "Linear functions", "Hyperbolas"]
    ),
    formulas: [
      formula("equivalent-ratios", "Equivalent ratios", "\\frac{a}{b}=\\frac{c}{d}", "a/b = c/d", "Equivalent ratios simplify to the same comparison.", "Equivalent Ratios", "Foundation", ["a", "b", "c", "d"], "fraction-percent", ["ratio", "equivalent"]),
      formula("cross-multiplication", "Cross multiplication", "\\frac{a}{b}=\\frac{c}{d}\\iff ad=bc", "a/b = c/d iff ad = bc", "Equal ratios have equal cross-products.", "Equivalent Ratios", "Foundation", ["a", "b", "c", "d"], "fraction-percent", ["cross multiplication"]),
      formula("missing-fourth-term", "Missing fourth term", "d=\\frac{bc}{a}", "d = bc / a", "Solves a/b = c/d when d is unknown.", "Equivalent Ratios", "Practice", ["a", "b", "c"], "fraction-percent", ["missing value"]),
      formula("representative-fraction", "Representative fraction", "RF=\\frac{\\text{map}}{\\text{actual}}", "RF = map / actual", "Convert both distances to the same unit before reducing.", "Map Scale", "Practice", ["a", "b"], "fraction-percent", ["map scale", "representative fraction"]),
      formula("actual-distance-map-scale", "Actual distance from map", "\\text{actual}=\\text{map}\\times n", "actual = map x n", "Scale 1:n turns map units into real units.", "Map Scale", "Practice", ["a", "b"], "fraction-percent", ["actual distance"]),
      formula("map-distance-actual-scale", "Map distance from actual", "\\text{map}=\\frac{\\text{actual}}{n}", "map = actual / n", "Divide real distance by scale denominator.", "Map Scale", "Practice", ["a", "b"], "fraction-percent", ["map distance"]),
      formula("multi-term-ratio-share", "Multi-term ratio share", "S_i=\\frac{r_i}{\\sum r}\\,T", "share = part / sum parts x total", "Divide a whole fairly using all ratio parts.", "Sharing", "Foundation", ["a", "b", "c", "n"], "fraction-percent", ["ratio split"]),
      formula("ratio-to-percentage", "Ratio to percentage", "P_i=\\frac{r_i}{\\sum r}\\times100", "percent = part / sum parts x 100", "Shows each ratio part as a percent of the whole.", "Sharing", "Foundation", ["a", "b", "c"], "fraction-percent", ["percentage"]),
      formula("ratio-to-pie-angle", "Ratio to pie angle", "\\theta_i=\\frac{r_i}{\\sum r}\\times360^\\circ", "angle = part / sum parts x 360", "Pie chart angles share 360 degrees in the same ratio.", "Sharing", "Practice", ["a", "b", "c"], "fraction-percent", ["pie chart"]),
      formula("direct-proportion", "Direct proportion", "y=kx", "y = kx", "The ratio y/x remains constant.", "Proportion Types", "Foundation", ["x", "y", "k"], "graph", ["direct proportion"]),
      formula("inverse-proportion", "Inverse proportion", "xy=k", "xy = k", "The product xy remains constant.", "Proportion Types", "Practice", ["x", "y", "k"], "graph", ["inverse proportion"]),
      formula("constant-check", "Direct or inverse check", "\\frac{y_1}{x_1}=\\frac{y_2}{x_2}\\;\\text{or}\\;x_1y_1=x_2y_2", "constant ratio or constant product", "Use the right constant to classify a situation.", "Proportion Types", "Exam", ["x", "y"], "graph", ["classification"]),
    ],
  },
  {
    id: "trigonometry",
...
export const formulaVisualizerCategoryRouteMap: Record<string, string> = {
  ...
  "proportional-reasoning-2": "/visual-formulas/proportional-reasoning-2",
  "ratio-proportion": "/visual-formulas/proportional-reasoning-2",
  ratios: "/visual-formulas/proportional-reasoning-2",
  proportion: "/visual-formulas/proportional-reasoning-2",
