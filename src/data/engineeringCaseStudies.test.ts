...
import {
  caseStudiesForDomain,
  caseStudySummary,
  engineeringCaseStudies,
} from "./engineeringCaseStudies";
...
describe("engineering case studies", () => {
  it("covers every engineering math domain with industry scenarios", () => {
    const coveredDomainIds = new Set(
      engineeringCaseStudies.map((caseStudy) => caseStudy.domainId),
    );
    expect(
      engineeringMathDomains.every((domain) => coveredDomainIds.has(domain.id)),
    ).toBe(true);
    expect(caseStudySummary().coveredDomainCount).toBe(
  ...
  it("connects every case study to labs, formulas, evidence, and success criteria", () => {
    expect(
      engineeringCaseStudies.every(
        (caseStudy) => caseStudy.labRoutes.length >= 2,
      ),
    ).toBe(true);
    expect(
      engineeringCaseStudies.every((caseStudy) =>
        caseStudy.labRoutes.every((route) => route.startsWith("/")),
      ),
    ).toBe(true);
    expect(
      engineeringCaseStudies
        .flatMap((caseStudy) => caseStudy.labRoutes)
        .every((route) => route.startsWith("/syllabus-lab/")),
    ).toBe(true);
    expect(
      engineeringCaseStudies.every(
        (caseStudy) => caseStudy.formulaQueries.length >= 2,
      ),
    ).toBe(true);
    expect(
      engineeringCaseStudies.every(
        (caseStudy) => caseStudy.portfolioEvidence.length >= 3,
      ),
    ).toBe(true);
    expect(
      engineeringCaseStudies.every(
        (caseStudy) => caseStudy.successCriteria.length >= 3,
      ),
    ).toBe(true);
    expect(
      engineeringCaseStudies.every(
        (caseStudy) => caseStudy.estimatedMinutes > 0,
      ),
    ).toBe(true);
  });
  ...
  it("summarizes industry depth for the hub", () => {
    ...
    expect(summary.evidenceCount).toBeGreaterThanOrEqual(
      engineeringCaseStudies.length * 3,
    );
  ...
  it("includes recognizable engineering industries", () => {
    expect(
      caseStudiesForDomain("engineering-calculus").some(
        (caseStudy) => caseStudy.industry === "Robotics",
      ),
    ).toBe(true);
    expect(
      caseStudiesForDomain("optimization-operations-research").some(
        (caseStudy) => caseStudy.title.includes("LP"),
      ),
    ).toBe(true);
    expect(
      caseStudiesForDomain("complex-special-control").some(
        (caseStudy) => caseStudy.industry === "Control systems",
      ),
    ).toBe(true);
  });
