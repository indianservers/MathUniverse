...
import {
  engineeringLabLaunchers,
  launcherCoverageSummary,
  launchersForDomain,
} from "./engineeringLabLaunchers";
...
describe("engineering lab launchers", () => {
  it("gives every engineering math domain multiple native launchers", () => {
    expect(
      engineeringMathDomains.every(
        (domain) => launchersForDomain(domain.id).length >= 4,
      ),
    ).toBe(true);
    expect(
      engineeringLabLaunchers.every((launcher) =>
        launcher.route.startsWith("/"),
      ),
    ).toBe(true);
    expect(
      engineeringLabLaunchers.every((launcher) => launcher.formulas.length > 0),
    ).toBe(true);
  });

  it("covers concept labs, formula maps, and practice-ready routes", () => {
    const kinds = new Set(
      engineeringLabLaunchers.map((launcher) => launcher.kind),
    );
    const summary = launcherCoverageSummary();

    expect(Array.from(kinds)).toEqual(
      expect.arrayContaining(["concept-lab", "formula-map", "practice"]),
    );
    expect(summary.coveredDomainCount).toBe(summary.domainCount);
    expect(summary.conceptLabLauncherCount).toBeGreaterThanOrEqual(
      engineeringMathDomains.length * 3,
    );
  ...
  it("opens concept-specific labs instead of broad module placeholders", () => {
    ...
    const broadRoutes = engineeringLabLaunchers
      ...
      .filter(
        (route) =>
          !route.startsWith("/syllabus-lab/") &&
          !allowedUtilityPrefixes.some((prefix) => route.startsWith(prefix)),
      );
