...
export type OlympyardSessionMode =
  "topic" | "adaptive" | "mixed" | "weak" | "speed" | "mock";
...
export const OLYMPYARD_PROGRESS_STORAGE_KEY =
  "math-universe-olympyard-progress";
...
export function normalizeOlympyardProgress(
  value: OlympyardProgress | null | undefined,
): OlympyardProgress {
  ...
  return {
    ...
    lastTopicId:
      typeof value.lastTopicId === "string" ? value.lastTopicId : undefined,
    lastSession:
      typeof value.lastSession === "string" ? value.lastSession : undefined,
...
export function updateOlympyardProgress(
  ...
  options: {
    lastTopicId?: string;
    sessionLabel?: string;
    mockResult?: Omit<OlympyardMockHistoryEntry, "id" | "date">;
  } = {},
): OlympyardProgress {
  const safe = normalizeOlympyardProgress(current);
  if (!attempts.length && !options.mockResult && !options.lastTopicId)
    return safe;

  const correctCount = attempts.filter((attempt) => attempt.correct).length;
  const topicMastery = { ...safe.topicMastery };
  for (const attempt of attempts) {
    const currentTopic = topicMastery[attempt.topicId] ?? {
      attempted: 0,
      correct: 0,
    };
  ...
  const nextBadges = awardOlympyardBadges({
    ...safe,
    attempted,
    correct,
    topicMastery,
...
export function selectOlympyardQuestions(
  filters: OlympyardQuestionFilters,
  progress?: OlympyardProgress,
): OlympyardQuestion[] {
  const safeProgress = normalizeOlympyardProgress(progress);
  let pool = olympyardQuestions.filter(
    (question) =>
  ...
  if (filters.mode === "weak") {
    const weakTopicIds = getWeakOlympyardTopics(safeProgress).map(
      (item) => item.topicId,
    );
    pool = pool.filter((question) => weakTopicIds.includes(question.topicId));
    if (!pool.length)
      pool = olympyardQuestions.filter(
        (question) => question.difficulty !== "speed",
      );
  }

  if (filters.mode === "adaptive") {
    const adaptiveTopicIds = getAdaptiveOlympyardTopics(safeProgress, 5).map(
      (item) => item.topicId,
    );
    pool = interleaveByTopic(
      pool.filter((question) => adaptiveTopicIds.includes(question.topicId)),
    );
  ...
  if (filters.mode === "speed") {
    pool = pool.filter(
      (question) =>
        question.difficulty === "speed" ||
        (question.estimatedSeconds ?? 999) <= 45,
    );
  }
  ...
  const fallbackPool = filters.topicId
    ? olympyardQuestions.filter(
        (question) => question.topicId === filters.topicId,
      )
    : olympyardQuestions;
...
export function summarizeOlympyardSession(
  ...
  const incorrectQuestions = questions.filter(
    (question) => attempts[question.id] && !attempts[question.id].correct,
  );
  const topicBreakdown = questions.reduce<
    Record<string, { title: string; attempted: number; correct: number }>
  >((summary, question) => {
    const topic = olympyardTopicById(question.topicId);
    const current = summary[question.topicId] ?? {
      title: topic?.title ?? question.topicId,
      attempted: 0,
      correct: 0,
    };
...
export function getAdaptiveOlympyardTopics(
  progress: OlympyardProgress,
  limit = 4,
) {
  const safe = normalizeOlympyardProgress(progress);
  const stats = olympyardQuestions.reduce<
    Record<
      string,
      {
        topicId: string;
    ...
    >
  >((summary, question) => {
    const mastery = safe.topicMastery[question.topicId] ?? {
      attempted: 0,
      correct: 0,
    };
...
export function summarizeOlympyardMastery(progress: OlympyardProgress) {
  ...
  const masteredTopicCount = Object.values(safe.topicMastery).filter(
    (item) => item.attempted >= 3 && item.correct / item.attempted >= 0.8,
  ).length;
  return {
...
function normalizeTopicMastery(
  value: unknown,
): Record<string, OlympyardTopicProgress> {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return Object.fromEntries(
    Object.entries(value).map(([topicId, item]) => {
      const record =
        item && typeof item === "object"
          ? (item as Partial<OlympyardTopicProgress>)
          : {};
      return [
        topicId,
        {
...
function isMockHistoryEntry(
  value: unknown,
): value is OlympyardMockHistoryEntry {
  return Boolean(
...
function awardOlympyardBadges(progress: OlympyardProgress) {
  ...
  const speedCorrect = olympyardQuestions.filter(
    (question) =>
      question.difficulty === "speed" &&
      (progress.topicMastery[question.topicId]?.correct ?? 0) > 0,
  ).length;
...
function interleaveByTopic(questions: OlympyardQuestion[]) {
  const groups = questions.reduce<Record<string, OlympyardQuestion[]>>(
    (summary, question) => {
      summary[question.topicId] = [
        ...(summary[question.topicId] ?? []),
        question,
      ];
      return summary;
...
function olympyardWarmStartPool() {
  ...
  return olympyardQuestions.filter(
    (question) =>
      warmStartTopics.has(question.topicId) && question.difficulty !== "speed",
  );
}
...
function recommendOlympyardTopics(
