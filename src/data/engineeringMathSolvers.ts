...
export const engineeringSolverPresets: EngineeringSolverPreset[] = [
  {
    ...
    summary:
      "Maps a unit design region through x=2u+v, y=u+3v and computes the transformed area factor.",
    formula: "dA = |J| du dv",
  ...
  },
  {
    ...
    summary:
      "Computes trace, determinant, and real eigenvalues for a vibration/stability matrix.",
    formula: "lambda^2 - tr(A)lambda + det(A)=0",
    metricLabel: "lambda",
    metricValue: runEigenPreset()
      .eigenvalues.map((value) => round(value, 4))
      .join(", "),
  },
  {
    ...
    summary:
      "Solves x^3 - x - 2 = 0 from x0 = 1.5 and records the iteration protocol.",
  ...
  },
  {
    ...
    route: "/syllabus-lab/newton-raphson-tangent-iteration",
    summary:
      "Brackets the same cubic root on [1, 2] with deterministic sign checks.",
    formula: "f(a)f(b)<0",
  ...
  },
  {
    ...
    summary:
      "Runs an explicit heat-equation grid with boundary values and stability check.",
  ...
  },
  {
    ...
    summary:
      "Evaluates divergence, curl, flux estimate, and circulation estimate for a linear field.",
  ...
  },
  {
    ...
    summary:
      "Evaluates feasible corner points for max Z=40x+30y under engineering resource limits.",
    formula: "max Z=c1x+c2y",
    metricLabel: "max Z",
    metricValue: round(
      runLinearProgrammingPreset().optimum.value,
      4,
    ).toString(),
  },
  {
    ...
    summary:
      "Classifies a second-order transfer response from natural frequency and damping ratio.",
...
export function jacobianAreaScale(
  ...
  return {
    determinant,
    sourceArea,
    mappedArea: Math.abs(determinant) * sourceArea,
  };
}
...
export function compareEulerRk4(
  ...
  for (let step = 1; step <= steps; step += 1) {
    ...
    const nextEulerY =
      previousEuler.y! + h * f(previousEuler.x, previousEuler.y!);
    euler.push({
      step,
      x: round(x0 + step * h, 10),
      y: round(nextEulerY, 10),
      error: Math.abs(exactFinal - nextEulerY),
    });
    ...
    rk4.push({
      ...
      x: round(x0 + step * h, 10),
      y: round(nextRkY, 10),
      error: Math.abs(exactFinal - nextRkY),
    });
  }
  return {
    equation: "y'=0.5y",
...
export function explicitHeatStep(
  initial: number[],
  r: number,
  steps: number,
): HeatGridReport {
  if (r <= 0 || r > 0.5)
    throw new Error(
      "Explicit heat scheme requires 0 < r <= 0.5 for this preset.",
    );
  ...
  for (let step = 0; step < steps; step += 1) {
    ...
    const next = previous.map((value, index) => {
      ...
      return (
        value + r * (previous[index + 1] - 2 * value + previous[index - 1])
      );
    });
  ...
  return {
    ...
    stable: r <= 0.5,
    grid,
    centerValue: grid.at(-1)?.[center] ?? 0,
  };
}

export function markovQueueReport(
  transition: number[][],
  initial: number[],
  steps: number,
  arrivalRate: number,
...
export function vectorFieldReport(
  field: { a: number; b: number; c: number; d: number },
  radius: number,
): VectorFieldReport {
...
export function linearProgrammingCorners(
  constraints: { a: number; b: number; c: number }[],
  objective: { x: number; y: number },
): LinearProgrammingReport {
  ...
  const feasibleCorners = candidates
    ...
    .map((point) => ({
      x: round(point.x, 8),
      y: round(point.y, 8),
      value: round(objective.x * point.x + objective.y * point.y, 8),
    }))
    .filter(
      (point, index, points) =>
        points.findIndex(
          (other) =>
            Math.abs(other.x - point.x) < 1e-7 &&
            Math.abs(other.y - point.y) < 1e-7,
        ) === index,
    )
...
export function secondOrderControlResponse(
  naturalFrequency: number,
  dampingRatio: number,
): ControlResponseReport {
  if (naturalFrequency <= 0 || dampingRatio <= 0)
    throw new Error(
      "Second-order response requires positive natural frequency and damping ratio.",
    );
