...
describe("engineering math blueprint", () => {
  it("maps the complete engineering syllabus into major B.Tech domains", () => {
    ...
    expect(engineeringMathSummary.topicCount).toBe(
      engineeringSyllabusTopics.length,
    );
    expect(engineeringMathSummary.topicCount).toBeGreaterThan(40);
    expect(
      engineeringMathDomains.every((domain) => domain.topics.length > 0),
    ).toBe(true);
  });

  it("keeps every engineering domain native, formula-rich, and workspace-aware", () => {
    expect(
      engineeringMathDomains.every((domain) =>
        domain.nativeRoutes.every((route) => route.startsWith("/")),
      ),
    ).toBe(true);
    expect(
      engineeringMathDomains.every((domain) => domain.nativeRoutes.length > 0),
    ).toBe(true);
    expect(
      engineeringMathDomains.every(
        (domain) => domain.formulaFamilies.length >= 4,
      ),
    ).toBe(true);
    expect(
      engineeringMathDomains.every(
        (domain) => domain.workspaceTargets.length >= 3,
      ),
    ).toBe(true);
  ...
  it("defines phased milestones without touching protected 2D/3D workspaces", () => {
    ...
    expect(
      engineeringMathMilestones[0].acceptanceTargets.some((target) =>
        target.includes("No existing 2D/3D workspace layout"),
      ),
    ).toBe(true);
    expect(
      engineeringMathMilestones.every(
        (milestone) => milestone.deliverables.length > 0,
      ),
    ).toBe(true);
  });
