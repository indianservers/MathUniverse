...
export function normalizeOlympyardProgressLite(
  value: OlympyardProgressLite | null | undefined,
): OlympyardProgressLite {
  ...
  return {
    ...
    mockHistory: Array.isArray(value.mockHistory)
      ? value.mockHistory.slice(-10)
      : [],
  };
}

function normalizeTopicMasteryLite(
  value: unknown,
): Record<string, OlympyardTopicProgressLite> {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  return Object.fromEntries(
    Object.entries(value).map(([topicId, item]) => {
      const record =
        item && typeof item === "object"
          ? (item as Partial<OlympyardTopicProgressLite>)
          : {};
      return [
        topicId,
        {
