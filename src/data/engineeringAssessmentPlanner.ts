...
export type EngineeringAssessmentBand =
  "Exam ready" | "Practice ready" | "Build depth" | "Needs foundation";
...
export function buildEngineeringAssessmentPlans(): EngineeringAssessmentPlan[] {
  return engineeringMathDomains
    .map((domain) => {
      const launchers = launchersForDomain(domain.id);
      const solverCount = engineeringSolverPresets.filter(
      ...
      const examPrompt =
        practicePack?.prompts.find((prompt) => prompt.level === "exam") ??
        practicePack?.prompts[0];
...
export function assessmentSummary() {
  ...
  return {
    ...
    averageScore: Math.round(
      plans.reduce((sum, plan) => sum + plan.score, 0) /
        Math.max(1, plans.length),
    ),
    examReadyCount: plans.filter((plan) => plan.band === "Exam ready").length,
    needsFoundationCount: plans.filter(
      (plan) => plan.band === "Needs foundation",
    ).length,
...
export const engineeringExamSprints: EngineeringExamSprint[] = [
  {
    ...
    objective:
      "Run one calculus setup, one ODE classification, and one rank/eigen checkpoint.",
  ...
  },
  {
    ...
    objective:
      "Connect transform-domain reasoning, boundary-value models, and pole/residue interpretation.",
  ...
  },
  {
    ...
    objective:
      "Build iteration tables, probability checks, and optimization decisions with reproducible evidence.",
  ...
  },
  {
    ...
    objective:
      "Move from gradients and surfaces to flux, circulation, and PDE boundary behavior.",
    route: "/syllabus-lab/vector-calculus-field-theorems",
  },
];

export function sprintReadiness(sprint: EngineeringExamSprint) {
  const plans = buildEngineeringAssessmentPlans().filter((plan) =>
    sprint.domainIds.includes(plan.domainId),
  );
  ...
  return {
    ...
    averageScore,
    weakestDomain:
      plans.sort((left, right) => left.score - right.score)[0]?.title ?? "",
  };
}
...
function nextActionFor(
  ...
  if (solverCount === 0)
    return "Use the practice pack, then add solver-backed examples.";
  if (score < 85)
    return "Run the exam prompt and compare against the expected checkpoint.";
