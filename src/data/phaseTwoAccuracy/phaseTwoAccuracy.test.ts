...
import {
  ...
  validateAdvancedContracts,
  validateSystemContracts,
} from "./validatePhaseTwo";
import {
  advancedExampleCoverage,
  advancedExampleRequirements,
  auditLearningLibraries,
  ...
  phaseTwoCertificationEvidence,
  phaseTwoCertificationSummary,
  phaseTwoCurriculumMappings,
} from ".";
...
describe("Phase 2 accuracy roadmap contracts", () => {
  it("covers every advanced roadmap row with a valid contract", () => {
    ...
    expect(new Set(phaseTwoAdvancedContracts.map((item) => item.id)).size).toBe(
      54,
    );
  });
  ...
  it("matches the roadmap count for every advanced domain", () => {
    ...
    for (const domain of PHASE_TWO_ADVANCED_DOMAINS)
      expect(
        phaseTwoAdvancedContracts.filter((item) => item.domain === domain),
      ).toHaveLength(expected.get(domain)!);
  });
  ...
  it("covers every library, tool, AR, practice, curriculum, and certification system", () => {
    ...
    expect(new Set(phaseTwoSystemContracts.map((item) => item.area))).toEqual(
      new Set([
        "libraries",
        "tools-workspaces",
        "ar-xr",
  ...
  it("maps every contract to an existing top-level application route", async () => {
    ...
    for (const contract of [
      ...phaseTwoAdvancedContracts,
      ...phaseTwoSystemContracts,
    ]) {
      const topLevel = contract.route.split("/").filter(Boolean)[0];
      expect(appSource, `${contract.id} route is not registered`).toContain(
        `path="${topLevel}`,
      );
    }
  });
  ...
  it("maps every advanced contract to curriculum metadata with no duplicate keys", () => {
    expect(curriculumCoverageSummary()).toMatchObject({
      mapped: 54,
      total: 54,
      percentage: 100,
    });
    ...
    expect(
      new Set(
        phaseTwoCurriculumMappings.map(
          (item) =>
            `${item.board}:${item.grade}:${item.chapter}:${item.conceptId}`,
        ),
      ).size,
    ).toBe(54);
  ...
  it("reports evidence honestly without self-certifying unfinished work", () => {
    ...
    expect(phaseTwoCertificationSummary()).toEqual({
      total: 69,
      inventory: 0,
      inProgress: 69,
      certified: 0,
  ...
  it("provides the complete 14-example depth target for every advanced concept", () => {
    ...
    expect(new Set(phaseTwoAdvancedExamples.map((item) => item.id)).size).toBe(
      54 * 14,
    );
    for (const contract of phaseTwoAdvancedContracts)
      expect(advancedExampleCoverage(contract.id)).toEqual(
        advancedExampleRequirements,
      );
  });
  ...
  it("provides all five assessed modes with hints, rubric, misconception, and oracle", () => {
    ...
    for (const contract of phaseTwoAdvancedContracts) {
      const items = phaseTwoAdvancedAssessments.filter(
        (item) => item.conceptId === contract.id,
      );
      expect(new Set(items.map((item) => item.mode))).toEqual(
        new Set(contract.assessmentModes),
      );
      expect(
        items.every(
          (item) =>
            item.hintLadder.length === 2 &&
            item.scoringRule.criteria.length === 4 &&
            item.oracle,
        ),
      ).toBe(true);
    }
  ...
  it("publishes automated evidence without claiming manual certification", () => {
    expect(automatedEvidenceSummary()).toEqual({
      contracts: 54,
      withFullExamples: 54,
      withFullAssessments: 54,
      awaitingManualReview: 54,
    });
  });
});
