...
export const ADVANCED_EXAMPLE_KINDS = [
  "foundational",
  "visual",
  "real-world",
  "common-mistake",
...
export const phaseTwoAdvancedExamples: AdvancedConceptExample[] =
  phaseTwoAdvancedContracts.flatMap((contract) =>
    ADVANCED_EXAMPLE_KINDS.flatMap((kind) =>
      Array.from({ length: advancedExampleRequirements[kind] }, (_, index) => {
        const context =
          contract.exampleContexts[index % contract.exampleContexts.length];
        ...
        return {
          id: `${contract.id}.example.${kind}.${index + 1}`,
          conceptId: contract.id,
          kind,
          title: `${contract.topic}: ${kind.replace("-", " ")} ${index + 1}`,
          prompt: promptByKind[kind],
          accuracyCheck: contract.oracle,
          assumptions: [...contract.restrictions],
          ...(kind === "real-world"
            ? {
                units: "Use context-appropriate SI units and show conversions.",
              }
            : {}),
        };
      }),
...
export function advancedExampleCoverage(conceptId: string) {
  const examples = phaseTwoAdvancedExamples.filter(
    (example) => example.conceptId === conceptId,
  );
  return Object.fromEntries(
    ADVANCED_EXAMPLE_KINDS.map((kind) => [
      kind,
      examples.filter((example) => example.kind === kind).length,
    ]),
  ) as Record<AdvancedExampleKind, number>;
}
