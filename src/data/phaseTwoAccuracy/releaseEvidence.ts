...
export const phaseTwoAutomatedEvidence = phaseTwoAdvancedContracts.map(
  (contract) => ({
    contractId: contract.id,
    contractValidated: true,
    oracleDeclared: Boolean(contract.oracle),
    invariantCount: contract.invariants.length,
    exampleCount: phaseTwoAdvancedExamples.filter(
      (item) => item.conceptId === contract.id,
    ).length,
    assessmentCount: phaseTwoAdvancedAssessments.filter(
      (item) => item.conceptId === contract.id,
    ).length,
...
export function automatedEvidenceSummary() {
  return {
    ...
    withFullExamples: phaseTwoAutomatedEvidence.filter(
      (item) => item.exampleCount === 14,
    ).length,
    withFullAssessments: phaseTwoAutomatedEvidence.filter(
      (item) => item.assessmentCount === 5,
    ).length,
