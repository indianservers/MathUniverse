...
type StoredState = {
  records: { id: string; source: string; revision: number }[];
  assumptions: MathAssumption[];
};
...
export class MathDependencyGraph {
  ...
  define(source: string, stableId?: string): DependencyRecord | undefined {
        ...
        parsed.ast?.type !== "DEFINITION"
          ? [
              ...parsed.diagnostics,
              {
                ...
                severity: "ERROR",
                message:
                  "Dependency graph entries must be symbol or function definitions.",
              },
            ]
          : parsed.diagnostics;
      this.emit([]);
    ...
    const dependencies = collectSymbols(
      ast.expression,
      new Set(ast.parameters),
    ).filter((symbol) => symbol !== ast.name);
    ...
    if (existing && existing.symbol !== record.symbol)
      this.symbolIds.delete(existing.symbol);
    this.records.set(id, record);
    this.symbolIds.set(record.symbol, id);
    this.revision += 1;
    this.redoStack = [];
  ...
  remove(id: string) {
    ...
    this.symbolIds.delete(record.symbol);
    this.revision += 1;
    this.redoStack = [];
    this.recalculate([record.symbol]);
  }

  setAssumptions(assumptions: MathAssumption[]) {
    this.captureUndo();
    this.assumptions = clone(assumptions);
    this.revision += 1;
    this.redoStack = [];
    this.recalculate([...this.symbolIds.keys()]);
  }
  beginTransaction() {
    if (!this.transactionStart) this.transactionStart = this.exportState();
  }
  commitTransaction() {
  ...
  rollbackTransaction() {
    if (this.transactionStart) {
      const state = this.transactionStart;
      this.transactionStart = undefined;
  ...
  undo() {
    ...
    this.redoStack.push(this.exportState());
    this.restoreState(state, false);
    return true;
  }
  redo() {
  ...
  getSnapshot(): GraphSnapshot {
    return {
      ...
      assumptions: clone(this.assumptions),
      revision: this.revision,
      diagnostics: clone(this.diagnostics),
    };
  }
  getRecord(idOrSymbol: string) {
    return (
      this.records.get(idOrSymbol) ??
      (this.symbolIds.get(idOrSymbol)
        ? this.records.get(this.symbolIds.get(idOrSymbol) as string)
        : undefined)
    );
  }
  getEnvironment(): EvaluationEnvironment {
    const environment: EvaluationEnvironment = {};
    for (const record of this.topologicalRecords().records)
  ...
  private recalculate(changedSymbols: string[]) {
    ...
    while (expanded) {
      ...
      for (const record of this.records.values())
        if (
          record.dependencies.some((dependency) => affected.has(dependency)) &&
          !affected.has(record.symbol)
        ) {
          affected.add(record.symbol);
          expanded = true;
        }
    }
    for (const record of this.records.values())
      if (affected.has(record.symbol)) record.status = "DIRTY";
    ...
    for (const record of topology.records) {
      if (record.status === "DIRTY") {
        ...
        const missing = record.dependencies.filter(
          (dependency) => !this.symbolIds.has(dependency),
        );
        record.result = missing.length
          ? {
              status: "ERROR",
              assumptionsUsed: [],
              diagnostics: missing.map((symbol) => ({
                code: "UNDEFINED_SYMBOL",
                severity: "ERROR",
                message: `Symbol '${symbol}' is not defined.`,
                nodeId: record.ast.id,
              })),
              provenance: [],
            }
          : evaluateMath(record.ast, environment, this.assumptions);
        record.status = record.result.status === "ERROR" ? "ERROR" : "CLEAN";
        changedIds.push(record.id);
      }
      if (record.result.value) environment[record.symbol] = record.result.value;
    }
    for (const cycleId of topology.cycleIds) {
      ...
      if (record) {
        ...
        changedIds.push(cycleId);
      }
    }
    this.emit(changedIds);
  }

  private topologicalRecords(): {
    records: DependencyRecord[];
    cycleIds: string[];
    diagnostics: MathDiagnostic[];
  } {
    ...
    for (const record of this.records.values())
      for (const dependency of record.dependencies) {
        const parentId = this.symbolIds.get(dependency);
        if (parentId) {
    ...
    const ordered: DependencyRecord[] = [];
    while (queue.length) {
      const record = queue.shift() as DependencyRecord;
      ordered.push(record);
      for (const childId of (children.get(record.id) ?? []).sort()) {
    ...
    const cycleIds = [...this.records.keys()]
      .filter((id) => !ordered.some((record) => record.id === id))
      .sort();
    const diagnostics = cycleIds.length
      ? [
          {
            code: "CIRCULAR_DEPENDENCY",
            severity: "ERROR" as const,
            message: `Circular dependency detected: ${cycleIds.map((id) => this.records.get(id)?.symbol).join(" → ")}.`,
            details: { nodeIds: cycleIds },
          },
        ]
      : [];
  ...
  private captureUndo() {
    if (!this.transactionStart) this.undoStack.push(this.exportState());
  }
  private restoreState(state: StoredState, capture = true) {
    if (capture) this.captureUndo();
    this.records.clear();
    this.symbolIds.clear();
    this.assumptions = clone(state.assumptions);
    for (const stored of state.records) {
      const parsed = parseMath(stored.source);
      const ast = parsed.ast;
      if (ast?.type === "DEFINITION") {
        ...
        this.symbolIds.set(record.symbol, record.id);
      }
    }
    this.revision += 1;
    this.recalculate([...this.symbolIds.keys()]);
  }
  private emit(changedIds: string[]) {
    const snapshot = this.getSnapshot();
    this.listeners.forEach((listener) => listener(snapshot, changedIds));
  }
}
