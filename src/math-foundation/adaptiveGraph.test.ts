...
describe("adaptive graph sampling", () => {
  it("refines curved functions and declares its numerical evidence", () => {
    const result = adaptiveSampleExplicit(
      (x) => x * x,
      viewport,
      "HIGH_ACCURACY",
    );
  ...
  it("does not bridge an undefined rational discontinuity", () => {
    ...
    expect(
      result.segments.every(
        (segment) => !(segment.points[0].x < 1 && segment.points.at(-1)!.x > 1),
      ),
    ).toBe(true);
  });

  it("supports cancellable stale calculations", () => {
    const result = adaptiveSampleExplicit(Math.sin, viewport, "BALANCED", {
      cancelled: true,
    });
    expect(result.evidence.convergence).toBe("CANCELLED");
    expect(
      result.diagnostics.some((entry) => entry.code === "SAMPLING_CANCELLED"),
    ).toBe(true);
  });
});
