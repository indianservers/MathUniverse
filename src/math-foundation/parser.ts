...
function tokenize(source: string): {
  ...
  while (index < source.length) {
    ...
    if (/[A-Za-z_\u03c0\u03b1-\u03c9\u0391-\u03a9]/.test(character)) {
      ...
      while (
        index < source.length &&
        /[A-Za-z0-9_\u03b1-\u03c9\u0391-\u03a9]/.test(source[index])
      )
        index += 1;
      tokens.push({
        kind: "identifier",
...
class Parser {
  ...
  private tryDefinition(): MathAstNode | undefined {
    ...
    return this.node("DEFINITION", range, {
      name: name.value,
      parameters,
      expression,
      mathType: parameters.length ? "FUNCTION" : expression.mathType,
  ...
  private parseComparison(): MathAstNode {
    ...
    if (token.value === "=") {
      ...
      return this.node("EQUATION", this.span(left, right), {
        left,
        right,
        mathType: "EQUATION",
        domain: "BOOLEAN",
    ...
    if (comparisonOperators.has(token.value)) {
      this.consume();
      const right = this.parseBinary(0);
      return this.node("INEQUALITY", this.span(left, right), {
  ...
  private parseBinary(minimum: number): MathAstNode {
    ...
    while (true) {
      ...
      left = this.node("BINARY_OPERATION", this.span(left, right), {
  ...
  private parseUnary(): MathAstNode {
    ...
    if (token.value === "+" || token.value === "-") {
      ...
      return this.node(
        "UNARY_OPERATION",
        { start: token.start, end: operand.sourceRange.end },
        {
          operator: token.value,
          operand,
          mathType: operand.mathType,
  ...
  private parsePrimary(): MathAstNode {
    ...
    if (token.kind === "number")
      return this.node(
        ...
        {
          value: token.value,
          literalKind: token.value.includes(".") ? "DECIMAL" : "INTEGER",
          mathType: "SCALAR",
          domain: token.value.includes(".") ? "REAL" : "INTEGER",
        },
      );
    if (token.kind === "identifier") {
      ...
      if (this.match("(")) {
        ...
        if (!this.check(")")) {
          do {
            arguments_.push(this.parseComparison());
          } while (this.match(","));
        }
      ...
      return this.node(
        ...
        { start: token.start, end: token.end },
        {
          name: token.value === "π" ? "pi" : token.value,
          mathType: "SCALAR",
          domain: "UNKNOWN",
        },
      );
    ...
    if (token.value === "(") {
      ...
      if (this.match(",")) {
        ...
        return this.node(
          "VECTOR",
          { start: token.start, end: close.end },
          { items, mathType: "VECTOR", domain: "UNKNOWN" },
        );
      }
      this.expect(")", "MISSING_CLOSING_PARENTHESIS");
      return first;
    }
    if (token.value === "[") return this.parseBracket(token);
    this.error(
      "EXPECTED_EXPRESSION",
      "Expected a number, symbol, function, vector, list, or matrix.",
      token,
    );
    return this.node(
      "LITERAL",
    ...
    if (items.length > 0 && items.every((item) => item.type === "LIST")) {
      ...
      return this.node(
        ...
        { start: open.start, end: close.end },
        {
          ...
          mathType: "MATRIX",
          domain: "UNKNOWN",
          validation: widths.size > 1 ? "INVALID" : "VALID",
        },
      );
    }
    return this.node(
      "LIST",
      { start: open.start, end: close.end },
      { items, mathType: "LIST", domain: "UNKNOWN" },
    );
  }

  private node<T extends MathAstNode["type"]>(
    type: T,
    sourceRange: SourceRange,
    extra: Omit<
      Extract<MathAstNode, { type: T }>,
      "type" | "id" | "sourceRange" | "assumptions" | "validation"
    > & { validation?: "VALID" | "INVALID" | "UNVALIDATED" },
  ): Extract<MathAstNode, { type: T }> {
    const validation = extra.validation ?? "VALID";
    return {
      type,
      id: `ast-${sourceRange.start}-${sourceRange.end}-${this.sequence++}`,
      sourceRange,
      assumptions: [],
      validation,
      ...extra,
    } as unknown as Extract<MathAstNode, { type: T }>;
  }
  private span(left: MathAstNode, right: MathAstNode) {
    return { start: left.sourceRange.start, end: right.sourceRange.end };
  }
  ...
  private expect(value: string, code: string) {
    ...
    const token = this.peek();
    this.error(code, `Expected '${value}'.`, token);
    return token;
  }
  private error(code: string, message: string, token: Token) {
    this.diagnostics.push({
      code,
      severity: "ERROR",
      message,
      sourceRange: { start: token.start, end: token.end },
    });
  }
}
...
export function parseMath(source: string): ParseResult {
  ...
  return {
    source,
    ast,
    diagnostics: [...tokenized.diagnostics, ...parser.diagnostics],
  };
}
...
export function collectSymbols(
  ...
  const visit = (current: MathAstNode) => {
    switch (current.type) {
      case "SYMBOL":
        if (
          !excluded.has(current.name) &&
          !["pi", "e", "i"].includes(current.name)
        )
      ...
      case "FUNCTION_CALL":
        ...
        if (!excluded.has(current.name)) symbols.add(current.name);
        break;
      case "DEFINITION":
