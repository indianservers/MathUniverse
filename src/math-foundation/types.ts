...
export type MathDomain =
  "REAL" | "COMPLEX" | "INTEGER" | "NATURAL" | "BOOLEAN" | "UNKNOWN";
...
export type LiteralNode = AstBase & {
  type: "LITERAL";
  value: string;
  literalKind: "INTEGER" | "DECIMAL" | "BOOLEAN";
};
export type SymbolNode = AstBase & { type: "SYMBOL"; name: string };
export type UnaryOperationNode = AstBase & {
  type: "UNARY_OPERATION";
  operator: "+" | "-";
  operand: MathAstNode;
};
export type BinaryOperationNode = AstBase & {
  type: "BINARY_OPERATION";
  operator: "+" | "-" | "*" | "/" | "^";
  left: MathAstNode;
  right: MathAstNode;
};
export type FunctionCallNode = AstBase & {
  type: "FUNCTION_CALL";
  name: string;
  arguments: MathAstNode[];
};
export type DefinitionNode = AstBase & {
  type: "DEFINITION";
  name: string;
  parameters: string[];
  expression: MathAstNode;
};
export type EquationNode = AstBase & {
  type: "EQUATION";
  left: MathAstNode;
  right: MathAstNode;
};
export type InequalityNode = AstBase & {
  type: "INEQUALITY";
  operator: "<" | "<=" | ">" | ">=" | "!=";
  left: MathAstNode;
  right: MathAstNode;
...
export type PiecewiseNode = AstBase & {
  type: "PIECEWISE";
  cases: { value: MathAstNode; condition: MathAstNode }[];
  otherwise?: MathAstNode;
};
...
export type DecimalValue = {
  ...
  scale: number;
  precision: number;
  roundingMode: "HALF_EVEN" | "HALF_UP" | "DOWN";
};
export type SurdValue = {
  kind: "SURD";
...
export type ComplexValue = {
  kind: "COMPLEX";
  real: IntegerValue | RationalValue;
  imaginary: IntegerValue | RationalValue;
};
...
export type FunctionValue = {
  ...
  parameters: string[];
  body: MathAstNode;
  closure: Record<string, MathValue>;
};
