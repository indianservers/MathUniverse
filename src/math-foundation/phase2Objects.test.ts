...
describe("phase 2 object registry", () => {
  it("keeps objects backed by the shared dependency graph", () => {
    ...
    const registry = new Phase2ObjectRegistry(graph);
    const scalar = registry.add({
      id: "scalar",
      type: "SCALAR",
      name: "a",
      definition: "a=2",
    ...
    const fn = registry.add({
      id: "fn",
      type: "FUNCTION",
      name: "f",
      definition: "f(x)=x^2+a",
      visible: true,
      locked: false,
    });
    expect(graph.evaluateExpression("f(a)").exactForm).toBe("6");
    expect(registry.usedBy(scalar.id)).toContain(fn.nodeId);
    registry.select([scalar.id, fn.id]);
    registry.batch({ visible: false, style: { opacity: 0.4 } });
    expect(
      registry
        .list()
        .every((entry) => !entry.visible && entry.style.opacity === 0.4),
    ).toBe(true);
