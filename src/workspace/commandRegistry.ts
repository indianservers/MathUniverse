...
const specs: WorkspaceCommandSpec[] = [
  command("Plot", "Algebra", "Plot[expression]", "Graph a function or relation.", true, ["Plot[sin(x)]", "plot x^2-4"], ["Graph", "Function", "Draw", "Curve", "Visualize"]),
  command("Solve", "Algebra", "Solve[equation]", "Solve equations exactly where possible and numerically otherwise.", true, ["Solve[x^2-5*x+6=0]"], ["Roots", "Equation", "FindX"]),
  command("SolveSystem", "Algebra", "SolveSystem[equation1, equation2]", "Solve simultaneous equations exactly where possible.", true, ["SolveSystem[x+y=5, x-y=1]"], ["System", "Simultaneous", "Eliminate", "LinearSystem"]),
  command("Root", "Algebra", "Root[polynomial]", "Find real roots of a polynomial or expression.", true, ["Root[x^2-4]"], ["Zeros", "Zero", "RootsOf"]),
  command("Extremum", "Algebra", "Extremum[function]", "Find local extrema over the visible domain.", true, ["Extremum[x^2-4*x+1]"], ["Vertex", "Maximum", "Minimum", "MinMax", "TurningPoint"]),
  command("Factor", "Algebra", "Factor[expression]", "Factor supported algebraic expressions.", true, ["Factor[x^2-5*x+6]"], ["Factorize", "Factorise"]),
  command("Expand", "Algebra", "Expand[expression]", "Expand products and powers.", true, ["Expand[(x+2)(x+3)]"], ["MultiplyOut", "Distribute"]),
  command("Simplify", "Algebra", "Simplify[expression]", "Simplify exact expressions.", true, ["Simplify[2*x+x]"], ["Reduce", "Collect"]),
  command("Substitute", "Algebra", "Substitute[expression, a=value]", "Substitute values into an expression.", true, ["Substitute[x^2+a, a=3]"], ["Replace", "EvaluateAt", "PlugIn"]),
  command("PartialFractions", "Algebra", "PartialFractions[rational expression]", "Decompose rational expressions into simpler fractions where supported.", true, ["PartialFractions[(3*x+5)/((x+1)(x+2))]"], ["Apart", "PartialFraction"]),
  command("PolynomialDivide", "Algebra", "PolynomialDivide[dividend, divisor]", "Divide polynomials and report quotient/remainder.", true, ["PolynomialDivide[x^3-1, x-1]"], ["Divide", "LongDivision"]),
  command("Derivative", "Calculus", "Derivative[function]", "Differentiate a function symbolically where possible.", true, ["Derivative[x^3-2*x]"], ["Diff", "Differentiate", "Slope", "TangentSlope"]),
  command("Integral", "Calculus", "Integral[function]", "Compute an antiderivative where possible.", true, ["Integral[3*x^2]"], ["Integrate", "Antiderivative", "AreaUnder"]),
  command("Limit", "Calculus", "Limit[function, variable, value]", "Compute or estimate a limit.", true, ["Limit[sin(x)/x, x, 0]"], ["Lim", "Approaches"]),
  ...
  command("Intersect", "Geometry 2D", "Intersect[object1, object2]", "Find intersections of functions or supported geometry objects.", true, ["Intersect[x^2, 2*x+3]"], ["Intersection", "Intersections", "Meet", "Cross"]),
  command("Point", "Geometry 2D", "Point[x, y]", "Create a point from coordinates.", true, ["Point[2, 3]"], ["Coordinate", "Coordinates"]),
  command("Line", "Geometry 2D", "Line[pointA, pointB]", "Create a line from two points.", true, ["Line[(0,0), (4,3)]"], ["StraightLine"]),
  command("Segment", "Geometry 2D", "Segment[pointA, pointB]", "Create a bounded segment.", true, ["Segment[(0,0), (4,3)]"], ["LineSegment"]),
  command("Ray", "Geometry 2D", "Ray[pointA, pointB]", "Create a ray.", true, ["Ray[(0,0), (4,3)]"], ["HalfLine"]),
  command("Vector", "Geometry 2D", "Vector[pointA, pointB]", "Create a vector.", true, ["Vector[(0,0), (3,2)]"], ["Arrow", "Direction"]),
  command("Circle", "Geometry 2D", "Circle[center, radius]", "Create a circle by center and radius.", true, ["Circle[(0,0), 4]"], ["Circumference", "Disk"]),
  ...
  command("Area", "Geometry 2D", "Area[object]", "Measure area.", true, ["Area[circle radius 5]"], ["MeasureArea"]),
  command("Distance", "Geometry 2D", "Distance[object1, object2]", "Measure distance.", true, ["Distance[(0,0), (3,4)]"], ["Length", "MeasureDistance"]),
  command("Angle", "Geometry 2D", "Angle[A, B, C]", "Measure or construct an angle.", true, ["Angle[(1,0), (0,0), (0,1)]"], ["MeasureAngle", "Degrees"]),
  command("Surface", "Geometry 3D", "Surface[z=f(x,y)]", "Create a 3D surface.", true, ["Surface[z=sin(x)*cos(y)]"], ["Surface3D", "Graph3D", "Mesh"]),
  ...
  command("Table", "Lists And Tables", "Table[function, start, end, step]", "Create a value table.", true, ["Table[sin(x), -3, 3, 0.5]"], ["Values", "ValueTable"]),
  ...
  command("Matrix", "Matrices", "Matrix[[a,b],[c,d]]", "Create a matrix object.", true, ["Matrix[[1,2],[3,4]]"], ["Mat"]),
  command("Determinant", "Matrices", "Determinant[matrix]", "Compute a determinant.", true, ["Determinant[[1,2],[3,4]]"], ["Det"]),
  command("Transpose", "Matrices", "Transpose[matrix]", "Transpose a matrix.", true, ["Transpose[[1,2],[3,4]]"], ["Trans"]),
  command("Inverse", "Matrices", "Inverse[matrix]", "Invert a supported square matrix.", true, ["Inverse[[1,2],[3,4]]"], ["Inv"]),
