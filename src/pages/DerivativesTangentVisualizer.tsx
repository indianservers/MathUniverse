  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 xl:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <SectionCard title="Tangent and Secant Graph" description="Blue is f(x), orange is tangent, violet is secant, green is f'(x) when enabled.">
...
function DerivativeGraph({ samples, derivativeSamples, fn, a, h, derivative, showSecant, showDerivative }: { samples: Point[]; derivativeSamples: Point[]; fn: ((x: number) => number) | null; a: number; h: number; derivative: number; showSecant: boolean; showDerivative: boolean }) {
  ...
  return (
    <svg viewBox="0 0 760 460" className="h-[340px] w-full rounded-2xl bg-slate-50 dark:bg-slate-950 sm:h-[460px]">
      ...
      <path d={path(samples, sx, sy, yMin, yMax)} fill="none" stroke="#0891b2" strokeWidth="4" />
      {showDerivative && <path d={path(derivativeSamples, sx, sy, yMin, yMax)} fill="none" stroke="#10b981" strokeWidth="4" opacity="0.8" />}
      {tangent && <line x1={sx(tangent[0][0])} y1={sy(tangent[0][1])} x2={sx(tangent[1][0])} y2={sy(tangent[1][1])} stroke="#f59e0b" strokeWidth="4" />}
      {showSecant && secant && <line x1={sx(secant[0][0])} y1={sy(secant[0][1])} x2={sx(secant[1][0])} y2={sy(secant[1][1])} stroke="#8b5cf6" strokeWidth="4" strokeDasharray="8 7" />}
      {Number.isFinite(fa) && <g><circle cx={sx(a)} cy={sy(fa)} r="8" fill="#ef4444" stroke="#0f172a" strokeWidth="2" /><text x={sx(a) + 12} y={sy(fa) - 10} fontSize="13" fontWeight="900" fill="#0f172a">point of tangency</text></g>}
      {Number.isFinite(fa) && Number.isFinite(rise) && <g><line x1={sx(a)} y1={sy(fa)} x2={sx(a + run)} y2={sy(fa)} stroke="#0f172a" strokeWidth="3" /><line x1={sx(a + run)} y1={sy(fa)} x2={sx(a + run)} y2={sy(fa + rise)} stroke="#0f172a" strokeWidth="3" /><text x={sx(a + run) + 8} y={sy(fa + rise)} fontSize="13" fontWeight="900" fill="#0f172a">rise/run</text></g>}
