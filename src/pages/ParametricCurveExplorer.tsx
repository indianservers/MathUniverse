...
export default function ParametricCurveExplorer() {
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionCard title="Traced Parametric Curve" description="A glowing path traces the current x(t), y(t) sweep." compact tone="spotlight">
        <div className="cinematic-graph-stage p-2">
        <ResponsiveContainer width="100%" height="100%">
          <ScatterChart margin={{ top: 20, right: 20, bottom: 10, left: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(103,232,249,.2)" />
            <XAxis dataKey="x" type="number" stroke="#bae6fd" tick={{ fill: "#bae6fd", fontSize: 11 }} />
            <YAxis dataKey="y" type="number" stroke="#bae6fd" tick={{ fill: "#bae6fd", fontSize: 11 }} />
            <Tooltip contentStyle={{ borderRadius: 14, border: "1px solid rgba(103,232,249,.25)", background: "rgba(2,6,23,.94)", color: "#f8fafc" }} labelStyle={{ color: "#e0f2fe" }} />
            <Line dataKey="y" data={data} stroke="#22d3ee" strokeWidth={4} dot={false} />
...
function Expr({ label, value, onChange }: { label: string; value: string; onChange: (v: string) => void }) {
  return <label><span className="text-sm font-bold">{label}</span><input className="premium-input mt-2" value={value} onChange={(event) => onChange(event.target.value)} /></label>;
}
