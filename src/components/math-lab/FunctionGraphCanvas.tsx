...
export default function FunctionGraphCanvas({
  ...
  const xMax = axisValue(view.xMax, logX);
  const yMin = axisValue(view.yMin, logY);
  const yMax = axisValue(view.yMax, logY);
  ...
  const traceSeries =
    series.find((item) => item.id === selectedSeriesId && item.visible) ??
    series.find((item) => item.visible);
  const tracePoint =
    traceSeries && typeof traceX === "number"
      ? nearestPoint(traceSeries.points, traceX)
      : null;
  ...
  function handlePointerMove(event: PointerEvent<SVGSVGElement>) {
    if (dragRef.current && event.buttons === 1 && onViewChange) {
      ...
      const dx =
        ...
        (view.xMax - view.xMin);
      const dy =
        ((event.clientY - dragRef.current.clientY) / rect.height) *
        (view.yMax - view.yMin);
      onViewChange({
        xMin: dragRef.current.view.xMin - dx,
        xMax: dragRef.current.view.xMax - dx,
        yMin: dragRef.current.view.yMin + dy,
        yMax: dragRef.current.view.yMax + dy,
      });
      return;
    }
    ...
    onTraceChange(
      logX
        ? 10 ** (xMin + ratio * (xMax - xMin))
        : view.xMin + ratio * (view.xMax - view.xMin),
    );
  }
  ...
  function handleKeyDown(event: ReactKeyboardEvent<SVGSVGElement>) {
    ...
    if (
      ...
      onTraceChange(
        Math.min(
          view.xMax,
          Math.max(view.xMin, traceX + (direction * xSpan) / 100),
        ),
      );
      return;
    }
    ...
    const yDelta =
      event.key === "ArrowDown"
        ...
        : event.key === "ArrowUp"
          ...
          : 0;
    onViewChange({
      xMin: view.xMin + xDelta,
      xMax: view.xMax + xDelta,
      yMin: view.yMin + yDelta,
      yMax: view.yMax + yDelta,
    });
  }
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={
        tracePoint && typeof tracePoint.y === "number"
          ? `Interactive function graph. Trace point x ${formatTick(tracePoint.x)}, y ${formatTick(tracePoint.y)}`
    ...
    >
      <desc id="function-graph-keyboard-help">
        Use plus and minus to zoom, arrow keys to pan, zero to reset, and Shift
        plus left or right arrow to move the trace point.
      </desc>
      <rect
        width={WIDTH}
        height={HEIGHT}
      ...
      />
      <text x="694" y="222" fill="#64748b" fontSize="13" fontWeight="800">
        x
      </text>
      <text x="364" y="22" fill="#64748b" fontSize="13" fontWeight="800">
        y
      </text>
      {showGrid && (
      ...
      {imageLayers.map((image) => {
        ...
        if (
          ![topLeft.x, topLeft.y, bottomRight.x, bottomRight.y].every(
            Number.isFinite,
          )
        )
          return null;
      ...
      {integralArea && (
        <IntegralArea area={integralArea} view={view} toScreen={toScreen} />
      )}
      {series
            ...
            {item.style === "region"
              ...
              : item.style === "points"
                ...
                : item.style === "vectors"
                  ? vectorSegments(item.points, toScreen).map(
                      (segment, index) => (
                        <g key={`${item.id}-vector-${index}`}>
                          ...
                          />
                          <path
                            d={arrowHead(segment.start, segment.end)}
                            fill="none"
                            stroke={item.color}
                            strokeWidth="2"
                            strokeLinecap="round"
                  ...
                  : segmentsFor(item.points, view, toScreen).map(
                      (segment, index) => (
                        <polyline
                          key={`${item.id}-${index}`}
                          points={segment}
      ...
      {interactivePoints
        .filter((point) => (!logX || point.x > 0) && (!logY || point.y > 0))
        .map((point) => {
          const screen = toScreen(point.x, point.y);
          return (
            <g
              key={`interactive-${point.id}`}
              aria-label={`${point.label} at ${formatTick(point.x)}, ${formatTick(point.y)}`}
            >
              <circle
                className="gs2d-drag-handle"
                cx={screen.x}
                cy={screen.y}
                r="10"
                fill={point.color}
                ...
                onPointerMove={(event) => {
                  if (event.buttons !== 1 || !onInteractivePointChange) return;
                  event.stopPropagation();
                ...
                onKeyDown={(event) => {
                  if (
                    ...
                    return;
                  event.preventDefault();
                  const dx = (view.xMax - view.xMin) / 100;
                  const dy = (view.yMax - view.yMin) / 100;
                  onInteractivePointChange(
                    point.id,
                    point.x +
                      (event.key === "ArrowLeft"
      ...
      {tracePoint && traceSeries && typeof tracePoint.y === "number" && (
        <g>
          <line
            x1={toScreen(tracePoint.x, view.yMin).x}
            x2={toScreen(tracePoint.x, view.yMin).x}
            y1="0"
            y2={HEIGHT}
            stroke="#64748b"
            strokeDasharray="5 7"
          ...
          {precisionCrosshair && (
            <line
              x1="0"
              x2={WIDTH}
              y1={toScreen(tracePoint.x, tracePoint.y).y}
              y2={toScreen(tracePoint.x, tracePoint.y).y}
              stroke="#64748b"
              strokeDasharray="5 7"
              opacity="0.45"
            />
          ...
          <circle
            cx={toScreen(tracePoint.x, tracePoint.y).x}
            cy={toScreen(tracePoint.x, tracePoint.y).y}
            r="7"
            fill={traceSeries.color}
            stroke="#ffffff"
            strokeWidth="3"
          />
          <g
            transform={`translate(${Math.min(WIDTH - 190, Math.max(10, toScreen(tracePoint.x, tracePoint.y).x + 12))}, ${Math.min(HEIGHT - 72, Math.max(12, toScreen(tracePoint.x, tracePoint.y).y - 42))})`}
          >
            <rect
              width="178"
              height="62"
              ...
              opacity="0.92"
            />
            <text x="12" y="22" fill="#e0f2fe" fontSize="13" fontWeight="700">
              {traceSeries.label}
            </text>
            <text x="12" y="44" fill="#ffffff" fontSize="13">
              x=
              {precisionCrosshair
                ? formatPrecision(tracePoint.x)
                : formatTick(tracePoint.x)}
              , y=
              {precisionCrosshair
                ? formatPrecision(tracePoint.y)
                : formatTick(tracePoint.y)}
            </text>
          </g>
        </g>
      )}
...
function IntegralArea({
  area,
  view,
  toScreen,
}: {
  area: NonNullable<FunctionGraphCanvasProps["integralArea"]>;
  view: FunctionGraphView;
  toScreen: (x: number, y: number) => { x: number; y: number };
}) {
  const valid = area.points.filter(
    (point) =>
      point.valid &&
      point.y !== null &&
      point.x >= area.start &&
...
function segmentsFor(
  ...
  points.forEach((point) => {
    const jump =
      previousY !== null &&
      point.y !== null &&
      Math.abs(point.y - previousY) > visibleHeight * 0.65;
    if (
      !point.valid ||
      point.y === null ||
      jump ||
      point.y < view.yMin - visibleHeight ||
      point.y > view.yMax + visibleHeight
    ) {
...
function niceTicks(min: number, max: number, target: number) {
  ...
  for (
    let value = first;
    value <= max + step / 2 && ticks.length < 80;
    value += step
  ) {
...
function graphTicks(
  ...
  const ticks: number[] = [];
  for (let power = start; power <= end && ticks.length < 30; power += 1)
    ticks.push(10 ** power);
  return ticks;
}
...
function vectorSegments(
  ...
  for (let index = 0; index + 1 < points.length; index += 3) {
    ...
    if (!start?.valid || !end?.valid || start.y === null || end.y === null)
      continue;
    const screenStart = toScreen(start.x, start.y);
    const screenEnd = toScreen(end.x, end.y);
    if (
      [screenStart.x, screenStart.y, screenEnd.x, screenEnd.y].every(
        Number.isFinite,
      )
    )
      segments.push({ start: screenStart, end: screenEnd });
  }
  return segments;
}
