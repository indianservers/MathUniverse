...
export default function Grade7ArithmeticExpressionsLab() {
  ...
  const result = useMemo(() => {
    try {
      ...
      return {
        data: null,
        error: error instanceof Error ? error.message : "Check the expression.",
      };
    }
  }, [expression]);
  ...
  return (
    <NCERTChapterScaffold
      ...
      studentTask={{
        ...
        commonMistake:
          "Do not add before multiplying unless brackets tell you to.",
      }}
      teacherNote={{
        objective:
          "Students evaluate numeric expressions using order of operations.",
        prerequisite: "Whole-number operations and brackets.",
        prompt: "Ask students to write the first operation before calculating.",
        misconception:
          "Students often solve strictly left to right even when multiplication appears later.",
        extension:
          "Let students create two expressions with the same numbers but different brackets.",
      }}
      recap={[
        "Brackets are solved first.",
        "Multiplication and division happen before addition and subtraction.",
        "For the same priority, move left to right.",
      ]}
    >
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-950/80">
        <div className="grid gap-3 lg:grid-cols-[1fr_auto]">
          <label className="text-sm font-black text-slate-800 dark:text-slate-100">
            Expression
            <input
              aria-label="Arithmetic expression input"
              value={expression}
        ...
        {result.error || !result.data ? (
          ...
          <div
            className="mt-5 grid gap-4 xl:grid-cols-[minmax(280px,0.8fr)_minmax(0,1.2fr)]"
            aria-live="polite"
          >
            <div className="rounded-2xl bg-slate-950 p-5 text-white">
              <p className="text-xs font-black uppercase tracking-wide text-cyan-200">
                Expression tree idea
              </p>
                ...
                {normalizeExpression(expression)
                  ...
                  .map((char, index) => (
                    <span
                      ...
                      className={
                        char === "(" || char === ")"
                          ? "rounded-lg bg-violet-500/40 px-2 py-1"
              ...
              {result.data.warning && (
                <p className="mt-3 rounded-xl bg-amber-300/15 p-3 text-sm font-bold text-amber-100">
                  {result.data.warning}
                </p>
              )}
            </div>
            <ol className="space-y-3">
              {result.data.steps.length === 0 ? (
                <li className="rounded-2xl border border-slate-200 bg-slate-50 p-4 font-bold dark:border-white/10 dark:bg-slate-900">
                  This expression is already a single number.
                </li>
              ) : (
                result.data.steps.map((step, index) => (
                  <li
                    key={`${step.expression}-${index}`}
                    className="rounded-2xl border border-cyan-100 bg-cyan-50 p-4 dark:border-cyan-300/20 dark:bg-cyan-300/10"
                  >
