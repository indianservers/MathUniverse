...
const presets = [
  ...
  "9,99,999",
  "10,00,00,000",
  "Compare 45,60,000 and 4,560,000",
];
const places: RoundingPlace[] = [
  "ten",
  "hundred",
  "thousand",
  "lakh",
...
export default function Grade7LargeNumbersLab() {
  ...
  return (
    <NCERTChapterScaffold
      ...
      studentTask={{
        tryFirst:
          "Type a population or money amount and switch between Indian and international systems.",
        ...
        commonMistake:
          "Do not group every comma in pairs. Indian system uses 3 digits first, then pairs.",
      }}
      teacherNote={{
        objective:
          "Students connect digit position, commas, number names, rounding, and estimation.",
        prerequisite: "Place value up to lakhs or millions.",
        prompt:
          "Ask students to read the same number in both systems and compare the words.",
        misconception:
          "Students often confuse lakh with million or round by chopping digits.",
        extension:
          "Use real city population, distance, school budget, or stadium capacity data.",
      }}
      recap={[
        "A digit changes value when its place changes.",
        "Indian and international commas group the same digits differently.",
        "Rounding chooses the nearest useful benchmark.",
      ]}
    >
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-950/80">
        <div className="grid gap-3 lg:grid-cols-[1fr_1fr_auto]">
          <label className="text-sm font-black text-slate-800 dark:text-slate-100">
            ...
            <input
              ...
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 font-mono text-lg dark:border-white/10 dark:bg-slate-900"
            />
          </label>
          <label className="text-sm font-black text-slate-800 dark:text-slate-100">
            Compare with
            <input
              aria-label="Comparison number input"
              value={compareInput}
          ...
          <div className="grid grid-cols-2 gap-2 self-end">
            {(["indian", "international"] as const).map((item) => (
              <button
                ...
                className={
                  item === mode ? "action-primary" : "action-secondary"
                }
        ...
        {value === null || compareValue === null ? (
          <div className="mt-4">
            <InvalidMathStateMessage>
              Use whole numbers only. Commas are allowed.
            </InvalidMathStateMessage>
          </div>
        ) : (
          <div className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1.2fr)_minmax(280px,0.8fr)]">
                ...
                <thead>
                  <tr>
                    {rows.map((row) => (
                      <th
                        key={row.label}
                        className="px-2 py-2 text-xs text-slate-500 dark:text-slate-300"
                      >
                        {row.label}
                      </th>
                    ))}
                ...
                <tbody>
                  <tr>
                    {rows.map((row) => (
                      <td
                        key={row.label}
                        className="rounded-xl border border-white bg-white px-2 py-4 text-2xl font-black text-cyan-800 dark:border-white/10 dark:bg-slate-950 dark:text-cyan-100"
                      >
                        {row.digit}
                      </td>
                    ))}
            ...
            <div className="space-y-3" aria-live="polite">
              <Fact
                label="With commas"
                value={formatNumberBySystem(value, mode)}
              />
              ...
              <label className="block rounded-2xl border border-slate-200 bg-slate-50 p-3 text-sm font-bold dark:border-white/10 dark:bg-slate-900">
                ...
                <select
                  ...
                  value={place}
                  onChange={(event) =>
                    setPlace(event.target.value as RoundingPlace)
                  }
                  className="mt-2 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-950"
                >
                  {places.map((item) => (
              ...
              <Fact
                label="Rounded value"
                value={formatNumberBySystem(roundToPlace(value, place), mode)}
              />
              <Fact
                label="Comparison"
                value={
