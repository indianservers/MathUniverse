...
const presets = [
  {
    label: "parallel lines at 40 degrees",
    mode: "transversal" as Mode,
    angle: 40,
  },
  {
    label: "parallel lines at 65 degrees",
    mode: "transversal" as Mode,
    angle: 65,
  },
  ...
  {
    label: "vertical opposite angles",
    mode: "intersecting" as Mode,
    angle: 55,
  },
];
...
export default function Grade7LinesTrianglesLab() {
  ...
  return (
    <NCERTChapterScaffold
      ...
      studentTask={{
        tryFirst:
          "Set the transversal angle to 40 degrees and find the matching angle.",
        ...
        commonMistake:
          "Do not assume all angle pairs are equal. The line relationship matters.",
      }}
      teacherNote={{
        objective:
          "Students identify angle pairs and use triangle angle facts.",
        prerequisite:
          "Basic angle measurement and straight angle equals 180 degrees.",
        prompt: "Ask students to name the angle pair before calculating.",
        misconception:
          "Students often mix corresponding, alternate, and co-interior angles.",
        extension:
          "Ask students to create a triangle and calculate an exterior angle two ways.",
      }}
      recap={[
        "Parallel lines create predictable angle pairs.",
      ...
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <aside className="space-y-4">
          <label className="block text-sm font-black">
            Explorer mode
            <select
              aria-label="Lines and triangles mode"
              value={mode}
          ...
          <div
            className="rounded-2xl border border-cyan-100 bg-cyan-50 p-4 text-sm font-bold text-cyan-950 dark:border-cyan-300/20 dark:bg-cyan-300/10 dark:text-cyan-50"
            aria-live="polite"
          >
            {mode === "transversal" && (
              <p>
                Corresponding angle = {angle} degrees. Co-interior partner ={" "}
                {180 - angle} degrees.
              </p>
            )}
            {mode === "triangle" && (
              <p>
                {triangle.a} + {triangle.b} + {triangle.c} = 180 degrees.
              </p>
            )}
            {mode === "exterior" && (
              <p>Exterior angle = opposite interior angles added together.</p>
            )}
            {mode === "intersecting" && (
              <p>
                Vertical opposite angles are equal: {angle} degrees and {angle}{" "}
                degrees.
              </p>
            )}
          </div>
  ...
  return (
    <g>
      ...
      <g stroke="none" fontSize="32" fontWeight="900">
        <text x="420" y="118" fill="#fff">
          {angle} degrees
        </text>
        <text x="510" y="256" fill="#fff">
          corresponding
        </text>
        <text x="510" y="286" fill="#fff">
          = {angle} degrees
        </text>
        <text x="120" y="120" fill="#93c5fd">
          parallel
        </text>
      </g>
    </g>
...
function TriangleAngles({ a, b, c }: { a: number; b: number; c: number }) {
  return (
    <g>
      ...
      <g stroke="none" fontSize="32" fontWeight="900">
        <text x="190" y="300" fill="#facc15">
          {a} degrees
        </text>
        <text x="505" y="300" fill="#facc15">
          {b} degrees
        </text>
        <text x="345" y="135" fill="#facc15">
          {c} degrees
        </text>
        <text x="230" y="380" fill="#fff">
          {a} + {b} + {c} = 180 degrees
        </text>
      </g>
    </g>
...
function ExteriorAngle({ angle }: { angle: number }) {
  ...
  return (
    <g>
      ...
      <g stroke="none" fontSize="32" fontWeight="900">
        <text x="205" y="300" fill="#fff">
          {angle} degrees
        </text>
        <text x="335" y="155" fill="#fff">
          {other} degrees
        </text>
        <text x="525" y="292" fill="#facc15">
          exterior
        </text>
        <text x="525" y="322" fill="#facc15">
          {exterior} degrees
        </text>
        <text x="210" y="380" fill="#fff">
...
function Intersecting({ angle }: { angle: number }) {
  return (
    <g>
      ...
      <g stroke="none" fontSize="32" fontWeight="900">
        <text x="380" y="170" fill="#facc15">
          {angle} degrees
        </text>
        <text x="335" y="285" fill="#facc15">
          {angle} degrees
        </text>
        <text x="180" y="380" fill="#fff">
