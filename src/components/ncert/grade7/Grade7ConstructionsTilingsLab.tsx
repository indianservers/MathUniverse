...
const constructionSteps: Record<ConstructionMode, string[]> = {
  perpendicular: [
    "Draw segment AB.",
    "Open compass more than half of AB.",
    "Draw arcs above and below from A and B.",
    "Join arc intersections to get the perpendicular bisector.",
  ],
  angle: [
    "Draw an angle.",
    "Draw an arc cutting both arms.",
    "From both cut points, draw equal arcs inside the angle.",
    "Join the vertex to the arc intersection.",
  ],
  triangle: [
    "Draw base 3 units.",
    "From one end draw an arc of radius 4.",
    "From the other end draw an arc of radius 5.",
    "Join the intersection to both ends.",
  ],
};

export default function Grade7ConstructionsTilingsLab() {
  const [construction, setConstruction] =
    useState<ConstructionMode>("perpendicular");
  ...
  return (
    <NCERTChapterScaffold
      ...
      studentTask={{
        tryFirst:
          "Choose perpendicular bisector and read each step before looking at the diagram.",
        ...
        commonMistake:
          "Do not guess the middle point by sight. Use equal arcs or measurements.",
      }}
      teacherNote={{
        objective:
          "Students connect construction steps with geometric conditions and tiling patterns.",
        prerequisite: "Line segments, angles, triangles, and symmetry.",
        prompt:
          "Ask students which information is fixed and which point is constructed.",
        misconception:
          "Students may treat construction as drawing a neat picture instead of following exact conditions.",
        extension:
          "Ask which regular polygons tile the plane and why angles around a point matter.",
      }}
      recap={[
        "Compass arcs preserve equal distances.",
        "Construction steps create points that satisfy a condition.",
        "A tiling works when shapes repeat without gaps or overlaps.",
      ]}
    >
      <section className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
        <div className="rounded-3xl border border-slate-200 bg-slate-950 p-4 text-white shadow-sm">
          <svg
            viewBox="0 0 760 420"
            role="img"
            aria-label={`${construction} construction and ${tiling} tiling diagram`}
        ...
        <aside className="space-y-4">
          <label className="block text-sm font-black">
            ...
            <select
              ...
              value={construction}
              onChange={(event) =>
                setConstruction(event.target.value as ConstructionMode)
              }
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-white/10 dark:bg-slate-900"
            >
          ...
          <label className="block text-sm font-black">
            Tiling pattern
            <select
              aria-label="Tiling pattern"
              value={tiling}
              onChange={(event) => setTiling(event.target.value as TilingMode)}
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-white px-4 py-3 dark:border-white/10 dark:bg-slate-900"
            >
          ...
          <ol className="space-y-2" aria-live="polite">
            {constructionSteps[construction].map((step, index) => (
              <li
                key={step}
                className="rounded-2xl border border-cyan-100 bg-cyan-50 p-3 text-sm font-bold text-cyan-950 dark:border-cyan-300/20 dark:bg-cyan-300/10 dark:text-cyan-50"
              >
                {index + 1}. {step}
              </li>
            ))}
    ...
    return (
      <g transform="translate(60 60)">
        <g stroke="#e2e8f0" strokeWidth="5" fill="none">
          ...
          <path
            d="M145 206 A90 90 0 0 1 170 260"
            stroke="#22d3ee"
            strokeDasharray="8 8"
          />
          <path
            d="M190 170 A70 70 0 0 1 235 225 M175 260 A70 70 0 0 1 235 225"
            stroke="#facc15"
          />
    ...
    return (
      <g transform="translate(50 55)">
        <g stroke="#e2e8f0" strokeWidth="5" fill="none">
          ...
          <path
            d="M70 280 A180 180 0 0 1 250 100 M250 280 A240 240 0 0 0 250 40"
            stroke="#f472b6"
            strokeDasharray="8 8"
          />
        </g>
  ...
  return (
    <g transform="translate(50 70)">
      <g stroke="#e2e8f0" strokeWidth="5" fill="none">
        ...
        <path
          d="M60 200 A150 150 0 0 1 180 70 M60 200 A150 150 0 0 0 180 330 M300 200 A150 150 0 0 0 180 70 M300 200 A150 150 0 0 1 180 330"
          stroke="#22d3ee"
          strokeDasharray="8 8"
        />
      ...
      {cells.map((cell) => {
        ...
        if (mode === "triangle")
          return (
            <path
              key={cell}
              d={`M${x} ${y + 48} L${x + 23} ${y} L${x + 46} ${y + 48} Z`}
              fill={cell % 2 ? "#22d3ee" : "#a78bfa"}
              opacity="0.85"
            />
          );
        if (mode === "hexagon")
          return (
            <polygon
              key={cell}
              points={`${x + 18},${y} ${x + 38},${y + 10} ${x + 38},${y + 34} ${x + 18},${y + 44} ${x - 2},${y + 34} ${x - 2},${y + 10}`}
              fill={cell % 2 ? "#facc15" : "#22d3ee"}
              opacity="0.85"
            />
          );
        return (
          <rect
            ...
            width="42"
            height="42"
            fill={cell % 2 ? "#22d3ee" : "#facc15"}
            opacity="0.85"
          />
        );
      })}
      <text x="0" y="150" fill="#fff" fontWeight="900">
        tiling repeats with no gaps
      </text>
    </g>
  );
}
