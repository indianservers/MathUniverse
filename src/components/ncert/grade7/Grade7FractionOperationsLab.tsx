...
const presets = [
  { label: "1/2 + 1/3", a: [1, 2], b: [1, 3], op: "add" as FractionOperation },
  {
    ...
    a: [3, 4],
    b: [1, 6],
    op: "subtract" as FractionOperation,
  },
  {
    label: "2/3 * 3/5",
    a: [2, 3],
    b: [3, 5],
    op: "multiply" as FractionOperation,
  },
  {
    label: "4/5 / 2/3",
    a: [4, 5],
    b: [2, 3],
    op: "divide" as FractionOperation,
  },
  {
    label: "compare 5/8 and 3/4",
    a: [5, 8],
    b: [3, 4],
    op: "compare" as FractionOperation,
  },
  {
    label: "simplify 18/24",
    a: [18, 24],
    b: [1, 2],
    op: "simplify" as FractionOperation,
  },
];
...
export default function Grade7FractionOperationsLab() {
  ...
  const output = useMemo(() => {
    try {
      ...
      return {
        ...
        commonDenominator: 1,
        comparison: 0,
        error: error instanceof Error ? error.message : "Check the fractions.",
      };
    }
  }, [a, b, operation]);
  ...
  return (
    <NCERTChapterScaffold
      ...
      studentTask={{
        tryFirst:
          "Try 1/2 + 1/3 and predict whether the answer is bigger than 1/2.",
        ...
        commonMistake:
          "Do not add denominators when adding fractions. Make equal-sized parts first.",
      }}
      teacherNote={{
        objective:
          "Students connect fraction operations to equal parts and simplified results.",
        prerequisite:
          "Meaning of numerator, denominator, and equivalent fractions.",
        prompt:
          "Ask students to draw the common-denominator parts before calculating.",
        misconception: "Students may add numerator and denominator separately.",
        extension:
          "Use recipes, sharing, and distance walked as word-problem contexts.",
      }}
      recap={[
        "Equivalent fractions name the same amount.",
        "Addition and subtraction need common-sized parts.",
        "Multiplication and division use different fraction meanings.",
      ]}
    >
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-950/80">
        <div className="grid gap-3 lg:grid-cols-[1fr_1fr_220px]">
          ...
          <label className="text-sm font-black text-slate-800 dark:text-slate-100">
            ...
            <select
              ...
              value={operation}
              onChange={(event) =>
                setOperation(event.target.value as FractionOperation)
              }
        ...
        {output.error ? (
          <div className="mt-4">
            <InvalidMathStateMessage>{output.error}</InvalidMathStateMessage>
          </div>
        ) : (
          <div
            className="mt-5 grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]"
            aria-live="polite"
          >
            ...
            <div className="space-y-3">
              <Fact
                ...
                value={
                  operation === "compare"
                    ? comparisonText(output.comparison)
                    : `${output.result.numerator}/${output.result.denominator}`
                }
              />
              <Fact
                label="Mixed number"
              ...
              <Fact
                ...
                value={
                  operation === "add" || operation === "subtract"
                    ? `${output.commonDenominator}`
                    : "not needed"
                }
              />
              <Fact
                label="Simplest form"
                value={`${output.result.numerator}/${output.result.denominator}`}
              />
            </div>
          </div>
        )}
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          {presets.map((preset) => (
            <button
              ...
              onClick={() => {
                setA({ numerator: preset.a[0], denominator: preset.a[1] });
                setB({ numerator: preset.b[0], denominator: preset.b[1] });
...
function FractionInput({
  ...
  return (
    <fieldset className="rounded-2xl border border-slate-200 p-3 dark:border-white/10">
      <legend className="px-1 text-sm font-black text-slate-800 dark:text-slate-100">
        {label}
      </legend>
      <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2">
        <input
          ...
          className="rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-900"
        />
        ...
        <input
          ...
          onChange={(event) =>
            onChange({ ...value, denominator: Number(event.target.value) })
          }
          className="rounded-xl border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-900"
        />
      </div>
...
function FractionBar({
  ...
  const numerator = Math.min(
    denominator,
    Math.max(0, Math.abs(fraction.numerator)),
  );
  return (
    <div className="rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-white/10 dark:bg-slate-900">
      ...
      >
        {Array.from({ length: denominator }, (_, index) => (
          <span
            key={index}
            className={`h-10 rounded-md ${index < numerator ? color : "bg-white dark:bg-slate-950"}`}
          />
        ))}
      </div>
    </div>
