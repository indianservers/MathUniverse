...
export function isClass10PriorityRoute(
  id: string,
): id is Class10PriorityRouteId {
...
type LabPanel = {
  ...
  metrics: (
    values: number[],
    routeId: Class10PriorityRouteId,
...
export default function Class10BoardExamLab({
  concept,
}: {
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div className="overflow-hidden rounded-2xl border border-cyan-200 bg-slate-950 p-2 dark:border-cyan-300/20">
        <svg
          viewBox="0 0 760 480"
          role="img"
          aria-label={`${concept.title} board exam visualization`}
      ...
      <aside className="space-y-3">
        <NCERTCompactPanel
          title="Live values"
          description="Read these before solving."
          accent="emerald"
        >
          <div className="grid gap-2">
            {config.metrics(values, routeId).map((metric) => (
              <Metric
                key={metric.label}
                label={metric.label}
                value={metric.value}
        ...
        <NCERTCompactPanel
          title="Controls"
          description="Tune the current diagram."
          accent="cyan"
        >
          {controlsPanel}
        </NCERTCompactPanel>
  ...
  const proofPanel = (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
      <div className="space-y-3">
        ...
        <ol className="space-y-2">
          {config.steps(values, routeId).map((step, index) => (
            <li
              key={step}
              className="rounded-2xl border border-slate-200 bg-white p-3 text-sm font-semibold text-slate-700 dark:border-white/10 dark:bg-white/5 dark:text-slate-200"
            >
              <span className="font-black text-cyan-700 dark:text-cyan-200">
                Step {index + 1}:{" "}
              </span>
              {step}
            </li>
      ...
      <NCERTCompactPanel
        ...
        description="Use the current values."
        accent="violet"
      >
  ...
  const practicePanel = (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
      {bankPractice.length > 0 ? (
        <NCERTPracticeCheck
          title={`${concept.title} practice bank`}
          questions={bankPractice}
          conceptId={routeId}
          compact
        />
      ) : (
        <div className="rounded-2xl border border-emerald-200 bg-emerald-50 p-4 dark:border-emerald-300/20 dark:bg-emerald-300/10">
          ...
          />
          <p
            className={`mt-3 text-sm font-bold ${result.ok ? "text-emerald-700 dark:text-emerald-200" : "text-slate-600 dark:text-slate-300"}`}
          >
      ...
      <NCERTCompactPanel
        title="Try next"
        description="Change the preset, then answer again."
        accent="amber"
      >
        {controlsPanel}
      </NCERTCompactPanel>
  ...
  const linksPanel = (
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <Link to="/ncert" className="action-secondary justify-center">
        Open NCERT dashboard
      </Link>
      <Link to="/formulas" className="action-secondary justify-center">
        Open formulas
      </Link>
    </div>
  );
    ...
    <div className="grid gap-4 md:grid-cols-3">
      <NCERTCompactPanel title="Student task" accent="cyan">
        <p className="text-sm font-semibold leading-6 text-slate-700 dark:text-slate-200">
          {concept.tasks[0] ?? "Move values, predict, then verify."}
        </p>
      </NCERTCompactPanel>
      ...
      {bankPractice.length > 0 && (
        <div className="md:col-span-3">
          <NCERTTeacherModePanel
            ...
            questions={bankPractice}
            prompts={concept.tasks}
            misconception={config.feedback(values, routeId)}
            extension={concept.outcomes[1] ?? concept.summary}
          />
        </div>
      )}
    </div>
...
function getLabPanel(
  ...
  if (routeId.includes("solid") || routeId.includes("frustum"))
    return solidsPanel(concept);
...
function circleTangentPanel(concept: NCERTConcept): LabPanel {
  return {
    ...
    subtitle:
      "Move the point of contact and compare a true tangent with a nearby secant.",
    presets: [
    ...
    formula:
      "Radius at the point of contact is perpendicular to the tangent: OT perpendicular PT.",
    ...
    steps: () => [
      "Draw radius OT to the point of contact.",
      "Draw the line perpendicular to OT at T.",
      "A line not perpendicular crosses the circle at two points, so it is a secant, not a tangent.",
    ],
    practice: () => ({
      prompt: "What angle does the radius make with the tangent?",
      expected: 90,
      tolerance: 0.5,
      hint: "Think perpendicular.",
    }),
    feedback: (values) =>
      values[2] === 0
        ? "Mistake watch: the comparison line matches the tangent; tilt it to see a secant."
        : "The tangent touches once; the tilted comparison line cuts the circle twice.",
  };
}
...
function twoTangentsPanel(concept: NCERTConcept): LabPanel {
  return {
    ...
    subtitle:
      "Move point P inside, on, and outside the circle, then verify both tangent lengths.",
    presets: [
    ...
    steps: () => [
      "Join OA, OB, and OP.",
      "OA perpendicular PA and OB perpendicular PB.",
      "Right triangles OAP and OBP share OP and have OA = OB, so PA = PB by RHS.",
    ],
    practice: (values) => ({
      prompt: `If OP=${round(values[0])} and r=${round(values[1])}, find each tangent length.`,
      expected: round(tangentLength(values[0], values[1])),
      tolerance: 0.08,
      hint: "Use sqrt(OP^2 - r^2) when P is outside.",
    }),
...
function trianglePanel(concept: NCERTConcept): LabPanel {
  ...
  return {
    title: isArea
      ...
      : isCriteria
        ? "AA, SSS, SAS similarity checker"
        : "BPT and converse manipulative",
    subtitle: isArea
      ? "Scale one triangle and watch area grow by the square of the side ratio."
      : isCriteria
        ? "Toggle side and angle conditions to see when similarity is valid."
        : "Move D and E on two sides and compare AD/DB with AE/EC.",
    presets: [
      {
        label: "valid theorem case",
        values: isArea ? [1.5, 0, 1] : [0.45, 0.45, 1.5],
      },
      {
        label: "wrong condition",
        values: isArea ? [2, 8, 1] : [0.35, 0.55, 1.2],
      },
      { label: "exam friendly", values: isArea ? [2, 0, 1] : [0.6, 0.6, 2] },
    ],
    labels: isArea
      ? ["Side ratio k", "Angle mismatch", "Area model"]
      : ["Cut or scale A", "Cut or angle B", "Scale/check C"],
    ...
    metrics: (values, routeId) => {
      if (routeId.includes("areas"))
        return [
          ...
          {
            label: "area ratio",
            value: round(similarTriangleAreaRatio(values[0])).toString(),
          },
          {
            label: "verdict",
            value: Math.abs(values[1]) < 0.1 ? "similar" : "not similar",
          },
        ];
      ...
      return [
        ...
        {
          label: "verdict",
          value: ratios.parallel ? "parallel/similar" : "not verified",
        },
      ];
    },
    steps: (values, routeId) =>
      routeId.includes("areas")
        ? [
            "Similar triangles keep equal angles.",
            "All lengths scale by k.",
            "Area uses two dimensions, so area ratio is k^2.",
          ]
        : routeId.includes("criteria")
          ? [
              "Check angle equality for AA.",
              "Check all side ratios for SSS.",
              "Check two side ratios plus the included angle for SAS.",
            ]
          : [
              "Draw DE across the two sides.",
              "Compute AD/DB and AE/EC.",
              "If ratios match, the converse says DE is parallel to BC.",
            ],
    practice: (values, routeId) =>
      routeId.includes("areas")
        ? {
            prompt: `If side ratio k=${round(values[0])}, what is the area ratio?`,
            expected: round(similarTriangleAreaRatio(values[0])),
            tolerance: 0.08,
            hint: "Square the side ratio.",
          }
        : {
            prompt:
              "For a valid BPT/converse case, should the two ratios be equal? Type yes or no.",
            expected: bptRatios(values[0], values[1]).parallel ? "yes" : "no",
            hint: "Compare AD/DB and AE/EC.",
          },
    feedback: (values, routeId) =>
      routeId.includes("areas")
        ? "Common mistake: do not use k for area ratio; use k squared."
        : bptRatios(values[0], values[1]).parallel
          ? "The side ratios match, so the parallel/similarity condition is verified."
          : "The ratios do not match; this is the wrong-case visual.",
  };
}

function linearPanel(concept: NCERTConcept): LabPanel {
  return {
    title: concept.id.includes("consistency")
      ? "Consistency ratio and graph checker"
      : "Substitution and elimination stepper",
    subtitle:
      "Edit coefficients, read the algebra steps, and verify the result on a graph.",
    presets: [
    ...
    formula:
      "a1x + b1y = c1 and a2x + b2y = c2; determinant a1b2 - a2b1 decides uniqueness.",
    theoremLinks: resourceLinksForConcept(concept),
    render: linearSvg,
    metrics: (values) => {
      const verdict = classifyLinearSystem(
        values[0],
        values[1],
        values[2],
        values[3],
      ...
      return [
        ...
        { label: "verdict", value: verdict },
        {
          label: "solution",
          value: solution
            ? `(${round(solution.x)}, ${round(solution.y)})`
            : "not unique",
        },
      ];
    },
    steps: (values) => {
      ...
      return [
        ...
        `Elimination determinant is ${round(a1 * b2 - a2 * b1)}; classify before solving.`,
        "Substitution isolates one variable, then replaces it in the other equation.",
      ];
    },
    practice: (values) => {
      const solution = solveLinearSystem(
      ...
      return solution
        ? {
            prompt: "Find x for the unique solution.",
            expected: round(solution.x),
            tolerance: 0.08,
            hint: "Use elimination or the graph intersection.",
          }
        : {
            prompt: "Type the solution count: none or infinite.",
            expected:
              classifyLinearSystem(
                values[0],
                values[1],
                ...
                ? "none"
                : "infinite",
            hint: "Parallel means none; coincident means infinite.",
          };
    },
    feedback: (values) =>
      `System classification: ${classifyLinearSystem(values[0], values[1], values[2], values[3], values[4], values[5])}. Common mistake: compare all three ratios, not only slopes.`,
  };
}
...
function statsPanel(concept: NCERTConcept): LabPanel {
  return {
    ...
    subtitle:
      "Edit the distribution by preset and compare mean, mode, median, modal class, and median class.",
    presets: [
      {
        label: "marks distribution",
        values: [0, 10, 6, 10, 20, 10, 20, 30, 14, 30, 40, 18, 40, 50, 12],
      },
      {
        label: "height distribution",
        values: [
          140, 150, 4, 150, 160, 9, 160, 170, 16, 170, 180, 11, 180, 190, 5,
        ],
      },
      {
        label: "age distribution",
        values: [10, 20, 3, 20, 30, 8, 30, 40, 15, 40, 50, 7, 50, 60, 2],
      },
      {
        label: "daily wages",
        values: [
          100, 120, 5, 120, 140, 12, 140, 160, 20, 160, 180, 8, 180, 200, 4,
        ],
      },
    ],
    labels: [
    ...
    steps: (values, routeId) => {
      const stats = groupedStats(rowsFromValues(values));
      if (routeId.includes("mode"))
        return [
          `Modal class is row ${stats.modalIndex + 1}.`,
          "Use l + [(f1-f0)/(2f1-f0-f2)]h.",
          `Mode is ${round(stats.modal)}.`,
        ];
      if (routeId.includes("median"))
        return [
          `Total frequency n is ${stats.totalFrequency}.`,
          `Find n/2 = ${round(stats.totalFrequency / 2)} in cumulative frequency.`,
          `Median is ${round(stats.median)}.`,
        ];
      return [
        `Find each class mark xi.`,
        `Compute sum fi xi and divide by n=${stats.totalFrequency}.`,
        `Mean is ${round(stats.mean)}; assumed mean gives the same result.`,
      ];
    },
    practice: (values, routeId) => {
      const stats = groupedStats(rowsFromValues(values));
      const expected = routeId.includes("mode")
        ? stats.modal
        : routeId.includes("median")
          ? stats.median
          : stats.mean;
      return {
        prompt: `Calculate the ${routeId.includes("mode") ? "mode" : routeId.includes("median") ? "median" : "mean"} for this table.`,
        expected: round(expected),
        tolerance: 0.12,
        hint: "Use the highlighted formula and table values.",
      };
    },
    feedback: (values) => {
      ...
      return invalid
        ? "Invalid input: every class needs upper > lower and non-negative frequency."
        : "Valid grouped table. Common mistake: use class marks for mean, not lower limits.";
    },
  };
}
...
function circleRegionPanel(concept: NCERTConcept): LabPanel {
  return {
    ...
    subtitle:
      "Build sectors, segments, annuli, quadrants, semicircles, and cutout-style shaded regions.",
    presets: [
    ...
    formula:
      "Sector = theta/360 * pi r^2; segment = sector - triangle; annulus = pi(R^2-r^2).",
    theoremLinks: resourceLinksForConcept(concept),
    render: circleRegionSvg,
    metrics: (values) => [
      {
        label: "sector",
        value: round(sectorArea(values[1], values[0])).toString(),
      },
      {
        label: "segment",
        value: round(segmentArea(values[1], values[0])).toString(),
      },
      {
        label: "annulus",
        value: round(
          annulusArea(values[1], Math.min(values[1], values[2])),
        ).toString(),
      },
    ],
    steps: () => [
      "Choose the useful full shapes.",
      "Subtract the overlap or cutout.",
      "For segment area, subtract triangle area from sector area.",
    ],
    practice: (values) => ({
      prompt: "Find the sector area for the shown outer circle.",
      expected: round(sectorArea(values[1], values[0])),
      tolerance: 0.12,
      hint: "Use theta/360 times pi r squared.",
    }),
    feedback: (values) =>
      values[2] >= values[1]
        ? "Invalid input: inner radius must be smaller than outer radius."
        : "Common mistake: segment is not just a sector; subtract the triangle.",
  };
}
  ...
  return {
    title: isFrustum
      ...
      : isRecast
        ...
        : "Combined solids exposed-area lab",
    subtitle: isFrustum
      ? "Slice a cone and calculate CSA, TSA, and volume."
      : isRecast
        ? "Melt one solid and solve the missing dimension of another using equal volume."
        : "Stack or remove solids and separate visible surface from hidden joining faces.",
    presets: [
      {
        label: isFrustum
          ? "bucket shape"
      ...
      },
      {
        label: isFrustum
          ...
          : isRecast
            ? "cone to cylinder"
            : "cylinder + hemisphere",
        values: isFrustum ? [4, 3.5, 5] : [3, 5, 2],
      },
      {
        label: isFrustum
          ? "wide frustum"
          : isRecast
            ? "many small spheres"
            : "cone removed",
        values: isFrustum ? [6, 1, 7] : [2.5, 4, 1.5],
      },
    ],
    labels: isFrustum
      ? ["Large radius R", "Small radius r", "Height h"]
      : ["Radius", "Height", "Second radius/height"],
    ...
    metrics: (values, routeId) => {
      if (routeId.includes("frustum")) {
        ...
        return [
          ...
          {
            label: "CSA",
            value: round(frustumCSA(values[0], values[1], l)).toString(),
          },
          {
            label: "volume",
            value: round(
              frustumVolume(values[0], values[1], values[2]),
            ).toString(),
          },
        ];
      }
      ...
      return [
        {
          ...
          value: round(cylinderVolume(values[0], values[1])).toString(),
        },
        {
          label: "cone volume",
          value: round(coneVolume(values[0], values[2])).toString(),
        },
        {
          label: "hidden face",
          value: round(Math.PI * values[0] ** 2).toString(),
        },
      ];
    },
    steps: (values, routeId) =>
      routeId.includes("frustum")
        ? [
            `l = sqrt((R-r)^2+h^2) = ${round(frustumSlantHeight(values[0], values[1], values[2]))}.`,
            "CSA = pi(R+r)l.",
            "Volume = 1/3 pi h(R^2+r^2+Rr).",
          ]
        : routeId.includes("recasting")
          ? [
              "Write volume before = volume after.",
              "Keep pi symbolic until cancellation.",
              "Solve the missing dimension last.",
            ]
          : [
              "Add volumes of joined solids.",
              "For surface area, remove hidden common faces.",
              "Use curved surface only where flat faces are not exposed.",
            ],
    practice: (values, routeId) =>
      routeId.includes("frustum")
        ? {
            prompt: "Find the frustum volume.",
            expected: round(frustumVolume(values[0], values[1], values[2])),
            tolerance: 0.2,
            hint: "Use 1/3 pi h(R^2+r^2+Rr).",
          }
        : routeId.includes("recasting")
          ? {
              prompt: "Find the new cylinder height after recasting.",
              expected: round(
                cylinderVolume(values[0], values[1]) /
                  (Math.PI * Math.max(0.1, values[2]) ** 2),
              ),
              tolerance: 0.15,
              hint: "Set old volume equal to new volume.",
            }
          : {
              prompt: "Find total combined volume for cylinder plus cone.",
              expected: round(
                cylinderVolume(values[0], values[1]) +
                  coneVolume(values[0], values[2]),
              ),
              tolerance: 0.2,
              hint: "Volume adds even when one face is hidden.",
            },
    feedback: (values, routeId) =>
      routeId.includes("frustum") && values[1] >= values[0]
        ? "Warning: for a cone frustum, small radius should be less than large radius."
        : "Common mistake: hidden faces affect surface area, not volume.",
  };
}
...
function heightsPanel(concept: NCERTConcept): LabPanel {
  return {
    ...
    subtitle:
      "Move the observer, compare one-angle and two-angle cases, and calculate height or distance.",
    presets: [
    ...
    formula:
      "tan(theta) = opposite / adjacent, so height = distance * tan(theta).",
    theoremLinks: resourceLinksForConcept(concept),
    render: heightsSvg,
    metrics: (values) => [
      {
        label: "height",
        value: round(heightFromAngleDistance(values[0], values[1])).toString(),
      },
      {
        label: "distance from height",
        value: round(
          distanceFromHeightAngle(
            heightFromAngleDistance(values[0], values[1]),
            values[0],
          ),
        ).toString(),
      },
    ...
    steps: () => [
      "Mark the angle of elevation from the observer.",
      "Use tangent because opposite height and adjacent distance are known.",
      "For two-position problems, write two tangent equations and subtract distances.",
    ],
    practice: (values) => ({
      prompt: `Find the height when angle=${round(values[0])} deg and distance=${round(values[1])}.`,
      expected: round(heightFromAngleDistance(values[0], values[1])),
      tolerance: 0.15,
      hint: "Use distance times tan(angle).",
    }),
    feedback: () =>
      "Common mistake: use tan for height/distance, not sin or cos, when the ground distance is adjacent.",
  };
}
...
function checkPractice(answer: string, practice: Practice) {
  ...
  if (typeof practice.expected === "string") {
    ...
    return {
      ok,
      message: ok
        ? "Correct. Your reasoning matches the theorem."
        : `Not yet. ${practice.hint}`,
    };
  }
  ...
  const ok =
    Math.abs(parsed - practice.expected) <= (practice.tolerance ?? 0.05);
  return {
    ok,
    message: ok
      ? "Correct. Your calculation checks out."
      : `Not yet. Expected about ${round(practice.expected)}. ${practice.hint}`,
  };
}
...
function Label({
  ...
  return (
    <text x={x} y={y} fill={fill} fontSize={size} fontWeight="900">
      {text}
    </text>
  );
}
...
function circleTangentSvg(values: number[]) {
  ...
  return (
    <g>
      ...
      <path
        d={`M ${px - 22 * Math.cos(t)} ${py + 22 * Math.sin(t)} l ${18 * Math.cos(tangentAngle)} ${-18 * Math.sin(tangentAngle)} l ${22 * Math.cos(t)} ${-22 * Math.sin(t)}`}
        fill="none"
        stroke="#facc15"
        strokeWidth="3"
      />
      ...
      <Label
        x={70}
        y={70}
        text="True tangent touches once; secant cuts twice."
      />
    </g>
  );
}
...
function twoTangentsSvg(values: number[]) {
  ...
  const base = 0;
  const t1 = {
    x: cx + r * Math.cos(base + alpha),
    y: cy - r * Math.sin(base + alpha),
  };
  const t2 = {
    x: cx + r * Math.cos(base - alpha),
    y: cy - r * Math.sin(base - alpha),
      ...
      {canDraw ? (
        <>
          ...
          <Label
            x={70}
            y={70}
            text={`PA = PB = ${round(tangentLength(distance, radius))}`}
          />
        </>
      ) : (
        <Label
          x={70}
          y={70}
          text={tangentCase(distance, radius)}
          fill="#fecaca"
        />
      )}
    </g>
  ...
  if (routeId.includes("areas")) {
    ...
    return (
      <g>
        ...
        />
        <polygon
          points={`${B2.x},${B2.y} ${C2.x},${C2.y} ${A2.x},${A2.y}`}
          fill="#facc15"
          fillOpacity="0.42"
          stroke="#fde047"
          strokeWidth="4"
        ...
        <Label
          x={75}
          y={70}
          text={`Area ratio = k^2 = ${round(similarTriangleAreaRatio(k))}`}
        />
  ...
  return (
    <g>
      ...
      <line
        ...
        x2={E.x}
        y2={E.y}
        stroke={ratios.parallel ? "#facc15" : "#fb7185"}
        strokeWidth="5"
      />
      {[A, B, C, D, E].map((point, index) => (
        <circle
          key={index}
          cx={point.x}
          cy={point.y}
          r="8"
          fill={index < 3 ? "#f8fafc" : "#facc15"}
        />
      ))}
      <Label
        x={78}
        ...
        text={
          ratios.parallel
            ? "Ratios match: DE || BC"
            : "Wrong case: ratios do not match"
        }
        fill={ratios.parallel ? "#bbf7d0" : "#fecaca"}
      />
      <Label
        x={90}
        y={430}
        text={`AD/DB=${round(ratios.ratioAB)}  AE/EC=${round(ratios.ratioAC)}`}
      />
    </g>
  );
}
  ...
  return (
    <g>
      ...
      <Label
        x={70}
        y={70}
        text={`Verdict: ${classifyLinearSystem(a1, b1, c1, a2, b2, c2)}`}
      />
    </g>
  );
}
...
function statsSvg(values: number[]) {
  ...
  const tickValues = [
    ...new Set(
      Array.from({ length: 5 }, (_, index) => Math.round((maxF * index) / 4)),
    ),
  ];
  return (
    <g
      aria-label={`Grouped frequency chart. Bar heights in students: ${stats.rows.map((row) => row.frequency).join(", ")}`}
    >
        ...
        return (
          <g key={`${row.lower}-${row.upper}`}>
            <rect
              ...
              width="72"
              height={barH}
              fill={index === stats.modalIndex ? "#facc15" : "#22d3ee"}
              opacity="0.78"
              stroke="#e2e8f0"
              strokeWidth="1"
            />
      ...
      <polyline
        points={stats.cumulative
          .map(
            (cf, index) =>
              `${116 + index * 120},${390 - (cf / Math.max(1, stats.totalFrequency)) * 280}`,
          )
          .join(" ")}
        fill="none"
        stroke="#fb7185"
      ...
      <Label
        x={70}
        y={105}
        text={`n=${stats.totalFrequency}, mean=${round(stats.mean)}, mode=${round(stats.modal)}, median=${round(stats.median)}`}
        size={16}
      />
    </g>
  );
...
function circleRegionSvg(values: number[]) {
  ...
  return (
    <g>
      ...
      />
      <path
        d={`M ${cx} ${cy} L ${cx + R} ${cy} A ${R} ${R} 0 ${largeArc} 0 ${x} ${y} Z`}
        fill="#facc15"
        fillOpacity="0.42"
        stroke="#fde047"
        strokeWidth="3"
      ...
      <Label
        x={65}
        y={70}
        text={`sector=${round(sectorArea(outer, angle))}, segment=${round(segmentArea(outer, angle))}`}
      />
      <Label
        x={65}
        y={102}
        text={`annulus=${round(annulusArea(outer, Math.min(inner, outer)))}`}
      />
    </g>
  );
}
...
function solidsSvg(values: number[], routeId: Class10PriorityRouteId) {
  ...
  if (routeId.includes("frustum")) {
    ...
    return (
      <g>
        <path
          d="M 260 120 L 500 120 L 610 390 L 150 390 Z"
          fill="#0e7490"
          fillOpacity="0.38"
          stroke="#67e8f9"
          strokeWidth="5"
        ...
        <Label
          x={80}
          y={70}
          text={`R=${round(a)}, r=${round(c)}, h=${round(b)}, l=${round(l)}`}
        />
        <Label
          x={80}
          y={105}
          text={`CSA=${round(frustumCSA(a, c, l))}, TSA=${round(frustumTSA(a, c, l))}`}
        />
      </g>
    );
  }
  ...
  return (
    <g>
      ...
      {combined ? (
        <path
          d="M 250 210 L 360 80 L 470 210 Z"
          fill="#facc15"
          fillOpacity="0.55"
          stroke="#fde047"
          strokeWidth="4"
      ...
      <Label
        ...
        text={
          combined
            ? "Combined solid: hidden shared face is not exposed."
            : "Recasting: old volume equals new volume."
        }
      />
      <Label
        x={70}
        y={105}
        text={`Cylinder V=${round(cylinderVolume(a, b))}, cone V=${round(coneVolume(a, c))}, sphere V=${round(sphereVolume(a))}, hemi CSA=${round(hemisphereCSA(a))}, cone l=${round(coneSlantHeight(a, c))}`}
        size={15}
      />
    </g>
  );
...
function heightsSvg(values: number[]) {
  ...
  return (
    <g>
      ...
      />
      <path
        d={`M ${baseX + 45} ${baseY} A 45 45 0 0 0 ${baseX + 45 * Math.cos((-angle * Math.PI) / 180)} ${baseY + 45 * Math.sin((-angle * Math.PI) / 180)}`}
        fill="none"
        stroke="#fb7185"
        strokeWidth="4"
      />
      <Label
        x={70}
        y={70}
        text={`height = ${round(heightFromAngleDistance(angle, distance))}`}
      />
      <Label
        x={70}
        y={105}
        text={`tan(${round(angle)} deg) = height / ${round(distance)}`}
      />
    </g>
  );
}
