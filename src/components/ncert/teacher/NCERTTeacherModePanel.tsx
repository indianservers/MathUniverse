...
export default function NCERTTeacherModePanel({
  ...
  questions,
  prompts = [
    "Ask students to predict before checking.",
  ...
  const worksheetQuestions = useMemo(
    () => selectWorksheetQuestions(questions, difficulty, count),
      ...
      <div className="ncert-no-print flex flex-wrap items-start justify-between gap-3">
        <div>
          <h2 className="text-base font-black text-slate-950 dark:text-white">
            Teacher worksheet mode
          </h2>
          <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">
            Print-friendly HTML worksheet using the checked practice bank.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="ncert-no-print mt-4 grid gap-3 sm:grid-cols-3">
        <label className="text-sm font-black">
          ...
          <select
            ...
            className="mt-2 w-full rounded-2xl border border-amber-200 bg-white px-3 py-2 dark:border-amber-300/20 dark:bg-slate-950"
          >
...
function Info({ title, text }: { title: string; text: string }) {
  return (
    <div className="rounded-2xl bg-white/80 p-3 text-sm dark:bg-slate-950/40">
      ...
      <p className="mt-1 font-semibold leading-6 text-slate-600 dark:text-slate-300">
        {text}
      </p>
    </div>
  );
