...
export default function Grade8FractalsSolidViewsLab() {
  ...
  const defaultTabId = ["fractal", "solid", "practice"].includes(
    requestedTab ?? "",
  )
    ? requestedTab
    : "fractal";
        ...
        <div className="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-cyan-700 dark:text-cyan-200">
              Class 8 NCERT geometry lab
            </p>
            <h1 className="mt-2 text-2xl font-black text-slate-950 dark:text-white">
              Fractals and Solid Views
            </h1>
            <p className="mt-2 max-w-4xl text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">
              Explore Sierpinski carpet patterns, count retained and removed
              squares, then build cube stacks and read their top, front, and
              side views.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            {["Sierpinski carpet", "cube stacks", "orthographic views"].map(
              (item) => (
                <span
                  key={item}
        ...
        tabs={[
          {
            ...
            badge: "carpet",
            content: <FractalExplorer />,
          },
          {
            id: "solid",
            label: "Solid Views Lab",
            badge: "cubes",
            content: <SolidViewsLab />,
          },
          {
            id: "practice",
            label: "Practice + Teacher",
            badge: "checks",
            content: <PracticeAndResources />,
          },
        ]}
      />
    </div>
...
function FractalExplorer() {
  ...
  const removedCells = useMemo(
    () => generateSierpinskiRemovedSquares(iteration),
    [iteration],
  );
  const retainedMini = useMemo(
    () => generateSierpinskiRemovedSquares(Math.max(0, iteration - 1)),
    [iteration],
  );
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1.15fr)_minmax(320px,0.85fr)]">
      <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div>
            <h2 className="text-lg font-black text-slate-950 dark:text-white">
              Sierpinski carpet explorer
            </h2>
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">
              Remove the middle ninth, then repeat the same rule in every
              retained square.
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <IconButton
              ...
              onClick={() => setIteration((value) => Math.max(0, value - 1))}
            >
              Prev
            </IconButton>
            <IconButton
            ...
            <IconButton
              ...
              onClick={() => setIteration((value) => Math.min(5, value + 1))}
            >
              Next
            </IconButton>
            <IconButton
        ...
        <div className="mt-3 grid gap-2 md:grid-cols-4">
          <Metric
            label="Retained squares"
            value={stats.retainedSquares.toLocaleString()}
          />
          <Metric
            label="New removed"
            value={stats.newlyRemovedSquares.toLocaleString()}
          />
          <Metric
            label="Cumulative removed"
            value={stats.cumulativeRemovedSquares.toLocaleString()}
          />
      ...
      <aside className="space-y-4">
        <FormulaBlock
          title="Fractal count rule"
          formula="R_n=8^n,\quad A_n=(8/9)^n,\quad removed=1-(8/9)^n"
          explanation="Each retained square creates eight smaller copies in the next step."
        />
        <section className="rounded-2xl border border-cyan-200 bg-cyan-50/70 p-4 dark:border-cyan-300/20 dark:bg-cyan-300/10">
          <h3 className="font-black text-slate-950 dark:text-white">
            Self-similarity lens
          </h3>
          <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">
            Pick one retained third. It contains a smaller copy of the previous
            carpet.
          </p>
        ...
        <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
          ...
          <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">
            At iteration {iteration + 1}, how many smallest squares remain?
          </p>
          <div className="mt-3 flex gap-2">
            <input
              ...
              inputMode="numeric"
              className="min-w-0 flex-1 rounded-xl border border-slate-200 bg-white px-3 py-2 font-black text-slate-950 dark:border-white/10 dark:bg-white/10 dark:text-white"
            />
            <button
              type="button"
              onClick={() => setPrediction(String(expectedNext))}
          ...
          <p
            className={`mt-2 text-sm font-black ${answerState === "correct" ? "text-emerald-700" : answerState === "try" ? "text-amber-700" : "text-slate-500"}`}
          >
            {answerState === "correct"
              ? "Correct. Multiply the previous row by 8."
              : answerState === "try"
                ? `Try again: use 8^${iteration + 1}.`
              ...
              {table.map((row) => (
                ...
                >
                  ...
                  <td className="px-3 py-2">
                    {row.retainedAreaText} ({row.retainedPercent.toFixed(1)}%)
                  </td>
                  <td className="px-3 py-2">
                    {row.removedAreaText} ({row.removedPercent.toFixed(1)}%)
                  </td>
                </tr>
              ))}
            </tbody>
...
function SolidViewsLab() {
  ...
  const [grid, setGrid] = useState(() =>
    cloneGrid(getSolidPreset("staircase").grid),
  );
  ...
  const validation = useMemo(
    () => validateProjectionMatch(candidate, target, requireCubeCount),
    [candidate, requireCubeCount, target],
  );
  ...
  return (
    <div className="grid gap-4 xl:grid-cols-[300px_minmax(0,1fr)_320px]">
      <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
        ...
        <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">
          Select a grid cell, then add or remove cubes from that stack.
        </p>
        <select
          value={presetId}
          onChange={(event) => loadPreset(event.target.value)}
          className="mt-3 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-bold dark:border-white/10 dark:bg-white/10 dark:text-white"
        >
        ...
        <StackEditor
          ...
          onChange={(row, col, height) =>
            setGrid((old) => setStackHeight(old, row, col, height))
          }
        />
        <div className="mt-3 grid grid-cols-2 gap-2">
          <IconButton
            ...
            onClick={() =>
              setGrid((old) =>
                changeStackHeight(old, selected.row, selected.col, 1),
              )
            }
          >
            + Cube
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
        <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
          <div>
            ...
            <p className="text-sm font-semibold text-slate-600 dark:text-slate-300">
              Cube count: {cubeCount(grid)}
            </p>
          </div>
          <div className="flex flex-wrap gap-2">
            <IconButton
              label="Rotate view"
              onClick={() => setView((old) => (old + 1) % 4)}
            >
              <RotateCcw size={16} /> Rotate
            </IconButton>
            <IconButton label="Focus diagram" onClick={() => setView(0)}>
        ...
        <div className="mt-3 grid grid-cols-4 gap-2">
          {(["top", "front", "left", "right"] as ProjectionName[]).map(
            (name) => (
              <button
                key={name}
                type="button"
                onClick={() => setProjection(name)}
                className={`rounded-xl border px-3 py-2 text-sm font-black capitalize ${projection === name ? "border-cyan-400 bg-cyan-500 text-white" : "border-slate-200 bg-white text-slate-700"}`}
              >
                {name}
              </button>
            ),
        ...
        <ProjectionGrid
          ...
          grid={getProjection(grid, projection)}
        />
      </section>
      ...
      <section className="rounded-2xl border border-amber-200 bg-amber-50/70 p-4 dark:border-amber-300/20 dark:bg-amber-300/10">
        ...
        <p className="mt-1 text-sm font-semibold text-slate-600 dark:text-slate-300">
          Build any model that matches the challenge projections. Alternatives
          are accepted.
        </p>
        <StackEditor
          grid={candidate}
          ...
          onSelect={setSelected}
          onChange={(row, col, height) =>
            setCandidate((old) => setStackHeight(old, row, col, height))
          }
          compact
        />
        <div className="mt-3 grid grid-cols-2 gap-2">
          <IconButton
            label="Add cube to challenge"
            onClick={() =>
              setCandidate((old) =>
                changeStackHeight(old, selected.row, selected.col, 1),
              )
          ...
          <IconButton
            label="Remove cube from challenge"
            onClick={() =>
              setCandidate((old) =>
                changeStackHeight(old, selected.row, selected.col, -1),
              )
        ...
        <Toggle
          label="Require same cube count"
          checked={requireCubeCount}
          onChange={setRequireCubeCount}
        />
        <div
          className={`mt-3 rounded-xl border p-3 text-sm font-black ${validation.acceptsAlternative ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-amber-300 bg-white text-amber-800"}`}
        >
          {validation.message}
        </div>
        <div className="mt-3 grid grid-cols-2 gap-2 text-xs font-black">
          {[
            ...
            <span
              key={String(label)}
              className={`rounded-full px-3 py-2 ${ok ? "bg-emerald-100 text-emerald-800" : "bg-white text-slate-700"}`}
            >
              {label}: {ok ? "match" : "check"}
            </span>
          ))}
        </div>
        ...
        <div className="mt-3 rounded-2xl border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-slate-950/50">
          ...
          <p className="mt-1 text-xs font-semibold text-slate-600 dark:text-slate-300">
            Which view shows only footprint squares, not heights?
          </p>
          <div className="mt-2 grid grid-cols-3 gap-2">
            {(["top", "front", "left"] as ProjectionName[]).map((name) => (
              <button
                key={name}
                type="button"
                onClick={() => setViewAnswer(name)}
                className="rounded-xl border border-cyan-200 bg-cyan-50 px-2 py-2 text-xs font-black capitalize text-cyan-900"
              >
                {name}
              </button>
            ))}
          ...
          {viewAnswer && (
            <p
              className={`mt-2 text-xs font-black ${viewAnswer === "top" ? "text-emerald-700" : "text-amber-700"}`}
            >
              {viewAnswer === "top"
                ? "Correct. Top view ignores height and shows occupied cells."
                : "Try top view: it is the footprint from above."}
            </p>
          )}
        </div>
      </section>
  ...
  return (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
      <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
        ...
        <div className="mt-3 grid gap-3 md:grid-cols-2">
          {[
            [
              "Pattern rule",
              "Explain why retained squares follow 1, 8, 64, 512, ...",
            ],
            [
              "Area reasoning",
              "Use (8/9)^n to find retained area after n steps.",
            ],
            [
              "View reading",
              "Read top, front, left, and right projections of cube stacks.",
            ],
            [
              "Reconstruction",
              "Build a solid that matches given orthographic views.",
            ],
          ].map(([title, text]) => (
      ...
      <aside className="space-y-3">
        <FormulaBlock
          title="Core formulas"
          formula="R_n=8^n,\quad S_n=3^n,\quad area=(8/9)^n"
          explanation="Fractal growth is multiplicative; solid views are projections of the same object."
        />
...
function CarpetSvg({
  ...
  removedCells: CarpetCell[];
  highlight: (typeof retainedChoices)[number];
  showGrid: boolean;
  showRemoved: boolean;
  showLabels: boolean;
  zoom: number;
  ...
  return (
    <svg
      ...
      role="img"
      aria-label="Sierpinski carpet fractal"
    >
      ...
      <g transform={`translate(${panX} ${panY}) scale(${zoom})`}>
        ...
        <rect
          x={60 + highlight.x * 400}
          y={60 + highlight.y * 400}
          width={400 / 3}
          height={400 / 3}
          fill="none"
          stroke="#facc15"
...
function MiniCarpetSvg({ removedCells }: { removedCells: CarpetCell[] }) {
  return (
    <svg
      ...
      role="img"
      aria-label="Magnified smaller carpet"
    >
      ...
      <text x="18" y="114" fill="#facc15" fontSize="12" fontWeight="800">
        same rule at smaller scale
      </text>
    </svg>
  );
}
...
function StackEditor({
  ...
  onChange: (row: number, col: number, height: number) => void;
  compact?: boolean;
}) {
  return (
    <div
      className={`mt-3 grid grid-cols-4 gap-2 ${compact ? "text-xs" : "text-sm"}`}
    >
...
function SolidSvg({
  ...
  const cubes = rotated.flatMap((row, rowIndex) =>
  ...
  const sorted = cubes.sort(
    (a, b) => a.row + a.col + a.z - (b.row + b.col + b.z),
  );
  return (
    <svg
      viewBox="0 0 620 420"
      ...
      role="img"
      aria-label="Isometric cube stack model"
    >
      <rect width="620" height="420" rx="22" fill="#020617" />
      {showGrid && (
        <g stroke="#164e63" opacity="0.35">
...
function ProjectionGrid({ title, grid }: { title: string; grid: number[][] }) {
  return (
    <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3 dark:border-white/10 dark:bg-white/5">
      <h3 className="mb-2 text-sm font-black capitalize text-slate-900 dark:text-white">
        {title}
      </h3>
      <div
        className="inline-grid gap-1"
        style={{
          gridTemplateColumns: `repeat(${grid[0]?.length ?? 1}, minmax(28px, 1fr))`,
        }}
      >
        {grid.flatMap((row, rowIndex) =>
          row.map((value, colIndex) => (
            <div
              key={`${rowIndex}-${colIndex}`}
              className={`h-8 rounded-lg border ${value ? "border-cyan-400 bg-cyan-400" : "border-slate-200 bg-white dark:border-white/10 dark:bg-slate-900"}`}
            />
          )),
        )}
      </div>
...
function IconButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      onClick={onClick}
      className="inline-flex min-h-10 items-center justify-center gap-2 rounded-xl border border-cyan-200 bg-white px-3 py-2 text-sm font-black text-slate-800 shadow-sm hover:border-cyan-400 dark:border-cyan-300/20 dark:bg-white/10 dark:text-white"
    >
      {children}
    </button>
  );
...
function ResourceLink({ to, label }: { to: string; label: string }) {
  return (
    <Link to={to} className="action-secondary justify-between">
      {label}
      <Sparkles size={16} />
    </Link>
  );
