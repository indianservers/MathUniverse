...
export function validateRatioInputs(parts: number[]): MathResult<number[]> {
  if (parts.length < 2)
    return { ok: false, error: "A ratio needs at least two parts.", steps: [] };
  if (parts.length > 5)
    return {
      ok: false,
      error: "This Grade 8 lab supports up to five ratio parts.",
      steps: [],
    };
  if (parts.some((part) => !Number.isFinite(part)))
    return {
      ok: false,
      error: "Every ratio part must be a finite number.",
      steps: [],
    };
  if (parts.some((part) => part <= 0))
    return { ok: false, error: "Ratio parts must be positive.", steps: [] };
  return {
    ok: true,
    value: parts,
    steps: [`Ratio parts accepted: ${parts.join(":")}.`],
  };
}
...
export function areRatiosEquivalent(leftParts: number[], rightParts: number[]) {
  ...
  return (
    simplifyRatio(leftParts).join(":") === simplifyRatio(rightParts).join(":")
  );
}

export function solveCrossMultiplication(a: number, b: number, c: number) {
  if (a === 0 || b === 0)
    throw new Error("Known numerator and denominator must be non-zero.");
  return {
    left: a,
    right: b * c,
    missing: (b * c) / a,
    steps: [
      `Use a/b = c/d -> a x d = b x c.`,
      `${a} x d = ${b} x ${c}`,
      `d = ${(b * c) / a}`,
    ],
  };
...
export function getMissingProportionValue({
  ...
  const requireValue = (value: number | undefined, label: string) => {
    if (!Number.isFinite(value))
      throw new Error(`Known value ${label} is required.`);
    return value as number;
  };
  if (position === "a")
    return (requireValue(b, "b") * requireValue(c, "c")) / requireValue(d, "d");
  if (position === "b")
    return (requireValue(a, "a") * requireValue(d, "d")) / requireValue(c, "c");
  if (position === "c")
    return (requireValue(a, "a") * requireValue(d, "d")) / requireValue(b, "b");
  return (requireValue(b, "b") * requireValue(c, "c")) / requireValue(a, "a");
}

export function getDirectProportionValue({
  x1,
  y1,
  x2,
}: {
  ...
  return {
    ...
    steps: [
      `k = y1/x1 = ${y1}/${x1} = ${k}`,
      `y2 = k x x2 = ${k} x ${x2} = ${k * x2}`,
    ],
  };
}

export function getInverseProportionValue({
  x1,
  y1,
  x2,
}: {
...
export function getRepresentativeFraction({
  mapDistance,
  actualDistance,
  mapUnit,
  actualUnit,
  ...
  if (mapDistance <= 0 || actualDistance <= 0)
    throw new Error("Map and actual distances must be positive.");
  ...
  return {
    ...
    steps: [
      `Convert actual distance: ${actualDistance} ${actualUnit} = ${round(actualInMapUnit, 6)} ${mapUnit}.`,
      `RF = map/actual = ${mapDistance}/${round(actualInMapUnit, 6)} = 1:${round(denominator, 4)}.`,
    ],
  };
}

export function getActualDistanceFromMapScale({
  mapDistance,
  scaleDenominator,
  mapUnit,
  outputUnit,
  ...
  if (mapDistance <= 0 || scaleDenominator <= 0)
    throw new Error("Map distance and scale denominator must be positive.");
  const actualInMapUnit = mapDistance * scaleDenominator;
  const actual = convertLength(actualInMapUnit, mapUnit, outputUnit);
  return {
    actual,
    steps: [
      `Actual distance = ${mapDistance} x ${scaleDenominator} = ${actualInMapUnit} ${mapUnit}.`,
      `Convert ${actualInMapUnit} ${mapUnit} = ${round(actual, 6)} ${outputUnit}.`,
    ],
  };
}

export function getMapDistanceFromActualScale({
  actualDistance,
  scaleDenominator,
  actualUnit,
  outputUnit,
  ...
  if (actualDistance <= 0 || scaleDenominator <= 0)
    throw new Error("Actual distance and scale denominator must be positive.");
  const actualInOutputUnit = convertLength(
    actualDistance,
    actualUnit,
    outputUnit,
  );
  ...
  return {
    map,
    steps: [
      `Convert actual distance: ${actualDistance} ${actualUnit} = ${round(actualInOutputUnit, 6)} ${outputUnit}.`,
      `Map distance = ${round(actualInOutputUnit, 6)} / ${scaleDenominator} = ${round(map, 6)} ${outputUnit}.`,
    ],
  };
}
...
export function getConstantOfProportionality(
...
export function generateProportionTable({
  relationshipType,
  constant,
  xValues,
}: {
  ...
  return xValues.map((x) => {
    if (relationshipType === "inverse" && x === 0)
      throw new Error("x cannot be zero in an inverse proportion table.");
    return {
      x,
      y: relationshipType === "direct" ? constant * x : constant / x,
    };
  });
}
