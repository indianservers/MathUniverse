...
export function classifyDifferentialEquation(input: string) {
  ...
  const order = orderMatches
    ? Math.max(
        ...orderMatches.map((match) => Number(match.match(/\d/)?.[0] ?? 1)),
      )
    : 0;
...
export function differentialEquationPreset(preset: DEPreset) {
  ...
  > = {
    ...
    "xy-separable": {
      ...
      type: "Separable",
      solution: "y^2 = x^2 + C",
      verification: "Differentiate y^2 = x^2 + C to get dy/dx = x/y.",
      steps: [
        "Write y dy = x dx.",
        "Integrate both sides.",
        "y^2/2 = x^2/2 + C.",
    ...
    "linear-exp": {
      equation: "dy/dx + y = e^x",
      type: "Linear first order",
      solution: "y e^x = e^(2x)/2 + C",
      verification: "Integrating factor is e^x.",
      steps: [
        "Identify P(x)=1.",
        "IF = e^x.",
        "Multiply both sides.",
        "Integrate d/dx(y e^x) = e^(2x).",
      ],
    },
    "linear-x": {
      equation: "dy/dx + 2y = x",
      type: "Linear first order",
      solution: "y e^(2x) = integral x e^(2x) dx + C",
      verification: "Integrating factor is e^(2x).",
      steps: [
        "Identify P(x)=2.",
        "IF = e^(2x).",
...
export function applyInitialConditionForGrowth(
  k: number,
  x0: number,
  y0: number,
) {
