...
export function relationMatrix(
  ...
  return domain.map((row) =>
    codomain.map((col) => (pairSet.has(`${row}->${col}`) ? 1 : 0)),
  );
}

export function relationPropertyReport(set: string[], pairs: RelationPair[]) {
  const pairSet = new Set(pairs.map(([a, b]) => `${a}->${b}`));
  const missingReflexive = set.filter(
    (item) => !pairSet.has(`${item}->${item}`),
  );
...
export function functionClassifier(
  domain: string[],
  codomain: string[],
  pairs: RelationPair[],
) {
  ...
  const functionFailures = [...imagesByInput.entries()].filter(
    ([, outputs]) => outputs.length !== 1,
  );
  const isFunction = functionFailures.length === 0;
  const imageValues = pairs.map(([, output]) => output);
  const oneOneFailures = imageValues.filter(
    (output, index) => imageValues.indexOf(output) !== index,
  );
  const missingOutputs = codomain.filter(
    (output) => !imageValues.includes(output),
  );
  return {
    isFunction,
    ...
    bijective:
      isFunction && oneOneFailures.length === 0 && missingOutputs.length === 0,
    functionFailures,
