...
const configs: Record<Class12LabKind, LabConfig> = {
  "relations-functions": {
    ...
    subtitle:
      "Build a relation, inspect its matrix and arrow graph, then classify function behavior.",
    ...
    prompts: [
      "Which ordered pairs make the relation reflexive?",
      "When does every output get hit?",
      "What breaks the inverse?",
    ],
  },
  determinants: {
    title: "Determinants",
    subtitle:
      "Change a 2 by 2 matrix and watch area scale, cofactors, inverse, and Cramer's rule values update.",
    ...
    prompts: [
      ...
      "Which cofactor signs change?",
      "How does inverse verify A times A inverse?",
    ],
  },
  "continuity-differentiability": {
    title: "Continuity and Differentiability",
    subtitle:
      "Compare left and right limits for holes, jumps, corners, cusps, and smooth curves.",
    ...
    prompts: [
      ...
      "Can a tangent slope be assigned?",
      "Is continuity enough for differentiability?",
    ],
  },
  "integration-methods": {
    title: "Integration Methods",
    subtitle:
      "Choose a method and see how substitution, parts, partial fractions, and definite-area ideas connect.",
    ...
    prompts: [
      ...
      "Which term becomes u?",
      "Where does the shaded area come from?",
    ],
  },
  "differential-equations": {
    title: "Differential Equations",
    subtitle:
      "Classify order and degree, then compare a solution curve against a slope field.",
    ...
    prompts: [
      "What is the order?",
      "Where does the initial point sit?",
      "Does the curve follow the local slopes?",
    ],
  },
  "vectors-3d-geometry": {
    title: "Vectors and 3D Geometry",
    subtitle:
      "Move vectors in space and read dot product, cross product area, projection, and line direction.",
    ...
    prompts: [
      "What angle do the vectors make?",
      "Which area does the cross product measure?",
      "What is the projection length?",
    ],
  },
  "bayes-theorem": {
    title: "Bayes' Theorem",
    subtitle:
      "Update from prior probability to posterior probability with tree and table views.",
    ...
    prompts: [
      ...
      "How does the posterior change after a positive result?",
    ],
  },
  "linear-programming": {
    title: "Linear Programming",
    subtitle:
      "Move an objective function across a feasible polygon and compare corner values.",
    ...
    prompts: [
      "Which side of each constraint is feasible?",
      "Which corner maximizes Z?",
      "What changes when c1 or c2 changes?",
    ],
  },
  "inverse-trig": {
    title: "Inverse Trigonometric Functions",
    subtitle:
      "Restrict the original trig graph and read principal values from inverse graphs.",
    badge: "Principal value visualizer",
    examples: ["sin inverse", "cos inverse", "tan inverse", "identity card"],
    prompts: [
      "Why is the range restricted?",
      "Which principal value is chosen?",
      "What happens at domain endpoints?",
    ],
  },
};
...
export default function Class12GuidedLab({ kind }: { kind: Class12LabKind }) {
  ...
  const panel = useMemo(
    () => class12Panel(kind, preset, a, b, c),
    [kind, preset, a, b, c],
  );
  const details = useMemo(
    () => class12Details(kind, preset, a, b, c),
    [kind, preset, a, b, c],
  );
  const conceptId = conceptIdByKind[kind];
  const bankQuestions = getNCERTPracticeItems(conceptId);
  const practiceQuestions =
    bankQuestions.length > 0 ? bankQuestions : details.questions;
  const controlsPanel = (
      ...
      <label className="block text-sm font-black text-slate-700 dark:text-slate-200">
        ...
        >
          {config.examples.map((example, index) => (
            <option key={example} value={index}>
              {example}
            </option>
          ))}
        </select>
    ...
    <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_320px]">
      <div>
        <svg
          viewBox="0 0 760 460"
          role="img"
          aria-label={`${config.title} visual model`}
          className="h-[330px] w-full rounded-2xl bg-slate-950 sm:h-[380px]"
        >
          <Grid />
          {panel.svg}
        </svg>
      ...
      <NCERTCompactPanel
        title="Controls and live verdict"
        description="Change one value, then verify the method."
        accent="cyan"
      >
        {controlsPanel}
      </NCERTCompactPanel>
  ...
  const practicePanel = (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
      <NCERTPracticeCheck
        title={`${config.title} practice bank`}
      ...
      <NCERTCompactPanel
        ...
        description="Answer verbally or in notebook, then change values."
        accent="violet"
      >
        <p className="text-sm font-bold leading-6 text-slate-700 dark:text-slate-200">
          {panel.practice}
  ...
  const verificationPanel = (
    <div className="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
      <NCERTCompactPanel
        ...
        description="Every route shows how the result is checked, not only guessed."
        accent="emerald"
      >
        <div className="space-y-2">
          {details.verification.map((item) => (
            <p
              key={item}
              className="rounded-2xl bg-white/70 p-3 text-sm font-bold leading-6 text-slate-700 dark:bg-slate-950/40 dark:text-slate-100"
            >
              {item}
            </p>
          ))}
      ...
      <NCERTCompactPanel title="Examples" accent="cyan">
        <div className="flex flex-wrap gap-2">
          {config.examples.map((example) => (
            <span
              key={example}
              className="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-2 text-xs font-black text-cyan-900 dark:border-cyan-300/20 dark:bg-cyan-300/10 dark:text-cyan-50"
            >
              {example}
            </span>
          ))}
    ...
    <div className="space-y-5">
      <section className="rounded-3xl border border-cyan-200 bg-white/95 p-5 shadow-sm dark:border-cyan-300/20 dark:bg-slate-950/80">
        <p className="text-xs font-black uppercase tracking-wide text-cyan-700 dark:text-cyan-200">
          Class 12 NCERT guided visualization
        </p>
        ...
        tabs={[
          {
            ...
            content: (
              <NCERTSubTabs
                ariaLabel={`${config.title} method sub-tabs`}
                tabs={subTabs}
              />
            ),
          },
          ...
          { id: "practice", label: "Practice", content: practicePanel },
          {
            ...
            content: (
              <div className="grid gap-4 lg:grid-cols-2">
                ...
                >
                  {bankQuestions.length > 0 ? (
                    <NCERTTeacherModePanel
                      ...
                      misconception={details.verification[0]}
                      extension={panel.practice}
                    />
                  ) : (
                    <div className="space-y-2">
                ...
                <NCERTCompactPanel
                  title="Formula / theorem / proof links"
                  description="Exact links are used where available; category links are used when no exact route exists."
                  accent="cyan"
                >
                  <div className="flex flex-wrap gap-2">
                    {details.links.map((link) => (
                      <a
                        key={link.href}
                        href={link.href}
                        className="rounded-full border border-cyan-200 bg-white px-3 py-2 text-xs font-black text-cyan-900 hover:bg-cyan-50 dark:border-cyan-300/20 dark:bg-slate-950 dark:text-cyan-50"
                      >
                        {link.label}
                      </a>
                    ))}
...
function class12SubTabs(
  kind: Class12LabKind,
  visualPanel: JSX.Element,
  methodPanel: JSX.Element,
  controlsPanel: JSX.Element,
  practicePanel: JSX.Element,
) {
...
function class12Details(
  ...
  if (kind === "relations-functions") {
    ...
    const composed = composeRelations(
      [
        ["A", "B"],
        ["B", "C"],
      ],
    ...
    return {
      steps: [
        `Build the relation matrix row by row: ${relationMatrix(
          domain,
          codomain,
          pairs,
        )
        ...
        `Composition sample gives ${composed.map(([x, y]) => `${x}->${y}`).join(", ") || "no matching chain"}; inverse pairs are ${inverseRelation(
          pairs,
        )
          .map(([x, y]) => `${x}->${y}`)
          .join(", ")}.`,
      ...
      verification: [
        ...
        classifier.functionFailures.length
          ? `Function fails at ${classifier.functionFailures.map(([input]) => input).join(", ")}.`
          : "Every input has exactly one image.",
        classifier.repeatedOutputs.length
          ? `Not one-one because outputs repeat: ${classifier.repeatedOutputs.join(", ")}.`
          : "No output is repeated.",
        classifier.missingOutputs.length
          ? `Not onto because missing outputs: ${classifier.missingOutputs.join(", ")}.`
          : "Every codomain element is used.",
      ],
      questions: [
        {
          id: "rf-function",
          prompt: "In a function, how many outputs may one input have?",
          answer: "one",
          hint: "Use the vertical/arrow rule.",
          explanation: "A function gives each input exactly one output.",
          choices: ["one", "two", "any number", "none"],
        },
        {
          id: "rf-bijection",
          prompt: "A one-one and onto function is called what?",
          answer: "bijective",
          hint: "It has both injective and surjective behavior.",
          explanation: "One-one plus onto is bijective.",
          choices: ["bijective", "constant", "partial", "empty"],
        },
      ],
      diagramSummary:
        "Left nodes are inputs, right nodes are outputs, arrows are ordered pairs, and the matrix records the same relation.",
      links: commonLinks,
    };
  }
  if (kind === "determinants") {
    ...
    return {
      steps: [
        ...
        inv
          ? `Adjoint inverse exists. First inverse row is [${fmt(inv[0][0])}, ${fmt(inv[0][1])}].`
          : "Inverse does not exist because determinant is zero.",
        cramer
          ? `Cramer's rule preset solves x=${fmt(cramer.x)}, y=${fmt(cramer.y)}.`
          : "Cramer's rule needs a nonzero determinant.",
      ],
      verification: [
        `Singular check: ${Math.abs(determinant2) < 0.001 ? "singular" : "non-singular"}.`,
        inv
          ? `A times solution vector check: ${multiplyMatrixVector2(
              [
                [2, 1],
                [1, 1],
              ],
              [2, 1],
            ).join(", ")} equals constants 5, 3.`
          : "A inverse check is blocked because det(A)=0.",
      ...
      questions: [
        ...
        {
          id: "det-singular",
          prompt: "If det(A)=0, does A inverse exist?",
          answer: "no",
          hint: "Zero area scale collapses space.",
          explanation:
            "A matrix with determinant 0 is singular and has no inverse.",
          choices: ["yes", "no", "always", "only for 3x3"],
        },
      ],
      diagramSummary:
        "The parallelogram shows signed area scaling; determinant zero collapses the shape into a line.",
      links: commonLinks,
    };
  }
  if (kind === "continuity-differentiability") {
    ...
    return {
      ...
      questions: [
        {
          id: "cd-corner",
          prompt: "Is a sharp corner differentiable?",
          answer: "no",
          hint: "Check left and right slopes.",
          explanation: "A corner has different one-sided derivative values.",
          choices: ["yes", "no", "only if continuous", "always"],
        },
        {
          id: "cd-hole",
          prompt:
            "A removable hole has matching LHL/RHL but f(a) missing. Is it continuous?",
          answer: "no",
          hint: "Continuity also needs f(a).",
          explanation: "The value must equal the limit.",
        },
      ],
      diagramSummary:
        "The curve marks the test point; breaks, holes, jumps, corners, and tangent behavior decide the verdict.",
      links: commonLinks,
    };
  }
  if (kind === "integration-methods") {
    ...
    const integration = integrationPresetStepper(
      presets[preset] ?? "substitution-cos-x2",
    );
    return {
      ...
      questions: [
        {
          id: "int-sub",
          prompt: "What is integral 2x cos(x^2) dx?",
          answer: "sin(x^2)+c",
          hint: "Let u=x^2.",
          explanation: "du=2x dx, so the integral is sin(x^2)+C.",
          commonMistake: "Use substitution and include +C.",
        },
        {
          id: "int-area",
          ...
          answer: 1 / 6,
          tolerance: 0.001,
          hint: "Integrate x - x^2 from 0 to 1.",
          explanation: "1/2 - 1/3 = 1/6.",
        },
      ],
      diagramSummary:
        "The shaded region and method card connect the symbolic integral to a visual area or algebraic transformation.",
      links: commonLinks,
    };
  }
  if (kind === "differential-equations") {
    ...
    return {
      ...
      questions: [
        {
          id: "de-order",
          prompt: "What is the order of dy/dx + y = e^x?",
          answer: 1,
          tolerance: 0.001,
          hint: "Look at the highest derivative.",
          explanation: "Only first derivative appears.",
        },
        {
          id: "de-if",
          prompt: "For dy/dx + y = e^x, the integrating factor is?",
          answer: "e^x",
          hint: "IF = e^(integral P dx).",
          explanation: "Here P(x)=1, so IF=e^x.",
        },
      ],
      diagramSummary:
        "Small slope marks show the differential equation field; the solution curve should follow the local directions.",
      links: commonLinks,
    };
  }
  if (kind === "vectors-3d-geometry") {
    ...
    return {
      ...
      questions: [
        {
          id: "vec-dot",
          prompt: "Dot product of (1,2,3) and (4,5,6)?",
          answer: 32,
          tolerance: 0.001,
        ...
        {
          id: "vec-perp",
          prompt: "If a.b = 0 for nonzero vectors, the vectors are?",
          answer: "perpendicular",
          hint: "cos(theta)=0.",
          explanation: "The angle is 90 degrees.",
          choices: ["parallel", "perpendicular", "same", "zero"],
        },
      ],
      diagramSummary:
        "The 3D-like vector scene projects vectors to the plane while reporting dot, cross, projection, direction cosines, and line form.",
      links: commonLinks,
    };
  }
  if (kind === "bayes-theorem") {
    ...
    return {
      ...
      questions: [
        {
          id: "bayes-evidence",
          prompt: "If P(A)=0.5, P(E|A)=0.8, P(E|not A)=0.2, what is P(E)?",
          answer: 0.5,
          tolerance: 0.001,
          hint: "Use total probability.",
          explanation: "0.5*0.8 + 0.5*0.2 = 0.5.",
        },
        {
          id: "bayes-name",
          prompt:
            "Bayes theorem computes posterior probability after what is observed?",
          answer: "evidence",
          hint: "The event E updates belief.",
          explanation: "Evidence updates the prior into the posterior.",
          choices: ["evidence", "area", "slope", "matrix"],
        },
      ],
      diagramSummary:
        "The probability tree splits prior and non-prior cases, then recombines positive evidence into the posterior.",
      links: commonLinks,
    };
  }
  if (kind === "linear-programming") {
    ...
    return {
      steps: [
        ...
        optimum
          ? `Optimum shown at (${optimum.point.join(",")}) with value ${fmt(optimum.value)}.`
          : "No feasible corner detected.",
      ],
      verification: [
        "Linear objectives reach max/min at a corner when the feasible region is bounded.",
        "The corner table checks every candidate.",
        optimum
          ? `Highlighted optimum value is ${fmt(optimum.value)}.`
          : "Simple infeasible warning: no common feasible corner was found.",
      ],
      questions: [
        {
          id: "lpp-corner",
          prompt:
            "For a bounded feasible polygon, where does a linear objective attain optimum?",
          answer: "corner",
          hint: "Check the corner-point theorem.",
          explanation: "It is enough to evaluate corner points.",
          choices: ["corner", "middle only", "outside", "never"],
        },
        {
          id: "lpp-eval",
          prompt: "Evaluate Z=3x+2y at (3,1).",
          answer: 11,
          tolerance: 0.001,
          hint: "Substitute x=3,y=1.",
          explanation: "3*3 + 2*1 = 11.",
        },
      ],
      diagramSummary:
        "The shaded polygon is the feasible region, and the dashed objective line slides until it touches the best corner.",
      links: commonLinks,
    };
  }
  const principal = inverseTrigPrincipal(
    preset,
    Math.max(-1, Math.min(1, a / 8)),
  );
  return {
    steps: [
    ...
    questions: [
      {
        id: "it-sin",
        prompt: "sin^-1(1/2) equals?",
        answer: "pi/6",
        hint: "Use the principal range [-pi/2, pi/2].",
        explanation: "sin(pi/6)=1/2 and pi/6 is principal.",
        choices: ["pi/6", "5pi/6", "pi", "0"],
      },
      {
        id: "it-id",
        prompt: "sin^-1(x)+cos^-1(x) equals?",
        answer: "pi/2",
        hint: "This is the standard complementary identity.",
        explanation: "For x in [-1,1], the principal values sum to pi/2.",
      },
    ],
    diagramSummary:
      "The highlighted band marks the chosen principal branch so the inverse graph passes the function test.",
    links: commonLinks,
  };
}

function linksFor(kind: Class12LabKind) {
  return getNCERTConceptResourceLinks(conceptIdByKind[kind]).map(
    (resource) => ({
...
function relationPanel(preset: number): Panel {
  const relations = [
    {
      ...
      verdict:
        "Reflexive and symmetric; not transitive because A relates B and B relates A but A-A is okay while added chains can fail in variants.",
    },
    {
      pairs: [
        ["A", "1"],
      ...
      verdict:
        "One-one and onto for the shown codomain: each input has one output and every output is used.",
    },
    {
      pairs: [
        ["A", "1"],
      ...
      verdict: "Function but not one-one; two inputs share output 1.",
    },
    {
      pairs: [
        ["A", "1"],
      ...
      verdict: "Not a function because A has two images.",
    },
  ];
  ...
  return {
    ...
    visualDescription:
      "Arrows show ordered pairs; the matrix records the same information.",
    verdict: relation.verdict,
    practice:
      "Change examples and decide: function, one-one, onto, equivalence, or not.",
    metrics: [
    ...
    svg: (
      <g>
        {["A", "B", "C"].map((node, i) => (
          <Node key={node} x={150} y={130 + i * 90} label={node} />
        ))}
        {["1", "2", "3"].map((node, i) => (
          <Node key={node} x={520} y={130 + i * 90} label={node} />
        ))}
        {relation.pairs.map(([from, to], i) => {
          const fy = 130 + ["A", "B", "C"].indexOf(from) * 90;
          const ty =
            130 + (["1", "2", "3", "A", "B", "C"].indexOf(to) % 3) * 90;
          return (
            <Arrow
              key={`${from}-${to}-${i}`}
              x1={190}
...
function determinantPanel(a: number, b: number, c: number): Panel {
  ...
  return {
    ...
    visualDescription:
      "The transformed unit square has signed area equal to the determinant.",
    verdict: invertible
      ? `det(A) = ${fmt(det)}, so the inverse exists.`
      : "det(A) is 0, so rows/columns collapse and inverse does not exist.",
    practice:
      "Try to make determinant 0. What happens to the parallelogram area?",
    metrics: [
    ...
    svg: (
      <g>
        <Axis2D />
        <polygon
          points={`380,230 ${380 + a * 35},${230 - c * 35} ${380 + (a + b) * 35},${230 - (c + d) * 35} ${380 + b * 35},${230 - d * 35}`}
          fill="#22d3ee55"
          stroke="#67e8f9"
          strokeWidth="4"
        />
        ...
        <Text
          x={80}
          y={105}
          value={`= ${fmt(a)}*4 - ${fmt(b)}*${fmt(c)} = ${fmt(det)}`}
        />
      </g>
    ),
  };
...
function continuityPanel(preset: number): Panel {
  ...
  return {
    ...
    visualDescription:
      "The highlighted point tests continuity first, then differentiability.",
    verdict: verdicts[preset] ?? verdicts[0],
    practice:
      "Ask first: do the limits match? Then ask: is there one tangent slope?",
    metrics: [
      { label: "case", value: labels[preset] ?? labels[0] },
      {
        label: "continuous",
        value: preset === 0 || preset === 3 ? "yes" : "no",
      },
...
function integrationPanel(preset: number, a: number, b: number): Panel {
  const method =
    ["substitution", "by parts", "partial fractions", "area between curves"][
      preset
    ] ?? "substitution";
  ...
  return {
    ...
    visualDescription:
      "Each method is shown as a structural choice, not just a formula.",
    ...
    svg: (
      <g>
        <Axis2D />
        <path
          d="M100 330 C220 160 300 120 390 250 C470 360 570 220 670 135"
          fill="none"
          stroke="#22d3ee"
          strokeWidth="5"
        />
        <path
          d="M150 330 C240 210 330 190 420 260 C500 315 570 250 640 190 L640 330 Z"
          fill="#facc1555"
          stroke="#facc15"
          strokeWidth="3"
        />
        ...
        <Text
          x={95}
          y={105}
          value="Shaded part connects the symbolic integral to area."
        />
      </g>
    ),
  };
...
function differentialEquationPanel(a: number, b: number): Panel {
  return {
    ...
    visualDescription:
      "The blue curve should locally follow the small line segments.",
    verdict: `Example family: y = C e^(${fmt(a / 4)}x). Initial value slider gives C = ${fmt(b)}.`,
    practice:
      "Pick one point on the solution curve and compare it with the nearby slope marker.",
    metrics: [
      { label: "order", value: "1" },
    ...
    svg: (
      <g>
        ...
        <path
          d="M105 335 C210 310 310 260 420 205 C530 150 610 115 680 95"
          fill="none"
          stroke="#22d3ee"
          strokeWidth="5"
        />
...
function vectorPanel(a: number, b: number, c: number): Panel {
  ...
  return {
    ...
    visualDescription:
      "The same two vectors explain angle, area, and projection.",
    verdict: `Dot product controls angle; cross product magnitude gives parallelogram area ${fmt(area)}.`,
    practice:
      "Make the dot product near 0 and observe the angle becoming close to 90 degrees.",
    metrics: [
    ...
    svg: (
      <g>
        ...
        <polygon
          points={`380,250 ${380 + a * 40},${250 - c * 40} ${380 + (a + b) * 40},${170 - c * 40} ${380 + b * 40},170`}
          fill="#22d3ee33"
          stroke="#67e8f9"
        />
...
function bayesPanel(a: number, b: number, c: number): Panel {
  ...
  return {
    ...
    visualDescription:
      "Bayes theorem reweights the branch after evidence is observed.",
    verdict: `After positive evidence, posterior probability is ${fmt(100 * posterior)}%.`,
    practice:
      "Raise the false-positive rate and notice how the posterior can fall.",
    metrics: [
...
function lppPanel(a: number, b: number): Panel {
  ...
  const scored = corners
    .map(([x, y]) => ({ x, y, z: a * x + b * y }))
    .sort((p, q) => q.z - p.z);
  const best = scored[0];
  return {
    visualTitle: "Feasible region and corner evaluation",
    visualDescription:
      "The objective line reaches its optimum at a corner of the polygon.",
    verdict: `Best shown corner is (${best.x}, ${best.y}) with Z = ${fmt(best.z)}.`,
    practice:
      "Change objective coefficients and watch the winning corner change.",
    metrics: [
    ...
    svg: (
      <g>
        <Axis2D />
        <polygon
          points="160,330 520,330 440,180 160,80"
          fill="#22d3ee44"
          stroke="#67e8f9"
          strokeWidth="4"
        />
        ...
        <line
          x1="165"
          y1={330 - (best.z / Math.max(1, b || 1)) * 20}
          x2="610"
          y2={330 - (best.z / Math.max(1, a || 1)) * 20}
          stroke="#fb7185"
          strokeWidth="4"
...
function inverseTrigPanel(preset: number, x: number): Panel {
  ...
  return {
    ...
    visualDescription:
      "The restricted range makes the inverse relation a function.",
    verdict: `${fn}(${fmt(x)}) = ${fmt(value)} radians.`,
    practice:
      "Explain why inverse sine and inverse cosine choose different principal ranges.",
    metrics: [
    ...
    svg: (
      <g>
        <Axis2D />
        <path
          d="M115 250 C220 90 320 90 430 250 C530 410 620 410 690 250"
          fill="none"
          stroke="#22d3ee"
          strokeWidth="4"
        />
...
function CurveCase({ preset }: { preset: number }) {
  if (preset === 1) {
    return (
      <g>
        <Axis2D />
        <path
          d="M110 330 C230 220 320 185 370 175 M390 175 C470 185 560 240 660 335"
          fill="none"
          stroke="#22d3ee"
          strokeWidth="5"
        />
  ...
  if (preset === 2) {
    return (
      <g>
        <Axis2D />
        <path
          d="M120 310 C230 260 310 230 380 220"
          fill="none"
          stroke="#22d3ee"
          strokeWidth="5"
        />
        <path
          d="M390 130 C470 155 560 210 660 290"
          fill="none"
          stroke="#22d3ee"
          strokeWidth="5"
        />
  ...
  if (preset === 3) {
    return (
      <g>
        <Axis2D />
        <polyline
          points="120,330 380,175 650,325"
          fill="none"
          stroke="#22d3ee"
          strokeWidth="5"
        />
  ...
  return (
    <g>
      <Axis2D />
      <path
        d="M110 330 C210 270 300 210 380 180 C470 145 570 180 660 250"
        fill="none"
        stroke="#22d3ee"
        strokeWidth="5"
      />
...
function Text({ x, y, value }: { x: number; y: number; value: string }) {
  return (
    <text x={x} y={y} className="fill-slate-100 text-lg font-black">
      {value}
    </text>
  );
}
