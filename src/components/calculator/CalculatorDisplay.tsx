...
export default function CalculatorDisplay({ expression, result, error, angleMode, memory, accuracy }: CalculatorDisplayProps) {
  return (
    <div className="rounded-2xl bg-slate-950 p-5 text-right text-white shadow-inner">
      ...
      {result && !error ? <div className="mt-2 text-xs font-bold text-cyan-200">{accuracy === "exact" ? "Exact numeric result" : "Decimal approximation"}</div> : null}
    </div>
  );
}
