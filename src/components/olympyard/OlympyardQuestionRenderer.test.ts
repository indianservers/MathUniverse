...
const byType = (type: OlympyardQuestion["type"]) =>
  olympyardQuestions.find((question) => question.type === type)!;
...
describe("Olympyard question engine", () => {
  it("has a strong starter bank with topic coverage", () => {
    ...
    expect(olympyardTopics.every((topic) => (counts[topic.id] ?? 0) >= 6)).toBe(
      true,
    );
  });
  ...
  it("keeps question metadata complete and valid", () => {
    ...
    expect(
      olympyardQuestions.every((question) => question.prompt.trim().length > 0),
    ).toBe(true);
    expect(
      olympyardQuestions.every(
        (question) => String(question.answer).length > 0,
      ),
    ).toBe(true);
    expect(
      olympyardQuestions.every((question) => question.hints.length >= 3),
    ).toBe(true);
    expect(
      olympyardQuestions.every(
        (question) => question.solutionSteps.length >= 2,
      ),
    ).toBe(true);
    expect(
      olympyardQuestions.every((question) => Boolean(question.commonMistake)),
    ).toBe(true);
  ...
  it("ensures every MCQ-style question has a correct choice", () => {
    const choiceQuestions = olympyardQuestions.filter((question) =>
      ["mcq", "visual-mcq", "geometry-marker"].includes(question.type),
    );
  ...
  it("filters questions by grade and difficulty", () => {
    const gradeFiltered = filterOlympyardQuestions(
      olympyardQuestions,
      "class-3-4",
      "all",
    );
    const difficultyFiltered = filterOlympyardQuestions(
      olympyardQuestions,
      "all",
      "advanced",
    );
    ...
    expect(
      gradeFiltered.every((question) => question.gradeBand === "class-3-4"),
    ).toBe(true);
    expect(difficultyFiltered.length).toBeGreaterThan(0);
    expect(
      difficultyFiltered.every(
  ...
  it("validates numeric answers with tolerance", () => {
    ...
    expect(validateOlympyardAnswer(question, String(answer)).correct).toBe(
      true,
    );
    expect(validateOlympyardAnswer(question, String(answer + 2)).correct).toBe(
      false,
    );
    expect(
      validateOlympyardAnswer({ ...question, tolerance: 0.5 }, answer + 0.4)
        .correct,
    ).toBe(true);
  });
  ...
  it("validates click-match answers", () => {
    ...
    expect(
      validateOlympyardAnswer(question, {
        half: "fifty",
        quarter: "twentyfive",
        threequarter: "seventyfive",
      }).correct,
    ).toBe(true);
    expect(
      validateOlympyardAnswer(question, {
  ...
  it("validates pattern and step-fill style answers", () => {
    ...
    expect(validateOlympyardAnswer(stepFill, ["Add 3", "12"]).correct).toBe(
      true,
    );
    expect(validateOlympyardAnswer(stepFill, ["Add 2", "11"]).correct).toBe(
      false,
    );
  });
  ...
  it("keeps Phase 2 sample exports small and finite", () => {
    const ids = new Set(
      sampleOlympyardQuestions.map((question) => question.id),
    );
    ...
    expect(sampleOlympyardQuestions.every(hasOnlyFiniteQuestionValues)).toBe(
      true,
    );
  });
});
