  ...
  return (
    ...
    >
      <div className="flex flex-wrap items-center gap-2">
        <span className="rounded-full bg-cyan-100 px-3 py-1 text-xs font-bold text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-200">
          {question.topic}
        </span>
      ...
      <div className="mt-3">
        <button
          ...
          onClick={onHint}
          disabled={answered || assisted}
        >
          {assisted ? "Hint used" : "Use hint (-5 XP)"}
        </button>
        {assisted && (
          <p className="mt-3 rounded-2xl bg-amber-50 p-3 text-sm font-semibold text-amber-900 dark:bg-amber-400/10 dark:text-amber-100">
      ...
      {answered && (
        <div className="mt-4 rounded-2xl bg-slate-100 p-4 text-sm leading-6 dark:bg-white/10">
          <p>{question.explanation}</p>
          {selected !== question.correctAnswerIndex ? (
            <p className="mt-2 font-semibold text-rose-700 dark:text-rose-200">
              Why that option fails:{" "}
              {selected != null && selected >= 0
                ? question.distractorRationales[selected]
                : "No option was selected before the timer ended."}{" "}
              Tag: {question.misconceptionTag}.
            </p>
          ) : (
            <p className="mt-2 font-semibold text-emerald-700 dark:text-emerald-200">
              Oracle verified: {question.scoringRubric.fullCredit}
            </p>
          )}
          <p className="mt-2 text-xs text-slate-500 dark:text-slate-400">
            Next hint/solution step: {question.hints[1]}
          </p>
        </div>
      )}
    </motion.div>
