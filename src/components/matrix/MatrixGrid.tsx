...
export default function MatrixGrid({
  ...
  const update = (row: number, col: number, value: string) => {
    ...
    onChange(
      matrix.map((items, r) =>
        items.map((cell, c) =>
          r === row && c === col ? parseMatrixValue(value) : cell,
        ),
      ),
    );
  };
    ...
    <div className="inline-flex flex-col items-center gap-2">
      ...
      <div className="flex items-stretch gap-2">
        ...
        <div
          ...
          style={{
            gridTemplateColumns: `auto repeat(${Math.max(cols, 1)}, minmax(56px, 1fr))`,
          }}
        >
          <span />
          ...
          {matrix.flatMap((row, r) =>
            row.map((value, c) => [
              c === 0 ? (
                <span
                  key={`row-${r}`}
                  className="flex items-center justify-center text-xs font-black text-slate-400"
                >
                  R{r + 1}
                </span>
              ) : null,
              <div
                ...
                className={clsx(
                  ...
                  colorClass(color),
                  isHighlighted(r, c) &&
                    "scale-105 ring-4 ring-offset-2 ring-offset-white dark:ring-offset-slate-950",
...
function colorClass(color: MatrixGridProps["color"]) {
  if (color === "a")
    return "border-cyan-200 bg-cyan-50 text-cyan-950 dark:border-cyan-400/20 dark:bg-cyan-400/10 dark:text-cyan-100";
  if (color === "b")
    return "border-violet-200 bg-violet-50 text-violet-950 dark:border-violet-400/20 dark:bg-violet-400/10 dark:text-violet-100";
  if (color === "result")
    return "border-emerald-200 bg-emerald-50 text-emerald-950 dark:border-emerald-400/20 dark:bg-emerald-400/10 dark:text-emerald-100";
