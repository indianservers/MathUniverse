...
export function MatrixBreadcrumb({ current }: { current: string }) {
  return (
    <div className="flex flex-wrap items-center gap-2 text-sm font-semibold text-slate-500 dark:text-slate-400">
      ...
      <span>/</span>
      <Link to="/matrices" className="hover:text-cyan-600">
        Matrices
      </Link>
      <span>/</span>
...
export function MatrixBackLink() {
  return (
    <Link to="/matrices" className="action-secondary w-fit">
      <ArrowLeft className="h-4 w-4" />
      Back to Matrix Operations
    </Link>
  );
}

export function MatrixFormulaBlock({
  formula,
  title = "Formula",
}: {
  formula: string;
...
export function StepByStepPanel({
  ...
  useEffect(() => {
    ...
    const timer = window.setInterval(
      () => onActiveChange((active + 1) % steps.length),
      1200,
    );
      ...
      <div className="space-y-4">
        <div className="rounded-2xl bg-slate-100 p-4 dark:bg-white/10">
          ...
          <p className="mt-2 text-sm font-semibold text-slate-700 dark:text-slate-200">
            <MathExpression value={step.formula} />
          </p>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button
            className="tool-button"
            type="button"
            onClick={() => onActiveChange(steps.length - 1)}
          >
            Show All Steps
          </button>
          <button
...
export function MatrixSizeSelector({
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <select
        className="rounded-xl border border-slate-200 bg-white px-2 py-2 text-sm dark:border-white/10 dark:bg-slate-900"
        value={rows}
        onChange={(event) => onResize(Number(event.target.value), cols)}
      >
        {range(minRows, maxRows).map((value) => (
      ...
      <select
        className="rounded-xl border border-slate-200 bg-white px-2 py-2 text-sm dark:border-white/10 dark:bg-slate-900"
        value={cols}
        onChange={(event) => onResize(rows, Number(event.target.value))}
      >
        {range(minCols, maxCols).map((value) => (
          <option key={value} value={value}>
            {value} cols
          </option>
        ))}
...
export function MatrixErrorBox({ error }: { error?: string }) {
  ...
  return (
    <div className="rounded-2xl border border-rose-300 bg-rose-50 p-4 font-semibold text-rose-700 dark:border-rose-400/30 dark:bg-rose-400/10 dark:text-rose-100">
      {error}
    </div>
  );
}

export function MatrixPracticeCard({
  question,
  answer,
}: {
  question: string;
  ...
  return (
    <SectionCard title="Practice Question">
      ...
      {open && (
        <p className="mt-4 rounded-2xl bg-emerald-50 p-4 font-semibold text-emerald-800 dark:bg-emerald-400/10 dark:text-emerald-100">
          {answer}
        </p>
      )}
    </SectionCard>
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <SectionCard title="Required Condition">
          <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">
            {condition}
          </p>
        </SectionCard>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <SectionCard title="Common Mistake">
          <p className="text-sm leading-6 text-slate-600 dark:text-slate-300">
            {mistake}
          </p>
        </SectionCard>
