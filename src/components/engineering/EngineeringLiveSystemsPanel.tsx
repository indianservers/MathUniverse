...
export default function EngineeringLiveSystemsPanel({
  onWorkspaceObjectsChange,
  workspaceParent,
}: EngineeringLiveSystemsPanelProps) {
  ...
  const bode = useMemo(
    () => buildBodeResponse(gain, timeConstant),
    [gain, timeConstant],
  );
  const fourier = useMemo(
    () => buildFourierSynthesis(harmonics, 1.2),
    [harmonics],
  );
  const convolution = useMemo(
    () => buildConvolutionResponse(timeConstant, pulseWidth),
    [pulseWidth, timeConstant],
  );
  const ode = useMemo(
    () => solveFirstOrderOde(odeRate, forcing, 0),
    [forcing, odeRate],
  );
  const heat = useMemo(() => simulateHeatRod(0.9, heatSteps), [heatSteps]);
  const wave = useMemo(
    () => simulateWaveString(waveMode, 1.1, time),
    [time, waveMode],
  );
  const field = useMemo(() => buildVectorField(swirl, source), [source, swirl]);
  const stress = useMemo(
    () => buildStressStrainCurve(elasticity, 0.012),
    [elasticity],
  );
  const eigen = useMemo(
    () => buildEigenMode(eigenMode, elasticity, 12),
    [eigenMode, elasticity],
  );
      ...
      buildEngineeringLiveSystemWorkspaceObjects({
        ...
        systems: [
          ...
          },
          {
            ...
            summary:
              "Rectangular pulse through a first-order impulse response.",
            color: "#f59e0b",
          },
          {
            id: "numerical-ode",
          ...
          },
          {
            ...
            summary:
              "One-dimensional heat diffusion and standing-wave mode profiles.",
            color: "#fb7185",
          },
          {
            id: "vector-field",
            ...
            summary:
              "Adjustable linear field with divergence and curl metrics.",
            color: "#f59e0b",
          },
          {
            id: "stress-strain",
            ...
            controls: { elasticity },
            summary:
              "Simplified bilinear material response with resilience estimate.",
            color: "#a855f7",
          },
          {
            id: "eigenmode",
    ...
    <section className="rounded-lg border border-slate-200 bg-white p-4 shadow-sm dark:border-white/10 dark:bg-slate-950/60">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-black uppercase text-cyan-700 dark:text-cyan-200">
            Live Engineering Systems
          </p>
          <h3 className="mt-1 text-lg font-black text-slate-950 dark:text-white">
            Control, Signals, PDEs, Fields, Materials, Modes
          </h3>
        </div>
      ...
      <div className="mt-4 grid gap-3 xl:grid-cols-2">
        <SystemCard
          ...
          metrics={[
            `wc=${round(bode.cutoffFrequency)}`,
            `DC=${round(bode.dcGainDb)} dB`,
          ]}
          controls={
            <MiniSlider
              label="tau"
        ...
        <SystemCard
          ...
          metrics={[
            `peak=${round(convolution.peak)}`,
            `area=${round(convolution.area)}`,
          ]}
          controls={
            <MiniSlider
              label="pulse"
        ...
        <SystemCard
          ...
          metrics={[
            `steady=${round(ode.steadyState)}`,
            `settle=${round(ode.settlingTime)}s`,
          ]}
          controls={
        ...
        <SystemCard
          ...
          metrics={[
            `div=${round(field.divergence)}`,
            `curl=${round(field.curl)}`,
          ]}
          controls={
            <div className="grid gap-2">
              <MiniSlider
        ...
        <SystemCard
          ...
          metrics={[
            `yield=${round(stress.yieldStress)} MPa`,
            `U=${round(stress.resilience)}`,
          ]}
          controls={
            <MiniSlider
              label="E"
        ...
        <SystemCard
          ...
          metrics={[
            `mode=${eigenMode}`,
            `fn=${round(eigen.naturalFrequency)} Hz`,
            `${eigen.nodes.length} nodes`,
          ]}
          controls={
            <MiniSlider
              label="mode"
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-2 flex flex-wrap gap-2">
            {metrics.map((metric) => (
              <span key={metric} className="mini-chip">
                {metric}
              </span>
            ))}
          </div>
...
function LineChart({
  ...
  const path = points
    .map((point, index) => {
      ...
      const y =
        padding + (1 - (point.y - minY) / ySpan) * (height - padding * 2);
  ...
  return (
    <svg
      ...
      className="h-40 w-full rounded-lg bg-slate-950"
    >
      ...
      {zero && zeroY >= padding && zeroY <= height - padding && (
        <line
          x1={padding}
          x2={width - padding}
          y1={zeroY}
      ...
      />
      <text x={padding} y={height - 8} fill="#94a3b8" fontSize="10">
        min {round(minY)} max {round(maxY)}
      </text>
    </svg>
  );
}
      ...
      {points.map((point, index) => {
        ...
        return (
          <rect
            ...
            height={height}
            rx="3"
            fill={index % 2 ? "#38bdf8" : "#f59e0b"}
          />
        );
      })}
      <text x="12" y="150" fill="#94a3b8" fontSize="10">
        harmonics
      </text>
    </svg>
  );
...
function VectorFieldChart({
  samples,
}: {
  samples: Array<{ x: number; y: number; u: number; v: number }>;
}) {
  const size = 220;
  const center = size / 2;
  return (
