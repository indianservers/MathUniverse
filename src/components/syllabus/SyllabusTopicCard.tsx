...
} from "../../data/advancedSyllabusLabs";
...
export default function SyllabusTopicCard({ topic }: { topic: SyllabusTopic }) {
  const href = topic.linkedVisualization.available
    ? `${topic.linkedVisualization.route}${topic.linkedVisualization.section ? `#${topic.linkedVisualization.section}` : ""}`
    : `/syllabus#${topic.id}`;
  const conceptLabId = (concept: string) =>
  ...
  return (
    <article
      id={topic.id}
      className="glass-card flex min-h-[360px] scroll-mt-24 flex-col rounded-2xl p-5"
    >
      ...
      <div className="mt-4">
        <ConceptImagePanel
          title={topic.title}
          text={`${topic.unit} ${topic.description} ${topic.concepts.join(" ")} ${topic.keyFormulas.join(" ")}`}
          compact
        />
      </div>
      <div className="mt-4">
        ...
        <div className="mt-2 flex flex-wrap gap-2">
          {topic.concepts.slice(0, 5).map((concept, index) => (
            <Link
              key={`${concept}-${index}`}
              to={`/syllabus-lab/${conceptLabId(concept)}`}
              className="mini-chip gap-1.5 transition hover:border-cyan-300 hover:bg-cyan-100 hover:text-cyan-900 dark:hover:bg-cyan-300/15"
            >
              <ConceptIconBadge
                text={`${topic.title} ${concept}`}
        ...
        <ul className="mt-2 space-y-2 text-xs leading-5">
          {topic.keyFormulas.slice(0, 3).map((formula, index) => (
            <li key={`${formula}-${index}`} className="flex items-start gap-2">
              <ConceptIconBadge
                text={`${topic.title} ${formula}`}
                className="mt-0.5 h-5 w-5 [&_svg]:h-3 [&_svg]:w-3"
              />
              <span className="min-w-0 break-words font-semibold">
                <MathExpression value={formula} />
              </span>
            </li>
          ))}
        </ul>
      ...
      <p className="mt-4 text-sm leading-6 text-slate-600 dark:text-slate-300">
        ...
        {topic.recommendedVisualization}
      </p>
      {topic.linkedVisualization.available &&
      topic.linkedVisualization.isExternal ? (
        <a href={href} className="action-primary mt-auto w-full">
          {topic.linkedVisualization.label}
        </a>
      ) : topic.linkedVisualization.available ? (
        <Link to={href} className="action-primary mt-auto w-full">
          {topic.linkedVisualization.label}
        </Link>
      ) : (
        <Link to={href} className="action-secondary mt-auto w-full">
          View Concept Card
        </Link>
      )}
    </article>
