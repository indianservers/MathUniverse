...
} from "../../data/syllabusIntelligence";
import type { SyllabusTopic } from "../../data/syllabus";
import SectionCard from "../ui/SectionCard";

export default function SmartSyllabusPlanner({
  topics,
  levelId,
}: {
  ...
  return (
    <SectionCard
      title="Smart Syllabus Intelligence"
      description="Browser-only planner that ranks coverage, recommends guided workspace templates, and turns syllabus gaps into launchable teaching paths."
    >
      ...
      <div className="mt-5 grid gap-5 xl:grid-cols-[minmax(0,1fr)_420px]">
        <div className="rounded-2xl border border-slate-200 bg-white/70 p-4 dark:border-white/10 dark:bg-white/5">
          <div className="flex items-center gap-3">
            <span className="rounded-2xl bg-cyan-100 p-3 text-cyan-700 dark:bg-cyan-400/10 dark:text-cyan-200">
              <BrainCircuit className="h-5 w-5" />
            </span>
            <div>
              <h3 className="font-bold">Unit Priority Engine</h3>
              <p className="text-sm text-slate-500 dark:text-slate-400">
                Highest scores need new visual depth or stronger guided paths.
              </p>
            </div>
          </div>
          <div className="mt-4 space-y-3">
            {units.map((unit) => (
              <div
                key={unit.unit}
                className="rounded-2xl bg-slate-100 p-4 dark:bg-white/10"
              >
            ...
            {path.map((item) => (
              ...
              >
                <div className="flex items-start justify-between gap-3">
                  <div>
                    <p className="text-xs font-bold uppercase text-cyan-600 dark:text-cyan-300">
                      {item.topic.classLevel} / {item.topic.unit}
                    </p>
                ...
                {item.gap && (
                  <p className="mt-2 text-xs leading-5 text-slate-500 dark:text-slate-400">
                    Gap note: {item.gap.recommendedBuild}
                  </p>
                )}
                {item.workspaceRoute.startsWith("http") ? (
                  <a
                    href={item.workspaceRoute}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-cyan-600 dark:text-cyan-300"
                  >
                  ...
                  <Link
                    to={item.workspaceRoute}
                    className="mt-3 inline-flex items-center gap-2 text-sm font-bold text-cyan-600 dark:text-cyan-300"
                  >
                    Open guided lab <ArrowRight className="h-4 w-4" />
                  </Link>
                )}
              </div>
            ))}
...
function Metric({
  ...
  return (
    <div className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-white/5">
      <div className="mb-3 inline-flex rounded-2xl bg-slate-950 p-3 text-white dark:bg-white dark:text-slate-950">
        {icon}
      </div>
