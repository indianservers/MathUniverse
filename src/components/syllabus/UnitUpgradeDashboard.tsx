...
import {
  ...
  unitUpgradeSummary,
  type UnitUpgradeTarget,
} from "../../data/unitUpgradePlan";
...
function compactTargets(targets: UnitUpgradeTarget[]) {
  ...
  for (const target of targets) {
    ...
    if (!current || (target.priority === "High" && current.priority !== "High"))
      byUnit.set(target.unit, target);
  }
...
export default function UnitUpgradeDashboard({
  topics,
}: UnitUpgradeDashboardProps) {
  ...
  return (
    <SectionCard
      ...
      headerAction={
        <Link to="/workspace?mode=guided" className="action-secondary">
          Open workspace
        </Link>
      }
    >
      <div className="grid gap-2 sm:grid-cols-4">
        {[
          ...
          <div
            key={label}
            className="rounded-lg border border-slate-200 bg-white/70 p-3 dark:border-white/10 dark:bg-white/5"
          >
      ...
      <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {targets.map((target) => (
          <article
            key={target.id}
            className="rounded-lg border border-slate-200 bg-white p-3 dark:border-white/10 dark:bg-slate-950"
          >
            <div className="flex items-start justify-between gap-2">
              ...
              <span
                className={`shrink-0 rounded-full border px-2 py-0.5 text-xs font-bold ${priorityClasses[target.priority]}`}
              >
                {target.priority}
              </span>
            </div>
            <div className="mt-3 space-y-2 text-xs leading-5 text-slate-600 dark:text-slate-300">
              ...
              <p>
                ...
                {target.qualityTargets[0]}
              </p>
            </div>
            <Link
              to={target.route}
              className="mt-3 inline-flex text-sm font-bold text-cyan-700 hover:text-cyan-900 dark:text-cyan-300 dark:hover:text-cyan-100"
            >
              Open unit
            </Link>
          </article>
