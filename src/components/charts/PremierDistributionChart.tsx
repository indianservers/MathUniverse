...
export default function PremierDistributionChart({
  points,
  kind,
  title,
}: PremierDistributionChartProps) {
  const rawId = useId().replace(/:/g, "");
  const isDiscrete = kind === "discrete";
  const chart = useMemo(
    () => buildChart(points, isDiscrete),
    [points, isDiscrete],
  );
  ...
  const activeLabel = isDiscrete
    ? "PMF: probability at exact outcomes"
    : "PDF: probability comes from area";
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={title ?? `${kind} distribution chart`}
    ...
    >
      ...
      <g>
        ...
        {chart.xTicks.map((tick) => (
          <g key={`x-${tick.x}-${tick.label}`}>
            <line
              x1={chart.xScale(tick.x)}
              x2={chart.xScale(tick.x)}
              y1={margin.top}
              y2={chart.baseline}
              stroke="#e0f2fe"
              strokeWidth="1"
      ...
      <text
        ...
        y="26"
        textAnchor="end"
        className="fill-cyan-700 text-[14px] font-black"
      >
        {activeLabel}
      </text>
          ...
          {chart.discreteBars.map((bar) => (
            <g key={`${bar.label}-${bar.x}`}>
              ...
              <rect
                x={bar.x0 + 2}
                y={bar.y + 2}
                width={Math.max(1, bar.width * 0.24)}
                height={Math.max(0, chart.baseline - bar.y - 4)}
                rx="4"
                fill="#cffafe"
                opacity="0.34"
              />
      ...
      {chart.peak && (
        <g>
          ...
          />
          <text
            x={Math.min(chartWidth - 140, chart.peak.x + 14)}
            y={Math.max(54, chart.peak.y - 12)}
            className="fill-orange-700 text-[12px] font-black"
          >
...
function buildChart(points: PremierDistributionPoint[], isDiscrete: boolean) {
  ...
  const xPadding = isDiscrete
    ? Math.max(0.5, (rawMaxX - rawMinX) * 0.035)
    : Math.max(0.01, (rawMaxX - rawMinX) * 0.02);
  const xScale = d3
    .scaleLinear()
  ...
  const line = d3
    .line<PremierDistributionPoint>()
    .x((point) => xScale(point.x))
    .y((point) => yScale(point.y))
    .curve(d3.curveMonotoneX);
  const area = d3
    .area<PremierDistributionPoint>()
    .x((point) => xScale(point.x))
    .y0(baseline)
  ...
  const xTicks = ordered
    .filter(
      (_, index) => index % labelStride === 0 || index === ordered.length - 1,
    )
  ...
  const minStep =
    d3.min(
      ordered
        .slice(1)
        .map((point, index) => Math.abs(point.x - ordered[index].x)),
    ) ?? 1;
  const barWidth = Math.max(
    5,
    Math.min(24, Math.abs(xScale(rawMinX + minStep) - xScale(rawMinX)) * 0.32),
  );
  ...
  const peakPoint =
    d3.max(ordered, (point) => point.y) === undefined
      ? undefined
      : ordered.reduce(
  ...
  const peak = peakPoint
    ? { x: xScale(peakPoint.x), y: yScale(peakPoint.y), label: peakPoint.label }
    : undefined;
