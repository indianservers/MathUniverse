...
import {
  accuracyConceptsForDomain,
  type PhaseOneDomain,
  type StrengthenedConcept,
} from "../../data/conceptAccuracy";
import MathExpression from "./MathExpression";
import SectionCard from "./SectionCard";

export default function ConceptAccuracyPanel({
  domain,
}: {
        ...
        {concepts.map((concept) => {
          ...
          return (
            ...
            >
              ...
              >
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-gradient-to-br from-emerald-400 to-cyan-500 text-white shadow-sm">
                ...
                <span className="min-w-0 flex-1">
                  ...
                  <span className="mt-0.5 block text-xs leading-5 text-slate-600 dark:text-slate-300">
                    {concept.definition.learner}
                  </span>
                </span>
                <span className="mini-chip shrink-0">{concept.level}</span>
                <ChevronDown
                  className={`mt-2 h-4 w-4 shrink-0 transition-transform ${expanded ? "rotate-180" : ""}`}
                />
              </button>
              <div
                id={contentId}
    ...
    <div className="grid gap-3 xl:grid-cols-2">
      <div className="space-y-3">
        ...
        >
          ...
          <p className="mt-2">
            <strong>Domain and exclusions:</strong> {concept.domainStatement}
          </p>
        </AccuracyBlock>
        <AccuracyBlock
          icon={<CheckCircle2 className="h-4 w-4" />}
          title="Formula and validity"
        >
        ...
        <AccuracyBlock
          ...
          title="Misconceptions and counterexamples"
          tone="amber"
        >
          <div className="grid gap-2">
            {concept.misconceptions.map((item) => (
              <div
                key={item.claim}
                className="rounded-lg bg-white/70 p-2 dark:bg-slate-950/30"
              >
                <p>
                  <strong>Not:</strong> {item.claim}
                </p>
                <p className="mt-1">
                  <strong>Correct:</strong> {item.correction}
                </p>
                <p className="mt-1 text-xs opacity-80">
                  <strong>Counterexample:</strong> {item.counterexample}
                </p>
              </div>
            ))}
          </div>
      ...
      <div className="space-y-3">
        <AccuracyBlock
          ...
          title="Topic-specific example bank"
          tone="violet"
        >
          <div className="grid gap-2">
            {concept.examples.map((example) => (
              <div
                key={example.id}
                className="rounded-lg border border-violet-100 bg-white/70 p-2 dark:border-violet-300/15 dark:bg-slate-950/30"
              >
                <div className="flex flex-wrap items-center gap-2">
                  <span className="mini-chip">{example.kind}</span>
        ...
        >
          <p>
            <strong>Oracle:</strong> {concept.validation.oracle}
          </p>
          {concept.validation.tolerance != null && (
            <p className="mt-1">
              <strong>Tolerance:</strong> {concept.validation.tolerance}
            </p>
          )}
          <ul className="mt-2 grid gap-1">
            {concept.invariants.map((invariant) => (
...
function AccuracyBlock({
  ...
  const tones = {
    ...
    amber:
      "border-amber-100 bg-amber-50/60 dark:border-amber-300/15 dark:bg-amber-300/10",
    violet:
      "border-violet-100 bg-violet-50/60 dark:border-violet-300/15 dark:bg-violet-300/10",
    emerald:
      "border-emerald-100 bg-emerald-50/60 dark:border-emerald-300/15 dark:bg-emerald-300/10",
  };
  return (
    <section
      className={`rounded-xl border p-3 text-sm leading-6 text-slate-700 dark:text-slate-200 ${tones[tone]}`}
    >
      <h3 className="mb-2 flex items-center gap-2 font-black text-slate-950 dark:text-white">
        {icon}
        {title}
      </h3>
      {children}
