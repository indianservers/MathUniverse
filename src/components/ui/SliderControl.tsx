...
export function SliderGroup({
  ...
  return (
    <div
      className={`rounded-xl border border-slate-200 bg-white/75 p-3 shadow-sm dark:border-white/10 dark:bg-slate-950/40 ${className}`}
    >
      {(title || description) && (
        <div className="mb-3">
          {title && (
...
export default function SliderControl({
  ...
  const control = useRef<HTMLLabelElement>(null);
  ...
  useEffect(() => {
    function onKeyDown(event: KeyboardEvent) {
      if (
        !locked &&
        control.current?.contains(event.target as Node) &&
  ...
  const commit = (next: number, remember = true) => {
    ...
    const clamped = Math.min(
      max,
      Math.max(min, Number.isFinite(next) ? next : value),
    );
  ...
  if (compact) {
    return (
      ...
      >
        ...
        <div className="grid min-w-0 grid-cols-[minmax(0,1fr)_auto] gap-1 md:col-span-2">
          <input
  ...
  return (
    <label
      ref={control}
      className="block rounded-xl border border-slate-200 bg-white/75 p-3 shadow-sm transition hover:border-cyan-300 dark:border-white/10 dark:bg-slate-950/40 dark:hover:border-cyan-400/40"
    >
