...
export default function SliderControl({ label, value, min, max, step, onChange, unit = "", description, density = "default" }: SliderControlProps) {
  ...
  if (compact) {
    return (
      <label className="grid min-w-0 gap-2 py-2 first:pt-0 last:pb-0 md:grid-cols-[minmax(72px,0.7fr)_minmax(120px,1.6fr)] md:items-center">
        ...
        <button type="button" className={locked ? "mini-chip h-8 w-full justify-center bg-cyan-100 p-0 text-cyan-700 dark:bg-cyan-400/15 dark:text-cyan-100 md:col-span-2" : "mini-chip h-8 w-full justify-center p-0 md:col-span-2"} onClick={() => setLocked((value) => !value)} title="Range lock marker for paired slider work">
