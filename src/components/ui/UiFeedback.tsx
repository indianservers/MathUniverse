...
export function BackToPreviousButton({ label = "Back" }: { label?: string }) {
  ...
  return (
    <button
      type="button"
      className="action-secondary min-h-10 px-3 py-2"
      onClick={() => navigate(-1)}
    >
...
export function EmptyState({
  ...
  message = "Add input or change the settings to generate results.",
}: {
  title?: string;
  message?: string;
}) {
...
export function LoadingSkeleton({
  label = "Loading interactive view",
}: {
  label?: string;
}) {
  return (
    <div
      className="space-y-3 rounded-xl border border-slate-200 bg-white/70 p-3 dark:border-white/10 dark:bg-white/5"
      aria-live="polite"
    >
...
export function CopyResultButton({
  ...
  return (
    <button
      ...
      onClick={copy}
      disabled={disabled}
      title={`${label} (Ctrl/Cmd+C after selecting also works)`}
    >
...
export function ApproxBadge() {
  return (
    <span className="inline-flex w-fit rounded-full bg-amber-100 px-2.5 py-1 text-[11px] font-black uppercase text-amber-800 dark:bg-amber-400/15 dark:text-amber-100">
      Approximate
    </span>
  );
}
...
export function ResetExampleButton({
  ...
  return (
    <button
      ...
      className="tool-button"
      onClick={onClick}
      title="Reset to the default worked example"
    >
...
export function ResetValuesButton({
  onClick,
  label = "Reset values",
}: {
  onClick: () => void;
  ...
  return (
    <button
      ...
      className="tool-button"
      onClick={onClick}
      title="Clear current values and return controls to their default state"
    >
      <RotateCcw className="h-4 w-4" />
      {label}
    </button>
...
export function ExampleValuesButton({
  onClick,
  ...
  return (
    <button
      ...
      className="tool-button"
      onClick={onClick}
      title="Fill the tool with a complete example"
    >
...
export function PresetChips({
  ...
  useEffect(() => {
    try {
      setRecent(
        JSON.parse(localStorage.getItem("math-universe-last-examples") ?? "[]"),
      );
    } catch {
      setRecent([]);
    }
  ...
  function choose(example: string) {
    onSelect(example);
    const next = [example, ...recent.filter((item) => item !== example)].slice(
      0,
      5,
    );
    setRecent(next);
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {recent.map((example) => (
            <button
              key={`recent-${example}`}
              type="button"
              className="mini-chip bg-cyan-50 text-cyan-700 transition hover:bg-cyan-100 dark:bg-cyan-400/10 dark:text-cyan-100"
...
export function BookmarkToolButton({
  id,
  title,
  label = "Bookmark tool",
}: {
  ...
  return (
    <button
      type="button"
      className={
        bookmarked ? "action-primary min-h-10 px-3 py-2" : "tool-button"
      }
...
export function PracticeModeToggle() {
  const [enabled, setEnabled] = useState(
    () => localStorage.getItem(practiceModeKey) === "true",
  );
  ...
  return (
    <button
      type="button"
      className={enabled ? "action-primary min-h-10 px-3 py-2" : "tool-button"}
      onClick={() => setEnabled((value) => !value)}
      title="Hide heavier theory panels while practicing"
    >
      <Sparkles className="h-4 w-4" />
      Practice mode
    </button>
      ...
      <div className="mt-2 grid gap-1.5">
        {items.map((item) => (
          <a
            key={item.id}
            href={`#${item.id}`}
            className="text-sm font-semibold text-slate-600 transition hover:text-cyan-700 dark:text-slate-300 dark:hover:text-cyan-200"
          >
            {item.label}
          </a>
        ))}
...
export function CollapsibleTheorySection({
  title,
  children,
  defaultOpen = false,
}: {
  ...
  return (
    <div
      data-theory
      className="rounded-xl border border-slate-200 bg-white/80 p-3 dark:border-white/10 dark:bg-white/5"
    >
      <button
        type="button"
...
export function ExportImageButton({
  targetId,
  filename = "math-universe-visualization.png",
  label = "Export image",
}: {
  targetId: string;
  filename?: string;
  ...
  return (
    <button
      ...
      title="Export the current visualization as a PNG image"
    >
...
export function TeacherModeToggle() {
  const [enabled, setEnabled] = useState(
    () => localStorage.getItem("math-universe-teacher-mode") === "true",
  );
  ...
  useEffect(() => {
    ...
    return () =>
      window.removeEventListener("math-universe-toggle-teacher-mode", toggle);
  }, []);
  ...
  return (
    <button
      ...
      className={enabled ? "action-primary min-h-10 px-3 py-2" : "tool-button"}
      onClick={() => setEnabled((value) => !value)}
      title="Toggle larger classroom text"
    >
      <Sparkles className="h-4 w-4" />
      Teacher mode
    </button>
...
export function TermTooltip({ term, tip }: { term: string; tip: string }) {
  return (
    <span
      className="tooltip-icon cursor-help border-b border-dotted border-cyan-500 font-bold text-cyan-700 dark:text-cyan-200"
      data-tooltip={tip}
      title={tip}
    >
      {term}
