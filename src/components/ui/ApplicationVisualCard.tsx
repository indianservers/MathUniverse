...
export default function ApplicationVisualCard({
  title,
  description,
  visual,
  meta,
  ...
  return (
    <article className="group rounded-xl border border-slate-200 bg-white/75 p-3 transition hover:-translate-y-0.5 hover:border-cyan-300 hover:shadow-lg hover:shadow-cyan-950/10 dark:border-white/10 dark:bg-white/5 dark:hover:border-cyan-300/30">
      ...
      <div className={compact ? "mt-2" : "mt-3"}>
        {meta && (
          <p className="text-[11px] font-black uppercase tracking-normal text-cyan-700 dark:text-cyan-200">
            {meta}
          </p>
        )}
        <h3 className="mt-1 break-words text-sm font-black text-slate-950 dark:text-white">
          {title}
        </h3>
...
function ApplicationArtwork({
  kind,
  title,
  accent,
  compact,
}: {
  kind: ApplicationVisualKind;
  title: string;
  ...
  return (
    <svg
      className="block w-full overflow-hidden rounded-lg border border-slate-200/80 bg-slate-950 shadow-inner dark:border-white/10"
      viewBox={`0 0 320 ${height}`}
      role="img"
      aria-label={`${title} application image`}
    >
      <defs>
  ...
  return (
    <g opacity="0.18" stroke="#cbd5e1" strokeWidth="1">
      {[40, 80, 120, 160, 200, 240, 280].map((x) => (
        <line key={`x-${x}`} x1={x} y1="0" x2={x - 28} y2={height} />
      ))}
      {[24, 48, 72, 96].map((y) => (
...
function ArtworkShape({
  ...
  if (
    ["computer-graphics", "transform-3d", "game-design", "ar-vr"].includes(kind)
  ) {
    return (
      <g
        fill="none"
      ...
      >
        <path
          d="M74 72 L138 36 L214 58 L150 94 Z"
          fill={accent.from}
          opacity="0.22"
        />
        ...
        <path
          d="M74 72 L74 100 L150 128 L214 86"
          opacity="0.55"
          transform={`translate(0 ${height - 132})`}
        />
        <circle
          cx="246"
          cy={height - 34}
  ...
  if (["robotics", "engineering"].includes(kind)) {
    return (
      ...
      >
        <path
          d={`M74 ${height - 24} L124 ${height - 58} L180 ${height - 38} L236 ${height - 72}`}
        />
        {[74, 124, 180, 236].map((x, index) => (
          <circle
            key={x}
            cx={x}
            cy={[height - 24, height - 58, height - 38, height - 72][index]}
            r="11"
            fill="#020617"
            stroke={index % 2 ? accent.mid : accent.from}
            strokeWidth="4"
          />
        ))}
        <path
          d={`M232 ${height - 75} l24 -12 M232 ${height - 70} l28 4`}
          stroke={accent.to}
          strokeWidth="5"
        />
      </g>
  ...
  if (
    ["data-compression", "pca", "image-processing", "medical-imaging"].includes(
      kind,
    )
  ) {
    return (
        ...
        {[0, 1, 2, 3, 4].map((row) =>
          [0, 1, 2, 3, 4, 5].map((col) => (
            <rect
              ...
              height="10"
              rx="2"
              fill={col + row > 5 ? accent.mid : accent.from}
              opacity={0.35 + ((row + col) % 4) * 0.16}
            />
          )),
        )}
        <path
          d={`M204 ${height - 74} C230 ${height - 88}, 244 ${height - 40}, 274 ${height - 56}`}
          fill="none"
          stroke={accent.to}
          strokeWidth="5"
          strokeLinecap="round"
  ...
  if (["game-physics", "physics", "motion-model"].includes(kind)) {
    return (
      <g fill="none" strokeLinecap="round">
        <path
          d={`M38 ${height - 24} C92 ${height - 92}, 158 ${height - 6}, 242 ${height - 66}`}
          stroke={`url(#${gradientId})`}
          strokeWidth="5"
        />
        ...
        <path
          d={`M154 ${height - 40} l42 -22`}
          stroke={accent.mid}
          strokeWidth="4"
          markerEnd=""
        />
        <path
          d={`M194 ${height - 62} l-8 -1 l5 7`}
          stroke={accent.mid}
          strokeWidth="4"
        />
      </g>
  ...
  if (
    ...
    return (
      <g fill="none" strokeLinecap="round">
        <path
          d={`M38 ${height - 28} C82 ${height - 86}, 114 ${height - 12}, 152 ${height - 54} S234 ${height - 82}, 284 ${height - 32}`}
          stroke="#94a3b8"
          strokeWidth="2"
          opacity="0.45"
        />
        <polyline
          points={`54,${height - 30} 98,${height - 52} 140,${height - 42} 184,${height - 70} 238,${height - 60} 282,${height - 86}`}
          stroke={`url(#${gradientId})`}
          strokeWidth="5"
        />
        {[98, 184, 238].map((x, index) => (
          <circle
            key={x}
            cx={x}
            cy={[height - 52, height - 70, height - 60][index]}
            r="7"
            fill={index === 1 ? accent.to : accent.from}
          />
        ))}
      </g>
    );
  ...
  if (
    ...
    return (
      <g fill="none" strokeLinecap="round">
        <path
          d={`M34 ${height / 2} C54 18, 74 18, 94 ${height / 2} S134 ${height - 18}, 154 ${height / 2} S194 18, 214 ${height / 2} S254 ${height - 18}, 286 ${height / 2}`}
          stroke={`url(#${gradientId})`}
          strokeWidth="5"
        />
        {[72, 154, 236].map((x) => (
  ...
  if (kind === "gps") {
    return (
      <g fill="none" strokeLinecap="round">
        <path
          d={`M52 ${height - 26} L160 ${height - 78} L272 ${height - 34}`}
          stroke="#94a3b8"
          strokeWidth="2"
          opacity="0.45"
        />
        {[52, 160, 272].map((x, index) => (
          <circle
            key={x}
            cx={x}
            cy={[height - 26, height - 78, height - 34][index]}
            r={index === 1 ? 34 : 22}
            stroke={index === 1 ? accent.mid : accent.from}
            strokeWidth="3"
            opacity="0.75"
          />
        ))}
  ...
  if (kind === "cryptography") {
    return (
      <g>
        ...
        />
        <path
          d={`M112 ${height - 74} C112 ${height - 110}, 208 ${height - 110}, 208 ${height - 74}`}
          fill="none"
          stroke={accent.mid}
          strokeWidth="8"
          strokeLinecap="round"
        />
        {[116, 144, 172, 200].map((x) => (
          <rect
            key={x}
            x={x}
            y={height - 54}
  ...
  if (["architecture"].includes(kind)) {
    return (
      ...
      >
        <path
          d={`M56 ${height - 22} L116 ${height - 82} L176 ${height - 22} Z`}
          fill={accent.from}
          opacity="0.2"
        />
        <path
          d={`M128 ${height - 22} L202 ${height - 96} L276 ${height - 22} Z`}
          fill={accent.mid}
          opacity="0.18"
        />
        <line
  ...
  return (
    ...
    >
      ...
      <path
        d={`M68 ${height - 30} C112 ${height - 78}, 206 ${height - 78}, 252 ${height - 30}`}
      />
    </g>
  );
}
