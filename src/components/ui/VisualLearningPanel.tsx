...
export default function VisualLearningPanel({
  ...
  return (
    <SectionCard
      ...
      description="Realtime examples, proof steps, and practice that update with the visualizer."
      className="border-cyan-200/80 dark:border-cyan-400/20"
      compact
      headerAction={
        <button
          type="button"
          className="action-secondary min-h-9 shrink-0 rounded-xl px-3 py-2 text-xs"
          aria-expanded={expanded}
          aria-controls={contentId}
          onClick={() => setExpanded((value) => !value)}
        >
          {expanded ? "Collapse examples" : "Expand examples"}
          <ChevronDown
      ...
      <div id={contentId} hidden={!expanded}>
        <div className="mb-3 grid gap-2 md:grid-cols-3">
          <LearningCue
            ...
            label="Look"
            value="Find the fixed shape, equal marks, and moving point."
          />
          <LearningCue
            ...
            value={
              proofIdea ??
              "One relationship stays true while the diagram changes."
            }
          />
          <LearningCue
            ...
            value={
              teacherPrompt ??
              "Say the rule in your own words before using the formula."
            }
          />
        </div>
        ...
        <div className="grid gap-3 lg:grid-cols-2">
          <div className="space-y-3">
            ...
            {warning && (
              <div className="rounded-xl border border-amber-300 bg-amber-50 p-3 text-sm text-amber-800 dark:border-amber-400/30 dark:bg-amber-400/10 dark:text-amber-100">
                {warning}
              </div>
            )}
          </div>
          <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-1 xl:grid-cols-2">
            ...
            <div className="rounded-xl border border-violet-100 bg-violet-50/70 p-3 dark:border-violet-300/20 dark:bg-violet-300/10">
              <h3 className="flex items-center gap-2 font-semibold">
                <CheckCircle2 className="h-4 w-4 text-violet-600 dark:text-violet-200" />{" "}
                Try this
              </h3>
...
function LearningCue({
  ...
  return (
    <div className="rounded-xl border border-slate-200 bg-white/80 p-3 dark:border-white/10 dark:bg-white/5">
      ...
      <p className="mt-2 text-sm font-semibold leading-5 text-slate-700 dark:text-slate-200">
        {value}
      </p>
    </div>
  );
...
function Callout({
  ...
  return (
    <div
      className={`rounded-xl border p-3 text-sm ${danger ? "border-rose-200 bg-rose-50 text-rose-800 dark:border-rose-300/20 dark:bg-rose-400/10 dark:text-rose-100" : ""}`}
    >
