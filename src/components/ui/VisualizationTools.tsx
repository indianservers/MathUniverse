    ...
    <div className="space-y-3">
      <div className="visualization-tools flex flex-wrap items-center gap-2">
        ...
        <button
          ...
          title="Use browser full-screen mode"
        >
        ...
        <button
          className={
            showGrid
              ? "tool-button bg-cyan-50 text-cyan-700 dark:bg-cyan-400/15 dark:text-cyan-100"
              : "tool-button"
          }
          type="button"
          onClick={() => setShowGrid((value) => !value)}
          title="Show or hide coordinate grid"
        >
        ...
        <button
          ...
          title="Keep a faint ghost trail while parameters change"
        >
          <span>Trace</span>
        </button>
        ...
        <button
          ...
          title="Advance one explanatory step"
        >
      ...
      >
        ...
        {showGrid && (
          <div className="pointer-events-none absolute inset-0 rounded-2xl opacity-40 [background-image:linear-gradient(rgba(14,165,233,.18)_1px,transparent_1px),linear-gradient(90deg,rgba(14,165,233,.18)_1px,transparent_1px)] [background-size:32px_32px]" />
        )}
        {trace && (
          <div className="pointer-events-none absolute inset-0 rounded-2xl bg-cyan-400/5 ring-4 ring-inset ring-cyan-400/10" />
        )}
...
function CrosshairHud() {
  ...
  return (
    ...
    >
      {point && (
        <>
          <div className="pointer-events-none absolute bottom-3 left-3 rounded-full bg-slate-950 px-3 py-1 text-xs font-black text-white dark:bg-white dark:text-slate-950">
            x {point.x.toFixed(3)} · y {(1 - point.y).toFixed(3)}
          </div>
          <div
            className="pointer-events-none absolute top-0 h-full w-px bg-cyan-400/70"
            style={{ left: `${point.x * 100}%` }}
          />
          <div
            className="pointer-events-none absolute left-0 h-px w-full bg-cyan-400/70"
  ...
  return (
    <button
      className={clsx(
        ...
        active &&
          "bg-cyan-600 text-white hover:bg-cyan-600 hover:text-white dark:bg-cyan-400 dark:text-slate-950",
      )}
      type="button"
      onClick={onClick}
      title={title}
...
function AnnotationLayer({
  ...
  const [activeStroke, setActiveStroke] = useState<StrokeAnnotation | null>(
    null,
  );
  ...
  useEffect(() => {
    if (canvasRef.current)
      renderAnnotations(canvasRef.current, annotations, activeStroke);
  }, [activeStroke, annotations]);
  ...
  function move(event: React.PointerEvent<HTMLCanvasElement>) {
    ...
    setActiveStroke({
      ...activeStroke,
      points: [...activeStroke.points, pointerPoint(event)],
    });
  }
  ...
  function end() {
    ...
    if (activeStroke.points.length > 1)
      onChange([...annotations, activeStroke]);
  ...
  return (
    <canvas
      ref={canvasRef}
      className={clsx(
        "absolute inset-0 z-10 touch-none rounded-2xl",
        mode === "inspect"
...
function renderAnnotations(
  ...
  annotations: Annotation[],
  activeStroke: StrokeAnnotation | null,
) {
  const ctx = canvas.getContext("2d");
  if (!ctx) return;

  ctx.clearRect(0, 0, canvas.width, canvas.height);
  [...annotations, ...(activeStroke ? [activeStroke] : [])].forEach(
    (annotation) => {
...
function isNear(annotation: Annotation, point: Point) {
  ...
  return annotation.points.some(
    (strokePoint) =>
      Math.hypot(strokePoint.x - point.x, strokePoint.y - point.y) < 0.035,
  );
}
...
function annotationStorageKey(title: string) {
  const path =
    typeof window === "undefined" ? "visualization" : window.location.pathname;
...
function buildPrintableSheet(target: HTMLElement, title: string) {
  ...
  clone
    .querySelectorAll(".visualization-tools")
    .forEach((element) => element.remove());
  sheet.appendChild(clone);

  const formulas = Array.from(
    document.querySelectorAll<HTMLElement>("[data-formula-block]"),
  );
  if (formulas.length) {
