...
export default function FormulaBlock({
  ...
  const displayFormula = useMemo(
    () => formatFormulaForKatex(formula),
    [formula],
  );
  const html = useMemo(() => {
    try {
      return katex.renderToString(displayFormula, {
        throwOnError: false,
        displayMode: true,
      });
    } catch {
        ...
        <div className="formula-card min-w-0 overflow-x-auto overflow-y-visible rounded-xl border border-cyan-200/70 bg-gradient-to-br from-cyan-50 to-violet-50 px-3 py-4 text-center shadow-inner shadow-white/60 dark:border-cyan-400/20 dark:from-cyan-400/10 dark:to-violet-500/10 dark:shadow-black/10">
          {html ? (
            ...
            <MathExpression
              value={formula}
              display
              className="formula-plain text-base font-semibold md:text-lg"
            />
          )}
        </div>
      </SectionCard>
...
function formatFormulaForKatex(value: string) {
  ...
  if (normalized.includes("\\begin") || normalized.length < 34)
    return normalized;
  const pieces = normalized
    .split(/,\s+|;\s+/)
    .map((piece) => piece.trim())
    .filter(Boolean);
  if (pieces.length < 2 || pieces.some((piece) => piece.length < 6))
    return normalized;
  return `\\begin{aligned}${pieces.join("\\\\")}\\end{aligned}`;
}

function normalizeFormulaText(value: string) {
  return normalizeFormulaForKatex(value).replace(
