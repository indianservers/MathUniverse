...
const siteSearchTextByPath = new Map(
  siteLinks.map((link) => [
...
export default function Sidebar() {
  ...
  const [collapsed, setCollapsed] = useState(
    () => localStorage.getItem("math-universe-sidebar-collapsed") === "true",
  );
  ...
  return (
    <aside
      className={`hidden h-screen min-h-screen shrink-0 overflow-hidden border-r border-cyan-200/25 bg-[radial-gradient(circle_at_10%_5%,rgba(34,211,238,0.3),transparent_25%),radial-gradient(circle_at_90%_48%,rgba(168,85,247,0.22),transparent_32%),linear-gradient(180deg,#082f49_0%,#172554_48%,#240c45_100%)] text-slate-100 shadow-2xl shadow-indigo-950/30 backdrop-blur-2xl transition-[width] dark:border-white/10 min-[960px]:sticky min-[960px]:top-0 min-[960px]:block ${collapsed ? "w-28" : "w-[216px] xl:w-[274px]"}`}
      data-testid="desktop-sidebar"
      data-collapsed={collapsed}
    >
      <div
        className={`thin-scrollbar flex h-full min-h-0 flex-col overflow-y-auto overflow-x-hidden ${collapsed ? "p-3 pb-20" : "p-4 pb-20"}`}
      >
        {collapsed && (
          <button
            type="button"
        ...
        <div
          className={`mb-5 flex items-center gap-3 rounded-2xl border border-white/10 bg-white/[0.06] p-2 shadow-lg shadow-black/10 ${collapsed ? "justify-center" : ""}`}
        >
          <button
            type="button"
          ...
          <button
            type="button"
            className="tooltip-icon ml-auto hidden h-9 w-9 items-center justify-center rounded-full border border-white/10 bg-white/10 text-cyan-50 transition hover:bg-cyan-300/15 min-[960px]:inline-flex"
        ...
        {!collapsed && (
          <label className="mb-4 flex min-h-11 items-center gap-2 rounded-xl border border-cyan-200/20 bg-white/[0.08] px-3 text-sm font-semibold shadow-inner shadow-black/10 focus-within:border-cyan-300/70 focus-within:bg-white/[0.12] focus-within:ring-2 focus-within:ring-cyan-300/25">
            <Search className="h-4 w-4 text-cyan-100/70" />
            <input
              value={query}
              onChange={(event) => setQuery(event.target.value)}
              placeholder="Search menu, formula, topic..."
              className="min-w-0 flex-1 bg-transparent text-white outline-none placeholder:text-cyan-100/55"
            />
          </label>
        )}
        {!collapsed && !query && recentTools.length > 0 && (
          <div className="mb-4 rounded-xl border border-cyan-300/25 bg-cyan-300/10 p-2 shadow-lg shadow-cyan-950/10">
            <p className="mb-1 flex items-center gap-2 px-2 text-xs font-black uppercase text-cyan-100">
            ...
            {recentTools.map((item) => {
              ...
              return (
                <NavLink
                  key={item.route}
                  to={item.route}
                  className="flex min-w-0 items-center gap-2 rounded-xl px-2 py-2 text-sm font-bold text-cyan-50 transition hover:bg-white/10"
                >
            ...
            return (
              <div
                ...
                className={
                  collapsed
                    ? "min-w-0 space-y-1"
                    : "min-w-0 rounded-xl border border-white/10 bg-white/[0.045] p-1 shadow-sm shadow-black/10"
                }
              >
                <button
                  type="button"
                  onClick={() =>
                    collapsed ? toggleCollapsed() : toggleSection(section.title)
                  }
                  ...
                  aria-expanded={open}
                  data-tooltip={
                    collapsed ? `Open menu: ${section.title}` : section.title
                  }
                >
      ...
      <div className={`${indent} min-w-0 space-y-1`}>
        <button
          ...
          className={`tooltip-icon flex w-full min-w-0 items-center gap-3 rounded-lg px-3 py-2 text-sm font-semibold transition ${collapsed ? "justify-center" : ""} ${
            active
              ? "bg-cyan-300/15 text-cyan-50 ring-1 ring-cyan-300/25"
...
export function filterNavItems(
  ...
  return items.reduce<NavItem[]>((matches, item) => {
    ...
    const match = searchTerms.every((term) =>
      navItemSearchText(item, sectionTitle).includes(term),
    );
...
function navItemSearchText(item: NavItem, sectionTitle: string) {
  const siteSearchText =
    siteSearchTextByPath.get(normalizeRoute(item.route)) ?? "";
  return normalizeNavSearchText(
    [basicNavItemSearchText(item, sectionTitle), siteSearchText].join(" "),
  );
}
