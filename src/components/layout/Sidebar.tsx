...
import { ChevronDown, Clock3, Orbit, Search } from "lucide-react";
...
export default function Sidebar() {
  ...
  return (
    <aside className="hidden h-screen w-72 shrink-0 overflow-y-auto border-r border-white/60 bg-white/78 p-5 backdrop-blur-xl dark:border-white/10 dark:bg-slate-950/72 lg:sticky lg:top-0 lg:block">
      ...
      <label className="mb-4 flex min-h-11 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-3 text-sm font-semibold shadow-sm focus-within:border-cyan-400 focus-within:ring-2 focus-within:ring-cyan-300/40 dark:border-white/10 dark:bg-white/5">
      ...
      <nav className="space-y-2" aria-label="Primary navigation">
        ...
        {filteredSections.map((section) => {
          ...
          return (
            <div key={section.title} className="rounded-2xl border border-slate-200/70 bg-white/55 p-1 dark:border-white/10 dark:bg-white/[0.03]">
              <button
                type="button"
                onClick={() => toggleSection(section.title)}
                className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-sm font-black transition ${active ? "text-cyan-700 dark:text-cyan-200" : "text-slate-700 hover:bg-slate-100 dark:text-slate-200 dark:hover:bg-white/10"}`}
                aria-expanded={open}
              >
                  ...
                  {section.items.map(({ title, route, icon, isExternal }) => {
                    ...
                    return isExternal ? (
                      <a
                        ...
                        href={route}
                        title={title}
                        className="flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-semibold text-slate-600 transition hover:translate-x-1 hover:bg-slate-100 dark:text-slate-300 dark:hover:bg-white/10"
                      >
                        <Icon className="h-4 w-4" />
                      ...
                      <NavLink
                        ...
                        className={({ isActive }) =>
                          `flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-semibold transition hover:translate-x-1 ${
                            isActive
