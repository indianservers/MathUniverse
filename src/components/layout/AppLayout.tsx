...
function AppFooter() {
  ...
  if (isAreaTarget || isFixedAngleTarget || isTranslationTarget || isReflectionTarget || isRotationPointTarget) return null;
  if (isRelationTarget) {
    return (
  ...
  if (isPointReflectionTarget || isCircleReflectionTarget || isDilationPointTarget || isMatrixTransformationTarget || isCompositeTransformationTarget) {
    return (
      <footer className={`mx-auto w-full max-w-[1440px] pt-0 ${isCompositeTransformationTarget ? "h-[62px] px-[12px]" : isMatrixTransformationTarget ? "h-[74px] px-[16px]" : isCircleReflectionTarget || isDilationPointTarget ? "h-[96px] pl-[10px] pr-[13px]" : "h-[118px] pl-[24px] pr-[19px]"}`} aria-label="Site footer">
        <div className={`relative rounded-lg border border-slate-200 bg-white/80 px-4 py-3 shadow-sm ${isCompositeTransformationTarget ? "h-[54px]" : isMatrixTransformationTarget ? "h-[66px]" : isCircleReflectionTarget || isDilationPointTarget ? "h-[88px]" : "h-[106px]"}`}>
      ...
      <div className="app-layout-rail flex min-h-screen">
        ...
        <div className="flex min-w-0 flex-1 flex-col">
          ...
          <main
            ref={mainContentRef}
            id="main-content"
              className={`app-fullscreen-target mx-auto w-full max-w-[1440px] flex-1 pb-24 pt-2 md:pb-0 md:pt-4 ${isCompassTarget || isSemicircleTarget || isRigidPolygonTarget ? "px-3" : isGeneralPolygonTarget || isCircleCentreRadiusTarget ? "px-4" : isCircularArcTarget ? "pl-[23px] pr-4" : isCircumcircularArcTarget ? "pl-[6px] pr-3" : isCircularSectorTarget ? "pl-[6px] pr-4" : isConicFiveTarget ? "px-[14px]" : isEllipseTarget ? "pl-[13px] pr-[14px]" : isHyperbolaTarget ? "pl-[9px] pr-4" : isParabolaTarget ? "pl-[23px] pr-[19px]" : isDistanceTarget ? "pl-[17px] pr-4" : isAreaTarget ? "pl-[18px] pr-[11px]" : isAngleTarget ? "pl-[11px] pr-[3px]" : isFixedAngleTarget ? "pl-[12px] pr-[13px]" : isRelationTarget ? "pl-[18px] pr-[17px]" : isStepsTarget ? "pl-[14px] pr-[16px]" : isTranslationTarget ? "pl-[25px] pr-[34px]" : isReflectionTarget ? "px-[19px]" : isPointReflectionTarget ? "pl-[24px] pr-[19px]" : isCircleReflectionTarget ? "pl-[10px] pr-[13px]" : isRotationPointTarget ? "px-[20px]" : isDilationPointTarget ? "pl-[20px] pr-[16px]" : isMatrixTransformationTarget ? "px-[16px]" : isCompositeTransformationTarget ? "px-[12px]" : isCircleThreePointsTarget ? "px-6" : "px-2 sm:px-4 md:px-5"}`}
          >
