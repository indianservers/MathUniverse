...
function AppFooter() {
  ...
  if (isPointReflectionTarget || isCircleReflectionTarget || isDilationPointTarget || isMatrixTransformationTarget) {
    return (
      <footer className={`mx-auto w-full max-w-[1440px] pt-0 ${isMatrixTransformationTarget ? "h-[74px] px-[16px]" : isCircleReflectionTarget || isDilationPointTarget ? "h-[96px] pl-[10px] pr-[13px]" : "h-[118px] pl-[24px] pr-[19px]"}`} aria-label="Site footer">
        <div className={`relative rounded-lg border border-slate-200 bg-white/80 px-4 py-3 shadow-sm ${isMatrixTransformationTarget ? "h-[66px]" : isCircleReflectionTarget || isDilationPointTarget ? "h-[88px]" : "h-[106px]"}`}>
