...
function AppFooter() {
  ...
  if (isAreaTarget || isFixedAngleTarget || isTranslationTarget || isReflectionTarget || isRotationPointTarget || isTransformationMappingTarget) return null;
  if (isRelationTarget) {
    return (
  ...
  if (isPointReflectionTarget || isCircleReflectionTarget || isDilationPointTarget || isMatrixTransformationTarget || isCompositeTransformationTarget || isInvariantsTarget || isSymmetryExplorerTarget) {
    return (
      <footer className={`mx-auto w-full max-w-[1440px] pt-0 ${isSymmetryExplorerTarget ? "h-[81px] px-[10px]" : isInvariantsTarget ? "h-[65px] px-[10px]" : isCompositeTransformationTarget ? "h-[62px] px-[12px]" : isMatrixTransformationTarget ? "h-[74px] px-[16px]" : isCircleReflectionTarget || isDilationPointTarget ? "h-[96px] pl-[10px] pr-[13px]" : "h-[118px] pl-[24px] pr-[19px]"}`} aria-label="Site footer">
        <div className={`relative rounded-lg border border-slate-200 bg-white/80 px-4 shadow-sm ${isSymmetryExplorerTarget ? "h-[79px] py-2" : isInvariantsTarget ? "h-[65px] py-2" : isCompositeTransformationTarget ? "h-[54px] py-3" : isMatrixTransformationTarget ? "h-[66px] py-3" : isCircleReflectionTarget || isDilationPointTarget ? "h-[88px] py-3" : "h-[106px] py-3"}`}>
          ...
          <p className={`${isInvariantsTarget || isSymmetryExplorerTarget ? "mt-1 pt-1" : "mt-3 pt-2"} border-t border-slate-200 text-[6px] font-bold uppercase text-slate-500`}>&copy; {year} Indian Servers Private Limited. No rights to reproduce it.</p>
          <p className={`${isInvariantsTarget || isSymmetryExplorerTarget ? "absolute bottom-2 right-4" : "mt-1"} text-[6px] text-slate-500`}>www.IndianServers.com &nbsp;&nbsp; info@IndianServers.com</p>
        </div>
      </footer>
    );
  }
