    ...
    <AnimatePresence>
      {toast && (
        ...
        >
          ...
          <button
            type="button"
            className="rounded-xl bg-slate-100 px-3 py-1.5 text-xs font-black text-cyan-700 transition hover:bg-cyan-100 dark:bg-white/10 dark:text-cyan-200"
            onClick={() => {
              toast.onUndo();
              dismiss();
            }}
...
export function KeyboardShortcutsPanel() {
  ...
  useEffect(() => {
    ...
    window.addEventListener(
      "math-universe-open-shortcuts",
      openFromCommandCenter,
    );
    return () => {
    ...
    <>
      ...
      <AnimatePresence>
        {open && (
          ...
          >
            <motion.div
              className="mx-auto mt-24 max-w-md overflow-hidden rounded-2xl border border-white/20 bg-white shadow-2xl dark:bg-slate-950"
            ...
            >
              <div className="flex items-center justify-between border-b border-slate-200 px-5 py-4 dark:border-white/10">
                ...
                <button
                  type="button"
                  className="math-tool-button h-8 w-8 rounded-full"
                  onClick={() => setOpen(false)}
                >
                  <X className="h-4 w-4" />
                </button>
              ...
              <div className="divide-y divide-slate-100 dark:divide-white/5">
                {SHORTCUTS.map(({ keys, description }) => (
                  ...
                  >
                    ...
                    <kbd className="rounded-lg bg-slate-100 px-2.5 py-1 font-mono text-xs font-black text-slate-700 dark:bg-white/10 dark:text-slate-200">
                      {keys}
                    </kbd>
                  </div>
                ))}
...
export function BreadcrumbTrail() {
  ...
  const includesCoordinateGeometry =
    ...
    location.pathname === "/lessons/geometry/213-best-fit-line";
  const isBestFitLine =
    location.pathname === "/lessons/geometry/213-best-fit-line";
  ...
  return (
    <nav
      className="mb-2 flex flex-wrap items-center gap-2 text-sm font-bold text-slate-500 dark:text-slate-400"
    ...
    >
      ...
      {segments.map((segment, index) => {
        if ((isReflectionLine || isBestFitLine) && segment === "geometry")
          return null;
        const path = `/${segments.slice(0, index + 1).join("/")}`;
        const matched = navItems.find(
          (item) => !item.isExternal && item.route === path,
        );
        ...
        return (
          <span key={path} className="contents">
            {(includesCoordinateGeometry || isReflectionLine) && last && (
              <span className="flex items-center gap-2">
                <span aria-hidden>&gt;</span>
                <Link to="/lessons/geometry" className="hover:text-cyan-600">
                  Coordinate Geometry
                </Link>
              </span>
            )}
...
export function CommandPalette() {
  ...
  return (
    <>
      <button
        type="button"
        className="tool-button hidden !min-h-10 !whitespace-nowrap !rounded-xl !px-3 !py-2 !text-xs sm:inline-flex"
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            className="fixed inset-0 z-[90] bg-slate-950/55 p-4 backdrop-blur-sm"
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            onClick={() => setOpen(false)}
          >
            <motion.div
              className="mx-auto mt-14 max-w-3xl overflow-hidden rounded-2xl border border-white/20 bg-white shadow-2xl dark:bg-slate-950"
            ...
            >
              <div className="flex items-center gap-3 border-b border-slate-200 px-4 py-3 dark:border-white/10">
                ...
                <input
                  ...
                  placeholder="Search tools, formulas, workspace commands, exports..."
                />
                <kbd className="hidden rounded-lg bg-slate-100 px-2 py-1 font-mono text-[11px] font-black text-slate-500 dark:bg-white/10 dark:text-slate-300 sm:inline">
                  Enter
                </kbd>
                <button
                  type="button"
                  className="math-tool-button h-9 w-9 rounded-full"
                  onClick={() => setOpen(false)}
              ...
              <div className="grid gap-0 md:grid-cols-[1fr_240px]">
                  ...
                  {filtered.map((item, index) => {
                    ...
                    return (
                      <button
                        ...
                        onClick={() => launch(item)}
                        className={`flex w-full items-center gap-3 rounded-xl px-3 py-3 text-left transition ${active ? "bg-cyan-50 ring-1 ring-cyan-200 dark:bg-cyan-400/10 dark:ring-cyan-400/30" : "hover:bg-cyan-50 dark:hover:bg-cyan-400/10"}`}
                      >
                        ...
                        <span className="rounded-full bg-slate-100 px-2 py-1 text-[10px] font-black uppercase text-slate-500 dark:bg-white/10 dark:text-slate-300">
                          {item.group}
                        </span>
                      </button>
                    );
                ...
                <aside className="hidden border-l border-slate-200 bg-slate-50/70 p-4 dark:border-white/10 dark:bg-white/[0.03] md:block">
                  ...
                  <div className="mt-3 space-y-2 text-xs font-semibold leading-5 text-slate-600 dark:text-slate-300">
                    <p>
                      Type a topic, command, object, export, or workspace mode.
                    </p>
...
function buildCommandItems(): CommandItem[] {
  ...
  const actions: CommandItem[] = [
    {
      id: "action:workspace-polynomial",
      kind: "route",
      title: "Open Polynomial Workspace",
      subtitle: "Template with graph, algebra and guided controls",
      group: "Template",
      route: "/workspace?template=polynomials",
      keywords: "polynomial template graph roots workspace",
      icon: Sigma,
    },
    {
      id: "action:workspace-3d",
      kind: "route",
      title: "Open 3D Geometry",
      subtitle: "Surface, solid and vector tools",
      group: "Template",
      route: "/workspace?mode=3d",
      keywords: "3d geometry surface solid vector workspace",
      icon: Layers3,
    },
    {
      id: "action:workspace-geometry",
      kind: "route",
      title: "Open 2D Geometry",
      subtitle: "2D construction tools and object inspector",
      group: "Template",
      route: "/workspace?mode=geometry",
      keywords: "2d geometry constructor point line circle polygon",
      icon: MousePointer2,
    },
    {
      id: "action:formula-circles",
      kind: "route",
      title: "Circle Formulas",
      subtitle: "Jump to formula library for circle and sector formulas",
      group: "Formula",
      route:
        "/formulas?category=geometry&formula=geometry-0-0-circle-area#formula-geometry-0-0-circle-area",
      keywords: "circle formulas area circumference sector",
      icon: Sigma,
    },
    {
      id: "action:export-help",
      kind: "route",
      title: "Export Tools",
      subtitle: "Open workspace export panel",
      group: "Export",
      route: "/workspace?panel=export",
      keywords: "export png svg pdf json share url",
      icon: Download,
    },
    {
      id: "action:teacher",
      ...
      subtitle: "Switch classroom presentation controls",
      group: "Action",
      keywords: "teacher presentation lock reveal classroom",
      icon: Eye,
      run: () =>
        window.dispatchEvent(new Event("math-universe-toggle-teacher-mode")),
    },
    {
      id: "action:shortcuts",
      kind: "action",
      title: "Show Keyboard Shortcuts",
      subtitle: "Open the keyboard reference",
      group: "Help",
      keywords: "keyboard shortcut help",
      icon: Keyboard,
      run: () =>
        window.dispatchEvent(new Event("math-universe-open-shortcuts")),
    },
    {
      id: "action:focus-search",
      kind: "action",
      title: "Focus Page Search",
      subtitle: "Focus the first search field on this page",
      group: "Action",
      keywords: "search focus find page",
      icon: Search,
      run: () => focusFirstSearchInput(),
    },
    {
      id: "action:solve",
      kind: "route",
      title: "Solve Command",
      subtitle: "Open workspace with solve command examples",
      group: "Command",
      route: "/workspace?command=solve%20x%5E2-4%3D0",
      keywords: "solve equation command cas",
      icon: Calculator,
    },
    {
      id: "action:graph",
      ...
      subtitle: "Open workspace with graph command examples",
      group: "Command",
      route: "/workspace?command=graph%20y%3Dx%5E2-4",
      keywords: "graph function plot command",
      icon: Zap,
    },
    {
      id: "action:guided",
      kind: "route",
      title: "Guided Activity Mode",
      subtitle: "Prediction, manipulation, check, reflection",
      group: "Teaching",
      route: "/workspace?mode=guided",
      keywords: "guided activity predict manipulate check reflect",
      icon: Wand2,
    },
  ];
...
function scoreCommandItem(
  ...
  const recentBoost =
    item.kind === "route" && recentRoutes.includes(item.route) ? 25 : 0;
  if (!needle) return 50 + recentBoost;
  const haystack =
    `${item.title} ${item.subtitle} ${item.group} ${item.keywords}`.toLowerCase();
  ...
  return (
    ...
    groupBoost +
    recentBoost -
    Math.min(15, item.title.length / 10)
  );
}
...
export function BackToTopButton() {
  ...
  return (
    <button
      type="button"
      className="tooltip-icon fixed bottom-24 right-4 z-50 inline-flex h-12 w-12 items-center justify-center rounded-full bg-slate-950 text-white shadow-2xl dark:bg-white dark:text-slate-950 lg:bottom-6"
...
export function HeaderStats() {
  ...
  useEffect(() => {
    const read = () => {
      ...
      try {
        setStreak(
          Number(
            JSON.parse(localStorage.getItem(quizStreakKey) ?? "{}").count ?? 0,
          ),
        );
      } catch {
        setStreak(0);
  ...
  return (
    <div className="hidden items-center gap-2 md:flex">
      <span className="inline-flex items-center gap-1 rounded-full bg-orange-100 px-3 py-2 text-xs font-black text-orange-700 dark:bg-orange-400/15 dark:text-orange-100">
        <Flame className="h-4 w-4" />
        {streak}
      </span>
      <span className="inline-flex items-center gap-1 rounded-full bg-cyan-100 px-3 py-2 text-xs font-black text-cyan-700 dark:bg-cyan-400/15 dark:text-cyan-100">
...
export function recentRouteItems(limit = 5) {
  try {
    ...
    return (Array.isArray(routes) ? routes : [])
      ...
      .map((route) =>
        navItems.find((item) => !item.isExternal && item.route === route),
      )
...
export function trackQuizActivity() {
  ...
  const heatmap = rawHeatmap
    ? (JSON.parse(rawHeatmap) as Record<string, number>)
    : {};
  ...
  const streak = rawStreak
    ? (JSON.parse(rawStreak) as { date?: string; count?: number })
    : {};
  const yesterday = new Date(Date.now() - 86400000).toISOString().slice(0, 10);
  const count =
    streak.date === today
      ? (streak.count ?? 1)
      : streak.date === yesterday
        ? (streak.count ?? 0) + 1
        : 1;
...
export function GaugeBadge({ value }: { value: number }) {
  return (
    <span className="inline-flex items-center gap-1 rounded-full bg-slate-100 px-3 py-1 text-xs font-black dark:bg-white/10">
