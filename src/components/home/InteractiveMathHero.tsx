...
function gradientColor(value: number) {
  const scaled =
    MathUtils.clamp((value + 1.05) / 2.1, 0, 1) * (SURFACE_COLORS.length - 1);
  const index = Math.min(Math.floor(scaled), SURFACE_COLORS.length - 2);
  return SURFACE_COLORS[index]
    .clone()
    .lerp(SURFACE_COLORS[index + 1], scaled - index);
}
...
function DepthGrid({ mobile }: { mobile: boolean }) {
  const lines = useMemo(() => {
    ...
    for (let value = -extentX; value <= extentX + 0.01; value += step) {
      result.push({
        points: [
          new Vector3(value, -1.12, -extentZ),
          new Vector3(value, -1.12, extentZ),
        ],
        major: Math.abs(value % 1) < 0.01,
      });
    }
    for (let value = -extentZ; value <= extentZ + 0.01; value += step) {
      result.push({
        points: [
          new Vector3(-extentX, -1.12, value),
          new Vector3(extentX, -1.12, value),
        ],
        major: Math.abs(value % 1) < 0.01,
      });
    ...
    <group>
      {lines.map((line, index) => (
        <Line
          key={index}
          points={line.points}
          color={line.major ? "#38bdf8" : "#6366f1"}
          transparent
          opacity={line.major ? 0.2 : 0.075}
          lineWidth={line.major ? 0.75 : 0.35}
        />
      ))}
    </group>
  );
...
function GraphPoint({
  ...
  useFrame((_, delta) => {
    ...
    mesh.current.scale.setScalar(
      MathUtils.damp(mesh.current.scale.x, target, 10, delta),
    );
  });
  ...
  return (
    <group position={position}>
      <mesh scale={1.75}>
        <sphereGeometry args={[mobile ? 0.12 : 0.09, 18, 18]} />
        <meshBasicMaterial
          color={color}
          transparent
          opacity={0.18}
      ...
      >
        <sphereGeometry args={[mobile ? 0.105 : 0.085, 24, 24]} />
        <meshStandardMaterial
          color={hovered ? "#ffffff" : color}
          emissive={color}
          emissiveIntensity={hovered ? 1.5 : 0.58}
          roughness={0.28}
          metalness={0.08}
        />
      </mesh>
...
function Surface({
  ...
  const geometry = useMemo(
    () => buildSurfaceGeometry(formula, mobile),
    [formula, mobile],
  );
  const trace = useMemo(
    () =>
      Array.from({ length: mobile ? 56 : 96 }, (_, index) => {
        const x = MathUtils.lerp(-3.8, 3.8, index / (mobile ? 55 : 95));
        return new Vector3(x, surfaceFunctions[formula](x, 1.9) + 0.08, 1.9);
      }),
    [formula, mobile],
  );
  const points = useMemo(
    () =>
      [-2.4, 0, 2.25].map(
        (x) =>
          [x, surfaceFunctions[formula](x, 1.9) + 0.08, 1.9] as [
            number,
            number,
            number,
          ],
  ...
  return (
    <group ref={group} rotation={[-0.13, -0.32, 0]} position={[1, 0.02, 0]}>
...
export default function InteractiveMathHero() {
  ...
  return (
    <section className="home-live-hero" aria-labelledby="home-live-title">
      <div
        className="home-live-scene"
        aria-label={`Interactive three-dimensional mathematical surface: ${formulas[formula]}. ${descriptions[formula]} Drag to rotate and scroll to zoom.`}
      >
        <Canvas
          camera={{ position: [6, 4.2, 7], fov: 42 }}
          dpr={mobile ? [1, 1.2] : [1, 1.65]}
        ...
        >
          ...
          <hemisphereLight args={["#dff8ff", "#090b2c", 0.58]} />
          <directionalLight
            position={[4.5, 7, 5]}
            intensity={2.15}
            color="#dff8ff"
          ...
          <pointLight
            position={[-4, 2.5, 2]}
            intensity={mobile ? 12 : 16}
            distance={12}
            color="#6366f1"
          />
          <pointLight
            position={[4, 1.5, -4]}
            intensity={mobile ? 10 : 14}
            distance={11}
            color="#22d3ee"
          />
          <Surface
      ...
      <div className="home-live-copy">
        <div className="home-live-kickers">
          <span>
            <MousePointer2 />
            Interactive preview
          </span>
        ...
        <h1 id="home-live-title">
          See mathematics
          <br />
          come alive.
        </h1>
        <p>
          Explore formulas, proofs, graphs and geometry through interactive
          visual labs.
        </p>
        <div className="home-live-actions">
          ...
          <Link to="/problem-solver" className="secondary">
            Solve a Problem
          </Link>
          <Link to="/visual-formulas" className="secondary">
            View Formulas
          </Link>
        </div>
      </div>
      <div className="home-live-equation">
        <strong>
          <i aria-hidden="true" />
          {formulas[formula]}
        </strong>
        <span>Gradient surface · mint trace · three reference points</span>
        <button
          type="button"
          onClick={() => setFormula((value) => (value + 1) % formulas.length)}
        >
          Change surface
        </button>
      </div>
      <div className="home-live-controls">
        <button
          ...
          aria-label={
            running ? "Pause surface animation" : "Play surface animation"
          }
        >
          {running ? <Pause /> : <Play />}
        </button>
        <Link to="/math-lab/3d-graphing">
          Open 3D Lab
          <ArrowUpRight />
        </Link>
      </div>
