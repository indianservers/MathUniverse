...
export function buildAddedGraphPlots(
  plots: PlotItem[],
  expression: string,
  colors: string[],
  kind?: PlotKind,
  ...
  return [
    createGraphPlot(expression, colors[plots.length % colors.length], kind),
    ...plots,
  ].slice(0, 10);
}
...
export function samplePlotLayer(
  plot: PlotItem,
  ...
  if (plot.kind === "scatter" || plot.kind === "regression") {
    return {
      ...
      color: plot.color,
      kind: plot.kind,
      paths: [graphPath(stripInequality(expression), viewport)].filter(Boolean),
      cells: [],
    };
  }
  try {
    ...
    if (sample.kind === "implicit") {
      return {
        ...
        color: plot.color,
        kind: "implicit",
        paths: sample.segments
          .map((segment) => graphSegmentPath(segment.points, viewport))
          .filter(Boolean),
        cells: sample.cells,
      };
    }
    ...
    return {
      ...
      paths: [],
      cells: [],
      error: error instanceof Error ? error.message : "Graph sampling failed",
    };
  }
}

export function graphSegmentPath(
  points: { x: number; y: number; move?: boolean }[],
  viewport: GraphViewport,
) {
  return points
...
export function graphPath(expression: string, viewport: GraphViewport) {
  ...
  for (let i = 0; i <= 400; i += 1) {
    ...
    points.push(
      `${points.length ? "L" : "M"}${sx.toFixed(2)},${sy.toFixed(2)}`,
    );
  }
  return points.join(" ");
}
...
export function sampleTable(
  ...
  if (kind === "explicit" || kind === "piecewise" || kind === "inequality") {
    return generateValueTable(expression, start, end, step)
      .map((row) => ({ ...row, label }))
  ...
  try {
    ...
    const points =
      "segments" in sample
        ? sample.segments.flatMap((segment) => segment.points)
...
export function generateValueTable(
  expression: string,
  start: number,
  end: number,
  step: number,
  ...
  for (
    let x = start;
    direction > 0 ? x <= end + 1e-9 : x >= end - 1e-9;
    x += safeStep
  ) {
    try {
      rows.push({
        x: roundTo(x, 6),
        y: roundTo(evaluateGraphExpression(stripInequality(expression), x), 6),
      });
    } catch {
...
export function scaleX(x: number, viewport: GraphViewport) {
  return (
    ((x - viewport.xMin) / (viewport.xMax - viewport.xMin || 1)) *
    viewport.width
  );
}
...
export function linearRegression(points: ResultTableRow[]) {
  ...
  return {
    slope,
    intercept,
    expression: `${roundTo(slope, 4)}*x${intercept >= 0 ? "+" : ""}${roundTo(intercept, 4)}`,
  };
}
...
export function regressionModel(
  ...
  if (model === "linear") {
    ...
    return {
      ...
      detail: `Linear least-squares fit y = ${fit.expression}.`,
    };
  }
  if (model === "exponential") {
    ...
    const b = fit.slope;
    return {
      expression: `${roundTo(a, 5)}*exp(${roundTo(b, 5)}*x)`,
      detail: "Exponential regression by fitting ln(y)=ln(a)+b*x.",
    };
  }
  const degree = model === "quadratic" ? 2 : 3;
  const coeffs = polynomialRegression(clean, degree);
  const expression = coeffs
    .map((coeff, index) => {
  ...
  return {
    ...
    detail: `${model === "quadratic" ? "Quadratic" : "Cubic polynomial"} least-squares regression.`,
  };
}
...
function polynomialRegression(points: ResultTableRow[], degree: number) {
  ...
  const matrix = Array.from({ length: size }, (_, row) =>
    Array.from({ length: size }, (_, col) =>
      points.reduce((sum, point) => sum + point.x ** (row + col), 0),
    ),
  );
