...
export default function WorkspaceShell({
  ...
  const selectedObjectIds = useWorkspaceStore(
  ...
  const undoableCount = useMemo(
    () => history.filter((entry) => entry.before && entry.after).length,
    [history],
  );
  const redoableCount = useMemo(
    () => redoHistory.filter((entry) => entry.before && entry.after).length,
    [redoHistory],
  );
  ...
  const geometryObjects = useMemo(
    () =>
      objects.filter(
        (object) =>
          object.dimension === "2d" || object.id.startsWith("geometry:"),
      ).length,
    [objects],
  );
  const spaceObjects = useMemo(
    () =>
      objects.filter(
        (object) =>
          object.dimension === "3d" || object.id.startsWith("space3d:"),
      ).length,
    [objects],
  );
    ...
    <div ref={shellRef} className="workspace-app-shell" onKeyDown={onKeyDown}>
      <div className="workspace-topbar">
        <div className="min-w-0">
          <p className="text-[11px] font-black uppercase tracking-wide text-cyan-500 dark:text-cyan-300">
        ...
        <div className="hidden shrink-0 items-center gap-2 md:flex">
          <button
            ...
            title={
              undoableCount
                ? `Undo latest recoverable workspace action (${undoableCount} available)`
                : "Nothing recoverable to undo"
            }
          ...
          <button
            ...
            title={
              redoableCount
                ? `Redo latest recovered workspace action (${redoableCount} available)`
                : "Nothing to redo"
            }
          ...
          <button
            ...
            className="tool-button min-h-9 rounded-full px-3 py-1.5 text-xs"
            title={
              fullscreen ? "Exit full screen" : "Open workspace full screen"
            }
          >
            {fullscreen ? (
      ...
      <div
        ...
        style={
          {
            "--workspace-inspector": inspectorOpen ? "320px" : "48px",
          } as CSSProperties
        }
      >
        ...
        <aside
          className={`workspace-inspector thin-scrollbar hidden min-h-0 min-w-0 flex-col gap-2 overflow-auto xl:flex ${inspectorOpen ? "pr-1" : "items-center overflow-hidden pr-0"}`}
        ...
        >
          <button
            ...
            className={
              inspectorOpen
                ? "workspace-inspector-toggle workspace-inspector-toggle-open"
                : "workspace-inspector-toggle workspace-inspector-toggle-collapsed"
            }
            title={
              inspectorOpen
                ? "Collapse workspace side panel"
                : "Expand workspace side panel"
            }
            aria-label={
              inspectorOpen
                ? "Collapse workspace side panel"
          ...
          {inspectorOpen ? (
            <>
              <div className="workspace-inspector-tabs">
                <InspectorTab
                  active={inspectorMode === "objects"}
                  icon={ListTree}
                  label="Objects"
                  onClick={() => setInspectorMode("objects")}
                />
                <InspectorTab
                  active={inspectorMode === "inspector"}
                  icon={Info}
                  label="Inspector"
                  onClick={() => setInspectorMode("inspector")}
                />
                <InspectorTab
                  active={inspectorMode === "timeline"}
                  icon={History}
                  label="Timeline"
                  onClick={() => setInspectorMode("timeline")}
                />
              </div>
              <div className="rounded-xl border border-cyan-200 bg-cyan-50/80 p-2 text-xs font-bold text-cyan-900 dark:border-cyan-400/20 dark:bg-cyan-400/10 dark:text-cyan-100">
                <div className="flex items-center gap-2">
                  ...
                  <span className="truncate">
                    {selectedObject
                      ? `${selectedObject.label} selected`
                      : `${objects.length} workspace objects`}
                  </span>
                </div>
              </div>
              {inspectorMode === "objects" && (
                <ObjectList
                  objects={objects}
            ...
            <div className="workspace-inspector-rail">
              <button
                ...
                className="workspace-inspector-rail-button"
                title={`${objects.length} objects`}
              ...
              <button
                ...
                onClick={() => setInspectorOpen(true)}
                className="workspace-inspector-rail-button"
                title={`${history.length} timeline items`}
...
function InspectorTab({
  ...
  return (
    <button
      ...
      className={
        active
          ? "workspace-inspector-tab workspace-inspector-tab-active"
