...
import {
  nextDirectionalFocus,
  type FocusDirection,
} from "../../workspace/directionalFocus";
...
export default function MathWorkspaceLayout({
  ...
  const [viewportMode, setViewportMode] =
    useState<WorkspaceViewportMode>("desktop");
  ...
  useEffect(() => {
    ...
    const onRemoteKey = (event: KeyboardEvent) => {
      ...
      const rectFor = (element: HTMLElement) => {
        ...
        return {
          id: String(elements.indexOf(element)),
          left: rect.left,
          top: rect.top,
          width: rect.width,
          height: rect.height,
      ...
      const next = nextDirectionalFocus(
        rectFor(current),
        elements.map(rectFor),
        event.key as FocusDirection,
      );
      if (!next) return;
      event.preventDefault();
  ...
  const focusPrimary = () => {
    const target = rootRef.current?.querySelector<HTMLElement>(
      "canvas, [role='grid'], [role='application'], textarea, main button, main input",
    );
  ...
  const fitWorkspace = () => {
    ...
    window.dispatchEvent(
      new CustomEvent("math-workspace:fit", {
  ...
  return (
    ...
    >
      ...
      <div
        ...
        aria-label={
          workspace.id === "graphs" ? "Scrollable graph workspace" : undefined
        }
        onKeyDown={(event) => {
          if (workspace.id !== "graphs" || event.target !== event.currentTarget)
            return;
      ...
      <nav
        className="math-workspace-mobile-dock"
        aria-label={`${workspace.name} workspace controls`}
      >
        <Link to="/" aria-label="Home">
          <Home />
          <span>Home</span>
        </Link>
        <button type="button" onClick={focusPrimary}>
          <Focus />
          <span>Main</span>
        </button>
        <button
      ...
      {sheet && (
        <div
          className="math-workspace-sheet-backdrop"
          onMouseDown={(event) =>
            event.target === event.currentTarget && setSheet(null)
          }
        >
          <section
            ref={sheetRef}
            tabIndex={-1}
            className="math-workspace-sheet"
            role="dialog"
            aria-modal="true"
            aria-label={
              sheet === "help"
                ? "Workspace keyboard help"
            ...
            <header>
              <div>
                <strong>
                  {sheet === "help"
                    ? "Keyboard help"
                    : `${workspace.name} controls`}
                </strong>
                <span>
                  {sheet === "help"
                    ? "Keyboard and remote shortcuts"
                    : "View and presentation options"}
                </span>
              </div>
              <button
                type="button"
            ...
            {sheet === "help" ? (
              <dl className="math-workspace-shortcuts">
                <div>
                  ...
                  <dd>Move between workspace controls</dd>
                </div>
                <div>
                  <dt>
                    <kbd>Enter</kbd>
                ...
                <div>
                  ...
                  <dd>Close the current panel or sheet</dd>
                </div>
                <div>
                  <dt>
                    <kbd>Ctrl</kbd> + <kbd>F</kbd>
                  </dt>
                  <dd>Toggle workspace fullscreen</dd>
                </div>
                <div>
                  <dt>
                    <kbd>?</kbd>
              ...
              <div className="math-workspace-sheet-actions">
                <button type="button" onClick={fitWorkspace}>
                  <RotateCcw />
                  <span>
                    <strong>Fit workspace</strong>
                    <small>Recalculate the available canvas size</small>
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>
                    <strong>
                      {focusMode ? "Show panels" : "Focus on main view"}
                    </strong>
                    <small>Keep the mathematical activity unobstructed</small>
                  </span>
                </button>
                <button
                  type="button"
                ...
                >
                  ...
                  <span>
                    ...
                    <small>
                      Increase labels and simplify surrounding controls
                    </small>
                  </span>
                </button>
                <button
                ...
                >
                  ...
                  <span>
                    ...
                    <small>Use the complete display for this workspace</small>
                  </span>
                </button>
                <button type="button" onClick={() => setSheet("help")}>
                  <Keyboard />
                  <span>
                    <strong>Keyboard and remote help</strong>
                    <small>Review focus and navigation shortcuts</small>
                  </span>
                </button>
                <Link to="/?section=math-workspaces">
                  <Grid3X3 />
                  <span>
                    <strong>All Math Workspaces</strong>
                    <small>Switch to another connected studio</small>
                  </span>
                </Link>
              </div>
            )}
