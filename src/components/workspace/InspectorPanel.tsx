...
export default function InspectorPanel({
    ...
    return (
      <section className="rounded-xl border border-slate-200 bg-white/70 p-3 dark:border-white/10 dark:bg-white/5">
        ...
        <p className="mt-2 text-xs font-semibold leading-5 text-slate-500 dark:text-slate-400">
          Select a workspace object to inspect its value, linked views,
  ...
  const applyPatch = (
    patch: Partial<MathObject>,
    label = "Edited workspace object",
  ) => {
    onObjectChange?.(object, patch);
    updateObject(object.id, patch, label);
  };
  const updateTransform = (
    section: keyof MathTransform,
    axis: keyof MathVec3,
    value: number,
  ) => {
    ...
    applyPatch(
      {
        transform: {
          ...object.transform,
          [section]: {
            ...(object.transform[section] as MathVec3),
            [axis]: finiteOrZero(value),
          },
        },
      },
  ...
  const updateStyle = (patch: Partial<MathObjectStyle>) => {
    applyPatch(
      ...
      `Edited ${object.label} style`,
    );
  };
  const updateProperties = (patch: NonNullable<MathObject["properties"]>) => {
    applyPatch(
      { properties: { ...object.properties, ...patch } },
      `Edited ${object.label} properties`,
    );
  };
  ...
  const commitDynamicOpacity = (dynamicOpacity: string) => {
    ...
    updateProperties({
      dynamicStyle: {
        ...object.properties?.dynamicStyle,
  ...
  const updateInteractivity = (patch: Partial<MathObjectInteractivity>) => {
    ...
    applyPatch(
      { interactivity: { ...object.interactivity, ...patch } },
      `Edited ${object.label} interaction`,
    );
  };
  ...
  return (
    <section
      className="rounded-xl border border-slate-200 bg-white/70 p-3 dark:border-white/10 dark:bg-white/5"
      data-testid="workspace-inspector"
    >
      <div className="flex min-w-0 items-start gap-2">
        <div className="min-w-0 flex-1">
          <p className="text-[11px] font-black uppercase tracking-wide text-cyan-600 dark:text-cyan-300">
            {object.kind}
          </p>
          <h2 className="mt-1 break-words text-sm font-black text-slate-950 dark:text-white">
            {object.label}
          </h2>
        </div>
        <span
          className="rounded-full bg-slate-100 p-2 text-slate-600 dark:bg-white/10 dark:text-slate-200"
          title={object.locked ? "Locked" : "Editable"}
        >
          {object.locked ? (
      ...
      <div
        className="mt-3 rounded-xl bg-slate-100 p-3 dark:bg-slate-950/70"
      ...
      <div className="mt-3 grid gap-2">
        <label className="text-xs font-bold text-slate-600 dark:text-slate-300">
          ...
          <input
            ...
            onChange={(event) =>
              applyPatch(
                { label: event.target.value || object.label },
        ...
        <div className="grid grid-cols-2 gap-2">
          <ToggleEdit
            ...
            onChange={(checked) =>
              applyPatch(
                { visible: checked, status: checked ? "ready" : "hidden" },
                checked ? `Show ${object.label}` : `Hide ${object.label}`,
              )
            }
          />
          <ToggleEdit
            ...
            onChange={(checked) =>
              applyPatch(
                { locked: checked },
                checked ? `Locked ${object.label}` : `Unlocked ${object.label}`,
              )
            }
          />
        </div>
      ...
      <div className="mt-3 rounded-xl bg-slate-100 p-3 dark:bg-slate-950/70">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          <NumberEdit
            label="Layer"
            value={object.properties?.layer ?? 0}
            min={0}
            max={99}
            step={1}
            disabled={Boolean(object.locked)}
            onChange={(value) => updateProperties({ layer: value })}
          />
          <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400">
            ...
            <select
              ...
              onChange={(event) =>
                updateProperties({
                  labelMode: event.target.value as NonNullable<
        ...
        <label className="mt-2 block text-[10px] font-black uppercase text-slate-500 dark:text-slate-400">
          ...
          <input
            ...
            disabled={object.locked}
            onInput={(event) =>
              commitConditionalVisibility(event.currentTarget.value)
            }
            onBlur={(event) =>
              commitConditionalVisibility(event.currentTarget.value)
            }
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          <label className="text-[10px] font-black uppercase text-slate-500 dark:text-slate-400">
            ...
            <input
              ...
              disabled={object.locked}
              onInput={(event) =>
                commitDynamicOpacity(event.currentTarget.value)
              }
              onBlur={(event) =>
                commitDynamicOpacity(event.currentTarget.value)
              }
      ...
      <div className="mt-3 rounded-xl bg-slate-100 p-3 dark:bg-slate-950/70">
        ...
        <div className="mt-2 grid grid-cols-2 gap-2">
          ...
          <NumberEdit
            label="Stroke"
            value={object.style?.strokeWidth ?? 2}
            min={0.5}
            max={12}
            step={0.5}
            disabled={Boolean(object.locked)}
          ...
          <ToggleEdit
            label="Labels"
            checked={object.style?.labelVisible ?? true}
            disabled={Boolean(object.locked)}
            onChange={(checked) => updateStyle({ labelVisible: checked })}
          />
        </div>
        {object.dimension === "3d" && (
          <label className="mt-2 block text-[10px] font-black uppercase text-slate-500 dark:text-slate-400">
            ...
            <select
              ...
              onChange={(event) =>
                updateStyle({
                  material: event.target.value as MathObjectStyle["material"],
                })
              }
            >
        ...
        <div className="mt-3">
          ...
          <div className="mt-2 grid gap-1">
            {object.dependencies.slice(0, 8).map((dependency) => (
              <div
                key={`${dependency.id}-${dependency.role}`}
                className="rounded-lg bg-slate-100 px-2 py-1 dark:bg-white/10"
              >
      ...
      {object.interactivity && (
        <div className="mt-3">
          ...
          <div className="mt-2 flex flex-wrap gap-1.5">
            <ToggleEdit
              label="Selectable"
              checked={object.interactivity.selectable}
              disabled={Boolean(object.locked)}
              onChange={(checked) =>
                updateInteractivity({ selectable: checked })
              }
              compact
            />
            <ToggleEdit
              label="Draggable"
              checked={object.interactivity.draggable}
              disabled={Boolean(object.locked)}
              onChange={(checked) =>
                updateInteractivity({ draggable: checked })
              }
              compact
            />
            <ToggleEdit
              label="Editable"
              checked={object.interactivity.editable}
              disabled={Boolean(object.locked)}
              onChange={(checked) => updateInteractivity({ editable: checked })}
              compact
            />
            <ToggleEdit
              label="Resizable"
              checked={object.interactivity.resizable}
              disabled={Boolean(object.locked)}
              onChange={(checked) =>
                updateInteractivity({ resizable: checked })
              }
              compact
            />
            <ToggleEdit
              label="Rotatable"
              checked={object.interactivity.rotatable}
              disabled={Boolean(object.locked)}
              onChange={(checked) =>
                updateInteractivity({ rotatable: checked })
              }
              compact
            />
          </div>
        ...
        <div className="mt-3">
          ...
          <dl className="mt-2 space-y-1">
            {Object.entries(object.metadata).map(([key, value]) => (
              <div
                key={key}
                className="flex min-w-0 justify-between gap-2 rounded-lg bg-slate-100 px-2 py-1 dark:bg-white/10"
              >
                <dt className="truncate text-xs font-semibold text-slate-500 dark:text-slate-400">
                  {key}
                </dt>
                <dd className="truncate text-xs font-bold text-slate-800 dark:text-slate-100">
                  {String(value)}
                </dd>
              </div>
            ))}
    ...
    <label
      className={
        compact
          ...
          : "flex items-center gap-2 rounded-lg bg-slate-100 px-2 py-2 text-xs font-bold dark:bg-white/10"
      }
    >
      <input
        type="checkbox"
