...
function normalizeProgress(
  ...
  return {
    ...
    quizBestScores: {
      ...initialProgress.quizBestScores,
      ...(value?.quizBestScores ?? {}),
    },
    mastery: { ...initialProgress.mastery, ...(value?.mastery ?? {}) },
  };
}

export function updateMasteryEvidence(
  previous: MasteryEvidence | undefined,
  score: number,
  assessedAt = Date.now(),
): MasteryEvidence {
  ...
  const status =
    attempts >= 3 && average >= 85 && ageDays >= 7
      ? "durable"
      : attempts >= 2 && average >= 80
        ? "proficient"
        : "developing";
  const reviewDays =
    status === "durable"
  ...
  return {
    ...
    nextReviewAt: assessedAt + reviewDays * 86_400_000,
    status,
    reason,
  };
}
...
export function useProgress() {
  const [progress, setProgress] = useLocalStorage<ProgressState>(
    "math-universe-progress",
    initialProgress,
  );
  const safeProgress: ProgressState = useMemo(
    ...
    (topicId: string) => {
      setProgress((current) => {
        ...
        return {
          ...
          pageProgress: {
            ...base.pageProgress,
            [topicId]: Math.max(base.pageProgress[topicId] ?? 0, 25),
          },
        };
      });
    },
  ...
  const markTopicInteracted = useCallback(
    (topicId: string) => {
      setProgress((current) => {
        const base = normalizeProgress(current);
        return {
          ...
          pageProgress: {
            ...base.pageProgress,
            [topicId]: Math.max(base.pageProgress[topicId] ?? 0, 75),
          },
        };
      });
    },
  ...
  const setTopicProgress = useCallback(
    (topicId: string, percentage: number) => {
      setProgress((current) => {
        const base = normalizeProgress(current);
        return {
          ...base,
    ...
    (quizId: string, score: number) => {
      setProgress((current) => {
        ...
        return {
          ...
          quizBestScores: {
            ...base.quizBestScores,
            [quizId]: Math.max(base.quizBestScores[quizId] ?? 0, score),
          },
          mastery: {
            ...base.mastery,
            [quizId]: updateMasteryEvidence(base.mastery[quizId], score),
          },
        };
      });
    },
  ...
  const getTopicProgress = useCallback(
    (topicId: string) => {
      ...
      return (
        safeProgress.pageProgress[topicId] ??
        (safeProgress.visited[topicId] ? 25 : 0)
      );
    },
  ...
  const getTopicMastery = useCallback(
    (topicId: string) => safeProgress.mastery[topicId] ?? null,
    [safeProgress.mastery],
  );
