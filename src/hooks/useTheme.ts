...
export function useTheme() {
  ...
  const [fontScale, setFontScale] = useLocalStorage<
    "base" | "large" | "xlarge"
  >("math-universe-font-scale", "base");
  const [reducedMotion, setReducedMotion] = useLocalStorage(
    "math-universe-reduced-motion",
    false,
  );
  const [colorBlindPalette, setColorBlindPalette] = useLocalStorage(
    "math-universe-color-blind-palette",
    false,
  );
  ...
  useEffect(() => {
    ...
    root.style.fontSize =
      fontScale === "xlarge" ? "18px" : fontScale === "large" ? "17px" : "";
