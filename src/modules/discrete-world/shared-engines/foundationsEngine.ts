...
export function arithmeticSumInduction(n: number) {
  ...
  return {
    ...
    inductiveStep:
      "Assume S(k) = k(k + 1)/2. Then S(k + 1) = k(k + 1)/2 + (k + 1) = (k + 1)(k + 2)/2.",
...
export function pigeonhole(items: number, boxes: number) {
  ...
  return {
    minimumMaxLoad,
    distribution: Array.from({ length: safeBoxes }, (_, index) =>
      Math.floor((safeItems + safeBoxes - index - 1) / safeBoxes),
    ),
...
export function linearRecurrence(
  ...
  for (let index = 2; index < count; index += 1) {
    ...
    sequence.push(next);
    steps.push(
      `a${index} = ${coeffA}a${index - 1} + ${coeffB}a${index - 2} = ${next}`,
    );
  }
  return {
    sequence,
...
export function modularTable(modulus: number) {
  ...
  return Array.from({ length: mod }, (_, a) =>
    Array.from({ length: mod }, (_, b) => (a * b) % mod),
  );
}
...
export function sieve(limit: number) {
  ...
  for (let value = 2; value * value <= safeLimit; value += 1) {
    ...
    for (let multiple = value * value; multiple <= safeLimit; multiple += value)
      prime[multiple] = false;
  }
  return prime
    .map((isPrime, value) => (isPrime ? value : -1))
