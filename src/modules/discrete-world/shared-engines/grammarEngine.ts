...
export function parseGrammar(input: string, start = "S"): Grammar {
  const rules = input
    ...
    .map((line) => {
      ...
      const alternatives = right
        .split("|")
        .map((alt) => tokenizeProduction(alt.trim()));
  ...
  const terminals = Array.from(
    new Set(
      rules
        .flatMap((rule) => rule.alternatives.flat())
        .filter((symbol) => !variables.includes(symbol) && symbol !== "eps"),
    ),
  );
...
export function serializeGrammar(grammar: Grammar) {
  return grammar.rules
    .map(
      (rule) =>
        `${rule.from} -> ${rule.alternatives.map((alt) => alt.join(" ") || "eps").join(" | ")}`,
    )
    .join("\n");
}
...
export function deriveString(grammar: Grammar, target: string, maxSteps = 10) {
  ...
  const queue: DerivationStep[][] = [
    [{ sententialForm: [grammar.start], appliedRule: "start" }],
  ];
  ...
  while (queue.length) {
    ...
    if (
      ...
      terminalPrefixTooLong(current, targetSymbols, grammar.variables)
    )
      continue;

    const variableIndex = current.findIndex((symbol) =>
    ...
    for (const alt of rule.alternatives) {
      ...
      const next = [
        ...current.slice(0, variableIndex),
        ...replacement,
        ...current.slice(variableIndex + 1),
      ];
...
export function grammarClassification(grammar: Grammar) {
  const regular = grammar.rules.every((rule) =>
    rule.alternatives.every(
      (alt) =>
        alt.length <= 2 &&
        (alt.length < 2 || grammar.variables.includes(alt[1])),
    ),
  );
  const contextFree = grammar.rules.every(
    (rule) => rule.from.length === 1 && grammar.variables.includes(rule.from),
  );
  return {
    regular,
    contextFree,
    type: regular
      ...
      : contextFree
        ...
        : "Unrestricted grammar preview",
  };
}

export function boundedDerivations(
  grammar: Grammar,
  target: string,
  maxSteps = 8,
) {
  ...
  while (queue.length && results.length < 12) {
    ...
    if (
      path.length > maxSteps ||
      terminalPrefixTooLong(current, target.split(""), grammar.variables)
    )
      continue;
    const variableIndex = current.findIndex((symbol) =>
      grammar.variables.includes(symbol),
    ...
    const rule = grammar.rules.find(
      (item) => item.from === current[variableIndex],
    );
    if (!rule) continue;
...
export function parseTreeLevels(steps: DerivationStep[]) {
  return steps.map((step, index) => ({
    level: index,
    nodes: step.sententialForm.map((symbol, symbolIndex) => ({
      id: `${index}-${symbolIndex}-${symbol}`,
      label: symbol,
    })),
...
export function cnfPreview(grammar: Grammar) {
  ...
  const alreadyBinary = grammar.rules.every((rule) =>
    rule.alternatives.every((alt) => alt.length <= 2),
  );
...
function terminalPrefixTooLong(
  form: string[],
  target: string[],
  variables: string[],
) {
