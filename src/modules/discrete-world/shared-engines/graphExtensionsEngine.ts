...
export function eulerPath(project: GraphProject) {
  ...
  const odd = Array.from(degree.entries())
  ...
  for (let pass = 1; pass < project.nodes.length; pass += 1) {
    ...
    project.edges.forEach((edge) => {
      ...
      if (candidate < dist[edge.target]) {
        ...
        changed = true;
        steps.push(
          `Pass ${pass}: ${edge.target} becomes ${candidate} through ${edge.source}.`,
        );
      }
    });
    if (!changed) break;
  }
  const negativeCycle = project.edges.some(
    (edge) =>
      Number.isFinite(dist[edge.source]) &&
...
export function floydWarshall(project: GraphProject) {
  ...
  const dist = ids.map((row) =>
    ids.map((column) => (row === column ? 0 : Number.POSITIVE_INFINITY)),
  );
  const index = new Map(ids.map((id, i) => [id, i]));
  project.edges.forEach((edge) => {
    dist[index.get(edge.source)!][index.get(edge.target)!] = Math.min(
      dist[index.get(edge.source)!][index.get(edge.target)!],
      edge.weight,
    );
    if (!project.directed && !edge.directed)
      dist[index.get(edge.target)!][index.get(edge.source)!] = Math.min(
        dist[index.get(edge.target)!][index.get(edge.source)!],
        edge.weight,
      );
  });
  ids.forEach((_, k) =>
...
export function stronglyConnectedComponents(project: GraphProject) {
  ...
  const outgoing = new Map(
    project.nodes.map((node) => [node.id, [] as string[]]),
  );
  project.edges.forEach((edge) => {
