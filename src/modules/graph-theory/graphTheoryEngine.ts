...
export function directedDegreeMap(project: GraphProject) {
  const map = new Map(
    project.nodes.map((node) => [node.id, { in: 0, out: 0, total: 0 }]),
  );
  project.edges.forEach((edge) => {
...
export function graphMetrics(project: GraphProject) {
  ...
  const maxEdges = project.directed
    ? n * Math.max(0, n - 1)
    : (n * Math.max(0, n - 1)) / 2;
  ...
  return {
    ...
    regular:
      degrees.length > 0 && degrees.every((value) => value === degrees[0]),
...
export function adjacencyMatrix(project: GraphProject) {
  ...
  project.edges.forEach((edge) => {
    ...
    if (!project.directed && !edge.directed)
      matrix[column][row] += edge.weight || 1;
  });
  return { ids, matrix };
}
...
export function adjacencyList(project: GraphProject) {
  ...
  return project.nodes.map((node) => ({
    id: node.id,
    neighbors: (adj.get(node.id) ?? []).map(
      ({ to, edge }) => `${to}${edge.weight !== 1 ? `(${edge.weight})` : ""}`,
    ),
  }));
}
...
export function complementGraph(project: GraphProject): GraphProject {
  const existing = new Set(
    project.edges.map((edge) =>
      project.directed || edge.directed
        ? directedEdgeKey(edge.source, edge.target)
  ...
  project.nodes.forEach((source) => {
    project.nodes.forEach((target) => {
      ...
      const key = project.directed
        ? directedEdgeKey(source.id, target.id)
...
export function isBipartite(project: GraphProject) {
  ...
  return {
    ...
    left: project.nodes
      .filter((node) => color.get(node.id) === 0)
      .map((node) => node.id),
    right: project.nodes
      .filter((node) => color.get(node.id) === 1)
      .map((node) => node.id),
...
export function bridgesAndCutVertices(project: GraphProject) {
  ...
  const bridges = project.edges
    ...
    .map((edge) => edge.id);
  const cutVertices = project.nodes
    .filter((node) => {
      const next = {
        ...project,
        directed: false,
      ...
      return (
        next.nodes.length > 0 && connectedComponents(next).length > baseline
      );
    })
    .map((node) => node.id);
...
export function cliqueAndIndependentSets(project: GraphProject) {
  ...
  const clique =
    subsets
      .filter((subset) =>
        everyPair(subset, (a, b) => edgeSet.has(edgeKey(a, b))),
      )
...
export function floydWarshall(project: GraphProject) {
  const ids = project.nodes.map((node) => node.id);
  const dist = ids.map((from) =>
    ids.map((to) => (from === to ? 0 : Number.POSITIVE_INFINITY)),
  );
  const index = new Map(ids.map((id, row) => [id, row]));
  project.edges.forEach((edge) => {
    ...
    if (!project.directed && !edge.directed)
      dist[target][source] = Math.min(dist[target][source], edge.weight);
  });
...
export function graphDistanceMetrics(project: GraphProject) {
  ...
  const eccentricity = Object.fromEntries(
    table.ids.map((id, rowIndex) => {
      const reachable = table.dist[rowIndex].filter((value) =>
        Number.isFinite(value),
      );
      return [
        id,
        reachable.length ? Math.max(...reachable) : Number.POSITIVE_INFINITY,
      ];
    }),
  ) as Record<string, number>;
  const finite = Object.values(eccentricity).filter((value) =>
    Number.isFinite(value),
  );
  const radius = finite.length ? Math.min(...finite) : Number.POSITIVE_INFINITY;
  const diameter = finite.length
    ? Math.max(...finite)
    : Number.POSITIVE_INFINITY;
  return {
    radius,
    diameter,
    center: Object.entries(eccentricity)
  ...
  const local = Object.fromEntries(
    project.nodes.map((node) => {
      const neighbors = Array.from(
        new Set((adj.get(node.id) ?? []).map(({ to }) => to)),
      );
      const possible =
        (neighbors.length * Math.max(0, neighbors.length - 1)) / 2;
      let links = 0;
    ...
    while (queue.length) {
      ...
      for (const { to } of adj.get(node) ?? []) {
        if (!dist.has(to)) {
          ...
          best = Math.min(
            best,
            (dist.get(node) ?? 0) + (dist.get(to) ?? 0) + 1,
          );
        }
      }
    }
...
export function maxFlowMinCut(
  project: GraphProject,
  source = project.nodes[0]?.id ?? "",
  sink = project.nodes.at(-1)?.id ?? "",
) {
  const ids = project.nodes.map((node) => node.id);
  const capacity = new Map<string, Map<string, number>>();
  ids.forEach((id) =>
    capacity.set(id, new Map(ids.map((other) => [other, 0]))),
  );
  project.edges.forEach((edge) => {
    capacity
      .get(edge.source)
      ?.set(
        edge.target,
        (capacity.get(edge.source)?.get(edge.target) ?? 0) +
    ...
    if (!project.directed && !edge.directed) {
      capacity
        ...
        ?.set(
          edge.source,
          (capacity.get(edge.target)?.get(edge.source) ?? 0) +
            Math.max(0, edge.weight),
        );
    }
  });
  ...
  const bfsPath = () => {
    ...
    const bottleneck = Math.min(
      ...path.slice(1).map((to, index) => residual(path[index], to)),
    );
  ...
  const cutEdges = project.edges
    ...
    .map((edge) => edge.id);
  const value = Array.from(flow.get(source)?.values() ?? []).reduce(
...
export function planarityObstructionHint(project: GraphProject) {
  ...
  const isComplete =
    ...
    everyPair(
      ...
      (a, b) => edgeSet.has(edgeKey(a, b)),
    );
  if (isComplete && n === 5)
    return {
      obstruction: "K5",
      note: "K5 is a classic non-planar obstruction.",
    };
  const bipartite = isBipartite(project);
  if (bipartite.left.length === 3 && bipartite.right.length === 3) {
    const completeBipartite = bipartite.left.every((left) =>
      bipartite.right.every((right) => edgeSet.has(edgeKey(left, right))),
    );
    if (completeBipartite)
      return {
        obstruction: "K3,3",
        note: "K3,3 is the utility graph and is non-planar.",
      };
  }
  return {
    obstruction: "none detected",
    note: "No direct K5 or K3,3 pattern was detected in this classroom-scale graph.",
  };
}
...
export function bfs(
  ...
  while (queue.length) {
    ...
    steps.push({
      ...
      activeNodes: [node],
      activeEdges: [],
      note: `Visit ${node}; queue = [${queue.join(", ")}]`,
    });
...
export function dijkstra(
  ...
  const dist = Object.fromEntries(
    project.nodes.map((node) => [node.id, Number.POSITIVE_INFINITY]),
  ) as Record<string, number>;
  ...
  while (unvisited.size) {
    ...
    steps.push({
      ...
      activeNodes: [node],
      activeEdges: [],
      note: `Settle ${node} with distance ${dist[node]}.`,
    });
    for (const { to, edge } of adj.get(node) ?? []) {
  ...
  [...project.edges]
    ...
    .forEach((edge) => {
      if (find(edge.source) !== find(edge.target)) {
        ...
        tree.push(edge.id);
        steps.push({
          label: "Kruskal choose",
          activeNodes: [edge.source, edge.target],
          activeEdges: [edge.id],
          note: `Add ${edge.id} with weight ${edge.weight}.`,
        });
      } else {
        steps.push({
          label: "Kruskal skip",
          activeNodes: [edge.source, edge.target],
          activeEdges: [edge.id],
          note: `Skip ${edge.id}; it closes a cycle.`,
        });
      }
    });
  return { tree, steps };
...
export function prim(
  ...
  while (seen.size < project.nodes.length) {
    ...
    steps.push({
      ...
      activeNodes: [edge.source, edge.target],
      activeEdges: [edge.id],
      note: `Add lightest boundary edge ${edge.id}.`,
    });
  }
  return { tree, steps };
}
...
export function topologicalSort(project: GraphProject) {
  const indegree = Object.fromEntries(
    project.nodes.map((node) => [node.id, 0]),
  ) as Record<string, number>;
  project.edges.forEach((edge) => {
...
export function eulerCircuit(project: GraphProject) {
  ...
  return (
    ...
    components.length <= 1 &&
    Array.from(degree.values()).every((value) => value === 0 || value % 2 === 0)
  );
}

export function eulerCircuitPath(project: GraphProject) {
  if (!eulerCircuit(project))
    return {
      exists: false,
      path: [] as string[],
      steps: [] as AlgorithmStep[],
    };
  const unused = new Map<string, GraphEdge[]>(
    project.nodes.map((node) => [node.id, []]),
  );
  project.edges.forEach((edge) => {
  ...
  const stack = [project.edges[0]?.source ?? project.nodes[0]?.id ?? ""].filter(
    Boolean,
  );
  const path: string[] = [];
  const steps: AlgorithmStep[] = [];
  while (stack.length) {
    ...
    const edge = (unused.get(node) ?? []).find(
...
export function hamiltonianCycle(project: GraphProject) {
  ...
  const search = (node: string): boolean => {
    if (path.length === project.nodes.length)
      return Boolean(adj.get(node)?.some((next) => next.to === start));
    for (const { to } of adj.get(node) ?? []) {
...
export function graphColorConflicts(
  project: GraphProject,
  ...
  return project.edges
    .filter(
      (edge) =>
        assignment[edge.source] !== undefined &&
...
export function treeModel(kind: TreeKind) {
  ...
  return {
    nodes: [
      { id: "8", x: 260, y: 40 },
      { id: "4", x: avl ? 165 : 160, y: 120 },
      { id: "12", x: avl ? 355 : 360, y: 120 },
      { id: "2", x: 100, y: 205 },
      { id: "6", x: 220, y: 205 },
      { id: "10", x: 320, y: 205 },
      { id: "14", x: 430, y: 205 },
  ...
  for (let i = 0; i < project.edges.length; i += 1) {
    for (let j = i + 1; j < project.edges.length; j += 1) {
      ...
      if ([a.source, a.target].some((id) => id === b.source || id === b.target))
        continue;
      const p1 = nodeById.get(a.source),
        p2 = nodeById.get(a.target),
        p3 = nodeById.get(b.source),
        p4 = nodeById.get(b.target);
...
function segmentsIntersect(
  ...
  const ccw = (p: GraphNode, q: GraphNode, r: GraphNode) =>
...
function undirectedEdgeSet(project: GraphProject) {
  return new Set(
    project.edges.map((edge) => edgeKey(edge.source, edge.target)),
  );
}
      ...
      for (let color = 0; color < colors; color += 1) {
        ...
        if (
          edges.every(
            (edge) =>
              assignment[edge.source] === undefined ||
              assignment[edge.target] === undefined ||
              assignment[edge.source] !== assignment[edge.target],
          ) &&
          assign(index + 1)
        )
