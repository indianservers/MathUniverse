...
export function createBoardSessionMemory(
  boardId: string,
): SmartBoardSessionMemory {
  return {
    boardId,
...
export function createBoardIntelligencePersistence(
  boardId: string,
): BoardIntelligencePersistence {
  return {
...
export function buildBoardIntelligenceContext(input: {
  ...
  const maxTokens = clamp(
    input.maxEstimatedTokens ?? DEFAULT_MAX_TOKENS,
    128,
    12_000,
  );
  const elementMap = new Map(
    input.document.elements.map((element) => [element.id, element]),
  );
  ...
  if (input.activeProblemId && activeProblem?.id === input.activeProblemId) {
    seedIds.push(
      ...activeProblem.problemElementIds.filter((id) => elementMap.has(id)),
    );
  }
  ...
  for (const relationship of input.document.relationships) {
    if (
      includedIds.has(relationship.sourceElementId) &&
      elementMap.has(relationship.targetElementId)
    ) {
      includedIds.add(relationship.targetElementId);
    } else if (
      includedIds.has(relationship.targetElementId) &&
      elementMap.has(relationship.sourceElementId)
    ) {
  ...
  for (const [order, element] of ordered.entries()) {
    ...
    if (
      contextElements.length >= maxElements ||
      estimatedTokens + elementTokens > maxTokens
    ) {
  ...
  const relationships = input.document.relationships
    .filter(
      (relationship) =>
        finalIds.has(relationship.sourceElementId) &&
        finalIds.has(relationship.targetElementId),
    )
    .map(({ id, type, sourceElementId, targetElementId }) => ({
      id,
      type,
      sourceElementId,
      targetElementId,
  ...
  const ambiguities = extractContextAmbiguities(
    contextElements,
    input.document.intelligence.sessionMemory,
  );
  ...
  return {
    ...
    relationships,
    recentActions: input.document.actionHistory
      .slice(-10)
      .map(
        ({ id, actionType, sourceElementId, resultElementId, cancelled }) => ({
          id,
          actionType,
          sourceElementId,
          resultElementId,
...
export function understandBoardContext(
  ...
  const mathElement = context.elements.find(
    (element) => element.type === "math-expression",
  );
  const injectionDetected = context.elements.some((element) =>
    detectPromptInjection(element.content),
  );
  ...
  if (subject === "mathematics" && mathElement) {
    try {
      ...
      knownFacts.push(
        {
          label: "Expression",
          value: mathElement.content,
          sourceElementId: mathElement.id,
        },
        {
          label: "Structure",
      ...
      if (
        ...
        unknownFacts.push({
          ...
          reason: "No variable could be identified.",
        });
      }
      recommendations = mathematicsRecommendations(
        context,
        memory,
        analysis,
        mathElement.id,
      ...
      warnings.push(
        error instanceof Error
          ? error.message
          : "The selected expression could not be analyzed.",
      );
      unknownFacts.push({
        label: "Expression structure",
        reason: "Confirm the expression before running an engine.",
      });
    }
  } else if (subject && subject !== "unknown") {
    const subjectResult = unsupportedSubjectUnderstanding(
      subject,
      context,
      goal,
    );
    ...
    unknownFacts.push({
      ...
      reason: "Select a confirmed expression, text, or image region.",
    });
  }

  if (injectionDetected) {
    warnings.unshift(
      "Untrusted content contains instruction-like text. Tool permissions remain unchanged.",
    );
  }
  const activeProblem =
    context.elements.length && subject && subject !== "unknown"
      ? {
          id:
            memory.activeProblemId ??
            `problem-${context.boardId}-${context.elements[0].id}`,
          ...
          knownFacts,
          unknownFacts,
          attemptedStepIds: context.recentActions.map(
            (action) => action.resultElementId,
          ),
          verifiedStepIds: [],
          currentStage: context.pendingAmbiguities.some(
            (ambiguity) => ambiguity.requiresResolution,
          )
            ? ("blocked" as const)
            : ("understanding" as const),
          assumptions: [],
          warnings,
          completionStatus: context.pendingAmbiguities.some(
            (ambiguity) => ambiguity.requiresResolution,
          )
            ? ("blocked" as const)
...
export function inferBoardGoal(
  context: SmartBoardIntelligenceContext,
  command?: string,
): SmartBoardGoal {
  ...
  if (normalized) {
    ...
    evidence.push({
      source: "command",
      detail: `Explicit request classified as ${type}.`,
    });
  } else if (
    context.elements.some((element) => element.type === "math-expression")
  ) {
    type = "understand";
    evidence.push({
      source: "selection",
      detail: "A mathematical expression is selected.",
    });
  }
  return {
    type,
    ...
    confidence: normalized
      ...
      : context.elements.length === 1
        ...
        : "needs-confirmation",
    evidence,
    missingInformation: context.elements.length
      ? []
      : [
          {
            field: "selection",
            reason: "No active Board element is selected.",
          },
        ],
...
export function planBoardWorkflow(input: {
  ...
  add("confirm", "Confirm selected expression", {
    toolId: "board.inspect-selection",
    canSkip: false,
  });
  if (subject === "mathematics") {
    const math = input.context.elements.find(
      (element) => element.type === "math-expression",
    );
    let quadratic = false;
    if (math) {
    ...
    if (
      ...
      add("calculate", "Factor the quadratic", {
        toolId: "math.factor",
        boardActionType: "factor",
        permissionClass: "reversible-write",
      });
      add("calculate", "Find the roots", {
        toolId: "math.find-roots",
        boardActionType: "find-roots",
        permissionClass: "reversible-write",
      });
      add("verify", "Verify the roots", { toolId: "math.verify" });
    } else if (normalized.includes("solve")) {
      add("calculate", "Solve with the existing CAS", {
        toolId: "math.solve",
        boardActionType: "solve",
        permissionClass: "reversible-write",
      });
      add("verify", "Verify the result", { toolId: "math.verify" });
    } else if (normalized.includes("simplif")) {
      add("calculate", "Simplify with the existing CAS", {
        toolId: "math.simplify",
        boardActionType: "simplify",
        permissionClass: "reversible-write",
      });
    }
    if (
      ...
      add("visualize", "Plot with the existing 2D graph engine", {
        toolId: "math.plot-2d",
        boardActionType: "plot-2d",
        permissionClass: "reversible-write",
      });
    }
  } else if (subject === "physics" && normalized.includes("unit")) {
    add("open-module", "Open the existing unit converter", {
      toolId: "physics.open-unit-converter",
    });
  } else {
    add("analyze", `Review ${subject} capability availability`, {
      toolId: "board.inspect-problem",
    });
  }

  if (steps.length === 1) {
    add("analyze", "Understand the selected content", {
      toolId: "board.inspect-problem",
    });
  }
  const capabilities = steps
    .map((step) => toolCapability(step.toolId))
    .filter((item): item is SmartBoardCapability => Boolean(item));
  return {
    ...
    warnings:
      subject === "unknown"
        ? ["Subject confirmation is required before engine execution."]
        : [],
...
export function approveSafeWorkflowSteps(
  ...
  return {
    ...
    steps: plan.steps.map((step) =>
      step.permissionClass === "sensitive"
        ? step
        : {
            ...step,
...
export function cancelBoardWorkflow(
  plan: SmartBoardWorkflowPlan,
): SmartBoardWorkflowPlan {
  return {
    ...plan,
...
export function resolveBoardAmbiguity(
  ...
  return {
    ...
    sessionMemory: {
      ...
      resolvedAmbiguities: {
        ...persistence.sessionMemory.resolvedAmbiguities,
        [ambiguityId]: value,
      },
      updatedAt: new Date().toISOString(),
    },
  ...
  return {
    ...
    sessionMemory: {
      ...
      completedActionIds: unique([
        ...persistence.sessionMemory.completedActionIds,
        actionId,
      ]),
      updatedAt: new Date().toISOString(),
    },
...
export function verifyBoardResult(result: BoardResultElement): {
  ...
  if (result.status === "error" || result.status === "cancelled")
    return { status: "failed", label: "Verification failed" };
  if (result.status !== "success")
    return { status: "inconclusive", label: "Verification pending" };
  if (result.actionType === "verify")
    return { status: "verified", label: "Verified by symbolic engine" };
  if (
    result.actionType === "plot-2d" ||
    result.actionType === "table-of-values"
  ) {
    return result.graph?.series?.length
      ? {
          status: "numerically-verified",
          label: "Numerically sampled by graph engine",
        }
      : {
          status: "partially-verified",
          label: "Graph configuration validated",
        };
  }
  if (result.engine.adapter === "cas")
    return {
  ...
  return {
    status: "partially-verified",
    label: `Produced by ${result.engine.underlyingEngine ?? result.engine.adapter}`,
  };
}
...
export class BoardToolRegistry {
  ...
  constructor(
    definitions: SmartBoardToolDefinition[] = createBoardToolDefinitions(),
  ) {
  ...
  register(definition: SmartBoardToolDefinition, executor?: ToolExecutor) {
    if (this.definitions.has(definition.id))
      throw new Error(`Duplicate Board tool: ${definition.id}`);
  ...
  setExecutor(toolId: string, executor: ToolExecutor) {
    if (!this.definitions.has(toolId))
      throw new Error(`Unknown Board tool: ${toolId}`);
  ...
  listTools(context: SmartBoardIntelligenceContext) {
    return [...this.definitions.values()].filter(
      (definition) =>
        (!definition.subject ||
          definition.subject === context.primarySubject) &&
  ...
  async execute(
    ...
    if (signal?.aborted)
      return failure(call.id, "CANCELLED", "The workflow step was cancelled.");
    const definition = this.definitions.get(call.toolId);
    if (!definition)
      return failure(
        call.id,
        "INVALID_TOOL_CALL",
        "This tool is not allowlisted.",
      );
    if (!this.listTools(context).some((tool) => tool.id === definition.id)) {
      return failure(
        call.id,
        "CAPABILITY_UNAVAILABLE",
        "This tool is unavailable for the active subject or offline mode.",
      );
    }
    if (
      (definition.requiresUserConfirmation ||
        definition.permissionClass === "sensitive") &&
      !call.userConfirmed
    ) {
      return failure(
        call.id,
    ...
    if (
      call.sourceElementIds.some(
        (id) => !context.elements.some((element) => element.id === id),
      )
    ) {
      return failure(
        call.id,
        "INVALID_TOOL_CALL",
        "The tool call attempted to access content outside the selected context.",
      );
    }
    const validationError = validateToolArguments(definition, call.arguments);
    if (validationError)
      return failure(call.id, "INVALID_TOOL_CALL", validationError);
    const serializedArguments = JSON.stringify(call.arguments);
    if (detectPromptInjection(serializedArguments)) {
      return failure(
        call.id,
        "PROMPT_INJECTION_DETECTED",
        "Instruction-like Board content cannot change tool permissions.",
      );
    }
    ...
    if (!executor)
      return failure(
        call.id,
        "CAPABILITY_UNAVAILABLE",
        "The selected capability has no executable adapter.",
      );
    try {
      ...
      return failure(
        call.id,
        "UNKNOWN",
        error instanceof Error ? error.message : "The tool could not complete.",
      );
    }
  }
}
...
export function createBoardToolDefinitions(): SmartBoardToolDefinition[] {
  ...
  return [
    readOnly(
      "board.inspect-selection",
      "inspect-selection",
      "Inspect only the selected Board elements.",
    ),
    readOnly(
      "board.inspect-problem",
      "inspect-problem",
      "Inspect the active problem.",
    ),
    readOnly(
      "board.inspect-relationships",
      "inspect-relationships",
      "Inspect selected source-result relationships.",
    ),
    ...
    mathWrite(
      "math.find-roots",
      "find-roots",
      "Find roots with the existing CAS.",
    ),
    mathWrite(
      "math.verify",
      "verify",
      "Verify through the existing symbolic engine.",
    ),
    mathWrite(
      "math.plot-2d",
      "plot-2d",
      "Plot through the existing 2D graph engine.",
    ),
    readOnly(
      "physics.open-unit-converter",
      "open-unit-converter",
      "Open the existing unit-converter workspace.",
      "physics",
    ),
  ];
}
...
export class BoardIntelligenceOrchestrator {
  understand(
    ...
    signal?: AbortSignal,
  ): Promise<SmartBoardUnderstandingResult> {
    if (signal?.aborted)
  ...
  recommendActions(
    ...
    if (signal?.aborted)
      return Promise.reject(new DOMException("Cancelled", "AbortError"));
    return Promise.resolve(
      understandBoardContext(context, memory).recommendations,
    );
  }
...
export const boardIntelligenceOrchestrator =
  new BoardIntelligenceOrchestrator();
...
function contextElement(
  ...
  if (element.type === "math-expression") {
    return {
      ...
      recognitionStatus:
        element.recognitionConfidence === undefined
          ? "confirmed"
          : "recognized",
  ...
  if (element.type === "math-result") {
    return {
      ...
      type: element.type,
      content: (
        element.exactOutputLatex ??
        element.plainTextOutput ??
        element.title
  ...
  if (element.type === "text") {
    return {
      id: element.id,
      type: element.type,
      content: element.text.slice(0, MAX_EXPRESSION_LENGTH),
      subject: detectTextSubject(element.text),
      order,
      sourceElementIds: [],
      untrusted: true,
    };
  ...
  if (element.type === "image") {
    const regionTypes = element.recognitionRegions
    ...
    return {
      ...
      order,
      recognitionStatus: element.recognitionRegions.some(
        (region) => region.recognitionStatus === "success",
      )
        ? "recognized"
        : "unrecognized",
      sourceElementIds: [],
  ...
  if (element.type === "explanation") {
    return {
      ...
      subject: "mathematics",
      order,
      verificationStatus: element.verified ? "verified" : "ai-only",
      sourceElementIds: element.sourceElementIds,
      untrusted: true,
    };
  }
  return {
    ...
    content:
      element.type === "shape"
        ? element.shape
        : "Unrecognized handwriting stroke",
    subject: "unknown",
    order,
    sourceElementIds: [],
    untrusted: true,
...
function detectSubjects(elements: SmartBoardContextElement[]) {
  ...
  if (!subjects.length)
    return { primary: undefined, supporting: [] as SmartBoardSubject[] };
  const nonEnglish = subjects.filter((subject) => subject !== "english");
  const primary = nonEnglish[0] ?? subjects[0];
  return {
    primary,
    supporting: subjects.filter((subject) => subject !== primary),
  };
}
...
function detectTextSubject(text: string): SmartBoardSubject {
  ...
  if (
    /\b(velocity|acceleration|force|mass|newton|joule|voltage|current|m\/s|kinematic)\b/.test(
      lower,
    )
  )
    return "physics";
  if (
    /(→|->|\b(mole|molar|reaction|compound|oxidation|acid|alkali)\b)/.test(
      lower,
    )
  )
    return "chemistry";
  if (
    /\b(cell|organelle|mitosis|meiosis|gene|allele|taxonomy|photosynthesis)\b/.test(
      lower,
    )
  )
    return "biology";
  if (
    /[.!?]/.test(text) ||
    /\b(sentence|paragraph|grammar|verb|noun|adjective)\b/.test(lower)
  )
    return "english";
  return "unknown";
}

function detectImageSubject(
  element: Extract<BoardElement, { type: "image" }>,
): SmartBoardSubject {
  const content = element.recognitionRegions
    .map((region) => `${region.regionType} ${region.recognizedText ?? ""}`)
    .join(" ");
  return detectTextSubject(content);
}

function extractContextAmbiguities(
  ...
  for (const element of elements) {
    if (
      element.recognitionConfidence !== undefined &&
      element.recognitionConfidence < 0.75
    ) {
      ...
      if (!(id in memory.resolvedAmbiguities)) {
        ambiguities.push({
          ...
          description:
            "Recognition confidence is low. Confirm this content before calculation.",
          candidates: [
            { label: "Use current reading", value: element.content },
            { label: "Review recognition", value: "review" },
          ],
...
function mathematicsRecommendations(
  ...
  const preferred = analysis.suggestedActions.filter(
  ...
  return [...actionSet]
    .map((action, index): SmartBoardRecommendation => {
      ...
      const engineAvailable = capabilityAvailable(
        action,
        context.serviceAvailability,
      );
      const duplicate = memory.completedActionIds.includes(id);
      const ambiguity = context.pendingAmbiguities.some(
      ...
      const score = {
        ...
        userPreferenceFit:
          memory.userPreferences.intelligenceMode === "exploration" &&
          action.includes("plot")
            ? 10
            : 0,
        total: 0,
      };
      ...
      return {
        ...
        engine: {
          id: meta.engine,
          label: meta.engineLabel,
          localOrRemote: "local",
        },
        enabled: engineAvailable && !ambiguity,
        disabledReason: !engineAvailable
          ? `${meta.engineLabel} is unavailable.`
          : ambiguity
            ? "Confirm ambiguous recognition first."
            : undefined,
      };
    })
    .filter(
...
function unsupportedSubjectUnderstanding(
  ...
  const concepts = [
    {
      id: `${subject}-content`,
      label: `${capitalize(subject)} content`,
      confidence: "review-recommended" as const,
    },
  ];
  const knownFacts = [
    {
      label: "Detected subject",
      value: capitalize(subject),
      sourceElementId: sourceIds[0],
    },
  ];
  const unknownFacts = [
    {
      label: "Verified subject operation",
      reason: `No reusable ${capitalize(subject)} Board engine is registered.`,
    },
  ];
  const warnings = [
    `${capitalize(subject)} understanding is limited to local classification; no results are fabricated.`,
  ];
  const disabled = (
    ...
    score: {
      goalMatch: goal.type === action ? 30 : 0,
      subjectMatch: 20,
      conceptMatch: 10,
      prerequisiteFit: 0,
      learningValue: 5,
      engineAvailability: -50,
      duplicatePenalty: 0,
      ambiguityPenalty: 0,
      userPreferenceFit: 0,
      total: -15,
  ...
  if (subject === "physics") {
    recommendations.push({
      ...disabled(
        "open-unit-converter",
        "Open unit converter",
        "An existing unit workspace can help review conversions.",
        "convert",
      ),
      enabled: context.serviceAvailability.physicsUnits,
      disabledReason: context.serviceAvailability.physicsUnits
        ? undefined
        : "The unit-converter workspace is unavailable.",
      engine: {
        id: "unit-converter",
        label: "Existing Unit Converter",
        localOrRemote: "local",
      },
      priority: 45,
      score: {
    ...
    recommendations.push(
      disabled(
        ...
        "Balancing requires a verified Chemistry engine.",
        "verify",
      ),
    );
    ...
    recommendations.push(
      disabled(
        "check-grammar",
        "Check grammar",
        "Confirmed text requires an objective language engine.",
        "correct",
      ),
    );
    ...
    recommendations.push(
      disabled(
        "review-labels",
        "Review labels",
        "Label validation requires model reference data.",
        "verify",
      ),
    );
  }
...
function recommendationMeta(
  ...
  > = {
    factor: {
      title: "Factor",
      reason: quadratic
        ? "Factor is suggested because the selection is a quadratic polynomial."
        : "Factoring can expose the expression's multiplicative structure.",
      outcome: "A linked factorized result",
      engine: "cas",
      engineLabel: "Existing CAS",
      category: "solve",
      conceptMatch: quadratic ? 25 : 14,
      learningValue: 14,
    },
    "find-roots": {
      title: "Find roots",
      reason: quadratic
        ? "Roots are useful because this is a quadratic expression."
        : "Finding roots locates where the expression equals zero.",
      outcome: "Linked roots from the CAS",
      engine: "cas",
      engineLabel: "Existing CAS",
      category: "solve",
      conceptMatch: quadratic ? 25 : 15,
      learningValue: 15,
    },
    solve: {
      title: "Solve",
      reason: "Solve is suggested because the selection is an equation.",
      outcome: "A linked CAS solution",
      engine: "cas",
      engineLabel: "Existing CAS",
      category: "solve",
      conceptMatch: classification === "equation" ? 25 : 12,
      learningValue: 12,
    },
    verify: {
      title: "Verify",
      reason:
        "Verification checks the result with the existing symbolic engine.",
      outcome: "A visible verification status",
      engine: "verification",
      engineLabel: "Symbolic verifier",
      category: "verify",
      conceptMatch: 18,
      learningValue: 14,
    },
    "plot-2d": {
      title: "Graph",
      reason:
        "Graphing may help because the selection defines or implies a function.",
      outcome: "A linked interactive 2D graph",
      engine: "graph-2d",
      engineLabel: "Existing 2D graph engine",
      category: "visualize",
      conceptMatch: 18,
      learningValue: 18,
    },
    simplify: {
      title: "Simplify",
      reason: "Simplifying can expose an equivalent, easier-to-read form.",
      outcome: "A linked simplified result",
      engine: "cas",
      engineLabel: "Existing CAS",
      category: "continue",
      conceptMatch: 14,
    ...
    expand: {
      title: "Expand",
      reason: "Expanding can expose coefficients and like terms.",
      outcome: "A linked expanded result",
      engine: "cas",
      engineLabel: "Existing CAS",
      category: "explore",
      conceptMatch: 10,
    ...
    differentiate: {
      title: "Differentiate",
      reason:
        "The selected calculus content can be processed by the existing CAS.",
      outcome: "A linked derivative",
      engine: "cas",
      engineLabel: "Existing CAS",
      category: "solve",
      conceptMatch: classification === "derivative" ? 25 : 10,
      learningValue: 13,
    },
    integrate: {
      title: "Integrate",
      reason:
        "The selected calculus content can be processed by the existing CAS.",
      outcome: "A linked antiderivative",
      engine: "cas",
      engineLabel: "Existing CAS",
      category: "solve",
      conceptMatch: classification === "integral" ? 25 : 10,
      learningValue: 13,
    },
    statistics: {
      title: "Analyze data",
      reason: "The selection is structured as numerical data.",
      outcome: "A linked statistical summary",
      engine: "statistics",
      engineLabel: "Existing statistics engine",
      category: "explore",
      conceptMatch: 24,
      learningValue: 15,
    },
  ...
  return (
    data[action] ?? {
      ...
      reason: "This action matches the selected mathematical structure.",
      outcome: "A linked Board result",
      engine: "cas",
      engineLabel: "Existing engine",
...
function mathConcept(classification: string, degree?: number) {
  if (classification === "equation" && degree === 2)
    return { id: "quadratic-equation", label: "Quadratic equation" };
...
function subjectConfidence(
  ...
  if (
    context.elements.every(
      (element) =>
        element.subject === context.primarySubject ||
        element.subject === "unknown",
    )
  )
    return "high";
...
function intelligenceAvailability(services: SmartBoardServiceAvailability) {
  if (services.ai && services.recognition && services.cas && services.graph2d)
...
function resolveAvailableCapabilities(
  services: SmartBoardServiceAvailability,
): SmartBoardCapability[] {
  const capabilities: SmartBoardCapability[] = [
    "inspect-selection",
    "inspect-problem",
    "inspect-relationships",
  ...
  if (services.graph2d)
    capabilities.push("plot-2d", "plot-implicit", "table-of-values");
...
function capabilityAvailable(
  ...
  if (["plot-2d", "plot-implicit", "table-of-values"].includes(capability))
    return services.graph2d;
...
function commandGoal(command: string): SmartBoardGoalType {
  ...
  if (/\bverify|check (my |this )?(answer|work)\b/.test(command))
    return "verify";
  ...
  if (/\bgrammar|check (this )?(sentence|paragraph)\b/.test(command))
    return "check-grammar";
  if (/\breview\b.*\blabel/.test(command)) return "review-labels";
  if (/\bpractice|similar (question|problem)\b/.test(command))
    return "practice";
  if (/\bcontinue|complete\b/.test(command)) return "complete-work";
  return "understand";
}

function goalMatchesAction(
  goal: SmartBoardGoalType,
  action: SmartBoardCapability,
) {
  return (
    goal === action ||
...
function workflowTitle(goal: SmartBoardGoal, command: string) {
  if (
    ...
    (command.includes("graph") || command.includes("plot"))
  )
    return "Solve and graph selection";
...
function toolCapability(toolId?: string): SmartBoardCapability | undefined {
  ...
  return createBoardToolDefinitions().find((tool) => tool.id === toolId)
    ?.capability;
}

function validateToolArguments(
  definition: SmartBoardToolDefinition,
  ...
  if (
    ...
    expression.length >
      (definition.inputSchema.maxExpressionLength ?? MAX_EXPRESSION_LENGTH)
  ) {
    return "The expression exceeds the allowed length.";
  }
  const numericValues = Object.values(args).filter(
    (value): value is number => typeof value === "number",
  );
  if (
    numericValues.some(
      (value) => !Number.isFinite(value) || Math.abs(value) > 1_000_000_000,
    )
  ) {
...
function failure(
  callId: string,
  code: SmartBoardIntelligenceError["code"],
  userMessage: string,
): SmartBoardToolResult {
  return {
    callId,
