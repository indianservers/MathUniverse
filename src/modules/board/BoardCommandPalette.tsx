...
export default function BoardCommandPalette({
  open,
  commands,
  onClose,
}: {
  ...
  return (
    <div
      className="fixed inset-0 z-[100] flex items-start justify-center bg-slate-950/45 p-4 pt-[10vh]"
      role="dialog"
      aria-modal="true"
      aria-label="Board command palette"
      onMouseDown={(event) => {
      ...
      <div className="w-full max-w-xl overflow-hidden rounded-xl bg-white shadow-2xl dark:bg-slate-900">
        <div className="flex items-center gap-2 border-b border-slate-200 p-3 dark:border-white/10">
          ...
          <input
            ...
            placeholder="Search Board commands"
            aria-label="Search Board commands"
          />
          <button
            type="button"
