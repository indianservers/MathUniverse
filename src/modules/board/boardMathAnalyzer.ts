...
const actionDetails: Record<
  BoardActionType,
  Omit<BoardSuggestedAction, "id" | "type">
> = {
  evaluate: {
    label: "Evaluate",
    priority: 10,
  ...
  "plot-2d": {
    label: "Draw graph",
    priority: 10,
    engineAdapter: "graph-2d",
    enabled: true,
  },
  "plot-implicit": {
    label: "Open implicit graph",
    priority: 25,
    engineAdapter: "graph-2d",
    enabled: true,
  },
  ...
  statistics: {
    ...
    priority: 10,
    engineAdapter: "statistics",
    enabled: true,
  },
  geometry: {
    label: "Open in geometry",
    priority: 20,
    engineAdapter: "geometry",
    enabled: true,
  },
  verify: {
    label: "Verify",
    priority: 45,
    engineAdapter: "verification",
    enabled: true,
  },
};
...
export function normalizeBoardExpression(rawLatex: string) {
  ...
  const grouped = powers.replace(/[{}]/g, (token) =>
    token === "{" ? "(" : ")",
  );
  const functions = grouped.replace(
    /\\(sin|cos|tan|arcsin|arccos|arctan|ln|log|exp)\b/g,
    (_, name: string) =>
      name === "arcsin"
        ? "asin"
        : name === "arccos"
  ...
  const engineExpression = functions
    ...
    .replace(/\s+/g, " ")
    .trim();
  if (/[\\<>](?:script|iframe)|javascript:/i.test(engineExpression))
    throw new Error("Unsupported unsafe content.");
...
export function analyzeBoardExpression(rawLatex: string): BoardMathAnalysis {
  ...
  const suggestedActions = actionsForClassification(
    classification,
    normalized.engineExpression,
    variables,
    ambiguities,
  ...
  return {
    ...
    independentVariables: variables.includes("x")
      ? ["x"]
      : variables.slice(0, 1),
...
export function actionsForClassification(
  ...
  const rules: Record<BoardMathClassification, BoardActionType[]> = {
    ...
    equation:
      variables.length > 1
        ? ["solve", "plot-implicit", "geometry", "verify"]
        : ["solve", "plot-2d", "verify"],
  ...
  const unresolved = ambiguities.some(
  ...
  return (rules[classification] ?? [])
    .map((type) => ({
      ...
      disabledReason: unresolved
        ? "Resolve the highlighted ambiguity first."
        : undefined,
      defaultParameters: {
  ...
  return Array.from(
    new Set(
      tokens
        .filter(
          (token) =>
            !functionNames.has(token.toLowerCase()) &&
            !["pi", "e", "d", "dx", "dy"].includes(token.toLowerCase()),
        )
...
export function detectAmbiguities(
  ...
  if (/(?:^|[^a-z])l(?:[^a-z]|$)/.test(raw) || /1\s*\/\s*l/.test(raw)) {
    ambiguities.push({
      ...
      candidates: [
        { label: "Number 1", latex: raw.replace(/\bl\b/g, "1") },
        { label: "Variable l", latex: raw },
      ],
  ...
  if (/sin\s*\^\s*(?:\{-?1\}|\(-?1\)|-1)/i.test(raw)) {
    ambiguities.push({
      ...
      candidates: [
        {
          label: "Inverse sine",
          latex: raw.replace(/sin\s*\^\s*(?:\{-?1\}|\(-?1\)|-1)/i, "\\arcsin"),
        },
        {
          label: "Reciprocal",
          latex: `1/(${raw.replace(/\^\s*(?:\{-?1\}|\(-?1\)|-1)/, "")})`,
        },
      ],
      requiresResolution: true,
    });
  ...
  if (classification === "integral" && !/d[a-zA-Z]\b/.test(raw)) {
    ambiguities.push({
      ...
      candidates: [
        { label: "Integrate with respect to x", value: "x" },
        { label: "Integrate with respect to t", value: "t" },
      ],
      requiresResolution: true,
    });
  }
...
function detectStructures(expression: string) {
  ...
  const checks: Array<[RegExp, string]> = [
    [/\+/, "addition"],
    [/-/, "subtraction"],
    [/\*|(?:\d|\))(?=[a-z(])/, "multiplication"],
    [/\//, "division"],
    [/\^/, "power"],
    [/sqrt/, "root"],
    [/[a-z].*\^|[a-z].*[+-]/i, "polynomial"],
    [/\b(?:sin|cos|tan)\b/i, "trigonometric"],
    [/\b(?:ln|log)\b/i, "logarithmic"],
    [/\bexp\b|\be\^/i, "exponential"],
    [/=/, "equation"],
    [/[<>]=?/, "inequality"],
    [/d\/d|\\frac\{d/, "derivative"],
    [/\\int|\bintegr/, "integral"],
    [/\\lim|\blim/, "limit"],
    [/^\s*\[\s*\[/, "matrix"],
  ];
  checks.forEach(([pattern, label]) => {
    if (pattern.test(expression)) structures.push(label);
  });
  return structures;
}
...
function polynomialDegree(expression: string, variable: string) {
  ...
  const powers = Array.from(
    expression.matchAll(new RegExp(`${escaped}\\s*\\^\\s*\\(?([0-9]+)`, "g")),
