...
function expression(
  ...
  return {
    ...
    latex,
    sourceStrokeIds: [],
    bounds: { x: 20, y, width: 160, height: 50 },
...
describe("Board work verification", () => {
  it("orders lines spatially and creates sequence relationships", () => {
    const inputs = [
      expression("third", "x=5", 200),
      expression("first", "3x+7=22", 20),
      expression("second", "3x=15", 110),
    ];
    expect(orderMathElements(inputs).map((item) => item.id)).toEqual([
      "first",
      "second",
      "third",
    ]);
  ...
  it("finds the first invalid equation step without independently blaming later work", async () => {
    const inputs = [
      expression("a", "3x+7=22", 20),
      expression("b", "3x=15", 100),
      expression("c", "x=6", 180),
    ];
    const created = createSolutionSequence(inputs);
  ...
  it("treats low-confidence recognition as ambiguous", async () => {
    const inputs = [
      expression("a", "x+x", 20),
      { ...expression("b", "2x", 100), recognitionConfidence: 0.2 },
    ];
    const created = createSolutionSequence(inputs);
    const result = await verifySolutionSequence(
      created.sequence,
