...
} from "./boardImageImport";
import type { BoardImageElement } from "./types";

export default function BoardImageImportDialog({
  open,
  onClose,
  onInsert,
}: {
  open: boolean;
  onClose: () => void;
  onInsert: (image: BoardImageElement) => void;
}) {
  ...
  const [message, setMessage] = useState(
    "Images stay local until you explicitly insert them.",
  );
  ...
  async function loadFile(
    file?: File,
    nextSource: BoardImageElement["source"] = "upload",
  ) {
    if (!file) return;
    abortRef.current?.abort();
    ...
    try {
      ...
      setMessage(
        "Review the preview and detected reading regions before inserting.",
      );
    } catch (error) {
  ...
  async function startCamera() {
    if (!navigator.mediaDevices?.getUserMedia) {
      setMessage(
        "Camera unavailable in this browser. Upload an image instead.",
      );
      return;
    }
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Import image or use camera"
    >
      <div className="max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-xl bg-white p-4 shadow-2xl dark:bg-slate-900">
        <div className="flex items-start justify-between gap-3">
          <div>
            <h2 className="font-bold">Image and camera import</h2>
            <p className="text-sm text-slate-500">
              Nothing is uploaded. Imported text is treated as untrusted
              content.
            </p>
          </div>
          <button
        ...
        >
          ...
          <p className="mt-2 text-xs text-slate-500">
            PNG, JPEG or WebP · maximum 8 MB · drag and drop supported
          </p>
        </div>
        {cameraActive && (
          <video
            ref={videoRef}
            className="mt-3 max-h-80 w-full rounded-xl bg-slate-950 object-contain"
            playsInline
            muted
          ...
          <div className="mt-4 grid gap-4 md:grid-cols-[minmax(0,1fr)_240px]">
            <div className="relative overflow-hidden rounded-xl bg-slate-100 p-2 dark:bg-white/5">
              <img
                ...
                alt={`Imported worksheet preview with ${prepared.regions.length} detected regions`}
            ...
            <div className="space-y-3">
              <p className="text-sm">
                <strong>{prepared.regions.length}</strong> reading region
                {prepared.regions.length === 1 ? "" : "s"} detected from
              ...
              <p className="text-xs text-slate-500">
                A production vision provider is not configured. Regions are
                preserved for review; enter or correct their LaTeX after
                insertion.
              </p>
              <button
                type="button"
                  ...
                  onInsert(
                    createBoardImageElement(
                      ...
                      source,
                      {
                        ...
                        height: Math.max(
                          180,
                          (420 * prepared.height) / prepared.width,
                        ),
                      },
                      rotation,
                    ),
...
function imageErrorMessage(error: unknown) {
  ...
  if ((error as Error)?.name === "AbortError")
    return "Image preparation cancelled.";
