...
describe("Board export configuration", () => {
  it("exports structured JSON with optional tutor exclusion", () => {
    ...
    const output = buildBoardTextExport(document, "json", {
      includeTutor: false,
    });
    ...
    document.tutorMessages = [
      {
        ...
        role: "tutor",
        mode: "hint",
        text: "Inspect the exponent.",
        referencedElementIds: ["math"],
        verified: true,
        createdAt: new Date().toISOString(),
      },
    ];
    expect(
      buildBoardTextExport(document, "latex", { includeTutor: true }),
    ).toBe("x^2");
    expect(
      buildBoardTextExport(document, "tutor-text", { includeTutor: true }),
    ).toContain("Inspect the exponent");
  });
});
