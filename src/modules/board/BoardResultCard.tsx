...
const FunctionGraphCanvas = lazy(
  () => import("../../components/math-lab/FunctionGraphCanvas"),
);
...
export default function BoardResultCard({
  ...
  const defaultView = result.graph?.view ?? {
    xMin: -10,
    xMax: 10,
    yMin: -10,
    yMax: 10,
  ...
  return (
    ...
    >
      <header className="flex items-center gap-1 border-b border-slate-200 px-2 py-1.5 dark:border-white/10">
        <span
          className={`h-2 w-2 rounded-full ${result.status === "success" ? "bg-emerald-500" : result.status === "loading" ? "animate-pulse bg-cyan-500" : "bg-rose-500"}`}
        />
        ...
        <button
          ...
          aria-label={result.collapsed ? "Expand result" : "Collapse result"}
        >
          {result.collapsed ? (
        ...
        <button
          type="button"
          className="tool-button min-h-8 px-2 text-rose-600"
          onClick={(event) => {
            event.stopPropagation();
            onDelete();
          }}
        ...
        <div className="max-h-[440px] overflow-y-auto p-2">
          {result.status === "loading" && (
            <p
              ...
              aria-live="polite"
            >
              Calculating with {result.engine.adapter}…
            </p>
          )}
          {result.status === "error" && (
            <p
              className="rounded-lg bg-rose-50 p-2 text-sm text-rose-700 dark:bg-rose-500/10 dark:text-rose-200"
              role="alert"
            >
          ...
          {result.status === "success" && (
            <>
              {(result.exactOutputLatex || result.plainTextOutput) && (
                <pre className="whitespace-pre-wrap break-words rounded-lg bg-slate-100 p-2 font-mono text-sm dark:bg-white/5">
                  {result.exactOutputLatex ?? result.plainTextOutput}
                </pre>
              )}
                ...
                >
                  <div className="mb-2 flex flex-wrap gap-1">
                    <button
                      ...
                      className="tool-button min-h-8 px-2"
                      onClick={() =>
                        setView((current) => zoomView(current, 0.75))
                      }
                    >
                      Zoom in
                    </button>
                    <button
                      ...
                      className="tool-button min-h-8 px-2"
                      onClick={() =>
                        setView((current) => zoomView(current, 1.35))
                      }
                    >
                      Zoom out
                    </button>
                    ...
                    <button
                      ...
                      className={
                        showGrid
                          ? "action-primary min-h-8 px-2"
                          : "tool-button min-h-8 px-2"
                      }
                  ...
                  <div className="h-64 overflow-hidden">
                    ...
                    >
                      <FunctionGraphCanvas
                        ...
                        showGrid={showGrid}
                        showAxes={showAxes}
                        selectedSeriesId={graphSeries[0]?.id}
                        traceX={traceX}
                  ...
                  <p className="mt-1 text-xs text-slate-500">
                    {result.graph.accessibilitySummary} Trace x ={" "}
                    {traceX.toFixed(2)}
                  </p>
                </div>
              ) : null}
              {result.steps?.length ? (
                <details className="mt-2">
                  <summary className="cursor-pointer text-sm font-bold">
                    Engine steps ({result.steps.length})
                  </summary>
                  <ol className="mt-2 space-y-1 text-xs text-slate-600 dark:text-slate-300">
                    {result.steps.map((step) => (
                      <li key={step.id}>
                        {step.index + 1}. {step.explanation ?? step.outputLatex}
                      </li>
                    ))}
                  </ol>
                </details>
          ...
          <div className="mt-2 flex flex-wrap gap-1">
            ...
            <button
              type="button"
              className="tool-button min-h-8 px-2"
              disabled={result.status !== "success" || !result.exactOutputLatex}
              onClick={onInsert}
            >
          ...
          <div
            ...
            aria-label="Move result without dragging"
          >
            <span className="mr-1 text-xs text-slate-500">Move</span>
            <button
              type="button"
              className="tool-button min-h-8 px-2"
              aria-label="Move result left"
              onClick={() => onMove(-20, 0)}
            >
