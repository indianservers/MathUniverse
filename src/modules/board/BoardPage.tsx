...
import BoardCommandPalette, {
  type BoardCommand as BoardPaletteCommand,
...
import {
  applyVerification,
  createSolutionSequence,
  verifySolutionSequence,
} from "./boardWorkVerification";
import {
...
type RecognitionEntry = MathRecognitionResult & {
  id: string;
  createdAt: string;
};
...
export default function BoardPage() {
  const [document, setDocument] = useState<BoardDocument>(
    () => recoverDraft() ?? createBoardDocument(),
  );
  ...
  const [understanding, setUnderstanding] =
    useState<SmartBoardUnderstandingResult | null>(null);
  ...
  const [workVerification, setWorkVerification] =
    useState<BoardWorkVerificationResult | null>(null);
  ...
  const selectedStrokes = useMemo(
    () =>
      selectedElements.filter(
        (element): element is StrokeElement => element.type === "stroke",
      ),
    [selectedElements],
  );
  const selectedMath = useMemo(
    () =>
      selectedElements.find(
        (element): element is MathExpressionElement =>
          element.type === "math-expression",
      ) ?? null,
    [selectedElements],
  );
  const selectedImage = useMemo(
    () =>
      selectedElements.find(
        (element): element is BoardImageElement => element.type === "image",
      ) ?? null,
    [selectedElements],
  );
  ...
  const undo = useCallback(() => {
    setHistory((current) => {
      ...
      return {
        ...
        redo: [command, ...current.redo].slice(0, 80),
      };
    });
  }, []);
  ...
  const removeElements = useCallback((elements: BoardElement[]) => {
    ...
    setHistory((current) =>
      recordCommand(current, { type: "delete", elements }),
    );
    setSelectedIds((ids) =>
      ids.filter((id) => !elements.some((element) => element.id === id)),
    );
  }, []);
  ...
  const recognize = useCallback(async () => {
    ...
    try {
      ...
      const result = await mathRecognitionProvider.recognize(input, {
  ...
  const insertExpression = useCallback(() => {
    ...
    const element: BoardElement = {
      ...
      bounds: {
        ...sourceBounds,
        width: Math.max(180, sourceBounds.width),
        height: Math.max(58, sourceBounds.height),
      },
      createdAt: new Date().toISOString(),
    };
  ...
  const clearBoard = useCallback(() => {
    ...
    if (
      !globalThis.confirm(
        "Clear every element from this Board? You can undo this action.",
      )
    )
      return;
  ...
  const executeAction = useCallback(
    async (
      ...
      if (
        currentAnalysis.ambiguities.some(
          (ambiguity) => ambiguity.requiresResolution,
        )
      ) {
        setMessage(
          "Resolve the recognition ambiguity before running an engine.",
        );
        return false;
      }
      ...
      const repeatedActionCount = document.elements
        .filter(
          (element) =>
            element.type === "math-result" &&
            element.sourceElementIds.includes(source.id),
        )
        .filter(
          (element) =>
            element.type === "math-result" && element.actionType === actionType,
        ).length;
      ...
      const loading: BoardResultElement = {
        ...
        engine: {
          adapter:
            currentAnalysis.suggestedActions.find(
      ...
      const relationshipType = actionType.includes("plot")
        ? "graphs"
        : actionType === "verify"
          ? "verifies"
          : actionType === "geometry" || actionType === "plot-3d"
            ? "visualizes"
            : "derived-from";
      ...
      try {
        const response = await executeBoardAction({
          action: actionType,
        ...
        setDocument((current) => ({
          ...
          elements: current.elements.map((element) =>
            element.id === resultId && element.type === "math-result"
              ? {
                  ...element,
                  ...response,
                  status: "success",
          ...
          actionHistory: current.actionHistory.map((entry) =>
            entry.resultElementId === resultId
              ? {
                  ...entry,
                  completedAt,
  ...
  const analyzeIntelligence = useCallback(
    async (command?: string) => {
      ...
      try {
        ...
        setMessage(
          result.ambiguities.some((item) => item.requiresResolution)
        ...
        if ((error as Error).name !== "AbortError")
          setMessage(
            "Selection understanding was cancelled or could not complete.",
          );
      } finally {
  ...
  const handleRecommendation = useCallback(
    async (recommendation: SmartBoardRecommendation) => {
      ...
      if (recommendation.boardActionType) {
        ...
        if (completed) {
          setDocument((current) => ({
            ...current,
            intelligence: markBoardActionCompleted(
              current.intelligence,
              recommendation.id,
            ),
          }));
  ...
  const planIntelligenceWorkflow = useCallback(
    (command: string) => {
  ...
  const patchActiveWorkflow = useCallback(
    (updater: (plan: SmartBoardWorkflowPlan) => SmartBoardWorkflowPlan) => {
      setDocument((current) => {
  ...
  const runApprovedWorkflow = useCallback(async () => {
    ...
    const source =
      ...
      document.elements.find(
        (element): element is MathExpressionElement =>
          element.type === "math-expression" &&
          initial.goal.targetElementIds.includes(element.id),
      );
    const controller = new AbortController();
    ...
    try {
      for (const step of plan.steps) {
        ...
        const dependenciesReady = step.dependsOnStepIds.every((id) => {
          ...
          return (
            dependency?.status === "success" || dependency?.status === "skipped"
          );
        });
        if (!dependenciesReady) {
          plan = updateWorkflowStep(plan, step.id, {
            status: "failed",
            error: "A required earlier step did not complete.",
          });
          patchActiveWorkflow(() => plan);
          break;
        }
        ...
        patchActiveWorkflow(() => plan);
        let completed = true;
        if (step.boardActionType)
          completed = await executeAction(
            step.boardActionType,
            source,
        ...
        plan = updateWorkflowStep(plan, step.id, {
          ...
          error: completed
            ? undefined
            : "The existing engine did not complete this step.",
        });
        patchActiveWorkflow(() => plan);
        if (!completed) break;
      }
      if (
        ...
        plan.steps.every((step) =>
          ["success", "skipped", "pending"].includes(step.status),
        )
      ) {
        const unfinishedApproved = plan.steps.some(
          (step) => step.status === "approved" || step.status === "running",
        );
        if (
          !unfinishedApproved &&
          plan.steps.every((step) =>
          ...
          setMessage(
            "Approved workflow steps completed. Remaining steps await approval.",
          );
        }
      }
    } finally {
    ...
    actionParameters,
    document.elements,
    document.intelligence.activeWorkflow,
    executeAction,
    patchActiveWorkflow,
    selectedMath,
    workflowRunning,
  ...
  const insertTutorExplanation = useCallback(
    (response: {
      text: string;
      verified: boolean;
      referencedElementIds: string[];
    }) => {
      const source = document.elements.find((element) =>
        response.referencedElementIds.includes(element.id),
      );
  ...
  const insertImage = useCallback(
    (image: BoardImageElement) => {
      runCommand({ type: "add", elements: [image] });
      setSelectedIds([image.id]);
      setMessage(
        `Image inserted locally with ${image.recognitionRegions.length} review regions.`,
      );
    },
    [runCommand],
  );
  ...
  const checkSelectedWork = useCallback(async () => {
    const expressions = selectedElements.filter(
      (element): element is MathExpressionElement =>
        element.type === "math-expression",
    );
    if (expressions.length < 2) {
      setMessage(
        "Select at least two inserted expressions to check multi-line work.",
      );
      return;
    }
    try {
      ...
      const steps = created.steps.map((step, index) => ({
        ...
        bounds: {
          ...
          x: rightEdge + 36,
          y:
            Math.min(...expressions.map((element) => element.bounds.y)) +
            index * 78,
          width: Math.max(260, step.bounds.width),
        },
      }));
      const allElements = [...document.elements, ...steps];
      const result = await verifySolutionSequence(
        created.sequence,
        allElements,
      );
      const classification = analyzeBoardExpression(
        expressions[0].latex,
      ).classification;
      const confidences = expressions
        .map((element) => element.recognitionConfidence)
        .filter((value): value is number => typeof value === "number");
      recordBoardMasteryEvidence({
        classification,
        verification: result,
        recognitionConfidence: confidences.length
      ...
      setHistory((current) =>
        recordCommand(current, { type: "add", elements: steps }),
      );
      setWorkVerification(result);
      setSelectedIds(
        result.firstInvalidStepId
          ? [result.firstInvalidStepId]
      ...
      setMessage(
        result.firstInvalidStepId
          ? "First invalid step highlighted."
          : "Every recognized transformation is valid.",
      );
    } catch (error) {
      setMessage(
        error instanceof Error ? error.message : "Work verification failed.",
      );
    }
  }, [document.elements, selectedElements]);

  const performExport = useCallback(
    async (format: Parameters<typeof exportBoard>[1]) => {
      const target = boardSurfaceRef.current;
      if (!target) return;
      try {
        await exportBoard(document, format, target, {
          includeTutor: true,
          selectedElementIds: selectedIds,
        });
  ...
  const commands = useMemo<BoardPaletteCommand[]>(() => {
    ...
    const multiLine =
      selectedElements.filter((element) => element.type === "math-expression")
        .length >= 2;
    const action = (
      id: string,
      label: string,
    ...
    return [
      {
        ...
        enabled: selectedStrokes.length > 0,
        disabledReason: "Select handwriting",
        priority: 1,
        run: () => void recognize(),
      },
      {
        id: "understand",
        label: "Understand selection",
        keywords: "identify subject concept intelligence",
        enabled: selectedIds.length > 0,
        disabledReason: "Select Board content",
        priority: 1,
        run: () => {
      ...
      },
      {
        ...
        keywords: "recommend workflow goal",
        enabled: true,
        priority: 2,
        run: () => {
          setIntelligenceOpen(true);
      ...
      action("graph", "Draw graph", "plot-2d", 8),
      {
        ...
        label: "Check my work",
        enabled: multiLine,
        disabledReason: "Select two or more expressions",
        priority: 2,
        run: () => void checkSelectedWork(),
      },
      {
        id: "tutor",
        label: "Ask AI Math Tutor",
        enabled: hasMath || multiLine,
        disabledReason: "Select Board mathematics",
        priority: 3,
        run: () => {
          setTutorOpen(true);
          setRecognitionOpen(true);
      ...
      },
      {
        ...
        label: "Give a hint",
        enabled: hasMath,
        disabledReason: "Select an expression",
        priority: 4,
        run: () => {
          setTutorOpen(true);
          setRecognitionOpen(true);
  ...
  useEffect(() => {
    ...
    const strokes = document.elements.filter(
      (element): element is StrokeElement => element.type === "stroke",
    );
    if (
      settings.mode === "manual" ||
      settings.disabledForSession ||
      strokes.length < settings.minimumStrokeCount
    )
      return undefined;
    abortRef.current?.abort();
    const timer = window.setTimeout(() => {
      void fingerprintStrokes(strokes.flatMap((stroke) => stroke.points)).then(
        (fingerprint) => {
          if (fingerprint === document.automaticRecognition.lastFingerprint)
            return;
          setDocument((current) => ({
            ...current,
            automaticRecognition: {
              ...current.automaticRecognition,
  ...
  useEffect(() => {
    ...
    if (
      ...
      return undefined;
    const selectedFingerprint = selectedIds
      .map((id) => {
        const element = document.elements.find(
          (candidate) => candidate.id === id,
        );
        return element?.type === "math-expression"
          ? `${id}:${element.latex}`
          : `${id}:${element?.type ?? "missing"}`;
      })
      .join("|");
    if (intelligenceFingerprintRef.current === selectedFingerprint)
      return undefined;
    const timer = window.setTimeout(() => {
    ...
    >
      <div ref={boardSurfaceRef} className="space-y-3" data-testid="board-page">
        <section className="glass-card overflow-hidden rounded-xl">
          <div className="flex flex-wrap items-center gap-2 border-b border-slate-200 p-2 dark:border-white/10">
            ...
            <button
              ...
              onClick={() =>
                setViewport({
                  ...document.viewport,
                  zoom: Math.min(4, document.viewport.zoom * 1.2),
                })
              }
              aria-label="Zoom in"
            >
            ...
            <button
              ...
              onClick={() =>
                setViewport({
                  ...document.viewport,
                  zoom: Math.max(0.25, document.viewport.zoom / 1.2),
                })
              }
              aria-label="Zoom out"
            >
            ...
            <button
              ...
              disabled={recognitionStatus === "working"}
            >
              <ScanLine className="h-4 w-4" />{" "}
              {recognitionStatus === "working" ? "Recognizing…" : "Recognize"}
            </button>
            <button type="button" className="tool-button" onClick={saveCurrent}>
            ...
            >
              ...
              <span className="hidden 2xl:inline">Tutor</span>
            </button>
            <button
              type="button"
              className="tool-button"
            ...
            >
              ...
              <span className="hidden 2xl:inline">Intelligence</span>
            </button>
            <button
              type="button"
              className="tool-button"
            ...
            <button
              ...
              aria-label="Open Board command palette"
            >
            ...
            <button
              ...
              aria-label="Toggle Board Outline"
            >
            ...
            <button
              ...
              onClick={() => setRecognitionOpen((value) => !value)}
              aria-label="Toggle recognition panel"
            >
              {recognitionOpen ? (
          ...
          {settingsOpen && (
            <div className="grid gap-3 border-b border-slate-200 p-3 dark:border-white/10 sm:grid-cols-2 lg:grid-cols-5">
              ...
              <label className="text-sm font-semibold">
                ...
                <select
                  ...
                  onChange={(event) =>
                    runCommand({
                      type: "background",
                      before: document.background,
                      after: event.target.value as BoardDocument["background"],
                    })
                  }
                >
              ...
              <label className="text-sm font-semibold">
                ...
                <input
                  className="slider-range mt-2 w-full"
                  type="range"
                  min="1"
                  max="12"
                  value={strokeWidth}
              ...
              <label className="text-sm font-semibold">
                Ink color
                <input
                  className="mt-1 h-10 w-full rounded-lg border border-slate-200 bg-white p-1 dark:border-white/10 dark:bg-slate-950"
                  type="color"
                  value={color}
              ...
              <label className="text-sm font-semibold">
                Recognition after pause
                <select
                  className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-3 py-2 dark:border-white/10 dark:bg-slate-950"
                  value={document.automaticRecognition.mode}
                  onChange={(event) =>
                    setDocument((current) => ({
                      ...current,
                      automaticRecognition: {
                        ...current.automaticRecognition,
                        mode: event.target
                          .value as BoardDocument["automaticRecognition"]["mode"],
                      },
                    }))
                  }
                >
                  <option value="manual">Manual only</option>
                  <option value="suggest">Suggest after pause</option>
                  <option value="automatic">Recognize completed group</option>
                </select>
              </label>
              <label className="text-sm font-semibold">
                Pause: {document.automaticRecognition.pauseMs} ms
                <input
                  className="slider-range mt-2 w-full"
                  type="range"
                  min="800"
                  max="4000"
                  step="100"
                  value={document.automaticRecognition.pauseMs}
                  onChange={(event) =>
                    setDocument((current) => ({
                      ...current,
                      automaticRecognition: {
                        ...current.automaticRecognition,
                        pauseMs: Number(event.target.value),
                      },
                    }))
                  }
                />
          ...
          {exportOpen && (
            <div
              className="flex flex-wrap gap-2 border-b border-slate-200 p-3 dark:border-white/10"
              aria-label="Board export options"
            >
              <button
                type="button"
                className="tool-button"
                onClick={() => void performExport("png")}
              >
                PNG
              </button>
              <button
                type="button"
                className="tool-button"
                onClick={() => void performExport("pdf")}
              >
                PDF
              </button>
              <button
                type="button"
                className="tool-button"
                onClick={() => void performExport("json")}
              >
                Board JSON
              </button>
              <button
              ...
              <button
                type="button"
                className="tool-button"
                onClick={() => void performExport("tutor-text")}
              >
                Tutor transcript
              </button>
              <button
                type="button"
                className="tool-button"
                onClick={() => void performExport("print")}
              >
                Print
              </button>
              <span className="self-center text-xs text-slate-500">
                Account sharing is unavailable; local export remains available.
              </span>
            </div>
          )}
              ...
              {savedBoards.length ? (
                <div className="flex flex-wrap gap-2">
                  {savedBoards.map((board) => (
                    <div
                      key={board.id}
                      className="flex items-center rounded-lg border border-slate-200 dark:border-white/10"
                    >
                      <button
                        type="button"
                      ...
                      <button
                        ...
                        onClick={() => setSavedBoards(deleteBoard(board.id))}
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </div>
                ...
                <p className="text-sm text-slate-500">
                  No saved Boards yet. Your current work is still autosaved as a
                  draft.
                </p>
              )}
            </div>
          ...
          <div
            className={`grid min-w-0 ${recognitionOpen ? "xl:grid-cols-[minmax(0,1fr)_380px]" : ""}`}
          >
            <div className="min-w-0 p-2">
              <BoardCanvas
                ...
                overlay={
                  <>
                    {document.elements
                      .filter(
                        (element): element is BoardImageElement =>
                          element.type === "image" && Boolean(element.dataUrl),
                      )
                      .map((image) => {
                        const point = boardToScreen(
                          { x: image.bounds.x, y: image.bounds.y },
                          document.viewport,
                        );
                        return (
                          <img
                            ...
                            style={{
                              ...
                              top: point.y,
                              width:
                                image.bounds.width * document.viewport.zoom,
                              height:
                                image.bounds.height * document.viewport.zoom,
                    ...
                    {document.elements
                      ...
                      .map((result) => {
                        const source = document.elements.find(
                          (element) =>
                            element.id === result.sourceElementIds[0] &&
                            element.type === "math-expression",
                        ) as MathExpressionElement | undefined;
                        return (
                          <BoardResultCard
                            key={result.id}
                            result={result}
                            ...
                            onInsert={() => {
                              ...
                              const expression: MathExpressionElement = {
                                ...
                                bounds: {
                                  x: result.bounds.x,
                                  y:
                                    result.bounds.y + result.bounds.height + 24,
                                  width: 220,
                                  height: 64,
                                },
                                createdAt: new Date().toISOString(),
              ...
              >
                ...
                <span>·</span>
                <span>{selectedStrokes.length} selected strokes</span>
                {autoSuggestion && (
                  <button
                    type="button"
                    className="action-primary min-h-8 px-2"
                    onClick={() => {
                      setAutoSuggestion(false);
                      void recognize();
                    }}
                  >
                    Recognize completed group
                  </button>
                )}
                <button
                  type="button"
                  className="tool-button min-h-8 px-2"
                  onClick={() => void checkSelectedWork()}
                  disabled={
                    selectedElements.filter(
                  ...
                  <>
                    <button
                      ...
                      onClick={() => {
                        ...
                        setMessage(
                          "Selected strokes merged into one recognition group",
                        );
                      }}
                    >
                      Merge group
            ...
            {recognitionOpen && (
              <aside
                className="max-h-[72vh] overflow-y-auto border-t border-slate-200 p-3 dark:border-white/10 xl:border-l xl:border-t-0"
                aria-label="Recognition panel"
              >
                {intelligenceOpen && (
                  <Suspense
                    fallback={
                  ...
                  >
                    <BoardIntelligencePanel
                      ...
                      onRecommendation={(recommendation) =>
                        void handleRecommendation(recommendation)
                      }
                      onDismissRecommendation={(recommendation, mode) => {
                        setDocument((current) => ({
                          ...current,
                          intelligence: dismissBoardRecommendation(
                            current.intelligence,
                            recommendation.id,
                            mode,
                      ...
                      onApproveAll={() =>
                        patchActiveWorkflow(approveSafeWorkflowSteps)
                      }
                      onApproveStep={(stepId) =>
                        patchActiveWorkflow((plan) =>
                          updateWorkflowStep(plan, stepId, {
                            status: "approved",
                          }),
                        )
                      }
                ...
                {workVerification && (
                  <section
                    className="mb-4 rounded-lg bg-slate-100 p-3 dark:bg-white/5"
                    aria-label="Work verification result"
                    aria-live="polite"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <h2 className="font-bold">Work verification</h2>
                      <span className="mini-chip">
                        {workVerification.overallStatus}
                      </span>
                    </div>
                    <ol className="mt-2 space-y-1 text-sm">
                      {workVerification.verifiedSteps.map((step, index) => (
                        <li key={step.stepId}>
                          <button
                            type="button"
                            className={`w-full rounded-md px-2 py-1 text-left ${step.status === "invalid" ? "bg-rose-100 text-rose-900 dark:bg-rose-500/15 dark:text-rose-100" : ""}`}
                            onClick={() => setSelectedIds([step.stepId])}
                          >
                    ...
                    {workVerification.misconceptions?.map((item) => (
                      <p key={item.id} className="mt-2 text-sm">
                        <strong>{item.title}:</strong> {item.correctiveHint}
                      </p>
                    ))}
                  </section>
                )}
                {selectedImage && (
                  <section
                    ...
                    aria-label="Imported image region review"
                  >
                    <div className="flex items-center justify-between gap-2">
                      <div>
                        <h2 className="font-bold">Image regions</h2>
                        <p className="text-xs text-slate-500">
                          Review local detection and enter corrected LaTeX.
                        </p>
                      </div>
                      <span className="mini-chip">
                        {selectedImage.recognitionRegions.length}
                      </span>
                    </div>
                    <div className="mt-2 space-y-2">
                      {normalizeReadingOrder(
                        selectedImage.recognitionRegions,
                      ).map((region) => (
                        <div
                          key={region.id}
                          className="rounded-lg bg-slate-100 p-2 dark:bg-white/5"
                        >
                          <div className="flex items-center gap-2 text-xs">
                                  ...
                                  elements: current.elements.map((element) =>
                                    ...
                                    element.type === "image"
                                      ? {
                                          ...element,
                                          recognitionRegions:
                                            element.recognitionRegions.map(
                                              (item) =>
                                                item.id === region.id
                                                  ? {
                                                      ...item,
                            ...
                            />
                            <strong>
                              Region {(region.readingOrder ?? 0) + 1}
                            </strong>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      <button
                        ...
                        className="action-primary"
                        onClick={() => {
                          const regions = normalizeReadingOrder(
                            selectedImage.recognitionRegions,
                          ).filter(
                            (region) =>
                              region.selected && region.recognizedLatex?.trim(),
                          );
                          const expressions: MathExpressionElement[] =
                            regions.map((region, index) => ({
                              id: `math-image-${Date.now()}-${index}`,
                              type: "math-expression",
                              latex: region.recognizedLatex!.trim(),
                              normalizedExpression:
                                region.recognizedLatex!.trim(),
                              sourceStrokeIds: [],
                              bounds: {
                                x:
                                  ...
                                  36,
                                y: selectedImage.bounds.y + index * 72,
                                width: 240,
                                height: 58,
                              },
                              createdAt: new Date().toISOString(),
                          ...
                          if (!expressions.length) {
                            setMessage(
                              "Enter corrected LaTeX for at least one selected image region.",
                            );
                            return;
                          }
                          ...
                          setSelectedIds(
                            expressions.map((expression) => expression.id),
                          );
                          setMessage(
                            `${expressions.length} editable image expressions inserted.`,
                          );
                        }}
                      >
                        Insert selected expressions
                      </button>
                      <button
                        type="button"
                        className="tool-button"
                          ...
                          setDocument((current) => ({
                            ...
                            elements: current.elements.map((element) =>
                              ...
                              element.type === "image"
                                ? {
                                    ...element,
                                    recognitionRegions: normalizeReadingOrder([
                                      ...element.recognitionRegions,
                                      {
                                        id: `region-${Date.now()}`,
                ...
                {selectedMath && analysis && (
                  <section
                    className="mb-4 border-b border-slate-200 pb-4 dark:border-white/10"
                    aria-label="Contextual mathematical actions"
                  >
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="mini-chip">
                        {analysis.detectedStructures.slice(0, 2).join(" · ") ||
                          "expression"}
                      </span>
                    </div>
                    {analysis.ambiguities.map((ambiguity) => (
                      <div
                        key={ambiguity.id}
                        className="mt-2 rounded-lg bg-amber-50 p-2 text-sm text-amber-900 dark:bg-amber-400/10 dark:text-amber-100"
                      >
                        ...
                        <div className="mt-2 flex flex-wrap gap-1">
                          {ambiguity.candidates.map((candidate) => (
                            <button
                              ...
                              onClick={() => {
                                const replacement =
                                  candidate.latex ?? selectedMath.latex;
                                runCommand({
                                  type: "edit-math",
                                  id: selectedMath.id,
                                  before: selectedMath.latex,
                                  after: replacement,
                                });
                    ...
                    <div className="mt-3 flex flex-wrap gap-2">
                      {analysis.suggestedActions.slice(0, 5).map((action) => (
                        <button
                          ...
                          onClick={() => void executeAction(action.type)}
                        >
                          {action.label}
                        </button>
                      ))}
                    ...
                    {analysis.suggestedActions.length > 5 && (
                      <details className="mt-2">
                        ...
                        <div className="mt-2 flex flex-wrap gap-2">
                          {analysis.suggestedActions.slice(5).map((action) => (
                            <button
                              key={action.id}
                              type="button"
                              className="tool-button"
                    ...
                    <div className="mt-3 grid grid-cols-2 gap-2">
                      <label className="text-xs font-semibold">
                        Variable
                        <input
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 dark:border-white/10 dark:bg-slate-950"
                          value={String(actionParameters.variable ?? "x")}
                          onChange={(event) =>
                            setActionParameters((current) => ({
                              ...current,
                      ...
                      <label className="text-xs font-semibold">
                        ...
                        <input
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 dark:border-white/10 dark:bg-slate-950"
                          value={String(actionParameters.compareWith ?? "0")}
                          onChange={(event) =>
                            setActionParameters((current) => ({
                              ...current,
                              compareWith: event.target.value,
                            }))
                          }
                        />
                      </label>
                      <label className="text-xs font-semibold">
                        ...
                        <input
                          ...
                          max="99"
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 dark:border-white/10 dark:bg-slate-950"
                          value={Number(actionParameters.xMin ?? -10)}
                          onChange={(event) =>
                            setActionParameters((current) => ({
                              ...current,
                      ...
                      <label className="text-xs font-semibold">
                        ...
                        <input
                          ...
                          max="100"
                          className="mt-1 w-full rounded-lg border border-slate-200 bg-white px-2 py-1.5 dark:border-white/10 dark:bg-slate-950"
                          value={Number(actionParameters.xMax ?? 10)}
                          onChange={(event) =>
                            setActionParameters((current) => ({
                              ...current,
                ...
                <div className="mb-3 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-xs text-slate-500">
                      {mathRecognitionProvider.production
                        ? "Production provider"
                        : "Development adapter — mock results"}
                    </p>
                  </div>
                  <span
                    className={`mini-chip ${recognitionStatus === "error" ? "text-rose-600" : ""}`}
                  >
                    {recognitionStatus}
                  </span>
                </div>
                ...
                {recognition && (
                  <div className="mb-3 grid grid-cols-2 gap-2 text-sm">
                    ...
                    <div className="rounded-lg bg-slate-100 p-2 dark:bg-white/5">
                      ...
                      {recognition.detectedType ?? "unknown"}
                    </div>
                  </div>
                )}
                ...
                <MathKeyboardInput
                  value={latex}
                  onChange={setLatex}
                  label="Editable recognized LaTeX"
                  mode="formula"
                  rows={2}
                  defaultCompact
                />
                <div
                  className="mt-3 min-h-20 overflow-x-auto rounded-lg bg-slate-100 p-3 text-center dark:bg-white/5"
                  aria-label="Rendered recognition preview"
                >
                  {preview.valid ? (
                    <div dangerouslySetInnerHTML={{ __html: preview.html }} />
                  ) : (
                    <p className="text-sm text-rose-600">
                      Invalid LaTeX: {preview.error}
                    </p>
                  )}
                </div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-2">
                      {recognition.alternatives.map((alternative) => (
                        <button
                          type="button"
                          className="tool-button"
                          key={alternative.latex}
                          onClick={() => setLatex(alternative.latex)}
                        >
                          {alternative.latex}{" "}
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      setRecognitionHistory((items) =>
                        [corrected, ...items].slice(0, 12),
                      );
                      setMessage("Correction applied");
                    }}
                    disabled={!recognition || !preview.valid}
                  >
                  ...
                  <div className="mt-2 space-y-1">
                    {recognitionHistory.map((item) => (
                      <button
                        key={item.id}
                        type="button"
                        className="flex w-full items-center justify-between rounded-lg px-2 py-2 text-left text-sm hover:bg-slate-100 dark:hover:bg-white/5"
                        onClick={() => {
                          setRecognition(item);
                          setLatex(item.latex);
                        }}
        ...
        <SectionCard
          ...
          description="P/E/V/H switch tools. Ctrl/Cmd+Z undoes, Ctrl/Cmd+Shift+Z redoes, Ctrl/Cmd+S saves, and Ctrl/Cmd+Enter recognizes the selection."
        >
          <div className="flex flex-wrap gap-2 text-xs text-slate-500">
            <span className="mini-chip">
              <Brush className="h-3.5 w-3.5" />
              Pressure-aware vector ink
            </span>
            <span className="mini-chip">
...
export function renderBoardLatex(latex: string): {
  valid: boolean;
  html: string;
  error?: string;
} {
  ...
  try {
    ...
    return {
      valid: false,
      html: "",
      error: error instanceof Error ? error.message : "Malformed expression",
    };
  }
}
