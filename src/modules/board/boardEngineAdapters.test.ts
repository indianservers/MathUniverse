...
describe("Board existing-engine adapters", () => {
  it("translates factor and solve requests into the certified solver/CAS", async () => {
    ...
    const factored = await executeBoardAction({
      action: "factor",
      analysis,
      parameters: { variable: "x" },
    });
    const roots = await executeBoardAction({
      action: "find-roots",
      analysis,
      parameters: { variable: "x" },
    });
    expect(factored.exactOutputLatex?.replace(/\s/g, "")).toMatch(
      /\(x-2\).*\(x-3\)|\(x-3\).*\(x-2\)/,
    );
  ...
  it("builds an interactive graph configuration through graphSampler", async () => {
    ...
    const graph = await executeBoardAction({
      action: "plot-2d",
      analysis,
      parameters: { xMin: -5, xMax: 5, yMin: -5, yMax: 10 },
    });
  ...
  it("routes implicit equations and geometry to existing workspaces", async () => {
    ...
    const graph = await executeBoardAction({
      action: "plot-implicit",
      analysis,
      parameters: {},
    });
    const geometry = await executeBoardAction({
      action: "geometry",
      analysis,
      parameters: {},
    });
  ...
  it("normalizes cancellation, timeout, singular, and parsing errors", () => {
    expect(
      normalizeBoardEngineError(new DOMException("cancelled", "AbortError"))
        .code,
    ).toBe("CANCELLED");
    expect(normalizeBoardEngineError(new Error("engine timeout")).code).toBe(
      "TIMEOUT",
    );
    expect(normalizeBoardEngineError(new Error("singular matrix")).code).toBe(
      "SINGULAR_MATRIX",
    );
    expect(
      normalizeBoardEngineError(new Error("invalid expression parse")).code,
    ).toBe("PARSING_ERROR");
  });
