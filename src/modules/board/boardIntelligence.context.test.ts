...
describe("Board intelligence context", () => {
  it("includes only the selection and directly linked elements", () => {
    ...
    board.elements.push(
      expression("linked", "(x-2)(x-3)=0", 300, 20),
      expression("unrelated", "y=9", 20, 400),
    );
    ...
    expect(context.elements.map((item) => item.id)).toEqual([
      "quadratic",
      "linked",
    ]);
    expect(context.elements.some((item) => item.id === "unrelated")).toBe(
      false,
    );
  ...
  it("preserves logical top-to-bottom reading order", () => {
    ...
    const context = buildBoardIntelligenceContext({
      document: board,
      selectedElementIds: ["third", "first", "second"],
    });
    expect(context.elements.map((item) => item.id)).toEqual([
      "first",
      "second",
  ...
  it("identifies an active quadratic problem and local recommendations", () => {
    ...
    expect(result.recommendations.map((item) => item.action)).toEqual(
      expect.arrayContaining(["factor", "find-roots", "plot-2d"]),
    );
  ...
  it("extracts low-confidence recognition ambiguity and reuses a problem-scoped resolution", () => {
    ...
    let context = buildBoardIntelligenceContext({
      document: board,
      selectedElementIds: ["uncertain"],
    });
    expect(context.pendingAmbiguities[0]?.requiresResolution).toBe(true);
    board.intelligence = resolveBoardAmbiguity(
      board.intelligence,
  ...
  it("suppresses dismissed and similar recommendations through Board-scoped memory", () => {
    ...
    const first = understandBoardContext(
      context,
      createBoardSessionMemory(board.id),
    );
    const target = first.recommendations.find(
      (item) => item.action === "factor",
    )!;
    const persistence = dismissBoardRecommendation(
      board.intelligence,
      target.id,
      "dismiss",
      target.category,
    ...
    expect(second.recommendations.some((item) => item.id === target.id)).toBe(
      false,
    );
  });
  ...
  it("reports unavailable subject engines honestly", () => {
    ...
    expect(result.recommendations[0]?.disabledReason).toContain(
      "No verified English engine",
    );
  });
    ...
    board.elements.push(
      {
        id: "prompt",
        type: "text",
        text: "A car starts from rest. Find its final speed.",
        bounds: { x: 10, y: 10, width: 350, height: 50 },
        createdAt: board.createdAt,
      },
      {
        id: "values",
        type: "text",
        text: "velocity u = 0 m/s, acceleration a = 3 m/s^2",
        bounds: { x: 10, y: 80, width: 350, height: 50 },
        createdAt: board.createdAt,
      },
    );
    const context = buildBoardIntelligenceContext({
      document: board,
      selectedElementIds: ["prompt", "values"],
    });
    const result = understandBoardContext(
      context,
      board.intelligence.sessionMemory,
  ...
  it("disables recommendations when a deterministic engine is partially unavailable", () => {
    ...
    expect(
      result.recommendations.find((item) => item.action === "factor"),
    ).toMatchObject({
      enabled: false,
      disabledReason: "Existing CAS is unavailable.",
    });
    expect(
      result.recommendations.find((item) => item.action === "plot-2d")?.enabled,
    ).toBe(true);
  });
});
