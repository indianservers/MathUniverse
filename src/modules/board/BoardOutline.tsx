...
export default function BoardOutline({
  document,
  selectedIds,
  onSelect,
}: {
  ...
  return (
    <section aria-label="Board Outline">
      ...
      <ol className="mt-2 max-h-56 space-y-1 overflow-y-auto">
        {elements.map((element, index) => (
          <li key={element.id}>
            <button
              type="button"
              className={`w-full rounded-lg px-2 py-2 text-left text-sm ${selectedIds.includes(element.id) ? "bg-cyan-50 ring-1 ring-cyan-400 dark:bg-cyan-400/10" : "hover:bg-slate-100 dark:hover:bg-white/5"}`}
      ...
      {!elements.length && (
        <p className="mt-2 text-sm text-slate-500">The Board is empty.</p>
      )}
    </section>
  );
}
...
function describeElement(element: BoardElement) {
  ...
  if (element.type === "solution-step")
    return `Step ${element.order + 1}: ${element.latex} · ${element.verificationStatus ?? "unverified"}`;
  if (element.type === "math-result")
    return `${element.title}: ${element.exactOutputLatex ?? element.plainTextOutput ?? element.status}`;
  if (element.type === "image")
    return `Imported image with ${element.recognitionRegions.length} regions`;
