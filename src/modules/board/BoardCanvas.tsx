...
export default function BoardCanvas({
  ...
  const redraw = useCallback(() => {
    ...
    if (gesture?.kind === "select") {
      if (gesture.points?.length)
        drawLasso(context, gesture.points, document.viewport);
      else
        drawSelectionBox(
          context,
    ...
    if (gesture?.kind === "move") {
      ...
      const bounds = unionBounds(
        document.elements
          .filter((item) => gesture.ids.includes(item.id))
          .map((item) => ({
            ...item.bounds,
            x: item.bounds.x + dx,
            y: item.bounds.y + dy,
  ...
  const handlePointerDown = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (tool === "pen" || tool === "highlighter") {
      ...
      const hit = [...document.elements]
        .reverse()
        .find((element) => hitElement(element, point));
  ...
  const handlePointerMove = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (gesture.kind === "select" || gesture.kind === "move") {
      ...
      if (gesture.kind === "select" && gesture.points)
  ...
  const finishGesture = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    if (gesture.kind === "draw" && gesture.points.length) {
      ...
      const ids = document.elements
        .filter((element) => {
          if (!gesture.points?.length)
            return boxesIntersect(element.bounds, box);
          const center = {
      ...
      if (Math.abs(dx) > 0.1 || Math.abs(dy) > 0.1)
        onMoveSelection(gesture.ids, dx, dy);
    }
  ...
  const eraseAt = (point: BoardPoint) => {
    const hit = document.elements.filter((element) =>
      hitElement(element, point, 12 / document.viewport.zoom),
    );
  ...
  return (
    <div
      ref={hostRef}
      className="relative h-[62vh] min-h-[420px] w-full overflow-hidden rounded-xl bg-white shadow-inner dark:bg-slate-950"
    >
      <canvas
        ref={canvasRef}
...
function hitElement(
  ...
  return element.type === "stroke"
    ? strokeHitTest(element, point, radius)
...
function drawElement(
  ...
  context.fillStyle = globalThis.matchMedia?.("(prefers-color-scheme: dark)")
    .matches
    ? "#e2e8f0"
    : "#0f172a";
...
function drawStroke(
  ...
  stroke.points.slice(1).forEach((point, index) => {
    ...
    context.lineWidth =
      stroke.width * viewport.zoom * (0.55 + point.pressure * 0.9);
    context.beginPath();
