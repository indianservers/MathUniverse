...
export type BoardExportFormat =
  "json" | "latex" | "tutor-text" | "png" | "pdf" | "print";
...
export function buildBoardTextExport(
  document: BoardDocument,
  format: Extract<BoardExportFormat, "json" | "latex" | "tutor-text">,
  options: BoardExportOptions,
) {
  const selected = options.selectedElementIds?.length
    ? document.elements.filter((element) =>
        options.selectedElementIds?.includes(element.id),
      )
    : document.elements;
  if (format === "json") {
    const exportDocument = options.includeTutor
      ? document
    ...
    return selected
      .filter(
        (element) =>
          ...
          (element.type === "math-result" && element.exactOutputLatex),
      )
      .map((element) => {
  ...
  return options.includeTutor
    ? document.tutorMessages
        .map(
          (message) =>
            `[${message.role.toUpperCase()} · ${message.mode}] ${message.text}`,
        )
        .join("\n\n")
    : "";
}
...
export async function exportBoard(
  ...
  if (format === "json" || format === "latex" || format === "tutor-text") {
    ...
    downloadBlob(
      ...
      `${filename}.${format === "json" ? "json" : format === "latex" ? "tex" : "txt"}`,
    );
    return;
  }
  if (format === "print") {
