...
export type BoardCommand =
  ...
  | {
      type: "background";
      before: BoardDocument["background"];
      after: BoardDocument["background"];
    }
...
function apply(
  ...
  if (command.type === "add") {
    elements = reverse
      ? without(elements, command.elements)
      : merge(elements, command.elements);
    ...
    elements = elements.map((element) =>
      element.id === command.id && element.type === "math-expression"
