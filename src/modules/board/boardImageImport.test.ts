...
describe("Board image intake", () => {
  it("validates supported types and size limits", () => {
    ...
    expect(() =>
      validateBoardImage({ type: "image/gif", size: 2_000 }),
    ).toThrow("UNSUPPORTED_IMAGE");
    expect(() =>
      validateBoardImage({ type: "image/png", size: 9 * 1024 * 1024 }),
  ...
  it("detects separated horizontal ink regions", () => {
    ...
    for (const y of [10, 11, 12, 40, 41, 42])
    ...
    const regions = detectMathRegions({
      width,
      height,
      data,
      colorSpace: "srgb",
  ...
  it("normalizes manual multi-region reading order", () => {
    const base = {
      imageElementId: "image",
      regionType: "single-expression",
      selected: true,
    ...
    expect(normalizeReadingOrder(regions).map((region) => region.id)).toEqual([
      "upper",
      "lower",
    ]);
  });
  ...
  it("creates stable recognition fingerprints", async () => {
    ...
    expect(await fingerprintStrokes(points)).toBe(
      await fingerprintStrokes(points),
    );
    expect(await fingerprintStrokes(points)).not.toBe(
      await fingerprintStrokes([{ x: 9, y: 2 }]),
    );
  });
});
