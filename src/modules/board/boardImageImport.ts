...
export function validateBoardImage(file: Pick<File, "type" | "size">) {
  ...
  if (file.size <= 0 || file.size > BOARD_IMAGE_MAX_BYTES)
...
export async function prepareBoardImage(
  ...
  try {
    const scale = Math.min(
      1,
      BOARD_IMAGE_MAX_DIMENSION / Math.max(bitmap.width, bitmap.height),
    );
    ...
    return {
      ...
      height,
      regions,
      fingerprint: await fingerprintBytes(
...
export function detectMathRegions(
  image: ImageData,
): Array<Omit<BoardRecognitionRegion, "id" | "imageElementId">> {
  ...
  return source.map((band, readingOrder) => {
    ...
    return {
      bounds: {
        ...
        width,
        height:
          Math.min(height, band.end + padding) -
          Math.max(0, band.start - padding),
      },
...
export function createBoardImageElement(
  prepared: PreparedBoardImage,
  source: BoardImageElement["source"],
  bounds: BoundingBox,
  rotation = 0,
): BoardImageElement {
...
export async function fingerprintStrokes(
