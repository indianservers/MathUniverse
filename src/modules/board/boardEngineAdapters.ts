...
export async function executeBoardAction(
  request: BoardEngineRequest,
): Promise<BoardEngineResponse> {
...
export async function verifyBoardExpressions(
  left: string,
  right: string,
  variable = "x",
) {
  ...
  return {
    status: result.verification.equivalent
      ? ("verified" as const)
      : ("not-equivalent" as const),
    explanation: result.detail,
    exactDifferenceLatex: result.steps.find((step) => /difference/i.test(step)),
    counterExample: result.verification.samples.find(
      (sample) => Number(sample.difference) !== 0,
    )?.difference,
  };
}
...
export function normalizeBoardEngineError(error: unknown): BoardEngineError {
  ...
  if ((error as Error)?.name === "AbortError" || /cancel/i.test(message))
    return engineError("CANCELLED", message, "Calculation cancelled.", true);
  if (/timeout/i.test(message))
    return engineError(
      "TIMEOUT",
      message,
      "The calculation took too long. Try a smaller expression.",
      true,
    );
  if (/singular/i.test(message))
    return engineError(
      "SINGULAR_MATRIX",
      message,
      "This matrix is singular, so the requested inverse does not exist.",
      true,
    );
  if (/division by zero|divide by zero/i.test(message))
    return engineError(
      "DIVISION_BY_ZERO",
      message,
      "The expression divides by zero in the selected domain.",
      true,
    );
  if (/unsupported|not supported/i.test(message))
    return engineError(
      ...
      message,
      "That operation is not supported by the existing engine.",
      false,
    );
  if (/parse|invalid|expression/i.test(message))
    return engineError(
      "PARSING_ERROR",
      message,
      "The mathematical expression could not be parsed. Review the recognized notation.",
      true,
    );
  return engineError(
    "UNKNOWN",
    message,
    "The engine could not complete this calculation.",
    true,
  );
}
...
async function runAdapter(
  ...
  if (
    request.action === "plot-2d" ||
    request.action === "plot-implicit" ||
    request.action === "table-of-values"
  )
    return graphAdapter(request);
  if (request.action === "plot-3d")
    return routeAdapter(
      request,
      "/workspace/3d",
      "graph-3d",
      "Existing Three.js 3D Workspace",
    );
...
async function casAdapter(
  request: BoardEngineRequest,
): Promise<BoardEngineResponse> {
  const { solveProblem } =
    await import("../../problem-solver/problemSolverEngine");
  const expression = request.analysis.engineExpression;
  const transformExpression = expressionBody(expression);
  const variable = String(
    request.parameters.variable ?? request.analysis.variables[0] ?? "x",
  );
  ...
  if (!output.result.result || output.classification.kind === "unsupported") {
    throw new Error(
      output.result.unsupportedReason ??
        output.result.warnings[0] ??
  ...
  return {
    ...
    steps: output.result.steps.map((step, index) => ({
      id: `step-${index}`,
      index,
      explanation: step,
    })),
    ...
    engine: {
      ...
      underlyingEngine:
        ...
        "Existing certified problem solver / Nerdamer CAS",
    },
  };
}
...
async function graphAdapter(
  ...
  if (
    request.action === "plot-implicit" ||
    (request.analysis.classification === "equation" &&
      request.analysis.variables.length > 1)
  ) {
    return routeAdapter(
      request,
      `/workspace/graph?q=${encodeURIComponent(request.analysis.engineExpression)}`,
      "graph-2d",
      "Existing Graph Workspace (implicit routing)",
    );
  }
  const { generateTableValues, sampleFunction } =
    await import("../../utils/mathEngine/graphSampler");
  ...
  const table = generateTableValues(
    expression,
    Math.max(xMin, -5),
    Math.min(xMax, 5),
    1,
  ...
  return {
    plainTextOutput:
      request.action === "table-of-values"
        ? `${table.rows.length} table rows generated.`
        : `Interactive graph of ${expression}.`,
    graph: {
      mode: "explicit",
      expression,
      series: [
    ...
    engine: {
      adapter: "graph-2d",
      underlyingEngine: "FunctionGraphCanvas + graphSampler",
    },
  };
}

async function statisticsAdapter(
  request: BoardEngineRequest,
): Promise<BoardEngineResponse> {
  const { solveProblem } =
    await import("../../problem-solver/problemSolverEngine");
  const values = request.analysis.engineExpression
    .replaceAll("[", "")
    .replaceAll("]", "");
  const parsed = values.split(/[\s,;]+/).filter(Boolean);
  if (!parsed.length || parsed.some((value) => !Number.isFinite(Number(value))))
    throw new Error("Statistical data contains a non-numeric value.");
  const output = solveProblem(`statistics ${values}`);
  if (!output.result.result)
    throw new Error("Statistics engine did not return a result.");
  return {
    exactOutputLatex: output.result.result,
    plainTextOutput: output.result.result,
    steps: output.result.steps.map((step, index) => ({
    ...
    engine: {
      adapter: "statistics",
      underlyingEngine: "Existing certified statistics solver",
    },
  };
}

async function verificationAdapter(
  request: BoardEngineRequest,
): Promise<BoardEngineResponse> {
  const comparison = String(request.parameters.compareWith ?? "0");
  const verified = await verifyBoardExpressions(
    request.analysis.engineExpression,
    comparison,
    String(request.parameters.variable ?? "x"),
  );
  return {
    exactOutputLatex:
    ...
    warnings: verified.counterExample
      ? [`Counterexample difference: ${verified.counterExample}`]
      : undefined,
    engine: {
      adapter: "verification",
      underlyingEngine: "symbolicVerifyIdentity",
    },
  };
}
...
function validateRequest(request: BoardEngineRequest) {
  if (
    !request.analysis.engineExpression ||
    request.analysis.engineExpression.length > 2_000
  )
    throw new Error("Invalid expression length.");
  if (request.signal?.aborted)
    throw new DOMException("Calculation cancelled", "AbortError");
  if (
    request.analysis.ambiguities.some(
      (ambiguity) => ambiguity.requiresResolution,
    )
  )
    throw new Error("Resolve mathematical ambiguities before calculation.");
}
...
function stripLimit(expression: string) {
  return (
    expression.replace(
      /^.*?(?:lim|limit)[^(a-z]*[a-z]\s*(?:->|to)\s*[^\s]+\s*/i,
      "",
    ) || expression
  );
}
