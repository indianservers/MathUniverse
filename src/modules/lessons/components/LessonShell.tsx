...
import type { LessonContent, LessonDefinition, LessonLanguageCode, LessonProgress } from "../types";
...
export default function LessonShell({ lesson }: { lesson: LessonDefinition }) {
  ...
  useEffect(() => { setProgress(readLessonProgress(lesson)); setShareStatus(""); }, [lesson]);
  ...
  const usesImmersiveFunctionWorkspace = lesson.id >= 143 && lesson.id <= 152 || lesson.id === 153 || lesson.id === 154 || (lesson.id >= 156 && lesson.id <= 162) || lesson.id === 164 || lesson.id === 257 || lesson.id === 258 || lesson.id === 259 || lesson.id === 260 || lesson.id === 261 || lesson.id === 262 || lesson.id === 263 || lesson.id === 264 || lesson.id === 265 || lesson.id === 266 || lesson.id === 267;
  ...
  if (usesImmersiveFunctionWorkspace || usesImmersiveDynamicGeometryWorkspace) {
    return (
      <div className="lesson-page-shell space-y-3" data-testid="lesson-page" data-lesson-id={lesson.id}>
        ...
        {usesImmersiveDynamicGeometryWorkspace || lesson.id === 257 || lesson.id === 258 || lesson.id === 259 || lesson.id === 260 || lesson.id === 261 || lesson.id === 262 || lesson.id === 263 || lesson.id === 264 || lesson.id === 265 || lesson.id === 266 || lesson.id === 267 ? null : <nav className="lesson-adjacent-nav grid gap-3 sm:grid-cols-2" aria-label="Adjacent lessons">
    ...
    <div className="lesson-page-shell space-y-3" data-testid="lesson-page" data-lesson-id={lesson.id}>
      <header className="lesson-shell-header overflow-hidden rounded-2xl border border-slate-200 bg-white/95 shadow-xl shadow-cyan-950/5 backdrop-blur dark:border-white/10 dark:bg-slate-950/80">
        <div className="p-4 sm:p-5">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div className="min-w-0">
              <div className="lesson-topic-pills flex flex-wrap items-center gap-2 text-[10px] font-black uppercase tracking-wide">
                <Link className="lesson-topic-pill is-primary rounded-full bg-cyan-50 px-2.5 py-1 text-cyan-700 transition hover:bg-cyan-100 dark:bg-cyan-300/10 dark:text-cyan-100" to={`/lessons/${lesson.categorySlug}`}>{lesson.category}</Link>
                <span className="lesson-topic-pill rounded-full bg-slate-100 px-2.5 py-1 text-slate-600 dark:bg-white/10 dark:text-slate-300">{lesson.topic}</span>
              </div>
              <h1 className="lesson-shell-title mt-2 break-words text-2xl font-black leading-tight text-slate-950 dark:text-white sm:text-3xl">{lesson.title}</h1>
              <p className="lesson-shell-purpose mt-2 max-w-4xl text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300">{lesson.purpose}</p>
            ...
            <div className="lesson-shell-actions flex flex-wrap gap-2">
              <label className="lesson-language-select inline-flex min-h-10 items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 text-xs font-black text-slate-700 dark:border-white/10 dark:bg-white/10 dark:text-slate-200" title="Load a lesson language pack on demand">
          ...
          {shareStatus ? <p className="mt-3 rounded-lg bg-emerald-50 p-2 text-xs font-black text-emerald-800 dark:bg-emerald-300/10 dark:text-emerald-100" role="status">{shareStatus}</p> : null}
        </div>
      </header>
...
function buildShareUrl(state: LessonVisualState) {
  const href = typeof window === "undefined" ? "http://localhost/lessons" : window.location.href;
...
function LessonContentPanel({ lesson, content, language, status, activeTab, strengthenedLesson }: { lesson: LessonDefinition; content: LessonContent; language: (typeof lessonLanguageOptions)[number]; status: string; activeTab: LessonInfoTab; strengthenedLesson: StrengthenedLesson | null }) {
  return (
    <section id={`lesson-panel-${activeTab}`} role="tabpanel" className="space-y-3 rounded-2xl border border-slate-200 bg-white/90 p-4 text-[15px] shadow-lg shadow-cyan-950/5 dark:border-white/10 dark:bg-slate-950/70" aria-label="Lesson learning content" lang={language.code} dir={language.direction ?? "ltr"}>
      {activeTab === "learn" ? <div className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_300px]">
        <div className="rounded-xl border border-cyan-100 bg-gradient-to-br from-cyan-50 via-white to-violet-50 p-4 dark:border-cyan-300/20 dark:from-cyan-300/10 dark:via-slate-950 dark:to-violet-300/10">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <SectionTitle icon={<BookOpen className="h-4 w-4" />} label="Introduction" />
          ...
          <p className="mt-3 text-base leading-7 text-slate-700 dark:text-slate-200">{content.summary}</p>
          <p className="mt-2 text-base leading-7 text-slate-600 dark:text-slate-300">{content.explanation}</p>
          <p className="mt-3 rounded-xl border border-cyan-100 bg-white/80 p-3 text-base font-bold leading-7 text-cyan-950 dark:border-cyan-400/20 dark:bg-cyan-400/10 dark:text-cyan-100">{content.workedConnection}</p>
        </div>
        ...
        <DetailedLessonExplanation lesson={lesson} content={content} strengthenedLesson={strengthenedLesson} />
      </div> : null}

      {activeTab === "examples" ? <div id="lesson-examples" className="grid gap-3 lg:grid-cols-[minmax(0,1fr)_minmax(280px,0.9fr)]">
          ...
          <div className="mt-3 grid gap-2">
            {content.realWorldExamples.map((example, index) => (
              <article key={example} className="flex gap-3 rounded-xl border border-slate-200 bg-slate-50 p-3 transition hover:border-amber-200 hover:bg-amber-50 dark:border-white/10 dark:bg-white/10 dark:hover:bg-amber-300/10">
                ...
                <p className="text-base font-semibold leading-7 text-slate-700 dark:text-slate-200">{example}</p>
              </article>
            ))}
          </div>
          <ul className="mt-3 space-y-2">
            {content.keyIdeas.map((idea, index) => <li key={idea} className="flex gap-2 rounded-xl bg-slate-50 p-3 text-base font-semibold leading-7 text-slate-700 dark:bg-white/10 dark:text-slate-200"><CheckCircle2 className="mt-1 h-4 w-4 shrink-0 text-cyan-600" /><span><strong>Idea {index + 1}:</strong> {idea}</span></li>)}
          </ul>
        </section>
        ...
        <div className="space-y-3">
          <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
            ...
            <ol className="mt-3 space-y-2">
              {content.controlGuide.map((step, index) => <li key={step} className="flex gap-2 text-base leading-7 text-slate-600 dark:text-slate-300"><span className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-slate-100 text-sm font-black text-slate-700 dark:bg-white/10 dark:text-slate-100">{index + 1}</span><span>{step}</span></li>)}
            </ol>
            <div className="mt-3 rounded-xl border border-dashed border-cyan-200 bg-cyan-50 p-3 text-sm font-bold leading-6 text-cyan-900 dark:border-cyan-300/20 dark:bg-cyan-300/10 dark:text-cyan-100"><Keyboard className="mr-1 inline h-3.5 w-3.5" />{lesson.contract.keyboardAlternative}</div>
          </section>
          <LessonVisualBrief lesson={lesson} strengthenedLesson={strengthenedLesson} />
        </div>
      </div> : null}

      {activeTab === "formulas" ? <section id="lesson-formulas" className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
            <div className="flex flex-wrap items-center justify-between gap-2">
      ...
      {activeTab === "more" ? <details id="lesson-know-more" className="group rounded-xl border border-violet-200 bg-violet-50/70 p-2 dark:border-violet-300/20 dark:bg-violet-300/10" open>
        ...
        <div className="grid gap-2 px-2 pb-2 pt-1 sm:grid-cols-2">
          {content.knowMore.map((item) => <p key={item} className="rounded-lg bg-white/85 p-3 text-base font-semibold leading-7 text-slate-700 dark:bg-slate-950/50 dark:text-slate-200">{item}</p>)}
        </div>
      </details> : null}
    </section>
  );
...
function DetailedLessonExplanation({ lesson, content, strengthenedLesson }: { lesson: LessonDefinition; content: LessonContent; strengthenedLesson: StrengthenedLesson | null }) {
  const definitions = strengthenedLesson?.definitions.length ? strengthenedLesson.definitions.map((item) => item.statement) : [`${lesson.title} means: ${lesson.description}`];
  const vocabulary = strengthenedLesson?.keyVocabulary.length ? strengthenedLesson.keyVocabulary : [
    { term: lesson.contract.concept, meaning: lesson.outcome },
    { term: lesson.workspace, meaning: `The workspace representation used to inspect ${lesson.title}.` },
  ];
  const facts = strengthenedLesson?.facts.length ? strengthenedLesson.facts.map((item) => item.statement) : content.keyIdeas;
  const worked = strengthenedLesson?.workedExamples[0];
  const misconception = strengthenedLesson?.misconceptions[0];
  const practice = strengthenedLesson?.practice.slice(0, 3) ?? [];
  const restrictions = strengthenedLesson?.conditionsAndRestrictions.length ? strengthenedLesson.conditionsAndRestrictions : content.formulas.flatMap((item) => item.explanation).slice(0, 2);
  const formulaItems = strengthenedLesson?.formulas.length ? strengthenedLesson.formulas.map((item) => {
    const variables = item.variables.map((variable) => `${variable.symbol}: ${variable.meaning}`).join("; ");
    const restrictionsText = item.restrictions?.length ? ` Restrictions: ${item.restrictions.join("; ")}` : "";
    return `${item.label}: ${item.expression}. ${variables}${restrictionsText}`;
  }) : content.formulas.map((item) => `${item.label}: ${item.expression}. ${item.explanation}`);
  return (
    <section className="lg:col-span-2 rounded-2xl border border-slate-200 bg-white/90 p-4 dark:border-white/10 dark:bg-slate-950/60" aria-label="Detailed explanation">
      <div className="flex flex-wrap items-center justify-between gap-2">
        <SectionTitle icon={<NotebookText className="h-4 w-4" />} label="Detailed explanation" muted />
        <span className="rounded-full bg-cyan-50 px-2.5 py-1 text-[10px] font-black uppercase text-cyan-700 dark:bg-cyan-300/10 dark:text-cyan-100">Introduction first, depth next</span>
      </div>
      <div className="mt-3 grid gap-3 xl:grid-cols-[minmax(0,1.2fr)_minmax(260px,0.8fr)]">
        <div className="space-y-3">
          <ExplanationArticle title={`What ${lesson.title} means`} items={definitions} />
          <ExplanationArticle title="How to read it" items={[strengthenedLesson?.howItWorks ?? content.controlGuide.join(" "), strengthenedLesson?.whyItWorks ?? content.workedConnection]} />
          {worked ? (
            <article className="rounded-xl border border-cyan-100 bg-cyan-50/70 p-3 dark:border-cyan-300/20 dark:bg-cyan-300/10">
              <h3 className="text-sm font-black text-cyan-950 dark:text-cyan-100">Worked example</h3>
              <p className="mt-2 text-sm font-bold leading-6 text-slate-700 dark:text-slate-200">{worked.prompt}</p>
              <ol className="mt-2 space-y-1">
                {worked.steps.map((step, index) => <li key={step} className="text-sm leading-6 text-slate-600 dark:text-slate-300"><strong>Step {index + 1}:</strong> {step}</li>)}
              </ol>
              <p className="mt-2 rounded-lg bg-white/85 p-2 text-sm font-black text-cyan-900 dark:bg-slate-950/50 dark:text-cyan-100">Answer: {worked.answer}</p>
            </article>
          ) : null}
        </div>
        <aside className="space-y-3">
          <article className="rounded-xl border border-slate-200 bg-slate-50 p-3 dark:border-white/10 dark:bg-white/10">
            <h3 className="text-sm font-black text-slate-900 dark:text-white">Vocabulary</h3>
            <dl className="mt-2 space-y-2">
              {vocabulary.slice(0, 5).map((item) => <div key={item.term}><dt className="text-xs font-black uppercase text-cyan-700 dark:text-cyan-200">{item.term}</dt><dd className="text-sm leading-6 text-slate-600 dark:text-slate-300">{item.meaning}</dd></div>)}
            </dl>
          </article>
          <ExplanationArticle title="Must remember" items={facts.slice(0, 4)} compact />
          {formulaItems.length ? <ExplanationArticle title="Formula meaning" items={formulaItems.slice(0, 3)} compact /> : null}
          <ExplanationArticle title="Conditions" items={restrictions.slice(0, 4)} compact />
          {misconception ? <article className="rounded-xl border border-amber-200 bg-amber-50 p-3 dark:border-amber-300/20 dark:bg-amber-300/10"><h3 className="text-sm font-black text-amber-950 dark:text-amber-100">Common mistake</h3><p className="mt-2 text-sm leading-6 text-slate-700 dark:text-slate-200">{misconception.mistake}</p><p className="mt-2 text-sm font-black leading-6 text-amber-900 dark:text-amber-100">{misconception.correction}</p></article> : null}
          {practice.length ? <ExplanationArticle title="Try next" items={practice.map((item) => item.prompt)} compact /> : null}
        </aside>
      </div>
    </section>
  );
...
function ExplanationArticle({ title, items, compact = false }: { title: string; items: string[]; compact?: boolean }) {
  return (
    <article className="rounded-xl border border-slate-200 bg-slate-50/80 p-3 dark:border-white/10 dark:bg-white/10">
      ...
      <div className={compact ? "mt-2 space-y-1.5" : "mt-2 space-y-2"}>
        {items.map((item) => <p key={item} className={compact ? "text-sm font-semibold leading-6 text-slate-600 dark:text-slate-300" : "text-base leading-7 text-slate-700 dark:text-slate-200"}>{item}</p>)}
      </div>
    </article>
  );
}

function LessonTabBar({ active, onChange }: { active: LessonInfoTab; onChange: (tab: LessonInfoTab) => void }) {
  const tabs: Array<{ id: LessonInfoTab; label: string; icon: ReactNode }> = [
    { id: "interaction", label: "Interaction + visualization", icon: <Eye className="h-3.5 w-3.5" /> },
    { id: "learn", label: "Explain", icon: <NotebookText className="h-3.5 w-3.5" /> },
    { id: "examples", label: "Examples", icon: <Lightbulb className="h-3.5 w-3.5" /> },
    { id: "formulas", label: "Formulas", icon: <Sigma className="h-3.5 w-3.5" /> },
    { id: "more", label: "Know more", icon: <Sparkles className="h-3.5 w-3.5" /> },
  ];
  ...
  return (
    <nav className="mobile-safe-scroll flex gap-2 rounded-2xl border border-slate-200 bg-white/85 p-2 shadow-sm dark:border-white/10 dark:bg-slate-950/70" role="tablist" aria-label="Lesson content tabs">
      {tabs.map((tab) => (
        <button
          key={tab.id}
          type="button"
          ...
          className={active === tab.id ? "inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl bg-cyan-600 px-4 text-sm font-black text-white shadow" : "inline-flex min-h-10 shrink-0 items-center gap-2 rounded-xl px-4 text-sm font-black text-slate-600 transition hover:bg-cyan-50 hover:text-cyan-800 dark:text-slate-300 dark:hover:bg-cyan-300/10 dark:hover:text-cyan-100"}
        >
          {tab.icon}{tab.label}
        </button>
      ))}
