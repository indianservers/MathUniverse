...
export default function BaseSystemsTargetLesson73({
  ...
  const changeBase = (value: number) => {
    ...
    setBase(nextBase);
    setDigits((current) =>
      current.map((digit) => Math.min(digit, nextBase - 1)),
    );
    setPracticeLoaded(false);
    act();
  };
    ...
    >
      ...
      <main className="base73-main">
        <section className="base73-board">
          <header>
            <h2>
              Convert Binary {numberText}
              <sub>{base}</sub> to Decimal
            </h2>
            <p>Use place values, multiply, and add.</p>
          </header>
          <section className="base73-place-grid">
            {digits.map((digit, index) => (
              <article key={index}>
                ...
                <i>↓</i>
                <strong>
                  {digit} × {placeValues[index]}
                </strong>
              </article>
            ))}
            <svg viewBox="0 0 390 86" aria-hidden="true">
              <path d="M65 4v14q0 30 45 30h50q35 0 35 31M325 4v14q0 30-45 30h-50q-35 0-35 31M195 4v75" />
            </svg>
          </section>
          <section className="base73-sum" aria-label="Decimal sum">
            {products.map((product, index) => (
              <span key={index}>
                <b>{product}</b>
                {index < products.length - 1 ? <i>+</i> : null}
              </span>
            ))}
            <i>=</i>
            <strong>{decimal}</strong>
          </section>
          <section className="base73-verified">
            <Check />
            <b>
              {numberText}
              <sub>{base}</sub> = {decimal}
              <sub>10</sub>
            </b>
        ...
        <aside className="base73-side">
          <section className="base73-summary">
            ...
            <p>
              <ClipboardList /> Decimal value:<b>{decimal}</b>
            </p>
          </section>
          <section className="base73-allowed">
            <header>
              <CheckCircle2 />
              <h3>Allowed digits: {allowedDigits.join(" and ")}</h3>
            </header>
            <p>
              Valid digits for base {base} are {allowedDigits.join(" and ")}.
            </p>
            <div>
              {allowedDigits.map((digit) => (
                <button
                  type="button"
                  aria-label={`Set selected digit to ${digit}`}
                  className={digits[selectedIndex] === digit ? "active" : ""}
                  onClick={() => changeDigit(selectedIndex, digit)}
                  key={digit}
                >
                  {digit}
                </button>
          ...
          <section className="base73-why">
            <h3>
              <Info /> Why this works
            </h3>
            <p>
              <b>2</b>Each place is a power of the base.
            </p>
            <p>
              <CheckCircle2 />
          ...
          <section className="base73-number-line">
            <h3>Number line (decimal)</h3>
            <NumberLine value={decimal} maximum={lineMaximum} />
          </section>
          <section className="base73-practice">
            <header>
              <Rocket />
              <h3>Try it yourself</h3>
            </header>
            <p>
              Try: Convert 101<sub>2</sub> to decimal.
            </p>
            <button type="button" onClick={loadPractice}>
              {practiceLoaded ? `101₂ = ${decimal}` : "Start Practice"}
              <ArrowRight />
            </button>
          </section>
        </aside>
      ...
      <nav className="base73-navigation">
        <a href="/lessons/numbers-and-arithmetic/72-modular-arithmetic">
          <ArrowLeft />
          <span>
            PREVIOUS<b>Modular Arithmetic</b>
          </span>
        </a>
        <a href="/lessons/numbers-and-arithmetic/74-continued-fractions">
          <span>
            NEXT<b>Continued Fractions</b>
          </span>
          <ArrowRight />
        </a>
      </nav>
      ...
      <LessonTopicStudyBoard lessonId={73} view={tab} onInteraction={onInteraction} />
...
function NumberLine({ value, maximum }: { value: number; maximum: number }) {
  ...
  return (
    <svg
      viewBox="0 0 328 70"
      role="img"
      aria-label={`Decimal number line marker at ${value}`}
    >
      <line x1="18" y1="35" x2="310" y2="35" />
      {ticks.map((tick, index) => (
        <g key={index}>
          <line x1={18 + index * 58.4} x2={18 + index * 58.4} y1="29" y2="42" />
          <text x={18 + index * 58.4} y="62">
            {Number.isInteger(tick) ? tick : tick.toFixed(1)}
          </text>
        </g>
      ))}
      <circle cx={x} cy="27" r="6" />
      <text className="selected" x={x} y="62">
        {value}
      </text>
    </svg>
  );
