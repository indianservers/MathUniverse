...
export const twoDiceOutcomes: DiceOutcome[] = Array.from(
  { length: 6 },
  (_, first) =>
...
export function inEventB(
  [first, second]: DiceOutcome,
...
export function additionRuleSummary(mutuallyExclusive: boolean) {
  ...
  const b = twoDiceOutcomes.filter((outcome) =>
    inEventB(outcome, mutuallyExclusive),
  );
  const intersection = a.filter((outcome) =>
    inEventB(outcome, mutuallyExclusive),
  );
