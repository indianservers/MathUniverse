...
describe("Data Types dedicated adapter routing and surface", () => {
  function render() {
    return renderToStaticMarkup(<StatisticsLessonAdapter lesson={lessonCatalog.find(lesson => lesson.id === 467)!} resetToken={0} onInteraction={vi.fn()} />);
  }
  it("routes 467 to its own classification surface, not generic statistics controls", () => {
    ...
    expect(html).toContain("Classify the variables and match to the best graph");
    for (const text of ["Shift all data", "Add spread", "Extreme value", "Editable dataset", "Current model"]) expect(html).not.toContain(text);
  ...
  it("renders four graph previews, reference content and eight unfilled practice selects", () => {
    const html = render();
    for (const name of ["Bar chart", "Pie chart", "Dot plot", "Histogram"]) expect(html).toContain(`aria-label="${name} example"`);
    for (const text of ["Definition", "Classify correctly", "Quick reference", "Common mistake", "Check my answers", "0 / 4 correct (not checked)"]) expect(html).toContain(text);
