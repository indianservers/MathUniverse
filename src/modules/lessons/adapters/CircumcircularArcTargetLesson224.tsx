...
export default function CircumcircularArcTargetLesson224({
  ...
  const checkPractice = () => {
    ...
    setPracticeFeedback(
      Math.abs(arc - targetArc) <= 0.05 &&
        Math.abs(angle - targetArc / 2) <= 0.05
        ? "correct"
        : "incorrect",
    );
    onInteraction();
    ...
    >
        ...
        <div className="grid h-full grid-cols-[minmax(340px,1.5fr)_repeat(3,minmax(100px,.6fr))] items-center gap-3">
          <div>
            ...
            <div className="mt-3 flex gap-2">
              ...
              <button
                type="button"
                className="target-circ-header-action"
                onClick={reset}
              >
                <RotateCcw /> Reset
              </button>
      ...
      <nav className="mt-1 grid h-[59px] grid-cols-5 overflow-hidden rounded-xl border border-slate-200 bg-white px-2 shadow-sm">
        {[
          ...
          >
            <span className="grid h-6 w-6 place-items-center rounded-full border border-slate-200 text-[9px] font-black">
              {index + 1}
            </span>
            <span>
      ...
      <section
        id="circ-main"
        className="mt-[18px] grid h-[777px] grid-cols-[minmax(0,1fr)_233px] gap-[11px]"
      >
        <div className="h-[777px] overflow-hidden rounded-xl border border-slate-200 bg-white p-[10px] shadow-sm">
          <div className="flex items-start justify-between">
            <div>
              <h2 className="text-[12px] font-black">
                Three points define a circumcircular arc.
              </h2>
              <p className="mt-1 text-[8px] text-slate-600">
                Drag the points A, B, or C. Observe the arc measure and angle
                relationships.
              </p>
            </div>
            <button
              type="button"
              aria-label="Fullscreen circumcircular arc"
              className="target-circ-icon"
              onClick={() =>
          ...
          <div className="mt-3 grid h-10 grid-cols-4 items-center rounded-lg border border-slate-200 px-2 text-[8px]">
            ...
            />
            <Toggle
              label="Show central ∠AOC"
              checked={showCentral}
              onChange={setShowCentral}
            />
            <Toggle
              label="Show inscribed ∠ABC"
              checked={showInscribed}
              onChange={setShowInscribed}
            />
          </div>
          <div className="relative mt-1 h-[505px] overflow-hidden rounded-lg border border-slate-100">
            ...
            <div className="absolute bottom-3 left-3 flex overflow-hidden rounded-md border bg-white shadow-sm">
              <ToolButton
                active={tool === "select"}
                label="Select and drag points"
                icon={<MousePointer2 />}
                onClick={() => setTool("select")}
              />
              <ToolButton
                active={tool === "point"}
                label="Place missing point"
                icon={<CircleDot />}
                onClick={() => setTool("point")}
              />
              <ToolButton
                active={showSegments}
                label="Toggle triangle segments"
                icon={<Link2 />}
                onClick={() => setShowSegments((value) => !value)}
              />
              <ToolButton
                active={showCircle}
                label="Toggle circumcircle"
                icon={<Circle />}
                onClick={() => setShowCircle((value) => !value)}
              />
              <button
                type="button"
                className="target-circ-tool"
                aria-label={`Remove point ${names[selected]}`}
                onClick={deleteSelected}
              >
                <Trash2 />
              </button>
            </div>
            <label className="absolute bottom-3 right-3 flex h-8 items-center gap-2 rounded-md border bg-white px-3 text-[8px]">
              Grid{" "}
              <input
                type="checkbox"
                aria-label="Circumcircle grid"
                checked={showGrid}
                onChange={(event) => {
          ...
          <div id="circ-measures" className="mt-2 grid grid-cols-5 gap-2">
            ...
            <Metric
              label="Radius"
              value={model ? model.radius.toFixed(2) : "—"}
            />
            <Metric
              label="Arc ⌢ABC"
              value={model ? `${model.arcThroughA.toFixed(2)}°` : "—"}
              tone="text-blue-700"
            />
            <Metric
              label="Central ∠AOC"
              value={model ? `${model.centralAngle.toFixed(2)}°` : "—"}
              tone="text-green-700"
            />
            <Metric
              label="Inscribed ∠ABC"
              value={model ? `${model.inscribedAngle.toFixed(2)}°` : "—"}
        ...
        <aside className="h-[777px] overflow-hidden rounded-xl border border-slate-200 bg-white p-[10px] shadow-sm">
          ...
          <p className="mt-1 text-[8px] text-slate-500">
            Drag sliders or enter coordinates.
          </p>
          ...
          <section className="mt-2 rounded-lg border border-slate-200 p-3 text-[8px]">
            ...
            {model ? (
              <>
                <Measure
                  label="Radius (OA = OB = OC)"
                  value={model.radius.toFixed(2)}
                />
                <Measure
                  label="Arc ⌢ABC (through A)"
                  value={`${model.arcThroughA.toFixed(2)}°`}
                />
                <Measure
                  label="Complementary arc"
                  value={`${(360 - model.arcThroughA).toFixed(2)}°`}
                />
                <Measure
                  label="Central ∠AOC"
                ...
                <Measure
                  label="Inscribed ∠ABC"
                  value={`${model.inscribedAngle.toFixed(2)}°`}
                  tone="text-orange-600"
                />
              </>
            ) : (
              <p className="mt-3 rounded-md bg-rose-50 p-2 text-rose-700">
                Three non-collinear points are required.
              </p>
            )}
          </section>
          <section className="mt-2 rounded-lg border border-amber-200 bg-amber-50 p-3 text-[8px]">
            <h3 className="flex items-center gap-2 font-black text-amber-800">
              <Lightbulb className="h-4 w-4" /> Observation
            </h3>
            <p className="mt-2">
              The central angle is twice the inscribed angle intercepting the
              same arc.
            </p>
            <p className="mt-2 text-center font-serif text-[12px] italic">
              ∠AOC = 2∠ABC
            </p>
          </section>
        </aside>
      ...
      <section className="mt-3 grid h-[257px] grid-cols-[.85fr_1.1fr_1.3fr] gap-2">
        <article className="target-circ-card">
          ...
          {[
            ...
            <p key={step} className="target-circ-step">
              <b>{index + 1}</b>
              {step}
            </p>
          ))}
          <button type="button" className="target-circ-replay" onClick={replay}>
            <Play />
            {replaying ? `Step ${replayStage} of 5` : "Replay Steps"}
          </button>
        </article>
        <article id="circ-why" className="target-circ-card">
          <h2>Why it works</h2>
          <p>
            All points on the circle are equidistant from the center O. The
            central angle ∠AOC subtends arc AC. The inscribed angle ∠ABC
            subtends the same arc, so:
          </p>
          <div className="target-circ-formula">m∠AOC = 2m∠ABC</div>
          <p className="mt-5">Therefore,</p>
          <p className="mt-3 text-center font-serif text-[11px] italic">
            m⌢AC = m∠AOC = 2m∠ABC
          </p>
        </article>
        <article id="circ-practice" className="target-circ-card">
          ...
          <p className="mt-3">
            ...
            Adjust the points to get arc AC ≈ {targetArc}°.
          </p>
          <label className="mt-2 flex items-center gap-2">
            <input
              type="checkbox"
              checked={practiceFeedback === "correct"}
              onChange={() => setPracticeFeedback("idle")}
            />{" "}
            Record the inscribed angle.
          </label>
          <label className="mt-2 flex items-center gap-2">
            <input
              type="checkbox"
          ...
          <PracticeField
            label={`Arc AC (target ${targetArc}°)`}
            aria="Practice arc measure"
            value={practiceArc}
            onChange={(value) => {
              setPracticeArc(value);
              setPracticeFeedback("idle");
            }}
          />
          <PracticeField
            label="Inscribed ∠ABC"
            aria="Practice inscribed angle"
            value={practiceAngle}
            onChange={(value) => {
          ...
          <p
            role="status"
            className={`mt-2 rounded-md px-2 py-1 font-black ${practiceFeedback === "correct" ? "bg-green-50 text-green-700" : practiceFeedback === "incorrect" ? "bg-rose-50 text-rose-700" : "bg-slate-50 text-slate-500"}`}
          >
            {practiceFeedback === "correct"
              ? `Well done! Arc ≈ ${targetArc}° and ∠ABC ≈ ${targetArc / 2}°.`
              : practiceFeedback === "incorrect"
                ? "Use the inscribed-angle theorem: divide the arc by 2."
                : "Enter both measurements, then check."}
          </p>
          <button
            type="button"
            className="target-circ-new"
      ...
      <nav
        className="mt-[11px] grid h-12 grid-cols-2 gap-3"
        aria-label="Adjacent lessons"
      >
        <a
          className="target-circ-nav"
          href="/lessons/geometry/223-circular-arc"
        >
          <ArrowLeft />
          <span>
            <b>Previous</b>Circular Arc
          </span>
        </a>
        <a
          className="target-circ-nav justify-end text-right"
          href="/lessons/geometry/225-circular-sector"
        >
          <span>
            <b>Next</b>Circular Sector
          </span>
          <ArrowRight />
        </a>
      </nav>
...
function HeaderFact({
  ...
  return (
    <div className="flex h-12 items-start gap-2 rounded-md border border-slate-200 p-2 text-[8px]">
...
function Toggle({
  ...
  return (
    <label className="flex items-center gap-2">
      ...
      />
      <span
        className={`relative h-4 w-8 rounded-full ${checked ? "bg-blue-600" : "bg-slate-300"}`}
      >
        <span
          className={`absolute top-0.5 h-3 w-3 rounded-full bg-white transition-transform ${checked ? "translate-x-[17px]" : "translate-x-0.5"}`}
        />
      </span>
      {label}
    </label>
...
function CircumCanvas({
  ...
  return (
    <svg
      ...
      onClick={(event) => {
        if (tool === "point")
          onRestore(domain(event.currentTarget, event.clientX, event.clientY));
      }}
    >
      <defs>
        <pattern
      ...
      <rect
        width="500"
        height="500"
        fill={showGrid ? "url(#circ-grid)" : "white"}
      />
      {model && center && showCircle && (
        <circle
          data-testid="circumarc-circle"
          data-radius={model.radius.toFixed(6)}
          cx={center.x}
          cy={center.y}
          r={model.radius * scale}
          fill="none"
          stroke="#aeb8c7"
          strokeWidth="1.5"
        />
      )}
      {complete && showSegments && (
        <>
          <line
            x1={screenPoints[0]!.x}
            y1={screenPoints[0]!.y}
      ...
      {model && center && showArc && (
        <path
          data-testid="circumarc-through-a"
          data-arc-measure={model.arcThroughA.toFixed(6)}
          d={arcThroughAPath(
            screenPoints as [Point, Point, Point],
            model.radius * scale,
            model,
          )}
          fill="none"
      ...
      {model && center && showCenter && (
        <>
          <circle
            data-testid="circumarc-center"
            cx={center.x}
            cy={center.y}
            r="6"
            fill="#9333ea"
      ...
      {points.map(
        (point, index) =>
          point && (
            <g key={names[index]}>
              <circle
                data-testid={`circumarc-point-${names[index].toLowerCase()}`}
                cx={screenPoints[index]!.x}
                cy={screenPoints[index]!.y}
                r={selected === index ? 8 : 7}
                fill="#1677c8"
                stroke={selected === index ? "#0f172a" : "white"}
                strokeWidth="2"
      ...
      {!model && (
        <text x="250" y="245" textAnchor="middle" fill="#be123c" fontSize="13">
...
function ToolButton({
  ...
  return (
    <button
      ...
      aria-label={label}
      aria-pressed={active}
      className={`target-circ-tool ${active ? "is-active" : ""}`}
      onClick={onClick}
    >
      {icon}
    </button>
...
function PointControls({
  ...
  return (
    <section
      className={`rounded-lg border p-3 text-[8px] ${selected ? "border-blue-300 bg-blue-50/30" : "border-slate-200"}`}
      onClick={onSelect}
    >
      <h3 className="flex items-center gap-2 font-black">
        <span className="h-3 w-3 rounded-full bg-[#1677c8]" />
        Point {name}
        {!point && <span className="ml-auto text-rose-600">missing</span>}
      </h3>
      ...
      <label className="mt-2 grid grid-cols-[12px_1fr] items-center gap-2">
        x
        <input
          type="range"
          aria-label={`Point ${name} x slider`}
          disabled={!point}
          min="-10"
          max="10"
          step="0.1"
      ...
      <label className="mt-1 grid grid-cols-[12px_1fr] items-center gap-2">
        y
        <input
          type="range"
          aria-label={`Point ${name} y slider`}
          disabled={!point}
          min="-10"
          max="10"
          step="0.1"
...
function Metric({
  ...
  return (
    <div className="rounded-md border border-slate-200 p-2 text-center text-[7px]">
      <span>{label}</span>
      <b className={`mt-2 block text-[9px] ${tone}`}>{value}</b>
    </div>
  );
}
function Measure({
...
function PracticeField({
  ...
  return (
    <label className="mt-2 grid grid-cols-[1fr_120px] items-center gap-2">
      <span>{label}</span>
      <input
        aria-label={aria}
        inputMode="decimal"
...
function arcThroughAPath(
  points: [Point, Point, Point],
  radius: number,
  model: NonNullable<ReturnType<typeof circumModel>>,
) {
  const b = points[1],
    c = points[2];
  return `M ${b.x} ${b.y} A ${radius} ${radius} 0 ${model.arcThroughA > 180 ? 1 : 0} ${model.arcThroughAClockwise ? 1 : 0} ${c.x} ${c.y}`;
}
function angleAt(a: Point, vertex: Point, c: Point) {
  const u = { x: a.x - vertex.x, y: a.y - vertex.y },
    v = { x: c.x - vertex.x, y: c.y - vertex.y };
  return radToDeg(
    Math.acos(
      clamp(
...
function angleOf(center: Point, point: Point) {
  return radToDeg(Math.atan2(point.y - center.y, point.x - center.x));
}
function smallerAngle(a: number, b: number) {
  const delta = normalize360(b - a);
...
function clonePoints(points: PointTuple): PointTuple {
  return points.map((point) => (point ? { ...point } : null)) as PointTuple;
}
