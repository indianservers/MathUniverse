...
function guidanceFor(title: string) {
  ...
  if (name.includes("introduction")) return ["Vector rule", "A vector has size and direction.", "It describes movement, not just a point."];
  if (name.includes("component")) return ["Component form", "Write horizontal part first and vertical part second.", "Component order is (x,y)."];
  if (name.includes("position")) return ["Position vector", "A position vector starts at the origin.", "Point (a,b) has vector (a,b)."];
  if (name.includes("addition")) return ["Vector addition", "Add matching components.", "Head-to-tail arrows show the same result."];
  if (name.includes("subtraction")) return ["Vector subtraction", "Subtract matching components in order.", "u-v is different from v-u."];
  if (name.includes("scalar")) return ["Scalar multiplication", "Multiply every component by the scalar.", "A negative scalar reverses direction."];
  if (name.includes("magnitude")) return ["Magnitude", "Length is sqrt(x^2+y^2).", "A unit vector has length 1."];
  if (name.includes("dot product")) return ["Dot product", "Multiply matching components and add.", "The answer is a scalar."];
  if (name.includes("cross product")) return ["Cross product", "In 3D it gives a perpendicular vector.", "Magnitude gives parallelogram area."];
  if (name.includes("projection")) return ["Projection", "Projection is a vector shadow along another vector.", "It keeps only the target-direction part."];
  if (name.includes("linear combinations")) return ["Linear combination", "Scale vectors first, then add.", "This builds new vectors from given directions."];
  if (name.includes("line")) return ["Vector line", "r = a + tv uses a point and direction.", "t is a scalar parameter."];
  if (name.includes("plane")) return ["Vector plane", "A plane needs a point and two non-parallel directions.", "Two parameters move across the plane."];
  if (name.includes("relative motion")) return ["Relative motion", "Subtract velocities to compare from another object.", "Use direction as well as speed."];
  if (name.includes("force")) return ["Force vectors", "Add force components to find resultant force.", "Force has size and direction."];
  return ["Vector rule", "Use components and arrows together.", "Check whether the result is a vector or scalar."];
}
...
export default function VectorLessonAdapter({ lesson, resetToken, onInteraction }: LessonAdapterProps) {
  ...
  if (lesson.id === 186) return <VectorAdditionTargetLesson186 lesson={lesson} resetToken={resetToken} onInteraction={onInteraction} />;
      ...
      <div className="lesson-engine lesson-engine-vector">
        <div className="lesson-engine-axis lesson-direct-surface" data-direct-interaction="true">
          ...
          <svg viewBox="0 0 640 360" className="h-full min-h-[360px] w-full" role="img" aria-label="Linked vector diagram" onPointerDown={updateVectorFromPointer} onPointerMove={updateVectorFromPointer}>
            ...
            <Arrow vector={u} color="#0891b2" marker="url(#cyan-arrow)" label="u" />
            <Arrow vector={v} color="#f59e0b" marker="url(#amber-arrow)" label="v" />
            <Arrow vector={sum as Vector2} color="#8b5cf6" marker="url(#cyan-arrow)" label="u+v" dashed />
        ...
        <div className="lesson-engine-controls">
          ...
          <SliderGroup title="Vector u">
            <SliderControl density="compact" label="u x" value={ux} min={-5} max={5} step={0.5} onChange={update(setUx)} />
            <SliderControl density="compact" label="u y" value={uy} min={-5} max={5} step={0.5} onChange={update(setUy)} />
          </SliderGroup>
          <SliderGroup title="Vector v">
            <SliderControl density="compact" label="v x" value={vx} min={-5} max={5} step={0.5} onChange={update(setVx)} />
            <SliderControl density="compact" label="v y" value={vy} min={-5} max={5} step={0.5} onChange={update(setVy)} />
          </SliderGroup>
          <div className="grid grid-cols-2 gap-2">
            ...
            <Metric label="proj_v u" value={`(${projection.map((n) => n.toFixed(1)).join(", ")})`} />
          </div>
        </div>
      </div>
    </AdapterFrame>
...
function Arrow({ vector, color, marker, label, dashed = false }: { vector: Vector2; color: string; marker: string; label: string; dashed?: boolean }) {
  const x = 320 + vector[0] * 38;
  const y = 180 - vector[1] * 38;
  return <g><line x1="320" y1="180" x2={x} y2={y} stroke={color} strokeWidth="4" markerEnd={marker} strokeDasharray={dashed ? "8 6" : undefined} /><text x={x + 8} y={y - 8} fill={color} fontWeight="900">{label}</text></g>;
}
