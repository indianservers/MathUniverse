...
const ROWS = [
  ...
  },
  {
    ...
    result: "8",
    time: "2 min ago",
    source: "56 is reused from row 1, then divided by 7.",
  },
  {
    id: 3,
    expression: "12 + 5",
    result: "17",
    time: "5 min ago",
    source: "12 and 5 are added directly.",
  },
  {
    id: 4,
    expression: "17 × 4",
    result: "68",
    time: "7 min ago",
    source: "17 is reused from row 3, then multiplied by 4.",
  },
] as const;
  ...
  return (
    ...
    >
      <nav className="history-breadcrumb">
        ...
        <span>›</span>
        <b>17 Calculation History</b>
      </nav>

      <header className="history-header">
        <div className="history-kickers">
          <span>CORE WORKSPACES</span>
          <span>SCIENTIFIC CALCULATOR</span>
        </div>
        <h1>Calculation History</h1>
        <p>
          A reliable record of expressions and results—so you can verify, reuse,
      ...
      <main className="history-main">
        <section className="history-lab">
          <div className="history-lab-title">
            <div>
              <small>CALCULATION HISTORY LAB</small>
              <h2>Every result has a source. Verify before you reuse.</h2>
            </div>
            <strong>△ &nbsp; Do not copy a result without its input</strong>
          </div>
          <div
            className="history-display"
            data-testid="history-equation-overlay"
          >
          ...
          <p className="history-timeline">
            <History size={14} /> History timeline (newest first)
          </p>
          ...
          >
            ...
            {ROWS.map((row) => (
              <div
                className={`history-row ${selectedRow === row.id ? "selected" : ""}`}
                role="row"
                key={row.id}
                ...
                <div className="history-row-actions">
                  <button
                    ...
                    onClick={(event) => {
                      ...
                      void runAction("reuse", row.id);
                    }}
                  >
                    <RotateCcw />
                    <small>Reuse input</small>
                  </button>
                  <button
                    type="button"
                  ...
                  <button
                    type="button"
                    className={pinnedRows.includes(row.id) ? "active" : ""}
                    aria-label={`Pin note for row ${row.id}`}
                    onClick={(event) => {
                      event.stopPropagation();
                      void runAction("pin", row.id);
                    }}
                  >
                    <Pin />
                    <small>Pin note</small>
                  </button>
                  <button
                    type="button"
          ...
          <div className="history-action-key">
            {(Object.keys(ACTION_LABELS) as HistoryAction[]).map((action) => (
              <button
                type="button"
                key={action}
                onClick={() => void runAction(action, active.id)}
              >
                {action === "reuse" ? (
                  <RotateCcw />
                ) : action === "copy" ? (
                ...
                <span>
                  ...
                  <small>
                    {action === "reuse"
                      ? "Use this expression again"
                      : action === "copy"
                        ? "Copy the result value"
                        : action === "pin"
                          ? "Add a quick note"
                          : "View full expression details"}
                  </small>
                </span>
              </button>
            ))}
        ...
        <aside className="history-side">
          <section>
            <small className="history-uppercase">Concept trace</small>
            <h3>Calculation history essentials</h3>
            <div className="history-trace">
              <i>
                <FileText />
              </i>
              <span>
                <small>LATEST INPUT</small>
                <b>{active.expression}</b>
              </span>
            </div>
            <div className="history-trace">
              <i>
            ...
            <div className="history-trace">
              ...
              <span>
                <small>REUSE SAFELY</small>
                <b>Check the source expression</b>
              </span>
            </div>
          </section>
          <section className="history-chain">
            <small>DEPENDENCY CHAIN</small>
            <h3>Reuse only with the source</h3>
            <div>
              <b>7 × 8 = 56</b>
              <span>(original expression)</span>
            </div>
            <em>↓</em>
            <div>
              <b>56 / 7 = 8</b>
              <span>(uses 56 from above)</span>
              <CheckCircle2 />
            </div>
            <em>↓</em>
            <div className="wrong">
              <b>56 + 3 = 59</b>
              <span>(do not reuse blindly)</span>
              <XCircle />
            </div>
            <p>
              <ShieldCheck /> Keep the source visible. Reuse with confidence.
            </p>
          </section>
        </aside>
      </main>
      ...
      <section className="history-practice">
        <div className="history-question">
          <Trophy />
          <span>
            <small>PRACTICE CHECK</small>
            <b>Which history row produced 56?</b>
          </span>
        </div>
        <div className="history-choices">
          {[2, 3, 4, 1].map((id, index) => (
            <button
              type="button"
              className={practiceChoice === id ? "selected" : ""}
              key={id}
              onClick={() => {
            ...
            >
              <i>{String.fromCharCode(65 + index)}</i> Row {id}
              {practiceChecked && practiceChoice === id && id === 1 ? (
                <CheckCircle2 />
              ) : null}
            </button>
          ))}
        ...
        <div
          className={`history-practice-result ${practiceChecked ? (correct ? "correct" : "wrong") : "pending"}`}
        >
          ...
          <span>
            ...
            <small>
              {practiceChecked
                ? correct
                  ? "Row 1 produced 56."
                  : "Inspect the expression paired with 56."
                : "Select Check answer to grade it."}
            </small>
            <strong>{correct ? "7 × 8 = 56" : "Expression + result"}</strong>
          </span>
        </div>
      </section>
      ...
      <nav className="history-neighbors">
        <a href="/lessons/foundational-advanced/16-constants-library">
          ...
          <span>
            ...
            <b>Constants Library</b>
          </span>
        </a>
        <a href="/lessons/foundational-advanced/18-exact-and-decimal-modes">
          <span>
            <small>NEXT</small>
            <b>Exact and Decimal Modes</b>
          </span>{" "}
          →
        </a>
      </nav>
