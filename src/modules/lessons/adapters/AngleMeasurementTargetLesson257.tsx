  ...
  return (
    ...
    >
      <header className="target-angle-measurement-header">
        ...
        <p>
          Convert angle units. Understand degrees and radians using the unit
          circle.
        </p>
        <section>
        ...
        <footer>
          ...
          <button
            ...
            onClick={() => {
              document
                .querySelector(".target-angle-measurement-explorer")
      ...
      <section className="target-angle-measurement-steps">
        {[
          [
            "1",
            "Observe",
            "Angles are measured from the positive x-axis. Degrees use °, radians use rad.",
          ],
          [
            "2",
            "Manipulate",
            "Drag the blue ray or the protractor/slider. Values update instantly.",
          ],
          [
            "3",
            "Notice",
            "Degrees and radians change together. sin, cos and tan values update too.",
          ],
          [
            "4",
            "Understand",
            "Every angle has unique cos θ, sin θ. Periodicity is 360° or 2π radians.",
          ],
          ...
          <article key={String(title)}>
            ...
            {index < 3 ? (
              <ArrowRight className="target-angle-stage-arrow" />
            ) : null}
          </article>
        ))}
      </section>
      ...
      <section
        className="target-angle-measurement-explorer"
        aria-label="Dual degree-radian explorer"
      >
        ...
        <div>
          <article className="target-angle-measurement-circle-panel">
            ...
            <section className="target-angle-signs">
              <strong>SIGNS BY QUADRANT</strong>
              <small>(sin θ, cos θ, tan θ)</small>
          ...
          <aside className="target-angle-measurement-controls">
            ...
            <strong>
              {unit === "degrees"
                ? formatDegrees(angle)
                : `${formatDecimal(model.radians, 3)} rad`}
            </strong>
            <p>
              = <span>{radianLabel(angle)}</span> rad
            </p>
            {unit === "degrees" ? (
              <input
              ...
              <input
                aria-label="Angle in radians"
                aria-valuetext={`${formatDecimal(model.radians, 3)} radians`}
                type="range"
                min="-6283"
                max="6283"
                step="1"
                value={Math.round(model.radians * 1000)}
                onChange={(e) =>
                  updateAngle(((Number(e.target.value) / 1000) * 180) / Math.PI)
                }
              />
            )}
            <label>
                ...
                onChange={(e) =>
                  updateAngle(
                    unit === "degrees"
                      ...
                      : (Number(e.target.value) * 180) / Math.PI,
                  )
                }
              />
              <span>{unit === "degrees" ? "360°" : "2π"}</span>
          ...
          <div>
            {SPECIAL_ANGLES.map((value) => (
              <button
                ...
                key={value}
                className={
                  near(model.normalized, value % 360) &&
                  (value !== 360 || angle === 360)
                    ? "active"
                    : ""
                }
                onClick={() => updateAngle(value)}
              >
      ...
      <section className="target-angle-measurement-values">
        ...
        <div>
          ...
          <Value
            label="tan θ"
            value={model.tan === null ? "undefined" : formatNumber(model.tan)}
          />
          <Value
            label="(cos θ, sin θ)"
            value={`(${formatNumber(model.cos)}, ${formatNumber(model.sin)})`}
          />
        </div>
        <footer>
          <b>Domains:</b> θ (deg) ∈ ℝ <i /> θ (rad) ∈ ℝ <i /> <b>Ranges:</b> −1
          ≤ sin θ ≤ 1 <i /> −1 ≤ cos θ ≤ 1 <i /> tan θ ∈ ℝ \ {`{π/2 + kπ}`}
        </footer>
      </section>
      ...
      <section className="target-angle-measurement-learning">
        <article>
          <h2>▣ RULE &amp; FORMULA</h2>
          <h3>Degree-Radian Conversion</h3>
          <p>
            <b>2π rad</b> = <b>360°</b>
          </p>
          <p>
            1 rad = <b>180° / π</b> ≈ 57.2958°
          </p>
          <p>
            θ (rad) = θ (deg) × <b>π / 180</b>
          </p>
          <p>
            θ (deg) = θ (rad) × <b>180 / π</b>
          </p>
        </article>
        <article>
          <h2>▣ WORKED EXAMPLE</h2>
          <h3>Example: Convert 135° to radians and find sin and cos.</h3>
          <p>
            1) θ (rad) = 135° × π/180 = <b>3π/4 rad</b>
          </p>
          <p>
            2) 135° is in Quadrant II.
            <br />
            Reference angle = 180° − 135° = 45°
          </p>
          <p>3) (cos θ, sin θ) = (−cos 45°, sin 45°)</p>
          <footer>
            <b>sin 135° = √2/2 ≈ 0.7071</b>
            <b>cos 135° = −√2/2 ≈ −0.7071</b>
          </footer>
        </article>
        <article>
          <h2>
            <TriangleAlert /> MISCONCEPTION ALERT
          </h2>
          <h3>Thinking degrees and radians are different kinds of angles.</h3>
          <p>
            <b>Truth:</b> They measure the same angle quantity. Only the unit
            changes.
          </p>
          <p>180° = π rad (not 3.14 degrees!)</p>
          <p>360° = 2π rad (not 6.28 degrees!)</p>
          <footer>
      ...
      <section className="target-angle-measurement-practice">
        <article>
          ...
          <h3>What is the radian measure of 45°?</h3>
          <div>
            {(["A", "B", "C", "D"] as Choice[]).map((choice) => (
              <button
                type="button"
                key={choice}
                className={answer === choice ? feedback : ""}
                onClick={() => choose(choice)}
              >
                <b>{choice}</b>
                <span>
                  {{ A: "π/6", B: "π/3", C: "π/4", D: "2π/3" }[choice]}
                </span>
                {answer === choice && choice === "C" ? <Check /> : null}
              </button>
            ))}
          </div>
          <footer className={feedback} role="status">
            {feedback === "correct" ? (
              <>
                <Check />
                <b>Correct!</b> 45° × π/180 = <strong>π/4</strong> rad.
              </>
            ) : (
              <>
                <TriangleAlert />
                <b>Try again.</b> Multiply 45° by π/180 and simplify.
              </>
            )}
          </footer>
        </article>
      ...
      <nav className="target-angle-measurement-nav">
        <a href="/lessons/trigonometry/258-unit-circle">
          <ArrowLeft />
          <span>
            <b>Previous</b>Unit Circle
          </span>
        </a>
        <a href="/lessons/trigonometry/267-compound-angle-formulae">
          <span>
            <b>Next</b>Angle Addition and Subtraction
          </span>
          <ArrowRight />
        </a>
      </nav>
...
function UnitCircle({
  ...
  const center = { x: 197, y: 165 },
    radius = 133;
  const point = {
    x: center.x + radius * model.cos,
    y: center.y - radius * model.sin,
  };
  const pointerAngle = (event: ReactPointerEvent<SVGSVGElement>) => {
    ...
    onAngle(
      candidates.reduce((best, value) =>
        Math.abs(value - angle) < Math.abs(best - angle) ? value : best,
      ),
    );
  };
  const arc = describeArc(
    ...
    43,
    0,
    Math.min(model.normalized, 359.99),
  );
  return (
    <svg
      ref={svg}
      className="target-angle-unit-circle"
      viewBox="0 0 430 330"
      role="img"
      aria-label="Draggable oriented ray on the unit circle"
      onPointerMove={(e) => {
        if (dragging.current) pointerAngle(e);
      }}
      onPointerUp={() => {
    ...
    >
      <defs>
        <marker
          ...
          orient="auto"
        >
          <path d="M0,0 L7,3.5 L0,7z" fill="#1473e6" />
        </marker>
      </defs>
      <line x1="28" x2="345" y1={center.y} y2={center.y} stroke="#172554" />
      <line x1={center.x} x2={center.x} y1="20" y2="310" stroke="#172554" />
      <text x="350" y={center.y + 4}>
        x
      </text>
      <text x={center.x - 5} y="15">
        y
      </text>
      <circle
        cx={center.x}
      ...
      <text
        x={center.x + 54 * Math.cos((model.normalized * Math.PI) / 180)}
        y={center.y - 54 * Math.sin((model.normalized * Math.PI) / 180)}
        fontSize="14"
        fontWeight="900"
      >
        θ
      ...
      <g transform="translate(330 29)">
        <rect width="112" height="52" rx="6" fill="white" stroke="#cbd5e1" />
        <text x="12" y="20" fontWeight="700">
          (cos θ, sin θ)
        </text>
        <text x="12" y="40" fontSize="12" fontWeight="900">
...
function Protractor({
  ...
  const display =
    normalize(angle) <= 180 ? normalize(angle) : 360 - normalize(angle);
  const radians = (display * Math.PI) / 180,
    point = {
      x: 140 + 108 * Math.cos(radians),
      y: 132 - 108 * Math.sin(radians),
    };
  ...
  return (
    <svg
      ref={svg}
      className="target-angle-protractor"
      viewBox="0 0 280 150"
      role="img"
      aria-label="Interactive semicircular protractor"
      onPointerMove={(e) => {
    ...
    >
      {Array.from({ length: 37 }, (_, i) => {
        ...
        const inner = i % 3 === 0 ? 92 : 99;
        return (
          <line
            key={i}
            x1={140 + inner * Math.cos(a)}
            y1={132 - inner * Math.sin(a)}
            x2={140 + 108 * Math.cos(a)}
            y2={132 - 108 * Math.sin(a)}
            stroke="#94a3b8"
      ...
      <path d="M32 132 A108 108 0 0 1 248 132" fill="none" stroke="#172554" />
      <line x1="26" x2="254" y1="132" y2="132" stroke="#172554" />
      <line
        x1="140"
        y1="132"
        x2={point.x}
...
function StageVisual({ index }: { index: number }) {
  if (index === 0)
    return (
      <svg
        className="target-angle-stage-visual"
        viewBox="0 0 90 52"
        aria-hidden="true"
      >
        <circle cx="42" cy="27" r="20" fill="none" stroke="#94a3b8" />
        <line x1="8" x2="78" y1="27" y2="27" stroke="#64748b" />
        <line x1="42" x2="42" y1="3" y2="50" stroke="#64748b" />
        <line
          x1="42"
          x2="70"
          y1="27"
  ...
  if (index === 1)
    return (
      <svg
        ...
        aria-hidden="true"
      >
        <path d="M10 43 A42 42 0 0 1 78 30" fill="none" stroke="#94a3b8" />
        <line
          x1="44"
          y1="43"
          x2="73"
  ...
  if (index === 2)
    return (
      ...
      >
        <path
          d="M2 28 C12 3 24 3 34 28 S55 52 66 28 S80 3 89 28"
          fill="none"
          stroke="#0798aa"
          strokeWidth="2"
        />
  ...
  return (
    ...
    >
      <path
        d="M22 42 A20 20 0 1 1 45 8"
        fill="none"
        stroke="#172554"
        strokeWidth="1.5"
      />
      <path d="M42 5l7 3-5 6" fill="none" stroke="#172554" />
...
function angleModel(angle: number) {
  const radians = (angle * Math.PI) / 180,
    ...
    cos = clean(Math.cos(radians)),
    sin = clean(Math.sin(radians));
...
function radianLabel(degrees: number) {
  ...
  const scaled = Math.round(degrees * 10),
    denominator = 1800,
    divisor = gcd(Math.abs(scaled), denominator),
    numerator = scaled / divisor,
    bottom = denominator / divisor;
  const sign = numerator < 0 ? "−" : "",
    top = Math.abs(numerator);
...
function formatNumber(value: number) {
  return Math.abs(value) < 1e-10 ? "0.000" : value.toFixed(3);
}
function formatDecimal(value: number, places: number) {
  return Math.abs(value) < 1e-10 ? "0" : value.toFixed(places);
}
