...
describe("MatrixLessonAdapter", () => {
  it("renders all 18 matrix routes through explicit activities", () => {
    ...
    for (const lesson of lessons) {
      ...
      if (![352, 356, 361, 364].includes(lesson.id))
  ...
  it("uses the dedicated interactive matrix builder for lesson 347", () => {
    ...
    expect(html).toContain(
      "editable-resizable-matrix-transpose-augmentation-validation-export-cell-drag",
    );
  ...
  it("uses a separate element-wise model for lesson 348", () => {
    ...
    expect(html).toContain(
      "two-editable-compatible-matrices-elementwise-addition-subtraction",
    );
  ...
  it("uses a dedicated scalar and determinant model for lesson 349", () => {
    ...
    expect(html).toContain(
      "editable-two-by-two-matrix-scalar-range-cellwise-product",
    );
  ...
  it("uses a compatibility-driven multiplication model for lesson 350", () => {
    ...
    expect(html).toContain(
      "independently-resizable-editable-matrices-compatibility-row-column-dot-product",
    );
  ...
  it("uses a left-and-right identity model for lesson 351", () => {
    ...
    expect(html).toContain(
      "editable-two-by-two-matrix-left-right-identity-products-expanded-row-column-proof",
    );
  ...
  it("uses a selectable symbolic and numeric transpose model for lesson 352", () => {
    ...
    expect(html).toContain(
      "symbolic-numeric-two-by-three-matrix-derived-transpose-selectable-entry",
    );
  ...
  it("uses a draggable signed-area determinant model for lesson 353", () => {
    ...
    expect(html).toContain(
      "editable-two-by-two-matrix-determinant-products-draggable-column-vectors-signed-area",
    );
  ...
  it("uses a determinant-gated inverse and Gauss-Jordan model for lesson 354", () => {
    ...
    expect(html).toContain(
      "editable-two-by-two-matrix-determinant-invertibility-gated-formula-inverse-real-gauss-jordan-states",
    );
  ...
  it("uses a reversible row-operation state machine for lesson 355", () => {
    ...
    expect(html).toContain(
      "augmented-three-by-four-matrix-swap-nonzero-scale-row-replacement-preview-commit-draggable-row-reorder",
    );
  ...
  it("uses a real Gauss-Jordan RREF and pivot model for lesson 356", () => {
    ...
    expect(html).toContain(
      "real-gauss-jordan-rref-operation-state-sequence-clickable-steps-pivot-detection-rank-nullity",
    );
  ...
  it("uses one live equation-system model for lesson 357", () => {
    ...
    expect(html).toContain(
      "editable-linear-system-derived-coefficient-variable-constant-augmented-matrices",
    );
  ...
  it("uses a draggable basis transformation model for lesson 358", () => {
    ...
    expect(html).toContain(
      "editable-two-by-two-linear-transformation-draggable-basis-columns",
    );
  ...
  it("uses a derived eigensystem and draggable vector model for lesson 359", () => {
    ...
    expect(html).toContain(
      "editable-real-two-by-two-eigensystem-characteristic-polynomial-derived-eigenpairs",
    );
  ...
  it("uses a determinant-based draggable basis model for lesson 360", () => {
    ...
    expect(html).toContain(
      "draggable-two-vector-basis-determinant-independence-span-dimension-coordinate-solve",
    );
  ...
  it("uses a homogeneous-relation independence model for lesson 361", () => {
    ...
    expect(html).toContain(
      "draggable-editable-vector-pair-determinant-area-rank-independence-homogeneous-relation",
    );
  ...
  it("uses candidate-specific closure rules for lesson 362", () => {
    ...
    expect(html).toContain(
      "candidate-set-membership-rule-zero-addition-scalar-closure-editable-linear-combination",
    );
  ...
  it("uses the real Gram-Schmidt projection pipeline for lesson 363", () => {
    ...
    expect(html).toContain(
      "editable-draggable-two-vector-real-gram-schmidt-projection-subtraction-normalization",
    );
  ...
  it("uses a derived least-squares regression model for lesson 364", () => {
    const lesson = lessonCatalog.find((item) => item.id === 364)!;
    const html = renderToStaticMarkup(
      <MatrixLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="matrix-mockup-0549"');
    expect(html).toContain(
      "editable-draggable-data-real-least-squares-fit-residuals-sse-rmse-mae-normal-equations",
    );
    expect(html).toContain('data-best-line="[0.7429,0.9619]"');
    expect(html).toContain('data-minimum-sse="3.6762"');
    expect(html).toContain('data-atr="[0,0]"');
  });
});
