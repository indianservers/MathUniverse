...
describe("FinanceLessonAdapter", () => {
  ...
  it("uses the dedicated periodic-model surface for lesson 610", () => {
    const lesson = lessonCatalog.find((candidate) => candidate.id === 610)!;
    const html = renderToStaticMarkup(
      <FinanceLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="finance-mockup-0667"');
    expect(html).toContain(
      "dedicated-draggable-sinusoidal-cycle-landmark-model",
    );
    expect(html).toContain('data-amplitude="3000"');
    expect(html).toContain('data-period="12"');
    expect(html).toContain('data-peak="13000"');
    expect(html).toContain('data-trough="7000"');
  });

  it("uses the dedicated piecewise-model surface for lesson 611", () => {
    const lesson = lessonCatalog.find((candidate) => candidate.id === 611)!;
    const html = renderToStaticMarkup(
      <FinanceLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="finance-mockup-0668"');
    expect(html).toContain(
      "dedicated-draggable-interval-rule-endpoint-piecewise-model",
    );
    expect(html).toContain('data-left="-1"');
    expect(html).toContain('data-right="3"');
    expect(html).toContain('data-value="3"');
    expect(html).toContain('data-left-continuous="true"');
    expect(html).toContain('data-right-continuous="false"');
  });

  it("uses the dedicated parameter-estimation surface for lesson 612", () => {
    const lesson = lessonCatalog.find((candidate) => candidate.id === 612)!;
    const html = renderToStaticMarkup(
      <FinanceLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="finance-mockup-0669"');
    expect(html).toContain(
      "dedicated-draggable-observations-least-squares-confidence-model",
    );
    expect(html).toContain('data-slope="4.000"');
    expect(html).toContain('data-intercept="10.000"');
    expect(html).toContain('data-points="32"');
  });

  it("uses the dedicated dimensional-analysis surface for lesson 613", () => {
    const lesson = lessonCatalog.find((candidate) => candidate.id === 613)!;
    const html = renderToStaticMarkup(
      <FinanceLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="finance-mockup-0670"');
    expect(html).toContain(
      "dedicated-draggable-unit-exponent-cancellation-chain-model",
    );
    expect(html).toContain('data-result="83.33"');
  ...
  it("uses the dedicated residual-error surface for lesson 615", () => {
    const lesson = lessonCatalog.find((candidate) => candidate.id === 615)!;
    const html = renderToStaticMarkup(
      <FinanceLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="finance-mockup-0672"');
    expect(html).toContain(
      "dedicated-draggable-linear-quadratic-residual-error-model",
    );
    expect(html).toContain('data-model="linear"');
    expect(html).toContain('data-sse="9.00"');
    expect(html).toContain('data-mae="1.00"');
    expect(html).toContain('data-points="7"');
  });

  it("uses the dedicated scenario-comparison surface for lesson 616", () => {
    const lesson = lessonCatalog.find((candidate) => candidate.id === 616)!;
    const html = renderToStaticMarkup(
      <FinanceLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="finance-mockup-0673"');
    expect(html).toContain(
      "dedicated-three-scenario-compound-growth-distribution-model",
    );
    expect(html).toContain('data-principal="25000"');
    expect(html).toContain('data-rate="8"');
    expect(html).toContain('data-years="10"');
    expect(html).toContain('data-base-output="53973.12"');
  });

  it("uses the dedicated linear-programming surface for lesson 617", () => {
    const lesson = lessonCatalog.find((candidate) => candidate.id === 617)!;
    const html = renderToStaticMarkup(
      <FinanceLessonAdapter
        lesson={lesson}
        resetToken={0}
    ...
    expect(html).toContain('data-testid="finance-mockup-0674"');
    expect(html).toContain(
      "dedicated-draggable-two-constraint-corner-point-optimization-model",
    );
    expect(html).toContain('data-optimal-point="8,0"');
    expect(html).toContain('data-optimal-value="24"');
    expect(html).toContain('data-corner-count="5"');
  });
  ...
  it("renders all 27 finance routes with explicit controls and no legacy fallback", () => {
    ...
    for (const lesson of lessons) {
      ...
      expect(html, String(lesson.id)).toMatch(
        /finance and modelling lab|simple-interest model|finance-mockup-0648|finance-mockup-0649|finance-mockup-0650|finance-mockup-0651|finance-mockup-0652|finance-mockup-0653|finance-mockup-0654|finance-mockup-0655|finance-mockup-0656|finance-mockup-0657|finance-mockup-0658|finance-mockup-0659|finance-mockup-0660|finance-mockup-0661|finance-mockup-0662|finance-mockup-0663|finance-mockup-0664|finance-mockup-0665|finance-mockup-0666|finance-mockup-0667|finance-mockup-0668|finance-mockup-0669|finance-mockup-0670|finance-mockup-0671|finance-mockup-0672|finance-mockup-0673|finance-mockup-0674/,
      );
      expect(html, String(lesson.id)).not.toContain("Legacy");
      expect(html, String(lesson.id)).toMatch(
        /type="range"|dedicated-editable-data-regression-prediction-model|dedicated-draggable-unit-exponent-cancellation-chain-model/,
      );
    }
  });
