export const clampPolarAngle = (degrees: number) => ((Math.round(degrees) % 360) + 360) % 360;
export const clampPolarScale = (value: number) => Math.max(1, Math.min(10, Math.round(value * 10) / 10));
export const clampPetalMultiplier = (value: number) => Math.max(1, Math.min(10, Math.round(value)));
...
export function polarRosePoint(a: number, n: number, angleDegrees: number) {
  ...
  return { theta, radius, x: radius * Math.cos(theta), y: radius * Math.sin(theta) };
}

export const polarPetalCount = (n: number) => Math.abs(n) % 2 === 0 ? Math.abs(n) * 2 : Math.abs(n);

const plot = (x: number, y: number, scale: number) => ({ x: 350 + x * scale, y: 300 - y * scale });
...
export function polarRosePath(a: number, n: number) {
  ...
  return Array.from({ length: 721 }, (_, index) => {
    ...
    const radius = a * Math.sin(n * theta);
    const point = plot(radius * Math.cos(theta), radius * Math.sin(theta), scale);
...
export function polarAngleFromPixels(pixelX: number, pixelY: number) {
  return clampPolarAngle(Math.atan2(300 - pixelY, pixelX - 350) * 180 / Math.PI);
}
