...
function ListChart({values,view,selected}:{values:number[];view:'bar'|'dot';selected:number}) {
 const max=Math.max(1,...values.map(Math.abs)),palette=['#65afea','#76c99c','#f5af4a','#a774dd'];
 return <LessonGraphWorkspace title={`${view==='bar'?'Bar chart':'Dot plot'} of ordered list`} description="Height shows magnitude (absolute value); labels retain each signed value."><LessonBarGraph label={`${view} visualization of ordered list`} kind={view} highlightedId={String(selected)} bars={values.map((value,index)=>({id:String(index),label:`Index ${index+1}`,value,color:palette[index%4],fraction:Math.abs(value)/max*85/111}))}/></LessonGraphWorkspace>;
}
