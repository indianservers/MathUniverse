...
import { checkPlacement, checkPractice, familyOf, graphExplanation, placeVariable, placementScore, practiceVariables, suitableGraphs, variables, type Placements } from "./dataTypesModel";
...
describe("Data Types dedicated model", () => {
  it("classifies all ten contextualized variables without numerical-label confusion", () => {
    ...
    expect(variables.filter(item => familyOf(item.type) === "Categorical").map(item => item.id)).toEqual(["blood", "rank", "satisfaction", "shirt"]);
    expect(variables.find(item => item.id === "score")?.type).toBe("Discrete");
    expect(variables.find(item => item.id === "height")?.type).toBe("Continuous");
  });
  it("retains independent type and graph placements, supports corrections and rejects foreign ids", () => {
    ...
    expect(checkPlacement(variables[0], second.blood)).toEqual({ familyCorrect: false, graphCorrect: true });
    const corrected = placeVariable(second, "blood", "Categorical");
    expect(corrected.blood).toEqual({ family: "Categorical", graph: "Pie chart" });
  ...
  it("only counts variables with both assignments correct", () => {
    ...
    for (const variable of variables) state = placeVariable(state, variable.id, familyOf(variable.type));
    expect(placementScore(state)).toBe(0);
    for (const variable of variables) state = placeVariable(state, variable.id, suitableGraphs(variable.type)[0]);
  ...
  it("accepts legitimate graph alternatives with their conditions", () => {
    ...
    expect(graphExplanation("Continuous", "Dot plot")).toContain("small sample");
  });
  it("requires both practice selections and distinguishes counts, duration and ratings", () => {
    ...
    const answers = Object.fromEntries(practiceVariables.map(variable => [variable.id, { type: variable.type, graph: suitableGraphs(variable.type)[0] }]));
    expect(checkPractice(answers)).toEqual([true, true, true, true]);
    expect(checkPractice({ ...answers, experience: { type: "Discrete", graph: "Dot plot" }, rating: { type: "Continuous", graph: "Histogram" } })).toEqual([true, false, true, false]);
  });
});
