...
export default function ExpandingBracketsTargetLesson95({
  ...
  const startDrag = (event: DragEvent<HTMLButtonElement>, source: string) => {
    event.dataTransfer.setData("text/distribution-source", source);
    setDragging(source);
  };
  const dropOnArea = (event: DragEvent<HTMLElement>) => {
    event.preventDefault();
    const source = event.dataTransfer.getData("text/distribution-source");
    if (!source) return;
    setShowArrows(true);
    setShowArea(true);
    setChecked(true);
  ...
  const nextPractice = () => {
    ...
    setPracticeAnswer(
      expanded(
        practices[next].factor,
        practices[next].variable,
        practices[next].constant,
      ),
    );
    setPracticeChecked(true);
    act();
  };
  const checkPractice = (event: KeyboardEvent<HTMLInputElement>) => {
    if (event.key === "Enter") {
      setPracticeChecked(true);
      act();
    }
  ...
  return (
    ...
    >
      <nav className="expand95-breadcrumb">
        <a href="/">⌂</a>
        <a href="/">Home</a>
        <span>&gt;</span>
        ...
        <span>&gt;</span>
        <b>Expanding Brackets</b>
      </nav>
      <header className="expand95-header">
        <div className="expand95-eyebrows">
          <b>ALGEBRA</b>
          <strong>EXPRESSIONS AND MANIPULATION</strong>
        </div>
        <h1>Expanding Brackets</h1>
        <p>
          Use the distributive law to multiply the outside factor by every term
          inside the bracket.
        </p>
        <div className="expand95-badges">
          <b>Intermediate</b>
          <b>Algebra</b>
          <b>6-10 min</b>
          <b>Area model</b>
      ...
      <main className={`expand95-layout ${tab === "Interact" ? "" : "hidden"}`}>
        <section className="expand95-area">
          <header>
            <div>
              <h2>Distributive area model</h2>
              <p>
                Multiply {factor} by each term inside the bracket ({variable}{" "}
                and {constant}).
              </p>
            </div>
            <aside>
              <Toggle
                label="Show distribution arrows"
                value={showArrows}
                onToggle={() => {
                  setShowArrows((current) => !current);
                  act();
                }}
              />
              <Toggle
                label="Show area model"
                value={showArea}
                onToggle={() => {
                  setShowArea((current) => !current);
                  act();
                }}
              />
            </aside>
          ...
          <section
            className={`expand95-visual ${showArea ? "visible" : "hidden"}`}
          ...
          >
            <button
              type="button"
              draggable
              aria-label="Drag outside factor"
            ...
            {showArrows && (
              <svg viewBox="0 0 430 85" aria-hidden="true">
                <defs>
                  <marker
                    id="expand-arrow"
                    markerWidth="7"
                    ...
                    orient="auto"
                  >
                    <path d="M0,0 L0,6 L6,3 z" fill="#7d32ee" />
                  </marker>
                </defs>
                <path d="M38 46 Q90 8 151 48" />
                <path d="M42 42 Q202 -14 330 47" />
              </svg>
            )}
            <h3>
              <span>{variable}</span>
            ...
            <footer>
              ...
              <i>+</i>
              <span>{constantProduct}</span>
            </footer>
          </section>
        </section>
        <aside className="expand95-controls">
          <h2>Controls</h2>
          <label>
            Outside factor
            <div>
              <input
                aria-label="Outside factor"
                type="number"
                value={factor}
                onChange={(event) => updateFactor(Number(event.target.value))}
              />
              <button
                type="button"
                aria-label="Decrease outside factor"
                onClick={() => updateFactor(factor - 1)}
              >
                −
              </button>
              <button
                type="button"
                aria-label="Increase outside factor"
                onClick={() => updateFactor(factor + 1)}
              >
                +
              </button>
          ...
          <label>
            Bracket terms
            <div className="expand95-terms">
              <input
                aria-label="Variable term"
                value={variable}
                maxLength={1}
                onChange={(event) => {
                  setVariable(
              ...
              <input
                ...
                type="number"
                value={constant}
                onChange={(event) => updateConstant(Number(event.target.value))}
              />
            </div>
          </label>
          <label>
            ...
            <div>
              ...
              <button
                type="button"
                aria-label="Decrease check value"
                onClick={() => {
                  setCheckValue((current) => current - 1);
                  setChecked(false);
                  act();
                }}
              >
              ...
              <button
                type="button"
                aria-label="Increase check value"
                onClick={() => {
                  setCheckValue((current) => current + 1);
                  setChecked(false);
                  act();
        ...
        <section className="expand95-result">
          <h2>Result</h2>
          <p>Expanded expression</p>
          <strong>{result}</strong>
          <h3>
            Same value when {variable} = {checkValue}
          </h3>
          <p>
            Original: &nbsp;&nbsp;&nbsp;&nbsp; {factor}({checkValue}{" "}
            {sign(constant)}) &nbsp;= &nbsp;{originalValue}
          </p>
          <p>
            Expanded: &nbsp; {factor}({checkValue}) {sign(constantProduct)}{" "}
            &nbsp;= &nbsp;{expandedValue}
          </p>
          <footer>
            <Check />
            {checked
              ...
              : "Values updated. Check by substitution."}
          </footer>
        </section>
        <section className="expand95-guided">
          <h2>Guided steps</h2>
          <div>
            <article>
              <i>1</i>
            ...
            <article>
              ...
              <span>
                <b>
                  Multiply {factor} by {constant}
                </b>
                <strong>
                  {factor} × {constant} = {constantProduct}
                </strong>
                <p>Area = {constantProduct}</p>
              </span>
            </article>
            <em>→</em>
            <article>
        ...
        <section className="expand95-cards">
          <article>
            <h2>
              <Mountain />
            ...
            <p>
              Multiply the outside factor by
              <br />
              every term inside the bracket.
            </p>
            <strong>a(b + c) = ab + ac</strong>
            <p>
              Here: &nbsp; {expression} = {result}
            </p>
          </article>
          <article>
            <h2>Worked example</h2>
            <p>
              Expand {expression} and check with {variable} = {checkValue}.
            </p>
            <strong>
              {expression} = {result}
            </strong>
            <p>
              At {variable} = {checkValue}:
            </p>
            <span>
              Original: &nbsp;&nbsp;&nbsp;&nbsp; {factor}({checkValue} +{" "}
              {constant}) = {factor} × {checkValue + constant} = {originalValue}
              <br />
              <br />
              Expanded: &nbsp; {factor}({checkValue}) + {constantProduct} ={" "}
              {factor * checkValue} + {constantProduct} = {expandedValue}
            </span>
            <footer>
              <Check />
              Both give {expandedValue} — same value.
            </footer>
          </article>
          <article>
            <h2>
            ...
            <p>Do not multiply only the first term.</p>
            <strong>
              {expression} is not {factor}
              {variable} + {constant}.
            </strong>
            <p>
              You must multiply {factor} by both {variable}
              <br />
              and {constant}.
            </p>
          </article>
        </section>
        <section className="expand95-practice">
          <h2>Practice challenge</h2>
          <p>Expand the expression using the distributive law.</p>
          <label>
            Expand {practice.factor}
            {bracket(practice.variable, practice.constant)}
            <input
              ...
              onKeyDown={checkPractice}
            />
            {practiceChecked && practiceCorrect && <Check />}
          </label>
          <aside
            className={practiceChecked && practiceCorrect ? "correct" : ""}
          >
            <b>
              {practiceChecked
                ? practiceCorrect
            ...
            <span>
              {practice.factor} × {practice.variable} = {practice.factor}
              {practice.variable} and {practice.factor} × {practice.constant} ={" "}
              {practice.factor * practice.constant}
              <br />
              So, {practice.factor}
              {bracket(practice.variable, practice.constant)} ={" "}
              {practiceExpected}.
        ...
        <nav className="expand95-navigation">
          <a href="/lessons/algebra/94-substitution">
            <ArrowLeft />
            <span>
              Previous<b>Substitution</b>
            </span>
          </a>
          <a href="/lessons/algebra/96-double-brackets">
            <span>
              Next<b>Double Brackets</b>
            </span>
            <ArrowRight />
          </a>
        </nav>
        <footer className="expand95-footer">
          <div>
            <b>Math Universe</b>
            <p>
              Interactive math labs, visual proofs, graphing,
              <br />
