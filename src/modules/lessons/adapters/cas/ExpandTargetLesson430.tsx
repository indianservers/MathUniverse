...
export default function ExpandTargetLesson430({
  ...
  useEffect(() => {
    setHeaderTarget(
      document.querySelector<HTMLElement>(".lesson-shell-header"),
    );
  }, []);
  ...
  return (
    ...
    >
      {headerTarget &&
        createPortal(
          <aside
            className="exp430-guide"
            aria-label="How to use this CAS Workspace"
          >
            ...
            {[
              [
                ...
                "1",
                "Observe",
                "See the expression as a structure (tree).",
              ],
              [
                Shapes,
                "2",
                "Manipulate",
                "Distribute and collect using algebra tiles.",
              ],
              ...
              [
                ...
                "4",
                "Understand",
                "Connect to the rule and try a challenge.",
              ],
      ...
      <section className="exp430-workspace">
        <article className="exp430-structure">
          ...
          <div className="exp430-legend">
            <span /> First x First <span /> First x Last <span /> Last x First{" "}
            <span /> Last x Last
          </div>
        </article>
        ...
        <article className="exp430-tiles">
          ...
          >
            ...
            <span>
              {stage <= 1
                ? "Build the product tiles again."
                : "Go back one step to explore."}
            </span>
          </button>
        </article>
      </section>
      ...
      <section className="exp430-result">
        ...
        {showAlgebra && (
          <p>
            {model.left.label}
            {model.right.label} = {model.products.join(" + ")} ={" "}
            {model.expanded}
          </p>
        )}
      </section>
      ...
      <section className="exp430-learning">
        ...
        <article>
          ...
          <p>
            <b>Incorrect:</b> (x+2)(x-3) = x + 2 - 3{" "}
            <span className="wrong">X</span>
          </p>
          <p>
            <b>Why it's wrong:</b> The parentheses mean multiplication of two
            expressions, not simple addition/subtraction.
          </p>
        </article>
      </section>
      ...
      <section className="exp430-practice">
        <div>
          ...
          <p>
            <b>Challenge:</b> Expand the expression.
          </p>
          <label>
            <input
        ...
        <div>
          <label>
            ...
            <span>
              ...
              <button
                type="button"
                data-lesson-control="expand-practice-check"
                onClick={checkAnswer}
              >
                Check
              </button>
          ...
          <button
            type="button"
            data-lesson-control="expand-hint"
            className="exp430-hint"
            onClick={() => interact(() => setHint((value) => !value))}
          >
            <Lightbulb />{" "}
            {hint ? "Multiply first, outside, inside, last." : "Show hint"}
          </button>
        </div>
        <div className={`exp430-answer ${feedback}`}>
          <h2>Answer</h2>
          <p>
            <Polynomial coefficients={challengeModel.coefficients} /> <Check />
          </p>
          {feedback === "incorrect" && (
            <small>Combine both middle x terms.</small>
          )}
          <button
            type="button"
            data-lesson-control="expand-practice-steps"
            onClick={() => interact(() => setPracticeSteps((value) => !value))}
          >
            <Shapes />{" "}
            {practiceSteps ? challengeModel.products.join(" + ") : "Show steps"}
          </button>
        </div>
      </section>

      <nav className="exp430-nav" aria-label="Adjacent lessons">
        <a href="/lessons/symbolic-mathematics/429-simplify">
          <ArrowLeft />
          <span>
            <small>Previous</small>Simplify
          </span>
        </a>
        <a href="/lessons/symbolic-mathematics/431-factor">
          <span>
            <small>Next</small>Factor
          </span>
          <ArrowRight />
        </a>
      </nav>
...
function ExpressionTree({ model }: { model: ExpansionModel }) {
  return (
    <div className="exp430-tree">
      <b>
        {model.valid
          ? `${model.left.label}${model.right.label}`
          : "Invalid expression"}
      </b>
      <div>
        <span>{model.left.label}</span>
        <span>{model.right.label}</span>
      </div>
      <div>
        <i>{term(model.left.x, "x")}</i>
        <i>{signed(model.left.constant)}</i>
        <i>{term(model.right.x, "x")}</i>
        <i>{signed(model.right.constant)}</i>
      </div>
    </div>
  );
}
...
function AreaModel({ model }: { model: ExpansionModel }) {
  ...
  return (
    <div className="exp430-area">
      <span />
      <b>{term(model.right.x, "x")}</b>
      <b>{signed(model.right.constant)}</b>
      <b>{term(model.left.x, "x")}</b>
      <i>{term(ac, "x²")}</i>
      <i>{term(ad, "x")}</i>
      <b>{signed(model.left.constant)}</b>
      <i>{term(bc, "x")}</i>
      <i>{signed(bd)}</i>
    </div>
  );
...
function ExpansionStages({
  ...
  return (
    <div className={`exp430-stages ${view} stage-${stage}`}>
      <section>
        <h3>
          Step 1: Distribute <span>(create product tiles)</span>
        </h3>
        <div>
          {view === "tiles" ? (
            <>
              {[term(ac, "x²"), term(ad, "x"), term(bc, "x"), signed(bd)].map(
                (label, index) => (
                  <ProductTile
                    key={`${index}-${label}`}
                    index={index}
                    label={label}
                    collected={collected.includes(index)}
                    onCollect={onCollect}
                  />
                ),
              )}
      ...
      <section>
        <h3>Step 2: Collect like terms</h3>
        <div
          className={collected.length === 4 ? "complete" : "collecting"}
          data-lesson-control="expand-collect-dropzone"
...
function ProductTile({
  ...
  const start = (event: DragEvent<HTMLElement>) =>
  ...
  return (
    <i
      draggable
      data-lesson-control={`expand-product-${index}`}
      data-collected={collected}
      className={collected ? "collected" : ""}
      role="button"
      tabIndex={0}
      aria-label={`Collect product ${label}`}
      onDragStart={start}
...
function Polynomial({
  ...
  return (
    <strong className="exp430-polynomial">
      {term(a, "x")}
      <sup>2</sup> {operator(b)} {Math.abs(b) === 1 ? "" : Math.abs(b)}x{" "}
      {operator(c)} {Math.abs(c)}
    </strong>
  );
}
...
function parseExpansion(value: string): ExpansionModel {
  ...
  const invalid: ExpansionModel = {
    valid: false,
    left: { x: 0, constant: 0, label: "(?)" },
    right: { x: 0, constant: 0, label: "(?)" },
    products: [0, 0, 0, 0],
    coefficients: [0, 0, 0],
    expanded: "Invalid expression",
  };
  if (!match) return invalid;
  const left = parseBinomial(match[1]),
    right = parseBinomial(match[2]);
  if (!left || !right) return invalid;
  const products: [number, number, number, number] = [
    left.x * right.x,
    left.x * right.constant,
    left.constant * right.x,
  ...
  return {
    ...
    products,
    coefficients,
    expanded: polynomialText(coefficients),
  };
}
...
function parseBinomial(value: string): Binomial | null {
  ...
  return {
    x,
    constant,
    label: `(${term(x, "x")}${constant >= 0 ? "+" : ""}${constant})`,
  };
}
function term(coefficient: number, variable: string) {
  return `${coefficient === 1 ? "" : coefficient === -1 ? "-" : coefficient}${variable}`;
}
function signed(value: number) {
  return value >= 0 ? `+${value}` : `${value}`;
}
function operator(value: number) {
  return value < 0 ? "-" : "+";
}
function polynomialText([a, b, c]: [number, number, number]) {
  return `${term(a, "x^2")} ${operator(b)} ${Math.abs(b) === 1 ? "" : Math.abs(b)}x ${operator(c)} ${Math.abs(c)}`;
}
function normalizePolynomial(value: string) {
  return value
    .replaceAll(" ", "")
    .replaceAll("*", "")
    .replaceAll("²", "^2")
