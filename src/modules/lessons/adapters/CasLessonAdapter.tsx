...
export default function CasLessonAdapter(props: LessonAdapterProps) {
  ...
  return (
    <AdapterFrame title={`${props.lesson.title} - reusable CAS engine`} value={params.casCommand} footer="This lesson uses the shared symbolic engine with only expression, result, and steps visible.">
      <ReusableLessonEngine engine="cas" params={params} resetToken={props.resetToken} onInteraction={props.onInteraction} />
    </AdapterFrame>
  );
}
