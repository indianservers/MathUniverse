...
function NumberLine({
  ...
  const handleKey = (point: DragPoint, key: string) => {
    ...
    if (point === "center") onCenter(center + delta);
      ...
      onDistance(Math.max(0, distance + (point === "left" ? -delta : delta)));
  };
  if (distance < 0) {
    return (
      <div className="abs121-no-line" role="status">
        <CircleAlert /> Distance cannot be negative, so there are no real
        solution points.
      </div>
    );
  }
  return (
    ...
    >
      ...
      <path
        className="distance"
        d={`M${px(center)} 105 Q${(px(center) + px(left)) / 2} 58 ${px(left)} 105`}
        markerEnd="url(#abs121-arrow)"
      />
      <path
        className="distance"
        d={`M${px(center)} 105 Q${(px(center) + px(right)) / 2} 58 ${px(right)} 105`}
        markerEnd="url(#abs121-arrow)"
      />
      <text className="distance-label" x={(px(center) + px(left)) / 2} y="54">
        Distance = {distance}
      </text>
      <text className="distance-label" x={(px(center) + px(right)) / 2} y="54">
        Distance = {distance}
      </text>
      {(["left", "center", "right"] as const).map((point) => {
        const value =
          point === "left" ? left : point === "right" ? right : center;
        ...
        return (
          <g key={point}>
            <circle
              ...
              aria-label={
                point === "center"
                  ? "Drag absolute value center"
                  : `Drag ${point} absolute value solution`
              }
              onPointerDown={(event) => {
            ...
            />
            <path
              className={`label-tip ${point}`}
              d={`M${px(value) - labelWidth / 2} 184h${labelWidth}v34h-${labelWidth}z M${px(value) - 6} 184l6-8 6 8z`}
            />
            <text className={`value-label ${point}`} x={px(value)} y="206">
              {point === "center" ? `Center: ${value}` : `x = ${value}`}
            </text>
          </g>
        );
      })}
      <g className="legend">
        <rect x="10" y="226" width="430" height="24" rx="6" />
        <circle className="solution" cx="35" cy="238" r="5" />
        <text x="72" y="242">
          Solution points
        </text>
        <circle className="center" cx="160" cy="238" r="5" />
        <text x="191" y="242">
          Center
        </text>
        <line className="distance" x1="264" x2="296" y1="238" y2="238" />
        <text x="359" y="242">
  ...
  return (
    ...
    >
      <nav className="abs121-breadcrumb">
        ...
        <span>&gt;</span>
        <b>121 Absolute Value Equations</b>
      </nav>
      <header className="abs121-intro">
        <small>
          <b>ALGEBRA</b>
        ...
        <div>
          <label>
            <Languages />
            <select
              aria-label="Absolute-value equations language"
              value={language}
              onChange={(event) => {
      ...
      <main className="abs121-lab">
        <header>
          <span>
            ...
            <h2>Distance solver on the number line</h2>
          </span>
        ...
        <section className="abs121-body">
          <div className="abs121-left">
            <section className="abs121-number-card">
              <h3>Solve the absolute-value equation</h3>
              <div className="abs121-equation">
                | x <span>{center >= 0 ? "−" : "+"}</span>
                <input
                  aria-label="Absolute-value center"
                  type="number"
                  value={Math.abs(center)}
                  onChange={(event) =>
                    updateCenter(
                      (center >= 0 ? 1 : -1) * Number(event.target.value),
                    )
                  }
                />{" "}
                | ={" "}
                <input
                  aria-label="Absolute-value distance"
                  type="number"
                  value={distance}
                  onChange={(event) =>
              ...
              <p>
                {solvable
                  ? `Distance from ${center} equals ${distance}.`
                  : "Absolute value cannot equal a negative distance."}
              </p>
              <NumberLine
                center={center}
                distance={distance}
            ...
            <section className="abs121-branches">
              <h3>⌘ Two branches (two cases)</h3>
              {solvable ? (
                <>
                  <div>
                    <article>
                      <b>← Left branch</b>
                      <strong>
                        x{signed(center)} = −{distance}
                      </strong>
                      <p>Move {Math.abs(center)} to the other side</p>
                      <em>x = {left}</em>
                      <small>
                        <Check />
                        Matches the left solution point.
                      </small>
                    </article>
                    <article>
                      <b>→ Right branch</b>
                      ...
                      <small>
                        <Check />
                        Matches the right solution point.
                      </small>
                    </article>
                  </div>
                  <footer>
                    ...
                    <span>
                      ...
                      <p>Both solutions are required.</p>
                    </span>
                  </footer>
                </>
              ) : (
                <div className="abs121-impossible">
                  <CircleAlert />
                  <strong>No solution</strong>
                  <p>An absolute value is never negative.</p>
                </div>
              )}
              <aside>
                <CircleAlert />
                An absolute-value equation gives two solutions that are the same
                distance from the center.
              </aside>
            </section>
          </div>
          <aside className="abs121-right">
            <section className="abs121-reasoning">
              <h2>Reasoning &amp; steps</h2>
              <article>
                <i>1</i>
                <div>
                  <h3>Split into two branches</h3>
                  <p>
                    For |x{signed(center)}| = {distance}, write the two possible
                    equations.
                  </p>
                  <strong>
                    x{signed(center)} = {distance}
                    <br />x{signed(center)} = −{distance}
                  </strong>
                </div>
              </article>
              <article>
                <i>2</i>
                <div>
                  <h3>Solve both branches</h3>
                  <p>Solve each linear equation.</p>
                  <strong>
                    x{signed(center)} = {distance} → x = {right}
                    <br />x{signed(center)} = −{distance} → x = {left}
                  </strong>
                </div>
              </article>
              <article>
                ...
                <div>
                  ...
                  <p>
                    Verify each solution is a distance {distance} from the
                    center {center}.
                  </p>
                  <strong>
                    |{left} − {center}| = {distance} <Check />
                    <br />|{right} − {center}| = {distance} <Check />
                  </strong>
                  <b>Both checks are true.</b>
                </div>
              </article>
            </section>
            <section className="abs121-pitfall">
              <TriangleAlert />
              <div>
                <b>Common pitfall</b>
                <h3>ONLY_POSITIVE_BRANCH</h3>
                <p>
                  Solving only x{signed(center)} = {distance} gives x = {right}{" "}
                  but misses the other valid solution x = {left}.
                </p>
              </div>
            </section>
            <section className="abs121-practice">
              <header>
                <b>♜ Try one like this</b>
                <button
                  onClick={() => {
        ...
        <footer className="abs121-tags">
          <span>☷ primary-control</span>
          <span>▣ expression</span>
          <span>▣ symbolic result</span>
        </footer>
      </main>
      <nav className="abs121-adjacent">
        <a href="/lessons/algebra/120-trigonometric-equations">
          <ArrowLeft />
          <span>
            <small>PREVIOUS</small>Trigonometric Equations
          </span>
        </a>
        <a href="/lessons/algebra/122-linear-inequalities">
          <span>
            <small>NEXT</small>Linear Inequalities
          </span>
          <ArrowRight />
        </a>
      </nav>
      <footer className="abs121-footer">
        <b>
          <Sparkles />
          Math Universe
        </b>
