...
describe("Geometry2DLessonAdapter", () => {
  ...
  it("renders coordinate transformations as its own polygon mapping surface", () => {
    const lesson = lessonCatalog.find((item) => item.id === 179)!;
    const html = renderToStaticMarkup(<Geometry2DLessonAdapter lesson={lesson} resetToken={0} onInteraction={vi.fn()} />);
    expect(html).toContain("geometry-mockup-0236");
    expect(html).toContain('data-dedicated-lesson="179"');
    expect(html).toContain('data-testid="transform-source-handle"');
    expect(html).toContain("Each point moved by the vector");
    expect(html).toContain("Translate the triangle by vector");
  });
  ...
  it("renders coordinate loci as its own constrained-circle surface", () => {
    ...
    expect(html).toContain("(x - 1)² + (y - 1)² = 16");
  ...
  it("renders point-to-line distance as its own orthogonal-projection surface", () => {
    ...
    expect(html).toContain("Compare different paths");
    expect(html).not.toContain("reusable 2D geometry engine");
  });
  ...
  it("renders parallel lines as its own equal-slope invariant surface", () => {
    ...
    expect(html).toContain("Construct a Parallel");
    expect(html).not.toContain("reusable 2D geometry engine");
  });
  ...
  it("renders equation of a line as its own equivalent-forms surface", () => {
    ...
    expect(html).toContain("Equations of the line");
    expect(html).not.toContain("reusable 2D geometry engine");
  });
  ...
  it("renders gradient slope as its own rise-over-run surface", () => {
    ...
    expect(html).toContain("UNDEFINED SLOPE");
    expect(html).not.toContain("reusable 2D geometry engine");
  });
  ...
  it("renders distance between points as its own right-triangle surface", () => {
    ...
    expect(html).toContain("Live result");
