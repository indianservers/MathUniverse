...
export default function DoubleBracketsTargetLesson96({
  ...
  const updateSecond = (next: number) => {
    setSecond(Math.max(1, Math.min(6, next)));
    setChecked(false);
    setMiddleDrops([]);
    act();
  };
  const startDrag = (event: DragEvent<HTMLButtonElement>, source: string) => {
    event.dataTransfer.setData("text/middle-product", source);
    setDragging(source);
  };
  const dropMiddle = (event: DragEvent<HTMLElement>) => {
    event.preventDefault();
    const source = event.dataTransfer.getData("text/middle-product");
    if (!source) return;
    setMiddleDrops((current) =>
  ...
  return (
    <div
      ...
      data-middle-drops={middleDrops.join(",")}
    ...
    >
      <nav className="double96-breadcrumb">
        <a href="/">Home</a>
        <span>&gt;</span>
        ...
        <span>&gt;</span>
        <b>96 Double Brackets</b>
      </nav>
      <header className="double96-header">
        <div>
          <b>ALGEBRA</b>
          <strong>EXPRESSIONS AND MANIPULATION</strong>
        </div>
        <h1>Double Brackets</h1>
        <p>Expand binomial products using area tiles.</p>
        <nav>
          <b>Intermediate</b>
          <b>Algebra</b>
          <b>6-10 min</b>
        ...
        <aside>
          ...
          <p>✓ Use area tiles to expand {expression}</p>
          <p>
            ✓ See the four products: {variable}², {second}
            {variable}, {first}
            {variable}, {constant}
          </p>
          <p>✓ Combine like middle terms</p>
          <p>
            ✓ Verify by substitution ({variable} = {checkValue})
          </p>
        </aside>
      </header>
      <nav className="double96-tabs">
      ...
      <main className={`double96-layout ${tab === "Interact" ? "" : "hidden"}`}>
        <section className="double96-area">
          <header>
            ...
            <p>Multiply each term in one bracket by each term in the other.</p>
          </header>
          <section
            className={`double96-visual ${showProducts ? "products" : ""}`}
          >
            <h3>
              <span>{variable}</span>
              <span>+ {second}</span>
            </h3>
            <b className="double96-side-x">{variable}</b>
            <b className="double96-side-c">+ {first}</b>
            <div className="double96-grid">
              <strong>{variable}²</strong>
              <button
                type="button"
                draggable
                aria-label="Drag second middle product"
              ...
              <button
                type="button"
                draggable
                aria-label="Drag first middle product"
              ...
              <div
                ...
                style={{
                  gridTemplateColumns: `repeat(${second},1fr)`,
                  gridTemplateRows: `repeat(${first},1fr)`,
                }}
              >
          ...
          <section
            className="double96-combine"
            onDragOver={(event) => event.preventDefault()}
            onDrop={dropMiddle}
          >
            <h3>Combine the middle terms</h3>
            <div>
              <b>
                {second}
                {variable}
            ...
            <footer>
              <b>Final result</b>
              <strong>{combineMiddle ? expanded : uncombined}</strong>
            </footer>
          </section>
        </section>
        <aside className="double96-builder">
          ...
          <label>
            ...
            <div>
              ...
              <span>+</span>
              <select
                aria-label="First constant"
                value={first}
                onChange={(event) => updateFirst(Number(event.target.value))}
              >
          ...
          <label>
            Bracket 2
            <div>
              <input aria-label="Second variable" value={variable} readOnly />
              <span>+</span>
              <select
                aria-label="Second constant"
                value={second}
                onChange={(event) => updateSecond(Number(event.target.value))}
              >
                {[1, 2, 3, 4, 5, 6].map((value) => (
                  <option key={value}>{value}</option>
                ))}
          ...
          <label>
            ...
            <div className="double96-check-value">
          ...
          />
          <Toggle
            label="Combine middle terms"
            value={combineMiddle}
            onToggle={() => {
              setCombineMiddle((current) => !current);
              act();
            }}
          />
          <Toggle
            ...
            value={checkEnabled}
            onToggle={() => {
              setCheckEnabled((current) => !current);
              act();
            }}
          />
        </aside>
        <section className="double96-result">
          <h2>Result</h2>
          ...
          {checkEnabled && (
            <>
              <h3>
                Check at {variable} = {checkValue}
                <b>{checked ? "✓ Matches" : "Ready"}</b>
              </h3>
              <p>
                Original: &nbsp;&nbsp;&nbsp;&nbsp;({checkValue} + {first})(
                {checkValue} + {second}) = {originalValue}
              </p>
              <p>
                Expanded: &nbsp; {checkValue}² + {middle}({checkValue}) +{" "}
                {constant} = {expandedValue}
              </p>
              <footer>
                <Check />
                Same value: {expandedValue}
              </footer>
            </>
          )}
        </section>
        <section className="double96-steps">
          ...
          <div>
            ...
            <article>
              ...
              <span>
                <b>
                  {variable} times {second} and {first} times {variable}
                </b>
                <p>
            ...
            <article>
              ...
              <span>
                ...
                <small>Final expression</small>
                <strong>{expanded}</strong>
              </span>
            </article>
          </div>
        </section>
        <section className="double96-cards">
          <article>
            <h2>General Rule</h2>
            <strong>(a + b)(c + d) = ac + ad + bc + bd</strong>
            <p>
              Each term in the first bracket is
              <br />
              multiplied by each term in the second.
            </p>
            <footer>Then, combine like terms.</footer>
          </article>
          <article>
            <h2>
              <TriangleAlert />
            ...
            <b>Don't miss the middle terms.</b>
            <p>
              Only multiplying {variable} × {variable} and {first} × {second}{" "}
              gives
              <br />
              {variable}² + {constant}, which is incomplete.
            </p>
            <strong>{expanded}.</strong>
          </article>
          <article>
            ...
            <footer>
              <Check />
              Verified at {variable} = {checkValue}: {expandedValue}
            </footer>
          </article>
        </section>
        <section className="double96-practice">
          ...
          <b className={challengeChecked && challengeCorrect ? "correct" : ""}>
            {challengeChecked
              ? challengeCorrect
                ? "✓ Correct!"
                : "Try again"
          ...
          >
            <Eye />
            {showSolution
              ? "Hide area tiles solution"
              : "Show area tiles solution"}
          </button>
          {showSolution && (
            <aside>
              {challenge.variable}² | {challenge.second}
              {challenge.variable}
              <br />
              {challenge.first}
              {challenge.variable} | {challenge.first * challenge.second}
            </aside>
          )}
          <button
            type="button"
            className="double96-new"
        ...
        <nav className="double96-navigation">
          <a href="/lessons/algebra/95-expanding-brackets">
            <ArrowLeft />
            <span>
              Previous<b>Expanding Brackets</b>
            </span>
          </a>
          <a href="/lessons/algebra/97-factorisation">
            <span>
              Next<b>Factorisation</b>
            </span>
            <ArrowRight />
          </a>
        </nav>
        <footer className="double96-footer">
          <div>
            <b>Math Universe</b>
            <p>
