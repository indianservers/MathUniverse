import { ArrowLeft, ArrowRight, BookOpen, CheckCircle2, ClipboardCheck, ListChecks, Route, SearchCheck } from "lucide-react";
    ...
    <div className="space-y-4" data-testid="school-lesson-page">
      <header className="rounded-3xl border border-cyan-100 bg-white p-5 shadow-xl shadow-cyan-950/5 dark:border-white/10 dark:bg-slate-950/75">
        ...
        <div className="mt-4 flex flex-wrap gap-2">
          ...
          {lesson.metadata.engineDependencies?.map((engine) => <Chip key={engine} label={engine} />)}
        </div>
      </header>
      ...
      <main className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
        ...
        <aside className="space-y-4 xl:sticky xl:top-20">
          ...
          <section className="rounded-2xl border border-slate-200 bg-white p-4 dark:border-white/10 dark:bg-slate-950/70">
            <h2 className="text-sm font-black uppercase text-cyan-600 dark:text-cyan-300">Syllabus tags</h2>
      ...
      <nav className="grid gap-3 sm:grid-cols-2" aria-label="Adjacent school lessons">
...
function QualitySection({ lessonTitle, family }: { lessonTitle: string; family: string }) {
  return (
    <section className="rounded-2xl border border-violet-200 bg-violet-50/70 p-4 dark:border-violet-300/20 dark:bg-violet-300/10">
      ...
      <div className="mt-3 grid gap-3 md:grid-cols-3">
        <article className="rounded-xl bg-white/85 p-3 dark:bg-slate-950/50">
          ...
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-700 dark:text-slate-200">Start with a prediction about {lessonTitle}, then change one input in the lab and name what stayed fixed.</p>
        </article>
        <article className="rounded-xl bg-white/85 p-3 dark:bg-slate-950/50">
          <h3 className="text-xs font-black uppercase text-slate-600 dark:text-slate-300">Worked connection</h3>
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-700 dark:text-slate-200">Connect the visual pattern to the formula, diagram, table, or proof language used in {family}.</p>
        </article>
        <article className="rounded-xl bg-white/85 p-3 dark:bg-slate-950/50">
          <h3 className="text-xs font-black uppercase text-slate-600 dark:text-slate-300">Exit check</h3>
          <p className="mt-2 text-sm font-semibold leading-6 text-slate-700 dark:text-slate-200">Ask for one correct example, one non-example, and one sentence explaining why the method works.</p>
        </article>
      </div>
    </section>
  );
...
function LessonNotFound() {
  return <div className="rounded-2xl border border-amber-200 bg-amber-50 p-6 text-amber-950"><h1 className="text-2xl font-black">School lesson not found</h1><p className="mt-2 text-sm">This generated school lesson is not registered.</p><Link className="action-secondary mt-4" to="/lessons/school">Open school lessons</Link></div>;
}
