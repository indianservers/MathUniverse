...
export const sequenceSeriesProofConfigs: Record<string, SequenceSeriesProofConfig> = {
  ...
  TriangularNumbersProof: {
    ...
    toggles: ["Show duplicate rectangle"],
    steps: [
      { id: "intro", title: "Target", description: "Start with the question: how many dots are in rows 1 through n?", focusLabel: "formula target" },
      { id: "build", title: "Build rows", description: "Arrange the dots as rows 1, 2, 3, and so on up to n.", focusLabel: "visible count" },
      { id: "copy", title: "Copy exactly", description: "Make a second congruent triangle, so the count is doubled but each copy is unchanged.", focusLabel: "same count twice" },
      { id: "fit", title: "Fit together", description: "Move the copy into the missing spaces beside the first triangle.", focusLabel: "rearrangement" },
      { id: "rectangle", title: "Count rectangle", description: "The two triangles now form a rectangle with n columns and n + 1 rows.", focusLabel: "n by n + 1" },
      { id: "halve", title: "Divide by two", description: "The rectangle contains two equal triangles, so one triangle is half the rectangle.", focusLabel: "divide by 2" },
      { id: "conclude", title: "Conclusion", description: "The triangular number is T_n = n(n + 1)/2 for every slider value.", focusLabel: "conclusion" },
    ],
    formulas: ["T_n = 1 + 2 + ... + n = n(n + 1)/2"],
    notes: [
      "The blue triangle visibly contains row lengths 1, 2, 3, ..., n.",
      "The orange copy is congruent to the blue one, so putting it elsewhere does not change its dot count.",
      "Together the two triangles occupy an n by n + 1 rectangle, which has n(n + 1) dot positions.",
      "Because that rectangle is made from two equal triangular numbers, one triangular number is n(n + 1)/2.",
    ],
    questions: ["Move n. What changes in the rectangle dimensions?", "Why is it valid to divide the rectangle count by 2?"],
  },
