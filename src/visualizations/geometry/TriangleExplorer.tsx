        ...
        <div className="rounded-2xl bg-white p-3 dark:bg-slate-950/60">
          <svg viewBox="0 0 360 360" className="h-[360px] w-full">
            ...
            {[["A", A, "#22d3ee"], ["B", B, "#a78bfa"], ["C", C, "#f59e0b"]].map(([label, p, color]) => (
              <g key={label as string}><circle cx={(p as typeof A).x} cy={(p as typeof A).y} r="7" fill={color as string} /><text x={(p as typeof A).x + 10} y={(p as typeof A).y - 8} fill={color as string} fontWeight="700">{label as string}</text></g>
            ))}
