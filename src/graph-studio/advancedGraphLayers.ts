...
export function sampleAdvancedGraphExpression(
  input: string,
  xMin: number,
  xMax: number,
): AdvancedGraphSample | null {
  ...
  if (sequence)
    return safeSample(normalized, "sequence", "points", () =>
      sampleSequence(sequence),
    );

  const recurrence = callArguments(normalized, "recur");
  if (recurrence)
    return safeSample(normalized, "recurrence", "points", () =>
      sampleRecurrence(recurrence),
    );

  const cobweb = callArguments(normalized, "cobweb");
  if (cobweb)
    return safeSample(normalized, "cobweb", "line", () =>
      sampleCobweb(Number(cobweb[0]), cobweb[1], Number(cobweb[2])),
    );

  const contour = callArguments(normalized, "contour");
  if (contour)
    return safeSample(normalized, "contour", "line", () =>
      sampleContours(contour, xMin, xMax),
    );

  const vector = callArguments(normalized, "vector");
  if (vector)
    return safeSample(normalized, "vector-field", "vectors", () =>
      sampleVectorField(vector, xMin, xMax),
    );

  const slope = callArguments(normalized, "slope");
  if (slope)
    return safeSample(normalized, "slope-field", "vectors", () =>
      sampleSlopeField(slope, xMin, xMax),
    );

  const parametric = callArguments(normalized, "param");
  if (parametric)
    return safeSample(normalized, "parametric", "line", () =>
      sampleParametricRange(parametric),
    );

  const polarRange = normalized.match(
    /^r=(.+),theta=(-?(?:pi|\d+(?:\.\d+)?))\.\.(-?(?:pi|\d+(?:\.\d+)?))$/i,
  );
  if (polarRange)
    return safeSample(normalized, "polar-range", "line", () =>
      samplePolarRange(
        polarRange[1],
        parseBound(polarRange[2]),
        parseBound(polarRange[3]),
      ),
    );
  return null;
}
...
function sampleParametricRange(args: string[]) {
  ...
  if (!(end > start))
    throw new Error("Parametric end must be greater than its start.");
  return Array.from({ length: 720 }, (_, index) => {
    const t = start + (index / 719) * (end - start);
    const x = xFn(t);
    const y = yFn(t);
    return Number.isFinite(x) && Number.isFinite(y)
      ? { x, y, valid: true }
      : { x: t, y: null, valid: false };
  });
}

function sampleSequence(args: string[]) {
  if (args.length < 1 || args.length > 3)
    throw new Error("Use seq(expression, start, end).");
...
function sampleRecurrence(args: string[]) {
  if (args.length !== 3)
    throw new Error(
      "Use recur(initial, next-expression, count). In the next expression, prev is the previous term and n is its index.",
    );
  let value = Number(args[0]);
  const count = Math.min(
    MAX_DISCRETE_TERMS,
    Math.max(1, integer(args[2], "Recurrence count")),
  );
...
function sampleContours(args: string[], min: number, max: number) {
  ...
  return levels.flatMap((level) =>
    sampleImplicit((x, y) => fn(x, y) - level, min, max, 92),
  );
}

function sampleVectorField(args: string[], min: number, max: number) {
  if (args.length !== 2)
    throw new Error("Use vector(x-component, y-component).");
...
function samplePolarRange(expression: string, start: number, end: number) {
  if (!(end > start))
    throw new Error("Polar angle end must be greater than the start.");
  ...
  for (let row = 0; row < resolution; row += 1) {
    ...
    for (let column = 0; column < resolution; column += 1) {
      ...
      const corners = [
        ...
        { x: x + step, y: y + step },
        { x, y: y + step },
      ].map((point) => ({ ...point, value: fn(point.x, point.y) }));
      ...
      const hits = [
        ...
        if (
          ...
          return [];
        const ratio =
          Math.abs(a.value - b.value) < 1e-12
            ? 0.5
      ...
      for (let index = 0; index + 1 < hits.length; index += 2)
        points.push(hits[index], hits[index + 1], { x, y: null, valid: false });
    }
  }
  return points;
}
...
function safeSample(
  normalized: string,
  family: AdvancedGraphSample["family"],
  style: AdvancedGraphStyle,
  sample: () => GraphSample[],
): AdvancedGraphSample {
  try {
    return { points: sample(), normalized, style, family };
  } catch (error) {
    return {
      points: [],
      normalized,
...
function callArguments(input: string, name: string) {
  const prefix = `${name}(`;
  if (!input.toLowerCase().startsWith(prefix) || !input.endsWith(")"))
    return null;
...
function parseBound(value: string) {
  ...
  try {
    ...
    /* Use the domain-specific message below. */
  }
  throw new Error(
    "Angle bounds must be numeric expressions such as 0, pi, or 2*pi.",
  );
}
