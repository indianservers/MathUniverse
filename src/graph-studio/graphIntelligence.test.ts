...
describe("Graph Studio intelligence", () => {
  it("keeps symbolic CAS results exact and supplies steps", () => {
    ...
    expect(factor?.result.replace(/\s/g, "")).toMatch(
      /\((-2\+x|x-2)\)\*\((-3\+x|x-3)\)/,
    );
  ...
  it("labels sampled function evidence as numerical or heuristic", () => {
    ...
    expect(findings.find((item) => item.label === "Symmetry")?.value).toContain(
      "Even",
    );
    expect(findings.find((item) => item.label === "Symmetry")?.status).toBe(
      "numerical",
    );
    expect(findings.find((item) => item.label === "Family")?.status).toBe(
      "heuristic",
    );
  });
  ...
  it("converts compatible units and blocks dimension mismatches", () => {
    expect(convertGraphUnit(1, "km", "m")).toMatchObject({
      ok: true,
      value: 1000,
    });
