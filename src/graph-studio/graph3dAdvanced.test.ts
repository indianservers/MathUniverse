...
describe("advanced 3D graph sampling", () => {
  it("normalizes and meshes an implicit sphere", () => {
    expect(normalizeImplicitEquation("x^2+y^2+z^2=9")).toBe(
      "(x^2+y^2+z^2)-(9)",
    );
  ...
  it("samples parametric surfaces and space curves", () => {
    ...
    const curve = sampleParametricCurve(
      { x: "cos(t)", y: "sin(t)", z: "t/4" },
      0,
      Math.PI * 4,
      120,
    );
  ...
  it("refines curved explicit regions and classifies critical points", () => {
    const mesh = sampleAdaptiveExplicitSurface(
      "sin(4*x)*cos(4*y)",
      2,
      2,
      10,
    ...
    expect(
      critical.some(
        (point) =>
          point.kind === "saddle" && Math.hypot(point.x, point.y) < 0.25,
      ),
    ).toBe(true);
  });
