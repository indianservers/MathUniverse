...
import {
  createGraphVariable,
  detectGraphVariables,
  reconcileGraphVariables,
  substituteGraphVariables,
...
describe("Graph Studio expression foundation", () => {
  it("detects parameters without treating coordinates and functions as sliders", () => {
    expect(
      detectGraphVariables(["y = a*sin(b*x+c)", "z = r*(x^2+y^2)+t"]),
    ).toEqual(["a", "b", "c", "r", "t"]);
    const variables = reconcileGraphVariables(
      ["y=a*x+b"],
      [createGraphVariable("a", 3)],
    );
    expect(variables.map((item) => [item.name, item.value])).toEqual([
      ["a", 3],
      ["b", 1],
    ]);
  ...
  it("keeps mathematical evaluation inside the safe parser", () => {
    ...
    expect(() => compileFunctionExpression("window.alert(1)")).toThrow(
      /Unsupported/,
    );
  });
  ...
  it("samples sequences and bounded recurrences with safe expressions", () => {
    ...
    const recurrence = sampleAdvancedGraphExpression(
      "recur(1, 2*prev+n, 4)",
      -10,
      10,
    );
  ...
  it("samples contour, vector, slope, and polar-range layers", () => {
    const contour = sampleAdvancedGraphExpression(
      "contour(x^2+y^2, 1;4)",
      -3,
      3,
    );
    ...
    const polar = sampleAdvancedGraphExpression(
      "r=2*sin(3*theta),theta=0..pi",
      -3,
      3,
    );
...
describe("Graph Studio offline projects", () => {
  ...
  it("saves, exports, imports, and migrates a versioned project", () => {
    const project = createGraphStudioProject("2d", "Wave lab", {
      expression: "sin(x)",
    });
    const saved = saveGraphStudioProject({
      ...project,
      variables: [createGraphVariable("a", 2)],
    });
    expect(readGraphStudioProjects("2d")).toHaveLength(1);
    const imported = importGraphStudioProject<{ expression: string }>(
      exportGraphStudioProject(saved),
      "2d",
    );
  ...
  it("rejects opening a 3D project in the 2D studio", () => {
    const project = createGraphStudioProject("3d", "Surface", {
      expression: "x^2+y^2",
    });
    expect(() =>
      importGraphStudioProject(exportGraphStudioProject(project), "2d"),
    ).toThrow(/Graph Studio 3D/);
  });
});
