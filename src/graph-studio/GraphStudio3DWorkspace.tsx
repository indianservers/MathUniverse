...
export type GraphStudio3DWorkspaceProps = {
  ...
  selectedSurface: "primary" | "secondary";
  onSelectedSurfaceChange: (surface: "primary" | "secondary") => void;
  onExpressionChange: (expression: string) => void;
  onSecondaryExpressionChange: (expression: string) => void;
  onSecondaryVisibleChange: (visible: boolean) => void;
  onAddExpression: () => void;
  onDuplicateExpression: () => void;
  onSwapExpressions: () => void;
  primaryError?: string;
  ...
  resolution: number;
  opacity: number;
  palette: "height" | "thermal" | "contour" | "mono";
  ...
  onPaletteChange: (value: "height" | "thermal" | "contour" | "mono") => void;
  ...
  onSamplingAnimationChange: (value: boolean) => void;
  ...
  return <div id="graph-studio-3d-root" className={`graph-studio-3d-shell ${leftOpen ? "has-left" : ""} ${rightOpen ? "has-right" : ""} ${dockOpen ? "has-dock" : ""}`}>
    ...
    <aside className={`gs3d-left-panel ${leftOpen ? "open" : ""}`} aria-label="Expressions and layers">
      ...
      <div className="gs3d-panel-scroll">
        <ExpressionCard active={props.selectedSurface === "primary"} visible expression={props.expression} color="cyan" error={props.primaryError} onSelect={() => props.onSelectedSurfaceChange("primary")} onChange={props.onExpressionChange} onDuplicate={props.onDuplicateExpression} />
        {props.secondaryVisible && <ExpressionCard active={props.selectedSurface === "secondary"} visible={props.secondaryVisible} expression={props.secondaryExpression} color="violet" error={props.secondaryError} onSelect={() => props.onSelectedSurfaceChange("secondary")} onChange={props.onSecondaryExpressionChange} onVisibility={() => props.onSecondaryVisibleChange(false)} visibilityLabel="Remove surface" onDuplicate={props.onSwapExpressions} />}
        <button type="button" className="gs3d-add-expression" onClick={props.onAddExpression} disabled={props.secondaryVisible}><Plus />{props.secondaryVisible ? "Two-surface limit reached" : "Add expression"}</button>
        ...
        <div className="gs3d-panel-section"><h3>Scene layers</h3>
          ...
          <ToggleRow label="Sample points" icon={<Activity />} checked={props.showPoints} onChange={props.onShowPointsChange} />
          <ToggleRow label="Wireframe" icon={<Grid3X3 />} checked={props.showWireframe} onChange={props.onShowWireframeChange} />
        </div>
      </div>
    </aside>
    ...
    <aside className={`gs3d-right-panel ${rightOpen ? "open" : ""}`} aria-label="Surface Inspector">
      ...
      <div className="gs3d-panel-scroll" aria-live="polite">
        {mode === "learn" ? <LearnPanel expression={props.expression} palette={props.palette} /> : inspectorTab === "properties" ? <PropertiesPanel props={props} /> : inspectorTab === "style" ? <StylePanel props={props} /> : <AnalysisPanel props={props} onCreateCrossSection={openCrossSection} />}
      </div>
    </aside>
    ...
    <footer className="gs3d-status"><span className="online-dot" />Offline ready <span>{fps} FPS</span><span>{props.resolution} x {props.resolution} adaptive mesh</span><span>{props.surface.error ? "Calculation error" : "Calculations current"}</span><span className="saved">Auto-saved locally</span></footer>
...
function ExpressionCard({ active, visible, expression, color, error, onSelect, onChange, onVisibility, visibilityLabel, onDuplicate }: { active: boolean; visible: boolean; expression: string; color: "cyan" | "violet"; error?: string; onSelect: () => void; onChange: (value: string) => void; onVisibility?: () => void; visibilityLabel?: string; onDuplicate: () => void }) {
  const [editing, setEditing] = useState(false);
  return <div className={`gs3d-expression ${active ? "active" : ""}`} onClick={onSelect}><span className={`swatch ${color}`} /><div className="expression-main"><button type="button" className="expression-text" onClick={() => setEditing((value) => !value)}>z = {expression}</button>{editing && <input autoFocus aria-label="Edit surface expression" value={expression} onChange={(event) => onChange(event.target.value)} onClick={(event) => event.stopPropagation()} />}{error && <p role="alert">{error}</p>}</div>{onVisibility && <button type="button" className="icon-action" onClick={(event) => { event.stopPropagation(); onVisibility(); }} aria-label={visibilityLabel ?? (visible ? "Hide surface" : "Show surface")}>{visible ? <Eye /> : <EyeOff />}</button>}<button type="button" className="icon-action" onClick={(event) => { event.stopPropagation(); onDuplicate(); }} aria-label={onVisibility ? "Swap primary and secondary surfaces" : "Duplicate surface"}><Copy /></button></div>;
}
