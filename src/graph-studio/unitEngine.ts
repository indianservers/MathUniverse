...
export function convertGraphUnit(
  value: number,
  fromSymbol: string,
  toSymbol: string,
) {
  ...
  if (!from || !to)
    return { ok: false as const, message: "Choose known units." };
  if (from.dimension !== to.dimension)
    return {
      ok: false as const,
      message: `Dimension mismatch: ${from.dimension} cannot convert to ${to.dimension}.`,
    };
  const result = (value * from.scale) / to.scale;
  return {
    ok: true as const,
    value: result,
    message: `${value} ${from.symbol} = ${format(result)} ${to.symbol}`,
