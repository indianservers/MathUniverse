...
export function linearRegression(points: NumericGraphDataPoint[]): LinearRegressionResult | null {
  ...
  return {
    ...
    mae: residuals.reduce((sum, point) => sum + Math.abs(point.observedY - point.predictedY), 0) / points.length,
    line: [{ x: minX, y: slope * minX + intercept }, { x: maxX, y: slope * maxX + intercept }],
    residuals,
  };
}

export function regressionModel(points: NumericGraphDataPoint[], kind: RegressionKind): LinearRegressionResult | null {
  if (kind === "linear") return linearRegression(points);
  if (kind === "exponential") {
    const positive = points.filter((point) => point.y > 0);
    const transformed = linearRegression(positive.map((point) => ({ x: point.x, y: Math.log(point.y) })));
    if (!transformed || positive.length !== points.length) return null;
    const a = Math.exp(transformed.intercept); const b = transformed.slope;
    return buildRegressionResult(points, kind, [a, b], (x) => a * Math.exp(b * x), `y = ${formatCoefficient(a)}e^(${formatCoefficient(b)}x)`);
  }
  if (points.length < 3) return null;
  const sums = (power: number) => points.reduce((sum, point) => sum + point.x ** power, 0);
  const rhs = (power: number) => points.reduce((sum, point) => sum + point.y * point.x ** power, 0);
  ...
  return buildRegressionResult(points, kind, coefficients, (x) => a + b * x + c * x * x, `y = ${formatCoefficient(c)}x² ${signed(b)}x ${signed(a)}`);
}

function buildRegressionResult(points: NumericGraphDataPoint[], kind: RegressionKind, coefficients: number[], predict: (x: number) => number, equation: string): LinearRegressionResult {
  const residuals = points.map((point) => ({ x: point.x, observedY: point.y, predictedY: predict(point.x) }));
  const meanY = points.reduce((sum, point) => sum + point.y, 0) / points.length;
  const total = points.reduce((sum, point) => sum + (point.y - meanY) ** 2, 0);
  const unexplained = residuals.reduce((sum, point) => sum + (point.observedY - point.predictedY) ** 2, 0);
  const xs = points.map((point) => point.x); const minX = Math.min(...xs); const maxX = Math.max(...xs);
  const line = Array.from({ length: 180 }, (_, index) => { const x = minX + index / 179 * (maxX - minX); return { x, y: predict(x) }; });
  return { kind, slope: coefficients[1] ?? 0, intercept: coefficients[0] ?? 0, coefficients, equation, rSquared: total < 1e-12 ? 1 : 1 - unexplained / total, rmse: Math.sqrt(unexplained / points.length), mae: residuals.reduce((sum, point) => sum + Math.abs(point.observedY - point.predictedY), 0) / points.length, line, residuals };
}
...
function solve3(matrix: number[][]) {
  ...
  for (let column = 0; column < 3; column += 1) {
    let pivot = column;
    for (let row = column + 1; row < 3; row += 1) if (Math.abs(next[row][column]) > Math.abs(next[pivot][column])) pivot = row;
    ...
    for (let index = column; index < 4; index += 1) next[column][index] /= divisor;
    for (let row = 0; row < 3; row += 1) if (row !== column) {
      const factor = next[row][column];
      for (let index = column; index < 4; index += 1) next[row][index] -= factor * next[column][index];
    }
  }
  return next.map((row) => row[3]);
}

function formatCoefficient(value: number) { return Number(value.toPrecision(6)).toString(); }
function signed(value: number) { return `${value < 0 ? "-" : "+"} ${formatCoefficient(Math.abs(value))}`; }
